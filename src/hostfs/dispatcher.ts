...
export class HostFsDispatcher {
   ...
   * $50: 空き容量。
   *
   * PRO-68K DOS呼び出しガイドp.150の記載は「使用可能クラスタ(W)/総クラスタ(W)/
   * クラスタあたりセクタ(W)/セクタあたりバイト(W)」の順。この順で+0を「使用可能クラスタ」、
   * +2を「総クラスタ」として書いたうえで、実機相当のHuman68k(WebX68k上のhuman302.xdf、
   * HostFSをCONFIG.SYSに登録した状態)で`dir c:`を実行し、表示される集計行を3通りの
   * 値の組み合わせで実測した(2026-09-12)。
   *
   * 実測結果: 「使用可能」欄は+0にどんな値(0/32000/65535)を書いても常に0Kのまま変化せず、
   * 「使用中」欄は常に「+2の値 × クラスタサイズ」がそのまま出た(+2=768→24576K、
   * +2=100→3200K、+2=0→0K)。つまりこのリモートデバイス経由のドライブでは、
   * Human68kのDIRコマンドは空き容量を「総容量から使用中を引いた残り」としては計算せず、
   * 何らかの理由で「使用可能」を常に0として扱う(HostFSは読み取り専用でHuman68k側からは
   * 書き込めないドライブのため、「空き」という概念自体が無いものとして扱われている
   * 可能性がある。XM6等の他実装は参照しておらず、この一文は実測結果からの推測)。
   *
   * この制約の下では「+0の値」は集計行の表示に一切影響しないため0のままにし、
   * 「+2の値」も0(使用中0K)にして、実際には存在しない使用量を作り物として見せない
   * ことを選んだ(0K/0Kは素っ気ないが、根拠のない数字を出すよりは誤解が少ないと判断)。
   */
  private handleFreeSpace(addr: number): void {
    const outPtr = readU32BE(this.mem, addr + HDR_ARG_PTR_OFFSET);
    writeU16BE(this.mem, outPtr + 0, 0); // 使用可能クラスタ(実測: 値を変えても表示は常に0)
    writeU16BE(this.mem, outPtr + 2, 0); // 総クラスタ(実測: 表示上の「使用中」に直結する欄)
