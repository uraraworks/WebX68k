...
/*
 * 【実測で決めた値、2026-09-04】
 * 当初 0.9 にしていたが、それでは無制限モードが固定倍率(4倍)より遅くなる。
 * 実測: 上限192fps に対し無制限173fps(= 192 × 0.9)。占有率がそのまま律速していた。
 * Worker はUIを持たないため、メインスレッド用の UNLIMITED_MAX_DUTY(0.7) のような
 * 「UIを守るための取り分」を確保する理由が無い。イベントループへは setInterval の
 * 刻みの間に自然に戻るので、上限を撤廃しても入力は届く
 * (実測で確認: 無制限中に打鍵して画面に "A>dir" が出た)。
 * 1.0 にした結果、同一実行内で 4倍121.6fps に対し無制限136fps となり、
 * 「無制限が一番速い」という当然の関係になった。
 */
export const WORKER_UNLIMITED_MAX_DUTY = 1.0;
