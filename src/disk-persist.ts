...
export interface DiskPersistenceDeps<S extends SlotId = SlotId> {
  ...
  saveDisk(args: { sourceKey: string; name: string; bytes: Uint8Array; savedAt: number }): Promise<void>;
...
export function createDiskPersistence<S extends SlotId = SlotId>(
  ...
  async function persistSlot(slot: S): Promise<boolean> {
    ...
    try {
      await deps.saveDisk({ sourceKey, name: pending.name, bytes: data, savedAt: deps.now() });
    } catch (err) {
