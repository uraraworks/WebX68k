...
// ここが持つのは SHA-256 で検証できる写し。
//
// **凍結されるのは ABI であって、実装ではない。** 版番号(v1, v2, ...)が保証するのは
// 「この版のペイロードがこの版のランタイムで動く」ことで、中身のバイナリが未来永劫
// 同一であることではない。実装の不具合修正や高速化では、同じ版のアセットを差し替える
// （2026-09-01 に v1 で実施。描画が最大10倍速くなり、過去の共有リンクも無改造のまま
// 速くなった。ABI・レイアウト・.xdf のセクタ数はいずれも不変）。
// 差し替えたら、この写しを取り直して test/sprout-share.test.ts の
// EXPECTED_MANIFEST_SHA256 も一緒に直すこと（直し忘れればテストが落ちる）。
//
