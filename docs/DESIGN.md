...
- 方向: `dpad-up` / `dpad-down` / `dpad-left` / `dpad-right`
  （描画は3x3の1部品で、斜めは隣接2 ID の同時押しとして表現する）
...
- バーチャルパッド = stage の**上に重ねる**オーバーレイ（画面は縮めない）

パッドを「下に積む」方式にするとゲーム中に表示が半分になって遊べないため、オーバーレイは必須。
一方、スマホの縦画面で両方出しても実用にならないので、既定は片方だけ表示する。
...
- **案C（採用）**: ツールバーは1ボタンのまま「入力パネル」トグル。**どちらを出すかは stage の隅に
  置いた小さなフローティングチップ（⌨ / 🎮）で切り替える。**
  - ツールバーのボタン数が増えず、ゲーム中は常に1タップでパッドが出る
    （最後に選んだ側を `localStorage['webx68k.inputPanel']` に保存し、トグルはそちらを出す）
  - チップをパネル内ではなく stage の隅に置くのは、パッド（オーバーレイ）と
    キーボード（積むパネル）で置き場所が揃わないため
...
4. `src/virtual-pad.ts`（描画・タッチ処理）＋ main.ts 配線＋切り替えチップ
5. 設定ダイアログへの「バーチャルパッド」タブ（割当編集。仮想キーボードをピッカーに流用）
6. 位置/サイズのドラッグ編集、連射、バイブレーション
