...
(各2バイト)ずつ書く欄がある。HostFSは実容量を持たない(読み取り専用のホストフォルダ)
ため作り物の値を返すしかないが、**どんな値を書いても実機相当のHuman68k(WebX68k上の
human302.xdf)の`dir`集計行では「使用可能」欄が常に0Kと表示される**ことを実測で
確認した(2026-09-12、`_local/hostfs/probes/p2b-install-e2e.json`。+0の値を
0/32000/65535と振っても使用可能は常に0、+2の値だけが「使用中」欄にそのまま
反映される)。XM6等の他実装は参照しておらず、原因(HostFSが読み取り専用のリモート
デバイスであることと関係があるかどうか含め)は未解決。この結果を受け、根拠のない
「使用中」の数字を作るよりはと考え、+0/+2とも0にしている(クラスタあたり
セクタ64・セクタあたりバイト512は形だけ有効な値を置いている)。詳細は
