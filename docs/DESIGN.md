...
| +18 | `$47`/`$48`のときはFILBUFへのポインタ(入力)。`$4c`/`$4d`のときは要求長(入力)→実際に読み書きしたバイト数(出力)。`$44`のときは新しい名前の`_NAMESTS`へのポインタ(入力)。`$49`のときは_CREATE/_NEWFILEの区別を示すフラグ(入力、下記参照)。**それ以外のコマンドでは戻り値(出力、long)** — 「戻り値は+18」というのは、この欄が入力(ポインタ/長さ/フラグ)と出力(エラーコード/読み書きした長さ)を兼ねているという意味 |
| +22 | `$4a`/`$4c`/`$4b`/`$4d`/`$49`のときはFCBへのポインタ(`$49`は出力、それ以外は入力) |
...
- 1回目の`_WRITE`から正しく書けたバイト数(D0=`$0D`=13)が返った。実験用ドライバで
  ...
  見られるが、原因までは確認していない。
- `_FILEDATE`は取得・設定ともD0=`-6`(ドライバに届かない)。
