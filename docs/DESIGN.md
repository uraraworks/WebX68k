...
iOS Safari は Pointer Lock API に対応しておらず、キャプチャモードが成立しない。一方で
追従モードは「目標位置(desiredRatio)」さえ与えれば Pointer Lock なしで動くため、
**タッチ位置をそのまま desiredRatio へ流し込めばタッチデバイスでもマウス操作が成立する**。
ツールバー「…」メニュー(入力グループ)の「タッチマウス」トグルで有効化し、設定は
`localStorage` の `webx68k.touchMouse` に保存する。

- ジェスチャの解釈(1本指=移動、タップ=左、2本指タップ=右、長押し=左押し込みドラッグ)は
  `src/touch-mouse.ts` の純ロジックが受け持つ。DOM に触れない形で切り出したのは
  platform.ts と同じ理由(タッチ実機なしで vitest から検証するため。`test/touch-mouse.test.ts`)
- main.ts 側は Pointer Events の正規化(`pointerType === 'mouse'` は既存経路に譲る)と、
  クリックパルスのタイミング制御だけを行う
- **タップのクリックは追従の収束を待ってから送る**。カーソルは目標へ数フレームかけて収束する
  ため、着地前にボタンを押すと移動中の座標でクリックされてしまう。`stepMouseTracking()` の
  収束判定(誤差0、または送信下限未満)で積んであるクリックを流し、IOCS ワークを使わない
  ソフト(収束を検知できない)向けに 400ms の期限切れ送信を保険として付ける
- ボタンパルスは押下100ms・間隔60ms。コアは `retro_run()` 中に1回しかボタンを読まないため、
  1フレームより十分長く保持しないと取りこぼす。連続タップ(ダブルクリック)はキューで直列化する
- ON の間は `canvas` に `touch-action: none` を当ててスクロール/ピンチへタッチを渡さない。
  `pointerdown` の `preventDefault` は iOS の互換マウスイベント(タップ後に合成される
  mousedown/click)の抑止が目的
...
- 移動方式は2つあり `webx68k.touchMouseMode` で永続化する。**既定はトラックパッド式(relative)**。
  absolute(タッチ位置へ移動)はカーソルが指の真下に来るため**指の影に隠れて見えない**という
  実使用上の難点が iPhone 実機で判明した(absolute は明示的に選んだ場合だけ使う)。
  relative は指の移動量をキャプチャモードの mousemove と同じ換算(canvas拡大率×感度)で
  求め、**加速テーブルを逆引きして「意図した移動量を超えない送信量」へ変換してから**
  `addMouseDelta` へ送る(`sendAmountFor()`/`predictedMoveFor()`)。モバイルは canvas の
  縮小表示で換算倍率が2倍超になり、生のまま送ると1イベントぶんでも IOCS の加速域に入って
  カーソルが画面端まで飛ぶため。送信ぶんの予測移動量を引いた残差は次のイベントへ繰り越し、
  指を離したら捨てる。閉ループは使わない(`hasDesiredRatio` を落として綱引きを防ぐ)。
  タップのクリックはカーソルが動いていないので収束待ちも不要(期限0で即パルス)
- relative の間は canvas の `mousemove` による追従目標の更新も止める。iOS Safari は
  `pointerdown` を `preventDefault` しても**互換マウスイベント(タップ後の mousemove)を
  発火することがあり**、拾うと閉ループがカーソルをタップ位置へ勝手に運んでしまう
  (タップ後にカーソルが飛ぶ・消えたように見える事象の原因になった)
- relative では**スロップ(TAP_SLOP_PX)以内の微動を送らない**。タップ時の指の揺れ(数px)を
  そのまま送ると、クリックのたびにカーソルが標的からずれ、特にダブルタップは1回目と2回目の
  クリック位置が食い違って**ゲスト側でダブルクリックとして成立しない**(iPhone実機で発覚)。
  スロップを超えた瞬間に始点からの累積差分をまとめて送って距離の欠損を防ぐ
