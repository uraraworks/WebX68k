...
HDD(SASI)とは別系統のスロットとして、SCSI ハードディスク(`.hds`)にも対応している。
実装は**HLE(High-Level Emulation)**で、SPC(MB89352)のバスフェーズ・DMA・割り込みまでは
エミュレーションしていない。px68k のSCSIボードROMスタブ(`$ea0000`)を Human68k に対する
デバイスドライバとして名乗らせ、セクタI/Oはホスト(JS)側へ抜けて OPFS を読み書きする
(構造は SASI とほぼ同じ)。検討経緯・実測ログの詳細は [docs/STORAGE-SCSI.md](STORAGE-SCSI.md) を参照。

- **役割の分担**: セクタI/Oは Worker 内で `FileSystemSyncAccessHandle`(ワーカー専用の同期API)を
  使って行う。メインスレッド側の `src/scsi-store.ts` は非同期APIだけを使い、一覧・取り込み・削除
  を担当する(`src/scsi-opfs.ts` がWorker側の同期I/O実装)。
- **ドライバヘッダの置き場所**: Human68k にドライバとして名乗るには、ドライバヘッダを置く
  ゲストRAMの番地が要る。Human68k の作業領域変数 `$66ca` の中身が使えればそれを使う。CONFIG.SYS
  でデバイスドライバを読み込まない構成などで `$66ca` が0のままの場合は、ベクタ設定エントリ入口の
  `a1`(Human68k が次に読み込むものの読み込み先)を代わりに使う。どちらも使えない(0・奇数番地・
  RAM上限を超える等)ときは**ドライバとして名乗らない**(理由をログに1行出し、d2に`$ffffffff`を
  返す)。無理に登録すると Human68k がヘッダを読めず起動そのものが止まるため、安全側に倒している。
- **既知の制限**: 起動ディスクの構成によっては SCSI ドライブが現れないことがある(起動そのものは
  妨げない)。
- **既知の制限**: SCSI からは起動できない。HLE 実装が SCSI ボードROMの「起動エントリ」を
  実装していないため、起動可能な `.HDS` を挿しても起動しない。フロッピーまたは HDD(SASI)
  から起動し、SCSI はデータ用のドライブとして使う(実機ではSRAM設定で起動デバイスに
  SCSIを選べるが、WebX68k では未対応)。

### GUI 側の仕様

- ブランク作成は 1〜2047MB の整数指定・FAT16 フォーマット。上限が2047MBなのは、SCSI の HLE 実装が
  イメージサイズを32bit符号あり整数で扱っているため。
- 区画が複数あるイメージは複数のドライブとして見える。
- 挿入は次回の起動から有効(起動中の差し替えは不可。差し替えるにはページ再読み込みが必要)。
- 保存先は FD/HDD のディスクライブラリ(IndexedDB)とは別で、OPFS の `scsi/`。ゲストの書き込みは
  ここへ永続化され、リロードをまたいで残る。OPFS は secure context 専用のため、**保護された接続
...
`?cpu=<10..1000|auto>`(`parseCpuSpeedParam()`)も同じ扱いで、`cpuSpeed` 変数のみを
...
手当てできない(全画面の対象は現在ページ全体。後述「切り欠き(セーフエリア)と全画面の対象」参照)。
...
切り替えチップがキーボード/パッド/マウスの3種になっている(`webx68k.inputPanel` に永続化)。
...
- **案C（採用）**: ツールバーは1ボタンのまま「入力パネル」トグル。**どちらを出すかは
  ツールバーの中に置いた小さなチップ（キーボード / パッドのアイコン）で切り替える。**
...
WebNP2 で同じ不具合を実機で追った結果を移植した(2026-08-20)。どちらも横持ち全画面
(`sides` 配置)でのみ表面化する。

- **全画面の対象はページ全体(`document.documentElement`)**。ネイティブ全画面は
  対象の部分木の外を一切描画しないため、`.console-card` を対象にすると、`sides` 配置で
  body 直下へ出したバーチャルパッドが丸ごと消える(Android 実機で発覚。iPhone は
  Fullscreen API 自体が無く疑似フルスクリーンへ倒れるのでこの経路を踏まない)。
  かつて対象を `.stage` → `.console-card` と移したのはツールバーを残すためだったが、
  ページ全体ならどこへ reparent しても消えない。`.console-card` を画面いっぱいへ広げる
  指定は `:root:fullscreen .console-card` へ移した
- **`sides` の左右ボックスはセーフエリアを引いて取る**(`vpadSideBoxesFor()`)。
  `index.html` の viewport は `viewport-fit=cover` なので、スタンドアロン表示や
  Android 全画面ではビューポートが切り欠きの下まで広がり、`x:0〜innerWidth` のまま
  取ると外縁の部品(左のスティック/右端のボタン)が隠れる。`env(safe-area-inset-*)` は
  カスタムプロパティ経由では解決されない環境があるため、実プロパティ(padding)へ入れた
  不可視要素 `.safe-area-probe` から読む
- **iOS は横向きで左右へ同じインセットを返すが、実際に塞がっているのは切り欠き側だけ**。
  両方避けると片側ぶん(実測59px)を無駄に捨てる。`resolveLandscapeInsets()` が
  左右同値のときだけ向き(`screen.orientation.angle`。90=左・270=右、いずれも実機確認)から
  片側を解放する。左右非対称・角度不明・縦向きは触らない(安全側)
- 縦向き全画面で切り欠きが実行画面に重なる件は、没入モードの `main` の padding を
  丸ごと0にせず `env(safe-area-inset-top)` だけ残して解決する。下と左右は入れない
  (下を空けると横向きで画面が縮み、左右は上記のボックス側と二重に引くことになる)
- `sides` の部品サイズは箱の幅でほぼ決まり縦向きより小さくなるので、はみ出さず重ならない
  範囲まで詰める。オプション(1/2)の帯は**スティックより先に**取り分ける(スティックを先に
  最大化して「余った隙間」を回すと、箱が横広のときオプションだけ潰れる)。ボタンの列/行
  ピッチは「実際に使うスロットの数」で割る(2ボタン構成で常に3列で割ると1列ぶん捨てる)
...
5. ✅ 割当編集 UI（`src/input-profile-ui.ts`。仮想キーボードのレイアウト定義をピッカーに流用。
   パッドのアイコンのプロファイルメニュー末尾の「割当を編集…」から開く。組み込みプロファイルを編集すると
   自動で複製を作る）
...
### 無制限モードは「1tickの総時間」で配分する

無制限モードには目標倍率が無いため、既存の倍率経路(`frameInterval`/`accumulator`/
`computeFrameBudget()`)を一切通らない別経路(`main.ts` のtick処理内)で動く。

**最初の実装は「コア実行だけに10msの予算」を与え、画面提示(RGB565→RGBA変換+putImageData)の
コストを予算の外に置いていた。これが誤りだった。** ブラウザでの実測(2026-09-03、513tick分の
中央値)は次の通り:

- tick間隔 19.74ms / コア実行 12.37ms / 画面提示 6.56ms / **メインスレッド占有率 95.9%**

この状態ではページが操作を受け付けなくなり、外部からの単純なJS実行すら45秒でタイムアウト
した。コア実行が予算10msに対し12.37msかかっていた理由は、**1フレームのコア時間が約3msあり、
deadline直前に始まったフレームが予算を踏み越える**ため。現在は
`deadline = tickStart + coreBudget - unlimitedFrameCostMs` として「入らないフレームは
始めない」ようにしている。

現在の設計では、`UNLIMITED_TICK_BUDGET_MS`(12ms)は**コア実行と画面提示を合わせた総量**。
画面提示は `UNLIMITED_PRESENT_INTERVAL_MS`(33ms)以上あけて約30fpsに落とし、固定費を分散
させる。提示コストとフレームコストは実測の移動平均(EMA、係数0.8/0.2)で持ち、そこから
配分を決める。

一般化した教訓: **「予算」を決めるときは、そのループが実際に払う費用を全部数えること。
固定費を予算の外に置くと、予算どおりに見えて実際には超過する。**

再発検出のため、開発ビルドの `__webx68kDebug.unlimitedTiming()` で配分の実測値を外から
読めるようにしてある。

**予算を守っても占有率は守れなかった。** 上記の修正後、2026-09-03 の別の実測(481tick分の
中央値)では1tickのコストは予算どおり(10.71ms)だったのに、tick間隔が10.73msしかなく
**占有率99.8%**になった。原因は駆動経路が3つあること: `requestAnimationFrame` /
`setTimeout(32)` / **AudioWorklet の tick**(`engine.setTickHandler` → `enterLoop`)。
3つ目はタイマーがスロットルされる環境でも駆動を止めないための経路で、通常モードでは
`accumulator` により余分な呼び出しが空振りするので無害だが、無制限モードは呼ばれるたびに
予算を使い切るため頻度がそのまま占有率になった。実測: rAF 60→8.4fps、`setTimeout(0)` の
遅延中央値 0.1→229ms、96 tick/秒。

対処は `UNLIMITED_MAX_DUTY`(0.7)と `unlimitedNextAllowedAt` による実時間ベースの占有率
ゲート(`frameBudget.ts`/`main.ts`)。1tickの実コストから「次に走ってよい時刻」を決めるので、
駆動経路がいくつあっても、またホストが遅くても占有率は上限を超えない。

一般化した教訓: **「1回あたりのコスト」を決めても、呼ばれる回数を決めていなければ総量は
決まらない。駆動経路が複数ある実装では、経路ごとの制御ではなく末端の実時間で上限を掛ける。**

対処後のブラウザ実測(2026-09-03、ゲスト側の時計 = MOUSETST.X の IOCS `_ONTIME` で測定):

| 指標 | 基準(OFF) | 無制限 | OFF復帰後 |
|---|---|---|---|
| ゲスト実速度 | 1.00倍 | 6.21倍 | 1.01倍 |
| rAF 維持レート | 60.1fps | 59.9fps | 60.4fps |
| `setTimeout(0)` 遅延の中央値 | 0.1ms | 10.5ms | 0.1ms |
| 画面更新 | 49.4fps | 19.8fps | 48.6fps |
| 設定パネルの実測表示 | 100% | 649% | 103% |

rAF が 60fps を維持できており(修正前は 8.4fps)、`setTimeout(0)` の遅延も最大12.1msで
頭打ちしている。占有率の上限(0.7)が効いている。

速度の検証に**ホスト側のフレーム数カウンタを使っていない**こと。ホスト側のカウンタは
「自分が何回呼んだか」を数えているだけでゲストが進んだ証拠にならないため、ゲスト側の
1/100秒タイマー(`_ONTIME`)を読んでいる。設定パネルの実測表示(649%)がゲスト側の
実測(6.21倍)と一致したことで、表示側の妥当性も同時に確認できた。

OFF に戻すと基準値へ完全に復帰しており、無制限モードを使った後遺症は無い。
...
コアのフレームバッファは RGB565。ImageData は RGBA なので毎フレーム変換が要る。
以前はピクセルごとに `r5/g6/b5` を展開して4バイト個別に書いていた。

現在は `src/rgb565.ts` の `buildRgb565Lut()` が作る 65536 要素の `Uint32Array` を引いて、
`Uint32Array` ビュー越しに1ピクセル1回書きにしている。表は256KBあるため最初に必要になった
時に1回だけ生成する。

`ImageData` は `[R,G,B,A]` のバイト列なので、32bitでまとめ書きするときの詰め方が
実行環境のバイト順で変わる。`buildRgb565Lut(littleEndian)` は純粋関数で、
`test/rgb565.test.ts` が**両方のバイト順について全65536値**を独立計算した期待値と突き合わせている。

**`imageData` を作り直すときは `Uint32Array` のビューも必ず同時に作り直すこと。**
忘れると解像度変化後だけ古いバッファへ書き続け、画面が固まる。

効果の実測(2026-09-03、同一ディスク・同一計測コードで前後比較):

- 画面提示1回のコスト(コア1フレーム＋変換＋putImageData の合計) **2.87ms → 2.37ms**
- コア1フレーム単体が約1.79msなので、**変換部分は約1.08ms → 約0.58ms**
- 無制限モードの実速度 6.21倍 → 6.36倍

出力バイト列が変わっていないことは、実画面(A>プロンプト、768x512)のフル canvas ハッシュが
変更前後で完全に一致すること(カーソル点滅による2種類のハッシュが、両方とも一致し、
それ以外は1つも出ない)で確認した。**式の網羅テストだけでは結線(バイト順・ストライド・
ビューの張り直し)の誤りは捕まらないため、実画面のバイト一致と2段構えで確認している。**
...
WebNP2(姉妹プロジェクト)ではコア(SDL2)が内部でメインループを持つため、ポーズには
コア側のフラグ(`webnp2_dbg_paused`)が要った。WebX68k は px68k-libretro で、メインループが
JS 側(`src/main.ts` の `loop()`)にあるため、コアの再ビルドは不要で `host.runFrame()`
(実体は `retro_run()`)を呼ばないだけでポーズになる。

状態は `pausedByUser`(モジュールスコープの `let`)一つだけを持つ。意図的に localStorage
へは保存しない。ポーズしたままリロードすると、次回起動時にコアはポーズされていない(コアの
状態はページ再読み込みで消える)のにこのフラグだけ「ポーズ中」を覚えていて画面が理由もなく
止まって見える、という事故を避けるため。

`loop(t)` は冒頭の `!running || !host` チェックの直後で `pausedByUser` を見て早期 return
する。その際 `lastFrameTime`/`accumulator` を 0 に戻す。戻さないと、再開直後の `dt` が
「ポーズしていた時間ぶん」の巨大な値になり、`computeFrameBudget()` がその dt を一気に
消化しようとして大量のフレームをまとめて回す(早送りになる)。`scheduleNext()` はポーズ中
rAF を張らず `setTimeout(PAUSE_SLEEP_MS)`(200ms)だけにして 60Hz の空回りを避ける。
AudioWorklet の tick からも `enterLoop()` が呼ばれ続けるが、`loop()` 冒頭の早期 return が
受けるので二重駆動にはならない。

音声は `AudioContext` を suspend しない。キューが枯れると AudioWorklet 側が約3msで
フェードアウトする実装(`src/audio.ts` の `_fade`)になっているため、フレーム供給を止める
だけでプツ音なく無音になり、再開時はフェードインする。suspend すると自動再生ポリシー絡みの
復帰リスク(一部ブラウザで resume() が無条件には効かない)があるため採らない。

状態遷移は `setPaused(next)` 一つに集約している(`pausedByUser` の更新、ボタンの
アイコン/title/aria-label/active クラス、オーバーレイの表示をここでまとめて行う)。
...
コアでなく JS 側の `pausedByUser` 一つだけにあるためポーリングが要らない。状態が
変わったときだけ DOM を触る設計にし、定期ポーリングは足していない。アイコンも
`<svg>` ごと作り直さず `<path id="btn-pause-icon">` の `d` 属性だけを `setAttribute`
で差し替えている(クリック対象の要素そのものを DOM から外さないため)。

再開はポーズ中オーバーレイ中央の再生ボタン(`#btn-pause-overlay-resume`)からのみ行う。
オーバーレイの余白クリックでは再開しない(WebNP2 の同種オーバーレイが後に「余白クリックでも
起動してしまう」誤操作を修正した経緯があり、同じ判断を最初から採用した)。

ツールバー上の「リセット」は誤操作時の被害が大きいため、頻繁に押す「ポーズ」等の操作からは
指1本ぶん離す設計にし、ツールバーを中央グループ(`.toolbar-center`)と右端グループ
(`.toolbar-end`、リセットのみ)に分離した。列構成・実測値は `src/style.css` の `.toolbar`
コメントおよびコミット履歴参照。

## 表示アスペクト比(4:3表示モード)

実機の X68000 は画面モード(256x256/512x512/768x512等)に関わらず、常に4:3のブラウン管
モニタいっぱいに表示される。一方 WebX68k はコアの実解像度をそのままドット等倍
(正方形ピクセル)描画すると、実機とは見た目のアスペクト比が異なる。この差を埋めるオプション
として「4:3表示」モードを追加した。判定・目標サイズ計算は DOM に触れない純関数として
`src/aspect.ts` に切り出し、`src/main.ts` の `rescale()` から呼ぶ。

- **既定は4:3表示。** 2026-08 時点では既存ユーザーの見た目を変えないことを優先してドット
  等倍を既定にしていたが、2026-09-12 のシャープ・バイリニア導入(`src/sharp-view.ts`)で
  4:3表示のぼやけが解消したため、2026-09-13 に既定を4:3へ変更した。ドット等倍は
  「明示的に切り替えられる」オプションという位置づけ(`resolveAspectMode()` のコメント参照)。
...
- **補間は4:3表示中のみ有効(シャープ・バイリニア)。** 4:3表示では `#screen` の上に表示専用の
  `#screen-sharp`(`src/sharp-view.ts`)を重ね、最近傍で物理ピクセルの整数倍(floor)へ拡大した
  中間バッファを作ってから端数ぶんだけCSS補間する。全体を一気に補間するとRetina 2倍表示で
  縦横4〜4.5倍の補間になり画面全体が滲んでいたため(2026-09-12 に置き換え)。`#screen` 自体は
  マウス換算・スクリーンショット・Bridge が実解像度バッファを前提にしているので触らない。
  ドット等倍表示は従来どおり `pixelated` のままくっきり保つ。
...
- **端数倍のときは物理ピクセル整数倍へ寄せる**(`fitDeviceScale()`、WebNP2 から移植)。
  CSSピクセル基準の端数倍 × `pixelated` は、最近傍で「物理2pxになる列」と「1pxになる列」が
  周期的に混ざり、1ドット幅の線や市松が間引かれてモアレになる。DPR を掛けた物理倍率が
  整数に近い(ロス8%以内)なら切り下げて乗せ、`pixelated` を保つ。ロスが大きいときは面積を
  優先して端数のまま使い、`#screen.smooth-scaled` で補間へ落としてモアレを消す
  (縮小方向のディザは補間したほうが実機CRTの滲みに近い。4:3の方針と同じ)。
  没入モード(全画面)は `fit` をそのまま使う設計で1倍以上でも端数倍が常態のため、
  1倍未満だけでなく没入モードにも効かせる。非没入かつ `fit>=1` のCSS整数倍経路は従来どおり。

## CPUクロックの走行中変更と ∞MHz(ホスト次第)

### なぜ速度倍率では「速い X68000」にならないか

速度倍率(`#btn-speed`)はホスト側のフレーム供給ペースを変えるだけで、エミュレートされる
マシン全体が同率で速く回る。CPU も CRTC も MFP タイマーも同じ比率で進むため、**ゲスト内で
測った比は一切変わらない**。実測(公開サイト・`?cpu=10`・チャチャベンチ):

| 速度 | ベンチ所要(実時間) | 結果 |
|---|---|---|
| 等倍 | 13,401 ms | 0079往復/10sec. |
| 無制限 | 1,561 ms | 0079往復/10sec. |

数字が同じなのは正常で、むしろ**倍率で数字が動くベンチのほうが壊れている**。ベンチの値を
伸ばしたい/実機より速い X68000 として使いたい場合に動かすのは `px68k_cpuspeed` のほう。

### GET_VARIABLE_UPDATE を実装しないとコアはオプションを二度と読まない

px68k-libretro は `retro_run()` の先頭で `RETRO_ENVIRONMENT_GET_VARIABLE_UPDATE`(cmd 17)を
毎フレーム問い合わせ、**真のときだけ** `update_variables()` を呼ぶ。`libretro-host.ts` は
この case を持っておらず `default: return 0` に落ちていたため、`setCoreOption()` を呼んでも
起動時に読まれた値のまま変わらなかった(UIが「CPU速度の変更にはリセットが要る」としていたのは
コア側の制約ではなくこれ)。

対応として `coreOptionsDirty` フラグと cmd 17 の case を足した。ただし**全オプションを
走行中更新にすると `px68k_ramsize` のようにマシン構成に属するものまで巻き込む**ため、
`LIVE_UPDATABLE_CORE_OPTIONS` のホワイトリストに載せたキーだけ「変わった」と申告する。
現在は `px68k_cpuspeed` のみ(`Config.clockmhz` を毎フレーム参照するだけなので走行中に
変えてよい)。

実測(計測用コアの水平フロントポーチ発生レートで観測): Human68k のプロンプトで走らせたまま
10MHz → 100MHz へ変更すると、リセット無しで 30,688回/秒 → 315,970回/秒 に切り替わった。

### ∞MHz の制御ループ

`cpuSpeed === 'auto'` のとき、1秒ごとに **実際に実時間を維持できたか** を見て上げ下げする。

- `achieved` = 直近区間で走ったエミュフレーム数 ÷ 期待フレーム数
- `achieved >= 0.99` なら上げる / `< 0.97` なら下げる / `< 0.9` なら一気に 0.7 倍へ
- 一度でも下げたら微調整幅(±5%)へ切り替える。それまでは粗く(+25%/-20%)
- 範囲は `CPU_MHZ_MIN`(10)〜`CPU_MHZ_MAX`(1000)、初期値 16MHz

**見積もりではなく結果で判定する。** 当初はコア実行1フレームの実時間が実時間の6割に収まる
範囲、という見積もりで判定していたが、それでは画面提示・音声・UIのコストを勘定に入れられず、
常に4割の余力を残したまま止まる。「ホストが許すかぎり」と言いながら限界の手前で頭打ちに
なっていた(実測: 同一ホスト・同一ディスクでベンチ値 0309 → 0399)。

ただし実時間を維持できていてもメインスレッドを食い尽くすと画面もUIも止まるため、コア実行
...
### 追いつけない設定でも操作を殺さない(占有率ゲート)

コアが実時間に追いつけない設定(高いCPUクロック、非力な端末)では、**1フレームの実行そのもの**が
実時間1フレームより長くなる。`computeFrameBudget()` は遅れを取り戻そうとフレーム数を増やすため、
tick同士が隙間なく連続してブラウザにイベントを配る時間が残らない。

実測(同一マシン・ローカルの本番ビルド同士で背中合わせ。`cpu=16` から `800Mhz (OC)` へ切替):

| | 旧 | 新 |
|---|---|---|
| JSの生存 | **完全停止**(1秒間隔の `setInterval` すら発火しない) | 生存 |
| rAF | — | 約21fps |
| `setTimeout(0)` | — | 約4.6ms |
| ゲスト速度 | — | 実測 10〜14% |

旧実装ではキーもツールバーのボタンも一切効かなくなる。JSが動かないのでブラウザがイベントを
配れないためで、キー処理の不具合ではない。

対策として実時間ベースの占有率ゲートを置いた(無制限モードの `UNLIMITED_MAX_DUTY` と同じ考え方)。
コア実行に C ミリ秒かかったら、次を始めるのは `C / FRAME_LOOP_MAX_DUTY`(0.85)ミリ秒後まで待つ。

**効かせるのは `frameCostMs > 1000/fps` のときだけ**、つまり1フレームの実行そのものが実時間
1フレームより長い=どうやっても追いつけない状態に限る。追いついている通常時・倍速時はこの条件が
偽なので、従来と完全に同じ経路を通る。**倍速を潰さないための線引き**であり、`frameInterval`
基準にすると倍率を上げるほど上限が縮んで倍速そのものを殺してしまう。
...
意図的に fps より遅く供給する。分母を `fps` にすると `achieved` が構造的に 1 未満へ張り付き、
下げ判定が出続けて**クロックが際限なく落ちる**。実測では 61 → 39 → 10MHz まで落ち込んだ。
「追いつけているか」を測るなら、比較対象は理論値ではなく**自分が狙った値**でなければならない。

**∞MHz のあいだは速度倍率ボタンを無効化する**(`updateSpeedAvailability()`)。どちらも同じ
ホスト時間を奪い合うため、両方を無制限にすると互いに意味を失う。起動時に全ツールバー
ボタンを一括で有効化する箇所があるので、その後にもう一度この関数を通して上書きしている。

コア側は `px68k_cpuspeed` が選択肢の文字列に一致しない場合、先頭の10進数を MHz として読む
(`PX68K_CLOCK_MHZ_MIN`〜`PX68K_CLOCK_MHZ_MAX`)。フロントエンドが任意のクロックを渡せるのは
このフォールバックによる。

### 制限

∞MHz は測定中にもクロックが動くため、**ベンチマークの数字は実行ごとにばらつく**。
比較可能な数字が要る場合は固定クロックを選ぶこと。実測では同じ端末でも
アイドル時 76MHz / ベンチ実行中 61MHz と動いた。

## HostFS(リモートデバイス、フォルダを直接ドライブとして見せる)

ブラウザで選んだフォルダを、Human68kのリモートデバイス(ブロックデバイスではない
論理デバイス、属性bit13)としてゲストへ見せる機能。ゲスト側はHOSTFS.SYS
(`tools/x68/hostfs.s`)という薄いデバイスドライバで、Human68kからのファイル操作要求を
そのままI/Oポート`$e8c000`経由でホスト(TypeScript、`src/hostfs/`)へ転送するだけの
作りにしている。実際の一覧・読み出し・検索・cdは全部ホスト側(`src/hostfs/dispatcher.ts`)
で処理する。

**この節の内容はすべて実機相当のHuman68k(WebX68k上のhuman302.xdf)を使った実測と、
PRO-68K DOS呼び出しガイドの記載に基づく。XM6等の他エミュレータの実装は参照していない。**

### リクエストヘッダの構造(実測)

`src/hostfs/dispatcher.ts`の先頭コメント、`tools/x68/remote-probe.s`(masterのC1〜C11実験)
で確定した構造。オフセットはヘッダ先頭からのバイト位置。

| オフセット | 内容 |
| --- | --- |
| +2 | コマンドコード(1バイト) |
| +3〜+4 | 状態(ドライバ/ホスト側からは常に0のまま。書き換えない) |
| +13 | `$47`(検索・初回)のときだけ検索属性 |
| +14 | `$47`/`$4a`/`$42`/`$43`/`$44`/`$45`/`$46`/`$49`のときは`_NAMESTS`へのポインタ(`$44`は元の名前)、`$41`のときはcdパス(区切り`$09`・NUL終端の生バッファ)、`$50`/`$4c`/`$4d`のときは出力先・バッファへのポインタ |
| +18 | `$47`/`$48`のときはFILBUFへのポインタ(入力)。`$4c`/`$4d`のときは要求長(入力)→実際に読み書きしたバイト数(出力)。`$44`のときは新しい名前の`_NAMESTS`へのポインタ(入力)。`$49`のときは_CREATE/_NEWFILEの区別を示すフラグ(入力、下記参照)。`$4f`のときはDATETIME(入力=0で取得・非0で設定/出力=結果、下記「$4f(_FILEDATE)」参照)。**それ以外のコマンドでは戻り値(出力、long)** — 「戻り値は+18」というのは、この欄が入力(ポインタ/長さ/フラグ/DATETIME)と出力(エラーコード/読み書きした長さ/日時)を兼ねているという意味 |
| +22 | `$4a`/`$4c`/`$4b`/`$4d`/`$49`/`$4f`のときはFCBへのポインタ(`$49`は出力、それ以外は入力) |

**状態欄(+3〜+4)は「デバイスエラー」の扱い**: Human68kのデバイスドライバは状態欄の
特定ビットが立っているとハードウェアエラーとして`$1003`などのエラーを返す規約になって
おり(master `remote-probe.s`のC1〜C3実験で確認)、HostFSでは常に0のまま(=エラー無し)
にして、成功/失敗は+18の戻り値だけで表現している。状態欄を使ったエラー通知はしない。

### コマンド番号と対応状況

| コマンド | 意味 | 同期/非同期 | 対応 |
| --- | --- | --- | --- |
| `$40` | 初期化(デバイスドライバ組み込み) | 同期 | ドライバ(`tools/x68/hostfs.s`)は自分の応答(+13/+14/+18/+3/+4)を書く前に、この要求をホストへ通知として転送する(親からの追加指示)。ホスト側(`HostFsDispatcher.handleInit`)はヘッダを一切書き換えず、+22(割り当てられたドライブ番号)を記録するだけ |
| `$41` | cd(カレントディレクトリ変更) | 同期 | 対応(P2a #1) |
| `$42` | `_MKDIR`(ディレクトリ作成) | **非同期** | 対応(W2b) |
| `$43` | `_RMDIR`(ディレクトリ削除) | **非同期** | 対応(W2b) |
| `$44` | `_RENAME`(改名) | **非同期** | 対応(W2b) |
| `$45` | `_DELETE`(ファイル削除) | **非同期** | 対応(W2b) |
| `$46` | `_CHMOD`(属性の取得/設定) | **非同期** | 対応(W2b) |
| `$47` | 検索・初回(ワイルドカード+属性) | **非同期**(Promise、ポーリングで完了) | 対応 |
| `$48` | 検索・次を返す | 同期(結果は`$47`実行時に確定済み) | 対応 |
| `$49` | `_CREATE`/`_NEWFILE`共用(新規作成) | **非同期** | 対応(W2b) |
| `$4a` | 開く | **非同期** | 対応(W2a以降は読み書き両用として開く) |
| `$4b` | 閉じる | 書き込みが無ければ同期、あれば**非同期**(実体への反映を待つ) | 対応 |
| `$4c` | 読む | 同期(内容は`$4a`/`$49`で取得済みのローカルバッファから) | 対応 |
| `$4d` | `_WRITE`(書く) | 同期(ローカルバッファへ反映するだけ。実I/Oは`$4b`で行う) | 対応(W2b) |
| `$4e` | シーク | 同期 | 対応 |
| `$4f` | `_FILEDATE`(日時の取得/設定) | **非同期** | 対応(本節末尾「$4f(_FILEDATE)」参照) |
| `$50` | 空き容量 | 同期 | 対応(値は本節末尾「$50空き容量」参照) |
| `$51` | `_DRVCTRL`(ドライブの状態問い合わせ/制御) | 同期 | 対応(MD=0のみ。本節末尾「$51(_DRVCTRL)」参照) |
| `$52` | `_GETDPB`(DPB取得) | — | **意図的に未実装のまま(未対応、-2で応答)**。リモートドライブでDPBの中身をどう埋めるか未計測のため、実測が揃うまで見送る |
| `$56`/`$57` | 不明(未解読) | 同期 | +18=0を返すだけ(実害の無いダミー応答) |
| 上記以外 | — | — | 未対応(-2で応答) |

`$47`と`$4a`だけを非同期にしているのは、「保留→ポーリング→完了」という本機能の
土台になる経路を、実際のディスク一覧・ファイルオープンという現実的な操作で
最低1回は通すため。W2b(書き込み系)の各コマンドも同じ経路(`HostFsDispatcher.beginAsync`)
に乗せている。

### VERIFY ON時はコマンド番号の最上位ビットが立つ(実測)

CONFIG.SYSで`VERIFY = ON`にすると、Human68kはリモートデバイスへ送る**コマンド番号
(+2)の最上位ビット($80)を立てて**送ってくる。実測(master `_local/hostfs-verify/v1〜v3*.json`、
human302.xdfへ`VERIFY = ON`を書いたCONFIG.SYSで起動):

- `dir`で届いたコマンド: `$d7`, `$c1`, `$c7`, `$d0`
- `type`で届いたコマンド: `$d7`, `$c1`, `$c7`, `$ca`

いずれも`$80`を外すと`$57`(未解読)/`$41`(cd)/`$47`(検索)/`$50`(空き容量)/`$4a`(開く)
という、既に対応済みのコマンドと一致する。PRO-68Kマニュアル第6章でブロックデバイスの
VERIFY ON時に書き込みコマンドが8→9になるのと同じ役割とみられる。

`HostFsDispatcher.request()`の入口で`verify = (cmd & 0x80) !== 0; cmd &= 0x7f;`として
印を外し、以後は元のコマンド番号として処理する(ヘッダそのものは書き換えない)。
未知コマンドの判定・記録は印を外した後の番号で行う(例: `$d1`は未知の`$51`として
記録する)。印付きで届いたかどうかは診断情報(HostFsUnknownCommandInfo.verify)に
1項目として残す。`$4d`(書く)で印が立っていても、ホスト側の書き込みはブラウザの
ファイル書き込みAPIで完結しており実ディスクのような取りこぼしが無いため、印を
受け取るだけで読み返しはしない。

### 検索($47/$48)の一致条件

- 名前8+拡張子3(空白埋め)を`_NAMESTS`から取り出し、`?`をワイルドカード(任意の1文字)
  として大文字小文字を無視で照合する。名前を無視して全件返すと、同じファイルが
  複数回見つかる不具合が実測で出たため、この照合は必須(`dispatcher.ts`のコメント)。
- 属性は「(エントリの属性 & 検索属性) != 0」で一致とする。PRO-68Kマニュアルp.184の
  「2つ以上のビットを立てた場合は、そのどれかに当てはまればよい」という記載どおり。
...
| --- | --- |
| `0` | 成功 |
| `-2` | ファイルが見つかりません(`DOS_ERR_NOT_FOUND`) |
| `-3` | ディレクトリが見つかりません(cd、`DOS_ERR_DIR_NOT_FOUND`) |
| `-18` | 該当ファイルなし(`$48`で次が無い、`DOS_ERR_NO_MORE_FILES`) |
| `-19` | 書き込み禁止です(`DOS_ERR_WRITE_PROTECTED`。読み取り専用モードでの書き込み系コマンド全般、
  および`move()`未対応環境でのディレクトリrenameに使う) |
| `-20` | ディレクトリがすでにあります(`$42`、`FS_ERR_DIR_EXISTS`) |
| `-21` | ディレクトリの中にファイルがあります(`$43`、`FS_ERR_DIR_NOT_EMPTY`) |
| `-22` | リネーム先がすでにあります(`$44`、`FS_ERR_RENAME_TARGET_EXISTS`) |
| `-23` | ディスクが一杯です(実書き込みの失敗、`QuotaExceededError`、`FS_ERR_DISK_FULL`) |
| `-25` | 指定の位置にはシークできません(`$4e`、`DOS_ERR_CANT_SEEK`) |
| `-80` | ファイルがすでに存在します(`$49`の_NEWFILE相当、`FS_ERR_FILE_EXISTS`) |

### 書き込み(W2a/W2b)

つなぐとき(フォルダ選択ダイアログ)に読み取り専用/書き込みも許可のどちらでATTACHするか
選ぶ(`HostFsConnectMode`、`'read'` / `'readwrite'`)。バックエンド
(`HostFolderFs.isWritable()`)がfalseなら、書き込み系コマンド(`$42`/`$43`/`$44`/`$45`/
`$46`の設定/`$49`/`$4d`)はすべて`-19`を返す。`$46`の取得(+13=`$FF`)だけは読み取り操作
なので読み取り専用モードでも成功する。

- **削除・上書きはそのまま反映する(ゴミ箱は作らない)**。`removeEntry`/
  `createWritable().close()`をそのまま使う。
- **閉じたときにまとめて反映する**。`$4a`(開く)/`$49`(作る)いずれの経路で開いても、
  以後の`$4c`(読む)/`$4d`(書く)/`$4e`(シーク)はディスパッチャが持つローカルバッファ
  (FCBポインタをキーにした`FcbState.content`)に対して行うだけで、実際のホスト書き込みは
  一切発生しない。`$4b`(閉じる)のときだけ、変更があれば(`dirty`)
  `HostFileSystem.openWrite()`→`write()`→`close()`(File System Access APIの
  `createWritable()`→`write()`→`close()`)で実体へ原子的に反映する。この反映は非同期
  なので、既存の「保留→ポーリング→完了」の経路(`beginAsync`)をそのまま使う
  (`$4b`は書き込みが無ければ同期即完了、あれば非同期になる)。
- **日時・属性はホストに保存できない**。`setFileDate`/`setAttr`は、書き込み可能モードなら
  何もせず成功(`FS_OK`)を返すだけ(File System Access APIに最終更新日時や属性を設定する
  手段が無いため)。`_FILEDATE`(`$FF57`)はそもそもHuman68kがドライバへ渡す前に`-6`で
  打ち切ることを実測で確認済み(開いたハンドル・閉じたハンドルのどちらでも同じ)。今回は
  このコマンド自体をディスパッチャで扱わない。
- **ディレクトリのrenameは`move()`が使える環境だけ**。`HostFolderFs.rename()`は、
  ファイルは`move()`があればそれを使い、無ければコピー+削除で代替する。ディレクトリは
  `move()`が無ければ`-19`を返す(コピー+削除による代替はディレクトリでは行わない)。
  `$44`のヘッダにはisDir相当の情報が無いため、ディスパッチャは`fs.getAttr()`で元エントリの
  属性を調べてからこの判断に回している(`handleRename()`)。
- **名前の変換・照合はW2a(読み取り)と同じ実装を使う**。ゲスト→ホストの新規名は
  `name-convert.ts`の`convertGuestNameToHostFileName`(CP932符号化)、既存ファイルの
  大文字小文字を無視した照合は`host-folder-fs.ts`の`findExistingByGuestName`。

#### `$49`(`_CREATE`/`_NEWFILE`共用)の+18フラグ

PRO-68Kマニュアル上は別々のDOSコール(`_CREATE`=`$FF3C`は既存なら上書き、`_NEWFILE`=
`$FF5B`は既存なら`-80`)だが、ドライバへは同じコマンド番号`$49`で届き、+18の値だけで
区別する。master `tools/x68/remote-probe.s`のC12実験、および本セッションの実測
(`tools/x68/write-test.s`のステップ1で`_CREATE`、ステップ14で`_NEWFILE`を実行し比較)で
以下を確認した:

- `_CREATE`のときは+18=**1**。
- `_NEWFILE`のときは+18=**0**。

ディスパッチャ(`handleCreate()`)は`flagIn === 0`のときだけ`failIfExists=true`
(`_NEWFILE`相当、既存なら`FS_ERR_FILE_EXISTS`=`-80`)として`HostFileSystem.createFile()`を
呼ぶ。実測(`_local/hostfs/probes/w2b-a2-writetest.json`、`14 NEWFILE D0=FFFFFFB0`=`-80`、
既存の`HELLO.TXT`に対して実行)で、この解釈が正しいことを確認済み。

成功時(`$49`)は、以後の`$4d`/`$4c`/`$4e`/`$4b`で使う書き込み用ファイルとして、
空のローカルバッファをそのFCBポインタへ登録する(`_CREATE`で上書きした場合も、
`_NEWFILE`で新規作成した場合も、ホスト側の実体は常に空になっている)。

#### 実測: `tools/x68/write-test.s`によるE2E確認(2026-09-12)

`_CREATE`→`_WRITE`→`_CLOSE`→`_OPEN`→`_SEEK`(末尾)→`_WRITE`→`_CLOSE`→`_FILEDATE`
(取得/設定)→`_CHMOD`(取得/設定)→`_RENAME`→`_DELETE`→`_MKDIR`→`_RMDIR`→`_NEWFILE`の
一連を実機相当のHuman68k上で実行し、各ステップのD0と、OPFS実体(`hostfs-test/`配下)の
両方を確認した。

- 1回目の`_WRITE`から正しく書けたバイト数(D0=`$0D`=13)が返った。実験用ドライバで
  観測された「`_CREATE`直後の最初の`_WRITE`だけD0=0になる」という現象は、この実装
  (ディスパッチャがFCBポインタをキーにローカルバッファへ同期的に書き込むだけの設計)
  では再現しなかった。実験用ドライバ側の別要因(記録専用の追従処理など)によるものと
  見られるが、原因までは確認していない。
- `_FILEDATE`は取得・設定ともD0=`-6`(ドライバに届かない)。**この`write-test.s`のステップは
  直前の`_CLOSE`(手順7)で閉じた**同じ**FCBに対して呼んでいる**ため、Human68k自身が
  「無効なファイルハンドル」として弾いていて、そもそもHostFSポートまで来ていない
  (このコマンド自体は下記「`$4f`(`_FILEDATE`)」のとおり対応済みで、`$4f`は
  実装後も同じ`-6`のまま。ドライバ側のコンソールログに`$4f`の受信自体が
  一切現れないことで裏取りした)。開いたまま(closeしていない)FCBに対して
  呼ぶcopyコマンドの経路では、下記のとおり実際にHostFSポートへ届く。
- `_CHMOD`取得はD0=`$20`(通常ファイル)、設定はD0=`0`(成功)。
- `_RENAME`→`_DELETE`→`_MKDIR`→`_RMDIR`はすべてD0=`0`(成功)。OPFS実体側でも、
  一時的に作られた`NEW.TXT`→`RENAMED.TXT`→削除、`NEWDIR`→削除がすべて反映され、
  最終的にHostFSのテスト用フォルダには元からあった検証用ファイルしか残らないことを
  確認した(ゲスト画面の表示だけでなく、OPFSの実体を直接読んで裏取りした)。
- `_NEWFILE`(既存の`HELLO.TXT`に対して実行)はD0=`-80`。

読み取り専用モード(`?hostfs=opfs-test`)で同じDOSコマンド列(`echo`/`copy`/`type`/`ren`/
`del`/`md`/`rd`)を実行すると、`echo`は「出力が作れません」、`copy`は「書き込み禁止です」
「コピーはされませんでした」、`ren`/`del`は「ファイルがありません」相当、`md`は
「ディレクトリが作れません」とゲストに表示され、OPFS実体には一切変化が無いことを
確認した。

### `$4f`(`_FILEDATE`)

+18=DATETIME(0なら取得・非0なら設定、`$4c`と同じ入出力兼用の番地)、+22=FCB。
PRO-68KマニュアルP.195のフォーマットそのまま(上位ワード=日付`(年-1980)<<9|月<<5|日`、
下位ワード=時刻`時<<11|分<<5|秒/2`。FILBUFの日付/時刻ワードと同じエンコード)。

- **取得**(DATETIME=0): `HostFileSystem.getFileDate()`でホストの`lastModified`を
  読み、上記フォーマットへ詰めて+18へ返す。読み取り操作なので`getAttr`と同じく
  読み取り専用モードでも成功する。ディレクトリはlastModifiedを持たないため
  `DIRECTORY_DATE`/`DIRECTORY_TIME`(1980-01-01 00:00)を返す(`listDir`と同じ扱い)。
- **設定**(DATETIME≠0): File System Access APIには最終更新日時を書き換える手段が
  無いため、`HostFileSystem.setFileDate()`は書き込み可能モードなら何もせず成功
  (`FS_OK`)を返すだけ(既存実装のまま、値は使わない)。成功時は、ホストへは実際には
  反映できないため入力のDATETIMEをそのまま+18へ返す(マニュアルの「D0.Lは
  DATETIMEと同じフォーマット」という記載に合わせた実装上の判断で、実測による
  裏取りではない)。読み取り専用モードでは、他の書き込み系コマンドと同じく`-19`。
- 該当FCB(`fcbStates`に無い、想定外)は`-2`(`DOS_ERR_NOT_FOUND`)。

#### 実測(w2c、`tools/x68/hostfs-dostest.xdf`の`COPY A:AUTOEXEC.BAT C:`、2026-09-12)

`$4f`が未対応だった時点では、`COPY A:AUTOEXEC.BAT C:`実行中に`[HostFS] 未知のコマンド:
$4f`という警告が1回出て、画面にも「ファイルがありません」が1行混ざっていた
(コピー自体は成功していた)。ヘッダをダンプして実測したところ:

```
hdr = 1a 00 4f 00 00 05 00 18 42 f0 00 00 02 00 00 00 00 00 5d 2c 4a e6 00 01 9a 4c
+14(u32) = 0
+18(u32) = 0x5d2c4ae6  ← DATETIME(非0なので「設定」)
+22(u32) = 105036      ← FCB(直前の$4dで書き込んだCOPY先ファイルのFCBと同じ)
```

`0x5d2c4ae6`を上記フォーマットで復号すると年月日=2026-09-12・時分秒=09:23:12前後
(実行時刻の実時計とほぼ一致)になり、`+14`は0のまま(未使用)だった。このことから:

- **確定**: `$4f`のDATETIMEは`+18`(`$4c`と同じ入出力兼用の番地)にあり、`+14`は
  このコマンドでは使わない。`+22`はFCB。
- **推測**: この呼び出しは「設定」で、値はゲストの現在時刻とみられる。つまり
  COPYコマンドは、コピー元の日付を引き継ぐのではなく、コピー先ファイルに
  「今」の日付を明示的に付け直している可能性がある(逆アセ等では確認していない、
  実測できた1件からの推測)。**「取得」(DATETIME=0)の経路は、このE2Eでは1回も
  観測できていない**(未解決)。

この実装(`$4f`をディスパッチャへ配線)後、同じ`COPY`を再実行すると「ファイルが
ありません」は出なくなり、`未知のコマンド: $4f`の警告も出なくなった。OPFS実体側にも
`AUTOEXEC.BAT`(186バイト、コピー元と同じ内容)が作られていることを確認した。

### `$50`空き容量

出力欄(+14が指すポインタ先)へ、PRO-68K DOS呼び出しガイドp.150の記載どおり
使用可能クラスタ/総クラスタ/クラスタあたりセクタ/セクタあたりバイトの順でワード
(各2バイト)ずつ書く欄がある。HostFSは実容量を持たないため作り物の値を返すしかないが、
**この8バイトだけでは「使用可能」欄の表示は決まらず、ヘッダ+18(通常は戻り値を書く欄)
に「使用可能バイト数」(_DSKFREがD0で返す値と同じ)を書く必要がある**ことが、
masterの`tools/x68/remote-probe.s`のC8実験で判明している:

- C8-a(+18に204800だけを書き、8バイト構造は書かない)→ dir集計が
  「536870712K」等に壊れる。
- C8-b(+18に204800を書き、かつ8バイト構造に使用可能クラスタ100/総クラスタ200/
  クラスタあたり2セクタ/セクタあたり1024バイトを書く)→ dir集計行が
  「200K Byte 使用中 / 200K Byte 使用可能」と正しく出た(master側
  `_local/remote-probe/c8b-dir-final.png`)。検算: 総バイト数200×2×1024=400K、
  使用可能(+18)=204800B=200K、400K−200K=200K=表示された「使用中」と一致する。

このため実際の意味は「+18=使用可能バイト数、使用中=(総クラスタ×クラスタあたり
セクタ×セクタあたりバイト)−+18」であり、8バイト構造の「使用可能クラスタ」欄(+0)
自体は表示に使われていないと見られる。(2026-09-12の1回目の実測では+18を常に0の
まま試していたため、「使用可能は常に0Kになる」という誤った結論を出していた。
その後の再実測でこの読み方に修正した。)

ホストの実際の空き容量はブラウザから取得できないため、WebX68kのHostFSはこの読み方に
基づき名目上の値を返す: FDXファイラの表示欄が9桁しかなく(実測: 2026-09-13、
2,147,450,880/1,048,576,000では先頭桁が欠けて「Total 147,450,880 /
Free 048,576,000」と表示された)、9桁に収まる最大値「999,997,440」がカンストした
ように見えるものを選んだ。総クラスタ=使用可能クラスタ=61035(使用中0)・
クラスタあたり32セクタ・セクタあたり512バイト(クラスタ=16384B)。
使用可能バイト数(+18)=61035×16384=999,997,440B。XM6等の他実装は
参照しておらず、すべてこの実測とPRO-68Kマニュアルに基づく。詳細は
`src/hostfs/dispatcher.ts`の`handleFreeSpace()`のコメント参照。

### `$51`(`_DRVCTRL`)

FDXファイラがD:へのドライブ切り替え直前に呼ぶ(実測: 2026-09-13、`MODE = MD*256+DRIVE`の
`MD`は常に0で届く)。ヘッダ+13が`MD`、+18が戻り値(呼び出し元へD0.Lとしてそのまま返る)。
+14のポインタ(用途不明)には書き込まない。

`MD=0`(状態問い合わせ)はXC20プログラマーズマニュアルp.97-98のビット定義に従う
(D01=メディア挿入、D02=NOT READY、D03=WRITE PROTECT、D06=イジェクト禁止、他省略。
PRO/RDYはメディア挿入時のみ意味を持つ)。実測(実ドライブ): 準備できたSASI HD=`$42`
(D06|D01)、空のフロッピー=`$0C`。WebX68kのHostFSはこれに合わせ、以下の3状態を返す
(`src/hostfs/dispatcher.ts`の`DRVCTRL_STATUS_*`/`handleDrvCtrl()`参照):

| HostFSの状態 | 戻り値(+18) | 内訳 |
| --- | --- | --- |
| フォルダ未接続 | `$04` | NOT READY(D02)のみ。空のフロッピーと同じ成分 |
| フォルダ接続・書き込みモード | `$42` | イジェクト禁止(D06)+メディア挿入(D01)。実測のSASI HDと同じ形 |
| フォルダ接続・読み取り専用モード | `$4A` | 上記にWRITE PROTECT(D03)を足したもの |

`MD>=1`(イジェクト・イジェクト禁止設定等の制御系)はマニュアルに戻り値の規定が無く、
実装もしていない。+18=0(成功扱い)を返すだけで完了する。

`$52`(`_GETDPB`)はDPB(ドライブパラメータブロック)の中身をリモートドライブで
どう埋めるべきか未計測のため、意図的に未実装のまま残してある(引き続き未知コマンドと
して-2で応答・記録される)。
...
ディスクライブラリの各行(HDD、またはFAT12/16として扱えるFD)から、対象ディスクの
ルートへ`HOSTFS.SYS`を書き、`CONFIG.SYS`に`DEVICE = \HOSTFS.SYS`を追加できる
(`src/hostfs/install-hostfs.ts`、`src/main.ts`の`installHostFsOnLibraryEntry`)。
書き込みは既存のファイル転送と同じ`fatReadFile`/`fatWriteFile`(`src/api/fat.ts`)を
使う。起動中にマウント中のディスクには書かず、同梱システムディスク
(`public/system/human302.xdf`)は書き換えずにコピーを作ってから組み込む。
`public/system/hostfs.sys`は`tools/x68/hostfs.s`から`tools/x68/build-hostfs.sh`で
アセンブルしたビルド済みバイナリの同梱で、`scripts/verify-hostfs-sys.sh`で
ソースからバイト一致で再現できることを検査できる。
...
- SCSI ハードディスクは HLE 実装のため、起動ディスクの構成によっては SCSI ドライブが
  現れないことがあります(起動そのものは妨げません。詳細は「SCSI ハードディスク(HLE)」節)。
