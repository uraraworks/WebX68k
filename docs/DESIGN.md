...
出力欄(+14が指すポインタ先)へ、使用可能クラスタ/総クラスタ/クラスタあたりセクタ/
セクタあたりバイトをこの順でワード(各2バイト)ずつ書く(PRO-68K DOS呼び出しガイド
p.150)。HostFSは実容量を持たない(読み取り専用のホストフォルダ)ため、どの欄も
ワード(0-65535)に収まる、もっともらしい値を作り物として返している:
クラスタ=32KB(512B×64セクタ)、総クラスタ32768(=1GB)、使用可能32000クラスタ
(≒1000MB)。詳細は`src/hostfs/dispatcher.ts`の`handleFreeSpace()`のコメント参照。
