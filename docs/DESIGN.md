...
| +14 | `$47`/`$4a`のときは`_NAMESTS`へのポインタ、`$41`のときはcdパス(区切り`$09`・NUL終端の生バッファ)、`$50`/`$4c`のときは出力先・バッファへのポインタ |
| +18 | `$47`/`$48`のときはFILBUFへのポインタ(入力)。`$4c`のときは要求長(入力)→実際に読んだバイト数(出力)。**それ以外のコマンドでは戻り値(出力、long)** — 「戻り値は+18」というのは、この欄が入力(ポインタ/長さ)と出力(エラーコード/読んだ長さ)を兼ねているという意味 |
| +22 | `$4a`/`$4c`/`$4b`のときはFCBへのポインタ |
...
| `-19` | 書き込み禁止です(`DOS_ERR_WRITE_PROTECTED`。定数は用意済みだが、書き込み系コマンド番号が
  未確定のため現状は未使用) |
