...
`cpuSpeed === 'auto'` のとき、`host.runFrame()` 1回の実時間を指数移動平均で測り、
1秒ごとに次の判定をする。

- コスト > 実時間1フレーム × `AUTO_CLOCK_TARGET_DUTY`(0.6) なら 0.8 倍に下げる
- コスト < その予算の 0.7 倍なら 1.25 倍に上げる
- 範囲は `CPU_MHZ_MIN`(10)〜`CPU_MHZ_MAX`(1000)、初期値 16MHz

予算を「実時間の一部」に切ってあるのは、コア実行がメインスレッドを使い切ると画面提示も
UI操作も入らなくなるため(無制限モードで実測した失敗と同じ理由)。上げ側だけ閾値を 0.7 倍に
寄せてあるのは境界で上下に往復し続けないため。
