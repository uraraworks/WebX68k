...
1. 負荷記録つきの新ハーネスで、**1組目から取り直す**。旧ハーネスで取った set1・set2 は基準値としては使わない（記録としては残す）。
2. 計測前後に `verdict` が `quiet` であることを確認し、`contended` だった組は基準値に採らない。
3. 2組目は別日に、同じくアセットのハッシュ一致と `verdict: quiet` を条件に取る。
4. 音声の `outputLatency` の変動（0.168秒／0.016秒）の原因を切り分ける。これが揃わない限り音声の日差は解釈できない。
