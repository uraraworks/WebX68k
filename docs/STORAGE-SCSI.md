...
### 主指標「KeyBuf 2回目」は移行前基準と比較可能か

**断定はできない。** 実測できた事実は以下:

- 既定経路の今回の中央値(5.915ms)は、移行前基準の「2回目」系列(4.3〜4.9ms)にほぼ近い値で、大きくは外れていない(「1回目」系列8.9〜12.5msほどには離れていない)。ただし今回は1組しか取っておらず、これが「1回目」相当か「2回目」相当かは、この1組だけでは決められない(移行前基準の「1回目/2回目」は同日に2回連続でスクリプトを起動して得た値であり、今回はどちらの経路も1回しか起動していない)。
- Worker経路の中央値(47.835ms)は既定経路の約8倍で、想定した「最大1フレーム(約16.7ms)の遅れ」という見積もりだけでは説明しにくい大きさである。上記「この設計の限界」で述べた「入力反映の遅れ」+「frame event相乗りの遅れ」の2段が効いている可能性、Worker側の駆動ループ(`setInterval`+accumulator、rAFではない)自体のタイミング精度、あるいは今回1件見つかったような後始末の食い込みに近い、リング読み取りタイミングの構造的なずれが複合している可能性があるが、**介入実験で切り分けたわけではないので原因は特定できていない**。
- したがって、**Worker経路のKeyBuf 2回目が移行前基準4.3〜4.9msと同じ意味で比較可能かどうかは、今回の実測だけでは判定できない。** 少なくとも「同じ物差しで測れている」(async化による往復遅延の上乗せが無い)ことは設計上・実装上保証したが、frame event相乗り方式自体が既定経路には無い遅延要因を持ち込んでおり、その遅延が主指標の水準を変えるほど大きいことは今回実測で確認できた。回帰判定にそのまま使うには、既定経路の「移行前基準の確定」節と同様に**複数組の反復測定が必要**であり、今回の1組だけでは「回帰した」とも「回帰していない」とも言えない。

### できなかったこと・未確認のこと

- 上記の理由により、Worker経路のKeyBuf計測結果を移行前基準(4.3〜4.9ms)との回帰判定にそのまま使うことはできない。反復測定(最低でも2組、できれば「移行前基準の確定」節と同様に複数組)が必要。
- Worker経路の中央値が既定経路の約8倍になる原因は特定していない(入力反映の遅れ・frame event相乗りの遅れ・駆動ループのタイミング精度・観測系自体の限界のどれがどれだけ効いているか、介入実験による切り分けは未実施)。
- 今回の主測定はいずれも1組(1回のスクリプト起動)のみ。日差・試行順序による変動(過去の「移行前基準:2組目の取得」節で見られたような外部負荷の影響)は考慮できていない。
- Worker経路の main trial は29/30成功(1件は上記の後始末食い込みによる誤読)であり、既定経路の30/30より劣る。この1件の性質(観測ハーネスの限界か、Worker経路特有の遅延が実際の計測に影響する範囲まで及ぶ実例か)の切り分けは今回行っていない。
- TVRAM経路も同様にWorker経路で中央値が約1.7倍(32.278ms→54.36ms)になっているが、こちらは元々ポーリング観測でありKeyBufほど厳密な基準を持たないため、今回は考察を深掘りしていない。

## 次にやること

移行前基準は2組そろい、**ワーカー移行に着手できる状態になった**。

1. ~~ワーカー移行の手順3(ステートと単純なFS転送)に進む。~~ → **2026-08-28 実施済み**（「ワーカー移行 手順3：ステートとFS転送の非同期化（実装）」参照）。~~次はワーカー移行の手順4(初期化、オプション、load/AV)に進む。~~ → **2026-08-28 実施済み**（「ワーカー移行 手順4：初期化・load/AVのスケルトン実装」参照）。~~手順5・7(映像・駆動ループ)に進む。~~ → **2026-08-28 実施済み**（「ワーカー移行 手順5・7：映像・駆動ループの実装、`?worker=1` を本体経路化」参照）。`?worker=1` でHuman68kが`A>`プロンプトまで到達することをdev・本番ビルド双方で確認済み。~~次はワーカー移行の手順6(入力)に進む。~~ → **2026-08-31、前半(キー・パッド・マウスボタン・加算マウスdelta・世代付きclear)を実装済み**（「ワーカー移行 手順6：入力の実装（2026-08-31）」参照）。単体テストと故障注入は通したが、~~実ブラウザでの動作確認~~ → **2026-08-31、コーディネータが実ブラウザ確認を行い、物理キーボード入力がゲストへ届かない欠陥を発見・修正、修正後は末端(TVRAM)まで届くことを確認済み**（「実ブラウザ確認で見つかった欠陥(2026-08-31)」「実ブラウザ確認(実測、2026-08-31)」参照。DIR実行・キーリピート・blur時の世代付きclearを確認、副産物として未移行のSRAMキーリピート追従がリピート速度の差(34文字 vs 20文字)として実際に出ていることも観測した）。**マウスの閉ループ追従(手順6の後半)、マウス入力(キャプチャ時の相対移動・ボタン)/ゲームパッドの実ブラウザ確認、実機(iOS/Android)確認はまだ行っていない。** ~~正式な計測ハーネスによる基準比較~~ → **2026-08-31、キー入力についてはKeyBufプローブをWorker対応させ、既定経路・Worker経路それぞれ1組ずつ実測した**（「KeyBufプローブのWorker対応と、Worker経路でのキー入力計測(実測、2026-08-31)」参照。故障注入3種は期待どおり検出。ただしWorker経路の主指標が既定経路の約8倍になっており、移行前基準4.3〜4.9msとの比較可否は1組だけでは判定できていない。反復測定が次の課題)。**起動・3ドライブ・音声の正式な基準比較はまだ行っていない。** 次はその点に進む。
