...
### 決定4：実ファイルへの同期I/O（COOP/COEP）は当面採らない

XM6 との資産のやりとりについて、3案を比較した。
...
**②を本命とする。**ブラウザは実ファイルへの同期書き込みを提供しておらず（`createSyncAccessHandle` は OPFS 専用）、③はその制約を回避するための重い仕掛けになる。②は標準APIの素直な使い方だけで「XM6のHDSを開いて、同じファイルに書き戻す」が成立する。

②の弱点は書き戻しが明示的なタイミングになることだが、これは現行の IndexedDB 保存と同じ性質であり、既に「平常時に短間隔で保存する」設計で対処している問題である。**書き戻しの契機（明示保存か自動か）は未決。**XM6 と共有するファイルを自動で書き換える是非があるため、別途決める。

なお COOP/COEP はページ単位のHTTPヘッダであり、「デスクトップのみ有効」にはできない。③を採る場合はスマートフォンを含む全アクセスに影響する。
...
- IndexedDB に載っていないことの確認は、`navigator.storage.estimate()` が OPFS と IndexedDB を合算した値を返すため、Chrome の `usageDetails`（内訳）が使えるかを先に確かめる。使えない場合は **IndexedDB のレコードを列挙してサイズを合計する**ほうが確実。
...
陽性対照として IndexedDB に 16MiB（`(i*13)&0xff` の周期パターン）を書いたところ、`usageDetails.indexedDB` の増分は **853,362 バイト**しかなかった。約20分の1に圧縮されている。OPFS 側は 16,777,394 バイトとほぼ生サイズだった。

決定3の受け入れ条件で「IndexedDB に載っていないこと」を `estimate()` の数字で確かめる場合、**圧縮率がデータ依存であるため増分の絶対値は根拠にならない**。既に書いていた「IndexedDB のレコードを列挙してサイズを合計するほうが確実」という方針を、この測定は裏づけている。
...
- **iOS WebKit は未確認。**同じページを実機で開く必要がある（`--serve` の LAN URL 経由。ただしFW許可は実体パスで判定される点に注意）。
...
1. **移行前基準の2組目を取る。**条件は (a) 同梱アセットのSHA-256が `newset1r-20260823-*` と一致、(b) `src/` が未変更、(c) 計測前後で `verdict: quiet`。**計測前にホストの静穏確認を行う**（`buildLoadReport` を直接呼ぶ。8/24はこれで見送りを判断した）。並行セッションのブラウザ・VM・dev サーバが競合するため、他の作業をしていない時間帯に取る。
2. キー入力1回目の KeyBuf が遅い件を2組目でも確認する。3組そろって再現すれば「1回目は暖機」として確定させる。
3. 音声の `outputLatency`（0.168秒／0.016秒）の切り分け。
4. **`persist()` の切り分け。**普段使いのプロファイルで `scripts/probe-opfs.mjs --serve` のページを開き、`false` が使い捨てプロファイル固有かどうかを見る。
5. **iOS WebKit での確認。**同じページを実機で開き、A〜Eが成立するかを見る。ここが崩れるとワーカー移行の対象範囲が変わる。
6. ワーカー移行の着手（2組目の取得後）。
