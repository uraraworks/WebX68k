...
1. **ワーカー移行** — SASI・MEMFSのまま。今日の基準値と突き合わせて回帰が無いことを確認する。`FileSystemSyncAccessHandle` がワーカー専用であるため、これは避けられない前提。
2. **SCSI HLE** — まず小さいテストイメージで、Human68kがドライブとして認識するところまで。I/Oの実体は仮でよい。
3. **I/Oの実体をOPFSへ差し替え** — 決定2のフックを差し替えるだけなので、壊れたら原因はそこに限定される。
4. **XM6のHDS取り込み** — ストリームでOPFSへ流し込む導線と、決定4の書き戻し。

2と3を分けられることで、「ドライブを認識しないのはIOCS実装のせいかI/Oのせいか」で切り分け不能になる事態を、フックの差し替えという形で構造的に避けられる。

## 次にやること

1. **2組目を別日に取る。**条件は (a) 同梱アセットのSHA-256が `newset1r-20260823-*` と一致、(b) `src/` が未変更、(c) 計測前後で `verdict: quiet`。`contended` の組は採らない。**計測前にホストの静穏確認を行い、残存プロセスがあれば止めてから取る**（今日はこれで1件止めた）。
2. キー入力1回目の KeyBuf が遅い件を2組目でも確認する。3組そろって再現すれば「1回目は暖機」として確定させる。
3. 音声の `outputLatency`（0.168秒／0.016秒）の切り分け。今日は2組とも0.016秒だった。
4. **HDSのバイト構造の調査**（上記「XM6のHDS資産の移行」の1）。計測やワーカー移行と独立して進められる。
5. 未確認事項：`navigator.storage.estimate()` の `usageDetails` が使えるか、iOS WebKit での OPFS 同期ハンドルとワーカー実機動作、OPFS の eviction と `navigator.storage.persist()`。
