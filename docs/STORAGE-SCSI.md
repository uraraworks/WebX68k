...
## 打鍵落ち修正・音声移行後の再計測(実測、2026-09-01 夜)

「make/breakの潰れの原因調査と修正」「ワーカー移行 手順5：音声出力の実装」の後、正式な計測ハーネスで起動・3ドライブ・音声を再計測した。条件はAC Power・内蔵スピーカー(`outputLatency: 0.032`)・コミット`0b19f58`・`dirty: false`(全ファイルで一致を確認)。**移行前基準(`newset2-20260825-*`)はBattery Power下で取られているため、絶対値の比較は引き続き成立しない**(宿題16、後述)。

### 1. 起動(最終値)

| ファイル | 経路 | verdict | 中央値 |
|---|---|---|---|
| `wm-20260901-boot-default-2.json` | 既定 | quiet(競合0) | 22,573.04ms |
| `wm-20260901-boot-worker.json` | Worker(音声未実装時) | quiet(競合0) | 24,619.6775ms |
| `wm-20260901-boot-worker-2.json` | Worker(音声移行後) | **contended**(`mds_stores`/`mds`、正規化中央値1.82) | 27,082.945ms(無効) |
| `wm-20260901-boot-worker-3.json` | Worker(音声移行後) | quiet(競合0) | **27,435.695ms** |

既定22,573.04ms・Worker(quiet、音声移行後)27,435.695msで**1.216倍**(27,435.695 / 22,573.04)。

経緯を辿ると、8/31時点で見えていた「1.3〜1.76倍」→CPU速度(Worker=10MHz/既定=16MHz)を揃えて**1.091倍**(`wm-20260901-boot-worker.json`、24,619.6775ms)と縮んだが、**この1.091倍は音声が未実装で仕事量が少ない状態の値だった**。音声移行後は1.216倍に戻っている。差分は24,619.6775→27,435.695msで**約2,815.9ms(≒2.8秒)**、これが音声処理の追加コストに相当すると見られる。

**1.091倍を「Worker化のコスト」として報告したのは誤りだった。** 音声未実装という条件差(仕事量が既定より少ない)を見落としたまま経路間コストとして扱っていた。区間内訳(`clickToWasmFetchComplete`/`wasmFetchCompleteToCoreReady`)はWorker経路で相変わらず`sampleCount: 0`のままで、この差がどの区間に乗っているかは今回も分からない(宿題14、変化なし)。

### 2. 3ドライブ

打鍵落ち(`input.attempts`中`matched: false`)の推移(実測件数を数え直した):

| ファイル | 状態 | 不一致件数 |
|---|---|---|
| `wm-20260901-drives-default.json` | 既定 | 0 |
| `wm-20260901-drives-worker.json` | 修正前 | 9 |
| `wm-20260901-drives-worker-2.json` | `e16c6fd`後(打鍵落ち未修正) | 5 |
| `wm-20260901-drives-worker-4.json` | 打鍵落ち修正(`a5aadbb`)後・contended(`backupd-helper`等) | **1** |
| `wm-20260901-drives-worker-5.json` | 同上・contended(`apfsd`) | **0** |

**修正が効いたと判定する。** ドライブ判定失敗(`judgement: "failed"`)は`e16c6fd`以降(worker-2/4/5)すべて0件で、これも実測で確認した。

**応答時間について、今朝の仮説を訂正する。** 今朝(「起動・3ドライブ・音声の基準比較」節)は「Workerで A:/B: が遅く C: だけ速いという向きの割れは、指標が観測側(`readTextScreen`のポーリング位相)を測っている疑い」と書いた。音声移行後の応答時間(`responseMs`中央値、実測)は次のとおり:

| ドライブ | 既定 | Worker(修正後、3回の実行) |
|---|---|---|
| A: | 137.75ms | 145.78ms(worker-4) / 143.645ms(worker-5) |
| C: | 542.365ms | 146.82ms(worker-2)† / 530.945ms(worker-4) / 200.35ms(worker-5) |

†worker-2は打鍵落ち修正前・音声移行前の実行だが応答時間の実測値としてはここに含めた。

A:はWorker 143.6〜145.8ms・既定137.8msと**既定に近づいた**。C:はworker-4で530.9msと既定542.4msに近い値が出た一方、同じWorker経路の3回の実行で146.8/530.9/200.4msと**大きくばらついている**。**向きの割れの主因は「音声が未実装でWorker側の仕事量が少なかったこと」だった可能性が高い**が、C:のばらつき自体は未解明で、**この指標の安定性そのものが未確認**であることも記録する。8/31時点で立てた「観測側を測っている疑い」は否定されたわけではなく、**主因ではなかった可能性が高い、という程度にとどめる**(断定しない)。

### 3. 音声

**音声は出るようになった**(未実装時のunderflow 100%相当が解消)。`queuedSec`中央値(実測):

| ファイル | 状態 | beep | idle |
|---|---|---|---|
| `wm-20260901-audio-default.json` | 既定・quiet | 82.154ms | 86.179ms |
| `wm-20260901-audio-worker.json` | Worker・音声未実装・quiet | 失敗 | 0ms(underflow 100%) |
| `wm-20260901-audio-worker-2.json` | Worker・音声移行後・**contended**(`mds_stores`、正規化中央値1.62) | 156.689ms(無効) | 110.476ms(無効) |
| `wm-20260901-audio-worker-3.json` | Worker・音声移行後・contended(`mobileassetd`等) | **197.868ms** | **164.342ms** |
| `wm-20260901-audio-worker-4.json` | Worker・音声移行後・contended(`apfsd`) | 失敗 | **143.991ms** |

Worker(音声移行後、有効な2本)のidleは143.991〜164.342msに対し既定86.179msで、**既定の約1.7〜1.9倍深い**(beepまで含めると197.868msで約2.3倍)。**この数字はいずれも`contended`下で取得しており、確定値ではない。静穏下での取り直しが必要。**

**beepシナリオがWorker経路で不安定**: 音声移行後の3回の実行(worker-2/3/4)のうち成功は`worker-3`の1回のみ(worker-2は`contended`につき無効扱いだが実行自体は成功、worker-4は失敗)。既定は同シナリオで毎回成功している。失敗理由はいずれも`"X-BASICの"Ok"プロンプトに到達できませんでした (lastLine=なし)"`で、**画面テキストが読めていない**。手順9で`screenText()`の未捕捉rejectionを`.catch(() => null)`で安全側へ倒した修正と関係する可能性があるが**未調査**。

### 4. 計測運用上の失敗(正直に書く)

**親セッションがハーネスの`contended`判定を自分の推論で上書きして計測を開始し、音声・起動の2本(`audio-worker-2`/`boot-worker-2`)を無効にした。** 根拠は「`ps`の`%cpu`は生涯平均だから偽陽性」という推論だったが、**macOSの`ps`の`%cpu`は直近1分程度の減衰平均であり、この前提自体が誤りだった**(`mds`が141%を示したことで発覚。生涯平均では説明できない値)。判定器は正しく「さっきまで本当に忙しかった」と言っていたことになる。**自分で設けた不合格判定は、原因(前提の誤り)を取り除いてから通すこと。**

参考として、無効と判定した`boot-worker-2`(27,082.945ms、正規化1.82)と静穏下の`boot-worker-3`(27,435.695ms)は**1.3%しか違わなかった**(実測)。起動時間はこの程度の負荷差には鈍感だったことになるが、**事前にそれは分からない**ので、無効判定を出したこと自体は当時の判断として妥当だったとする。

**8/25に一度だけ発生し「再発したら記録する」としていた`TimeoutError: Timed out after 30000 ms while waiting for the WS endpoint URL to appear in stdout!`(puppeteerのChrome起動待ちタイムアウト)が再発した。** 今回はload average 16以上の高負荷下で、3本連続実行の1本目で発生した(`wm-20260901-drives-worker-3.json`に対応する実行で、このファイルは存在しない=一次データが作られていない)。計測ロジックには到達しておらず、出力ファイルは作られなかった。**高負荷が条件の候補として1つ絞れたが確定ではない。**

計測環境そのものの問題として、Spotlightの索引再構築(`sudo mdutil -X`の副作用)でload averageが最大19.44まで上がり、計測不能な時間帯が数時間続いた。**再起動後に`/System/Volumes/Data`の`Indexing and searching disabled`が反映され解消した**(再起動前は`-405 unable to perform operation`で設定できなかった)。

## SCSI HLE 着手：呼び出し口の特定と、「IOCS は一度も呼ばれない」の実測(2026-09-02)

実施順序の手順2(SCSI HLE)に着手した。**実装の前に、まず何が呼ばれているかを測る**ことから始めた。

### コアを読んで分かった呼び出し口

px68k には外部SCSIボード(CZ-6BS1)の **ROMスタブだけ**が既にある(`x68k/scsi.c`、54行)。

- `$ea0020` に "Human68k" ID と IOCS ベクタ設定ルーチンが置かれ、IOCS `$F5` の実体は
  `move.b d1, $e9f800` を実行して外へ出すだけ
- `$e9f800` はメモリ書き込み表の添字 `(0x00e9f800 >> 13) & 0xff = 0x4f` に落ち、そこは
  **`wm_nop`** だった。つまり **SCSI IOCS 呼び出しはこれまで黙って捨てられていた**
- `SCSI_Write` は `$ea0000`(添字 `0x50`、ROM領域)の担当であり、呼び出し口ではない

**穴は1箇所**であり、ここに実装を入れれば HLE になる。

### 観測フック(動作を変えずログだけ取る)

`SCSI_IOCSPort_Write` を新設し、書き込み表の添字 `0x4f` を `wm_nop` から差し替えた。
IOCS番号(`d1` の下位バイト)・`d1`〜`d5`・`a1`・`pc` を先頭64件までログへ出す。
ゲストから見た動作は変えていない(従来も無視、今回もログのみで無視)。

### 陽性対照(毎回走る自己検査)

「IOCS が0件」という観測は、**呼ばれていない**のか**フックが繋がっていない**のかを
区別できなければ意味がない。そこで `SCSI_Init` の末尾で、CPU と同じ書き込み経路
(`cpu_writemem24`)で `$e9f800` を1回叩き、フックへ届いたかを毎回ログに出すようにした。

- 届いた   → `[SCSI-IOCS] selftest ok`（INFO）
- 届かない → `[SCSI-IOCS] selftest FAILED`（ERROR）

この自己検査は一時的な足場ではなく**常設**とする。将来メモリ表の並びが変わって結線が
外れたとき、無言で「0件」に戻るのを防ぐため。

### 故障注入で自己検査自体を検査した

書き込み表の該当エントリを `wm_nop` へ戻した版をビルドして実走した。

| 条件 | wasm サイズ | selftest | IOCS 呼び出し |
|---|---|---|---|
| 結線あり | 639,678 バイト | **ok** | 0件 |
| 結線を外す(故障注入) | 639,260 バイト | **FAILED** | 0件 |
| 結線を戻す | 639,678 バイト | **ok** | 0件 |

**成果物のサイズが実際に変わっている**ことを先に確認したうえで、検出されることを確かめた
(注入が効いていない状態で「検出されない」を見て安心する事故を避けるため)。

### 実測結果：起動しても SCSI IOCS は一度も呼ばれない

`?system=1&run=1`（バンドル済みシステムディスク `human302.xdf` から起動）で
Human68k の `A>` プロンプトまで到達(27.2秒)させ、到達後さらに3秒観測した。

- **SCSI IOCS 呼び出し：0件**
- selftest：ok（＝フックは生きている）

陽性対照が通り、その陽性対照が故障注入で落ちることも確認済みであるため、
これは観測系の故障ではなく**本当に呼ばれていない**という観測である。

### 決定6の前提が1つ動く：「認識」は IOCS の実装だけでは満たされない

決定6は HLE の必要範囲を「(1) セクタの読み書き (2) Human68k にドライブとして認識される」
の2点としていた。今回の実測は、**(2) は IOCS を実装しても満たされない**ことを示している。

- 外部SCSIボードROMスタブの「起動エントリ」(`$ea0034`)は **`rts` 一発**で、
  SCSIデバイスを探しも登録もしない。IOCS ベクタを `$7d4` に設定するだけである
- 内蔵SCSI機種向けの偽BIOS(`libretro.c` の `WinX68k_SCSICheck` が `$fc0000` へ置く
  `SCSIIMG`)も、IOCS 呼び出し口が `moveq #-1, d0; rts` で同じく空
- つまり**「SCSIデバイスが在る」とゲストへ知らせる経路が丸ごと無い**ため、
  Human68k が SCSI へ問い合わせる理由がそもそも無い

したがって、セクタ読み書きの実装より先に**誰がSCSIドライブを見つけて登録するのか**を
決める必要がある。ここは推測せず次の実測で切り分ける。

### 追測：IOCS ベクタ `$07d4` は設定されている(実測、2026-09-02)

「IOCS が呼ばれない」原因が「ROMスタブがそもそも拾われていない」ことなのかを切り分けるため、
起動後に IOCS ベクタ表($07c0〜$07ff)を `__webx68kDebug.peek()` で読んだ。

| 番地 | 値 | 対応 |
|---|---|---|
| `$07c0` | `$f0ff1b1e` | IOCS `$F0` 既定(未実装ハンドラ) |
| `$07c4` | `$0002c760` | IOCS `$F1`。RAM 上を指す(常駐側が差し替え済み) |
| `$07c8` | `$f2ff1b1e` | 既定 |
| `$07cc` | `$f3ff1b1e` | 既定 |
| `$07d0` | `$f4ff1b1e` | 既定 |
| **`$07d4`** | **`$00ea004a`** | **IOCS `$F5`。SCSIボードROMスタブの呼び出し口** |
| `$07d8` | `$f6ff1b1e` | 既定 |

**`$07d4` は既定値 `$f5ff1b1e` ではなく `$00ea004a` に書き換わっている。**
つまり IPL は `$ea002c` の "Human68k" ID を見つけ、`$ea0036` の
「IOCSベクタ設定エントリ」を呼んでおり、**ROMスタブは正しく拾われている**。

観測系の陽性対照として周辺ベクタも同時に読んだ。値は番地ごとに異なり
(`$f0…`/`$f2…`/`$f3…`/`$f4…`)、`peek()` が定数を返しているわけではない。
特に `$07d4` だけが `$f5ff1b1e` の並びから外れていることが、
書き換えが起きた直接の証拠になっている。

### したがって、欠けているのは「ベクタの設置」ではなく「デバイスの発見と登録」

ROMスタブは拾われ、IOCS ベクタも設置され、呼び出し口も結線済みである。
それでも呼び出しが0件なのは、**SCSIドライブが在ることを誰もゲストへ知らせていない**ためである。
スタブの起動エントリ(`$ea0034`)は `rts` 一発で、IDの走査もデバイスの登録もしない。

### 同梱 IPL ROM は SASI 機種のものであることの確認

内蔵SCSI機種の IPL で測る案(実施順序の検討時に挙げた候補3)は採れない。
同梱の `public/system/iplrom.dat` は 128KB の公開版 IPLROM であり、
`WinX68k_SCSICheck()` の SCSI 判定(`IPL[0x30600..0x30c00]` の走査)に掛からず
SASI 機種として扱われる。内蔵SCSI機種の ROM は手元に無く、同梱もできない。

**これは制約であると同時に設計を狭めてくれる。**製品として成立させるには
**外部SCSIボード(`$ea0000`)の経路だけで完結させる必要がある**。
実機でも CZ-6BS1 は SASI 機種に SCSI ディスクを足せるので、
必要な仕掛けは全てボード側ROMに入っていたはずである。ここは px68k の
偽ROMであり**自分たちが書ける領域**なので、他者の実装を読まずに構築できる。

### 次に検証する仮説

移行後イメージの実測(「移行後イメージの構造」節)では、セクタ0に `X68SCSI1` 署名と
`Human68K SCSI-DISK by Keisoku Giken` の文字列があり、セクタ2-3・6-13・20-31 に
IPLと管理領域が置かれていた。ここから立つ仮説は、

> **登録処理はディスク側にある。**ボードROMの仕事は「IDを走査してセクタ0を読み、
> `X68SCSI1` を確認して、ディスク上のコードへ渡す」までで、
> パーティションを Human68k へ登録するのはディスク上のコードである。

**検証方法**は今回作った観測フックがそのまま使える。IOCS `$F5` のセクタ読み出しだけを
実装し、起動エントリでセクタ0を読んで渡すようにしたうえで、
**ディスク上のコードが IOCS を叩き始めるか**をログで見る。叩き始めれば仮説は支持され、
必要なコマンドの一覧もそのログから直接得られる。叩かなければ仮説は外れであり、
その時点で別の経路を探す。
...
1. **`scripts/build-core.sh` の出力先が別 worktree に固定されていた。**
   `OUT_DIR` が `WebX68k/public/core`(master 側の作業ツリー)の絶対パスで、
   `WebX68k-storage` から実行すると**別ツリーの追跡ファイルを書き換える**。
   スクリプト自身の位置から `REPO_DIR` を求める形に直した(`SHIM_C` も同様)。
2. **vite dev は既定で `[::1]` にしか bind しない。**
   プローブの接続先を `127.0.0.1` にしていたため `ERR_CONNECTION_REFUSED` になった。
   既存の計測スクリプトが `localhost` を使っていたので今まで表面化していなかった。

### 宿題20（puppeteer の Chrome 起動待ちタイムアウト）に新しい観測

同じ `TimeoutError: Timed out after 30000 ms while waiting for the WS endpoint URL` が
今回も1回発生した。ただし**発生時の load average は 2.33** であり、
9/1 に記録した「高負荷(load 16以上)が条件の候補」とは食い違う。

切り分けとして、同時刻に Chrome を別プロファイル(`--user-data-dir` を temp に指定、
`--remote-debugging-port=0`)で手動起動したところ、**1秒で `DevTools listening on ws://…`
を stderr に出して正常に起動した**（利用者の Chrome が起動したままの状態で、である）。
したがって「利用者の Chrome が起動していること」は原因ではない。**高負荷も必要条件ではない。**

プローブ側には回避策として「起動タイムアウトを90秒に延ばし、失敗したら1回だけ再試行」を
入れた。**これは回避策であって原因の特定ではない。**

### できなかったこと・未確認のこと

- SCSIデバイスの登録経路(誰が見つけて誰が登録するか)は未特定
- SASI ディスクを付けた状態、SRAM のブートデバイス設定を変えた状態など、
  **条件を変えた場合に IOCS が呼ばれるか**は未測定（今回は1条件のみ）
- 内蔵SCSI機種の IPL(`iplromxv.dat` 等)を使った場合は未測定。
  現行の同梱 ROM は SASI 機種であり、`WinX68k_SCSICheck` の SCSI 分岐に入っていない

## SCSI HLE 手順2-1：ホスト側セクタI/Oの実装と疎通確認(実測、2026-09-02)

登録経路の仮説を検証するには、まず**セクタが読めること**が要る。IOCS のコマンド番号は
まだ1つも分かっていないが、**I/O の土台はコマンド番号を知らなくても作れて、
しかも既知の基準器で答え合わせができる**ため、そこから着手した。

### 経路の形

決定2 のとおり、emscripten のファイルシステムは経由しない。

    コア(x68k/scsi.c) → webx68k_scsi_read_sector(lba, buf) → WebX68k(src/core-shim.c) → ホスト

この段階の実体は**同期 XHR + Range リクエスト**である。最終形(OPFS の同期ハンドル)では
ないが、**同期・セクタ単位・wasmヒープに載るのは512バイトだけ**という形は最初から同じに
してある。手順3では `js_scsi_read_sector` の中身を差し替えるだけになる。

### 基準器での答え合わせ(実測)

`WEBX68K_SCSI_FIXTURE` の基準器を Range 対応の小さなサーバで配信し、コア起動時に
セクタ0を読ませた。結果:

    [SCSI] image size=104857600 bytes (204800 sectors) sector0="X68SCSI1" sector0[8..11]=$02000003

| 項目 | 基準器の記録値 | 実測 |
|---|---|---|
| サイズ | 104,857,600 バイト | 一致 |
| セクタ数 | 204,800 | 一致 |
| セクタ0 先頭8バイト | `X68SCSI1` | 一致 |
| セクタ0 の 8〜11 バイト目 | `02 00 00 03` | 一致 |

**陰性対照**：イメージを繋がずに起動すると
`__webx68kScsiUrl が未設定` → `[SCSI] イメージ未設定 (デバイス無しとして扱う)` となり、
「読めた」が出ない。

なお `sector0[8..11]` に名前を付けていないのは意図的である。この4バイトの意味は
まだ確定していない(総セクタ数 204,799 = `$00031fff` はこの並びの1バイト先から始まる)。
**意味を確定していない値に名前を付けない。**

### 途中で踏んだ罠2つ

1. **EM_JS の本体では正規表現のバックスラッシュが消える。**
   `Content-Range` の解析に `/\/(\d+)\s*$/` と書いたところ、生成された JS では
   `//(d+)s*$/` になっていた。EM_JS の本体は C の文字列化を通るため、
   `\/` `\d` `\s` のバックスラッシュが失われる。結果 `//` が行コメントになって
   関数の構造が壊れ、**エラーは無関係な行(`wasmExports = await createWasm();`)で
   報告された**。正規表現をやめ、`lastIndexOf('/')` による解析に置き換えた。
   **EM_JS の本体にバックスラッシュを書かない。**
2. **`self` を裸で参照すると Node 上の結合テストが落ちる。**
   `(typeof window !== 'undefined') ? window : self` と書いたため、
   コアを Node で読み込む結合テスト12件が `ReferenceError: self is not defined` で
   落ちた(単体テストが実際に捕まえた)。`globalThis` に統一して解消。
   ブラウザ・Worker・Node のいずれでも参照できる。

### `Range` は CORS のプリフライトを起こす

基準器を別ポートで配信したところ、最初は読めなかった。`Range` は CORS の
safelisted request header ではないため別オリジンではプリフライトが飛び、
さらに `Content-Range` を JS から読むには `Access-Control-Expose-Headers` が要る。
プローブ側のサーバに `OPTIONS` 応答・`Access-Control-Allow-Headers: Range`・
`Access-Control-Expose-Headers: Content-Range` を足して解決した。

**ページが COEP: require-corp で分離されている**ため `Cross-Origin-Resource-Policy:
cross-origin` も必要である。

### 次

`X68SCSI1` が読めたので、仮説(登録処理はディスク側にある)の検証に進める。
ROMスタブの起動エントリ(`$ea0034`)にセクタ0を読んでディスク側コードへ渡す処理を書き、
**ディスク側コードが IOCS を叩き始めるか**を観測フックで見る。
必要な IOCS コマンドの一覧は、その時点でログから直接得られる見込みである。

## SCSI HLE 手順2-2：起動エントリは呼ばれない(実測、2026-09-02)

仮説「登録処理はディスク側にあり、ボードROMの仕事はセクタ0を読んで渡すまで」を
検証するため、まず**その起動エントリが呼ばれるのか**を測った。呼ばれないなら
そこにローダを書いても意味がないためである。

### ROM の構造は動かせない（実測で判明）

最初、起動エントリのポインタ(`$ea0020`)を自前ルーチン(`$ea0060`)へ向けた。結果、
**`$07d4` が既定値 `$f5ff1b1e` に戻った**。つまり IPL がボードを丸ごと受け付けなくなった。
IPL は `$ea0020` の値を読んで検証しており、"Human68k" ID の直後という構造から外れると
拒否される。

**毎回 `$07d4` を測っていたおかげで即座に分かった。**この退行は起動自体には現れず
(Human68k は FD から普通に起動する)、SCSI 側のログも元から0件なので、
ベクタを見ていなければ「変化なし」として見逃していた。

対処として、**ROM の構造は一切動かさず** `$ea0034` の `rts`(2バイト)を
`bra.s $ea0060`(2バイト)に置き換える形にした。これで `$07d4` は `$00ea004a` に復帰した。

### 陽性対照：呼ばれれば印は出る

「起動エントリの印が出ない」を「呼ばれていない」と読むには、
**呼ばれれば印が出ること**を先に示す必要がある。そこで、実測で必ず呼ばれている
(＝`$07d4` が書き換わる)**ベクタ設定エントリ**の側にも同じ仕掛けを入れた。
`$ea0040` の `moveq #-1,d2` を `bra.s $ea0080` に置き換え、飛び先で印を出してから
元の `moveq`/`rts` を実行する。

結果:

    [SCSI] ベクタ設定エントリが呼ばれた(陽性対照) (pc=$00ea0088)

`pc` は印の命令の直後($ea0080 + 8)で、辻褄が合っている。`$07d4` も `$00ea004a` のままで、
移動させた `moveq #-1,d2` も正しく実行されている。

### 結果：起動エントリは呼ばれない

| 印 | 出たか |
|---|---|
| ベクタ設定エントリ(`$03`、陽性対照) | **出た** |
| 起動エントリ(`$01`) | **出ない** |
| セクタ0のゲストRAMへの読み込み(`$02`) | 出ない(起動エントリの中なので当然) |

**IPL は IOCS ベクタ設定エントリを呼ぶが、起動エントリは呼ばない。**
陽性対照が同じ仕掛けで動いているため、これは仕掛けの不備ではない。

### FDなし起動での確認は成立しなかった

「FDから即起動しているせいで起動エントリに到達していないのでは」を確かめるため
`?run=1`(system=1 なし)でも測ったが、ゲストが
「エラーが発生しました。リセットしてください。」で止まり、`$07d4` は既定値のまま、
陽性対照の印も出なかった。**IPL がROM初期化まで到達していない**とみられ、
この条件では起動エントリの可否を判定できない。**この試行は無効とする。**

### 分かっていないこと

- IPL がどういう条件で起動エントリを呼ぶのか。SRAM のブートデバイス設定
  (以前の候補2)が効く可能性があるが未測定
- FD から起動した場合に、SCSIドライブを Human68k へ足すのは誰の仕事なのか。
  起動エントリが呼ばれない以上、ボードROMの起動エントリ以外の経路がある可能性がある

**仮説(登録処理はディスク側にある)は、まだ支持も否定もされていない。**
起動エントリに到達できていないため、検証そのものが始められていない状態である。

## SCSI HLE 手順2-3：ボードを見に来ているのは Human68k だった(実測、2026-09-02)

「IPL がどういう条件で起動エントリを呼ぶのか」を調べるため、ROM ヘッダ($ea0020-$ea0053)が
読まれた瞬間の PC を記録した(`SCSI_Read` にフック)。命令フェッチは `OP_ROM` 経由で
`SCSI_Read` を通らないため、ここに来るのはデータ読み出しだけである。

### 読み出しの順序(実測)

| PC | 読んだ番地 | 意味 |
|---|---|---|
| `$0000e2f6` | `$ea0044` | ボードID "SCSI" |
| `$00006aea` | `$ea0044` | ボードID "SCSI" |
| `$00006af0` | `$ea0048` | ボードID "EX" |
| `$0000e2f6` | `$ea0020` | 起動エントリのポインタ |
| `$00007f16` | `$ea0020` | 起動エントリのポインタ |
| `$00007f1c` | `$ea0030` | ID "n68k" |
| `$00007f26` | `$ea002c` | ID "Huma" |
| `$00007f30` | `$ea0028` | IOCS入口のポインタ |
| `$00007f32` | `$ea0024` | ベクタ設定エントリのポインタ → 呼ぶ |

**PC は全て RAM 上($6xxx/$7xxx/$e2f6)である。**IPL は `$fc0000` 以降で動くので、
**ボードを見に来ているのは IPL ではなく Human68k(HUMAN.SYS)**ということになる。

流れは「ボードID "SCSIEX" を探す → `$ea0020` の起動エントリのポインタを取る →
その直前8バイトが "Human68k" か検証する → ベクタ設定エントリを呼ぶ」で終わっている。
**起動エントリは呼ばれないまま終わる。**

これで、先に出した退行(「`$ea0020` のポインタを書き換えるとボードごと拒否される」)の理由も
実測で裏が取れた。`$ea0020` が指す先の直前8バイトに "Human68k" が無ければ弾かれる。
**推測ではなく、読み出しの順序として観測できた。**

## d2 の戻り値は意味を持つ。ただし意味は未確定(実測、2026-09-02)

ベクタ設定エントリの最後にある `moveq #-1, d2`(px68k のダミー値)を疑い、値を振った。

| d2 | 起動 | 画面 |
|---|---|---|
| `$ffffffff`(-1、元の値) | **成功**(`A>` 到達) | 正常 |
| `$00000000` | **失敗**(2回とも再現) | 「おかしな命令を実行しました (SR=$2009:PC=$00000000)」 |
| `$00ea00a0`(自前ルーチンのアドレス) | 失敗 | 同上、**PC=$00000000 のまま** |
| `$00ea00c0`(全要素が自前ルーチンを指すポインタ表) | 失敗 | 同上、**PC=$00000000 のまま** |

**`d2` は確かに使われている。**-1 以外を返すと Human68k は別の経路へ進み、
最終的に `$00000000` を実行して落ちる。

しかし**意味は確定していない**。棄却できた仮説は2つある。

- **「d2 はルーチンのアドレス」は否定。**自前ルーチン($ea00a0、印を出すだけ)を渡しても
  印は出ず、PC=$00000000 のまま落ちた。Human68k は d2 へ飛んでいない
- **「d2 は構造体のアドレスで、その中のポインタへ飛ぶ」も否定。**16個の要素すべてを
  自前ルーチンに向けたポインタ表を渡しても、印は出ず PC=$00000000 のままだった

いずれの値でも落ちる先が `$00000000` で共通であることから、
**Human68k は d2 とは別の場所(既定値がゼロのまま)を経由して飛んでいる**とみられるが、
それがどこかは未特定である。

### 値を振るコストを下げた

`d2` の即値はコア初期化時に JS 側のグローバル `__webx68kScsiInitD2` から差し替える形にした。
**値を変えるたびの再ビルド(約2分)が不要になり、実測1回(約40秒)だけで済む。**
既定は -1 で、指定しなければ px68k 元来の挙動と同じである。
差し替え位置は決め打ちなので、命令語(`$243c` = `move.l #imm,d2`)を照合してから書き、
ずれていればエラーを出す。

プローブは `--init-d2=<値>` で指定する。

### 次に測ること

d2 ≠ -1 のときに Human68k がどこを経由して `$00000000` へ飛ぶのかを特定する。
候補は、d2 を使って何かを **書き込む** 先(ROM に書いたので落ちた)か、
d2 を使って読んだ先のさらに先である。いずれも「落ちる直前の経路」を見る必要があり、
現状の観測フック(ROM読み出しとポート書き込み)では届かない。

## SCSI HLE 手順2-4：$00000000 で捕まえたレジスタと、資料による裏取り(2026-09-02)

### アドレス0に印を置いて、飛んできた瞬間を捕まえた

Human68k は d2 ≠ -1 のとき `$00000000` を実行して落ちる。**そこは RAM なので、
印を出して戻るだけのコードを置いておけば飛んできた瞬間の状態が取れる。**
ベクタ設定エントリが呼ばれた時点(既にホストコマンドで捕まえている)で
`cpu_writemem24` により `move.b #$06,$e9f802 / rts` を書き込んだ。

結果(d2=$00000000 の条件、2回呼ばれた):

| 項目 | 値 |
|---|---|
| 呼び出し形式 | `jsr` 相当(スタックに戻り番地) |
| 戻り番地 | `$0000df48` / `$0000df4e`(6バイト違いの2箇所) |
| `a0` | `$00ea0020` … **ボードROMヘッダの先頭** |
| `a3` | `$00ea0036` … **ベクタ設定エントリのアドレス** |
| `a4` | `$00000000` … **空** |
| `d0`/`d7` | `$00007f02` / `$00000005` |

`a0` にヘッダ先頭、`a3` にベクタ設定エントリが入っているのに **`a4` だけが空**である。
そこで「`a4` も初期化ルーチンの戻り値ではないか」を試した(即値をホストから差し替え可能にし、
`--init-a4=` で振れるようにした)。

`a4` に自前ルーチン(`$ea00a0`)を渡すと、**`$00000000` へ飛ばなくなった**(印 `$06` が出ない)。
挙動は確かに変わる。しかし `a4` の指す先の印(`$05`)も出ず、起動は失敗のままだった。
**`a4` の意味も確定していない。**

なお、ROM 配列内の配置(`$ea0060`/`$ea0080`/`$ea00a0`/`$ea00c0`)は、ソースの配列を
機械的に数え直して意図どおりであることを確認済みである(番地の思い込みを排すため)。

## 資料：『Inside X68000』(1992) による裏取り

利用者から資料の提供を受けた。**追跡外の `_local/` に置かれており、リポジトリにも
配布物にも入らない。**扱いは従来方針どおりで、**事実(規約・番地)だけを採り、
文章やコードは写さない。資料で読んだことと実測したことは出所を分けて書く。**

### 資料が実測と一致した点(相互検証)

| 項目 | 資料 | 手元の実測 |
|---|---|---|
| CZ-6BS1 のボードID | `$EA0044` から `'SCSIEX'` | 一致(`$ea0044`/`$ea0048` が読まれた) |
| CZ-6BS1 の SCSI-ROM | `$EA0020`〜`$EA1FFF` | 一致(ヘッダは `$ea0020` から) |
| SCSI内蔵モデル | `$FC0000`〜`$FC1FFF`、ID `'SCSIIN'` | 未測定(同梱ROMはSASI機種) |

**先に自力で観測していた事実と食い違いが無い。**資料側の信頼性の目安になる。

### 資料から得た新しい事実(未実測を含む)

- SCSI 対応化にともない SRAM の **`$ED006F` / `$ED0070` / `$ED0071`** が使われる
- `$ED006F` は後ろ2つが有効かのフラグで、有効なら `$56`(`'V'`)。
  **`'V'` が無い場合は「SCSI ローダプログラム」が `'V'` を書き、
  `$ED0070` を `$07`、`$ED0071` を `$00` に設定する**
- `$ED0070`: bit3 = `0`:内蔵 / `1`:オプションボード、下位3ビット = 本体の SCSI ID(既定7)
- `$ED0071`: SASI ディスクを SCSI バスに繋いだときのフラグ(既定 `$00`)

**「SCSI ローダプログラム」という呼び名が資料から得られた。**これが我々の欠いている
ものの名前である可能性が高いが、**その中身や呼び出され方は資料のこの箇所には無い。**

## SRAM を資料どおりにしても、Human68k は SCSI を探しに来ない(実測)

手元の環境の SRAM を実測すると **3番地とも `$ff`** だった。
つまり **SCSI ローダは一度も走っていない。**

そこで、ローダが書くはずの値を我々が代わりに書いて測った(`--scsi-sram`)。

| 条件 | `$ed006f` | `$ed0070` | `$ed0071` | 起動 | SCSI IOCS |
|---|---|---|---|---|---|
| 既定 | `$ff` | `$ff` | `$ff` | 成功 | **0件** |
| 資料の既定値を書く | `$56`(`'V'`) | `$07` | `$00` | 成功 | **0件** |

**SRAM の設定だけでは Human68k は SCSI を探しに来ない。**
ボードは在り、IOCS ベクタも設置され、SRAM も資料どおりにしても、
IOCS `$F5` は一度も呼ばれない。欠けているのは依然として
「デバイスの発見と登録」を行う何か(＝資料の言う SCSI ローダに相当するもの)である。

### 罠：SRAM は書き込み許可を開けないと無言で捨てられる

最初 `SRAM_Write()` だけを呼んだところ、**エラーも警告も無く読み返しが `$ff` のままだった。**
`SRAM_WriteEnable(1)` が要る。書いた直後に読み返しをログへ出していたおかげで
「書いたつもりで書けていない」状態を測定として検出できた。開けたら必ず閉じる。

## 資料調査：『X68000 Human68k ver3.0』(利用者提供、2026-09-02)

**追跡外の `_local/` に置かれており、リポジトリにも配布物にも入らない。**
従来方針どおり**事実だけを採り、文章やコードは写さない。資料由来と実測は出所を分ける。**

### 資料から得た事実

- **Human68k ver3.0 は SCSI デバイスドライバを標準装備している。**
  FDから起動した場合でも `SCSIDRV.SYS` の組み込み無しに SCSI ドライブが使える。
  **SCSI ドライバを ROM 内に持っている**ため、と説明されている（第1章 1.3.2）
- **`CONFIG.SYS` の `SCSIDEV` は「SCSI デバイスドライバを登録するかどうか」の設定**（第7章）
  - `SCSIDEV=ON` … **SCSIROM のデバイスドライバを登録する**
  - `SCSIDEV=OFF` … SCSI デバイスドライバを登録しない（起動が速くなる）
  - **既定値はこのページには書かれていない**
- SWITCH コマンドの `BOOT` で `SCSI0`〜`SCSI7` からの起動を指定できる。
  `SCSI_ID` で本体のID(0〜7、工場出荷時7)を設定する（第5章）

### これが今日の実測と噛み合う

我々が探していた「デバイスの発見と登録」は、資料の言葉では
**「SCSIROM のデバイスドライバを登録する」**である。

- px68k の偽SCSI-ROMには**デバイスドライバが入っていない**。登録すべき実体が無い
- だから Human68k はボードを見つけ、IOCS ベクタを設置し、そこで止まる
- SRAM を資料どおりにしても変わらなかったのは、SRAM は**設定**であって
  **登録すべき実体**ではないため、と説明が付く

### 有力な仮説：`d2` はデバイスドライバヘッダのアドレス

「SCSIROM のデバイスドライバを登録する」という記述と、`$00000000` で捕まえた
実測とを合わせると、次の仮説が立つ。

> **ベクタ設定エントリが `d2` で返すのは、SCSI-ROM 内のデバイスドライバヘッダのアドレスである。**
> Human68k はそれを登録し、ヘッダ内のポインタ経由でドライバのルーチンを呼ぶ。

実測との符合:

- **2回呼ばれた**（`$0000df48` と `$0000df4e`、6バイト違いの2箇所）。
  デバイスドライバは2つのルーチンを続けて呼ばれるのが通例である
- **`a5` にスタック上のアドレス**(`$000067de`)が入っていた。要求の受け渡しに使われる形に見える
- `a0` にボードROMヘッダ、`a3` にベクタ設定エントリのアドレスが入っており、
  呼び出し側が**ボードの情報を持ったまま**呼んでいる
- `d2` に `$00000000` を返したとき、ヘッダとして読まれる先が全てゼロになるため
  **ポインタもゼロになり `$00000000` を呼ぶ**、と読める

**ただしヘッダの構造(どのオフセットに何があるか)は、提供された2冊のどちらにも無い。**
『Inside X68000』はハードウェア寄りで、Human68k のマニュアルは利用者向けであり、
デバイスドライバの作成仕様は扱っていない（「ストラテジ」「インタラプト」等の語は
どちらにも出てこないことを確認済み）。

### ROM に置いたポインタ表が呼ばれなかった理由の候補

`$ea00c0` に全要素が自前ルーチンを指すポインタ表を置いても呼ばれなかった件について、
**登録の際にヘッダへ書き込みが起きるなら、ROM では書き込みが捨てられて破綻する**
という説明が立つ。**未検証。**次はヘッダを **RAM に置いて** 同じ試験を行う。

### 同梱システムディスクの `CONFIG.SYS`(実測)

`public/system/human302.xdf` の `CONFIG.SYS` を読み出したところ、
`FILES` / `BUFFERS` / `LASTDRIVE` / `KEY` / `USKCG` / `BELL` / `DEVICE`×7 /
`ENVSET` / `EXCONFIG` の各行があり、**`SCSIDEV=` の行は無い**。
既定値が資料に書かれていないため、**これが `ON` 相当なのか `OFF` 相当なのかは未確定**である。
`SCSIDEV=ON` を明示した `CONFIG.SYS` で測れば切り分けられる（未実施）。

なお `HUMAN.SYS` 内には CONFIG キーワードの一覧があり、その中に `scsidev` が
含まれていることを実測で確認した（この版が `SCSIDEV` を解釈することの裏付け）。

### 次にやること（更新）

1. ~~**`SCSIDEV=ON` を明示した `CONFIG.SYS` で測る。**既定が `OFF` 相当なら、これだけで
   Human68k の振る舞いが変わる可能性がある。変わらなければ既定は `ON` 相当だと分かる~~
   → **2026-09-02 実施済み**（「CONFIG.SYS の SCSIDEV を振って測った」節を参照）。
   既定は `ON` 相当と確定し、「既定が OFF だから登録されない」という仮説は棄却された
2. ~~**デバイスドライバヘッダを RAM に置いて `d2` で渡す。**全ポインタを別々の印に向け、
   どのオフセットが呼ばれるかを実測してヘッダ構造を決める。
   ROM では登録時の書き込みが捨てられる可能性があるため RAM で行う~~
   → 2026-09-02 実施済み。d2 単独では読まれず、**a4 と両方**渡すと +4/+6 が読まれると判明（上記の節）
3. 上で構造が決まったら、偽SCSI-ROM に最小のデバイスドライバを実装する
   → 2026-09-02、ストラテジ／インタラプトの到達までは確認済み。次は初期化コマンドへの返答
   → 2026-09-02、初期化への返答まで実装。ただし返答の中身は結果を変えず、原因は別にある（上記の節）

## 第三者ドライバは要るのか（調査、2026-09-02）

「SCSI の HDD を使うには別途ドライバが要るのではないか」という問いを検討した。
**結論は不要**であり、我々の方針（偽SCSI-ROMにドライバを自前実装する）は変わらない。

### 公開ライブラリの掲載物は、基本のドライバではない

X68000 LIBRARY の SASI/SCSI の一覧を確認したところ、掲載されているのは
**ROM 内蔵ドライバでは足りない部分を足すもの**であった。

| ソフト | 用途 |
|---|---|
| `susie.x` | **MO や CD-ROM のための** SCSI デバイスドライバ |
| `GOVERHD.x` | 1GB 以上の HDD から**ブートできるようにする** |
| `HSCSI.X` | SCSI 読み込みの高速化 |
| `TWOSCSI.x` | 内蔵と拡張の SCSI ポートを同時使用 |
| `SCSITAI.SYS` 系 | SCSI ポートをターゲットとして使う |
| FAT32 ツール群・CD-DA 系 | FAT32 対応、CD 音声取り込み |

**「SCSI ハードディスクを使うために必要なドライバ」は1つも無い。**
資料（Human68k ver3.0 は SCSI ドライバを ROM 内に持つ）と整合する。

### 実測でも既に裏が取れていた

2026-08-23 の XM6 での移行では、**Human68k のシステムFDの `FORMAT.X` と `COPYALL.X` だけ**で
起動可能な SCSI ディスクを作れている。第三者のドライバは一切使っていない。
「素の SCSI HDD には追加ドライバが要らない」ことは、あの時点で実物で示していた。

### むしろ第三者ドライバに頼るほうが難しい

`x68k/scsi.c` の冒頭コメントは "SPC is not emulated" と明言しており、
メモリ表を確認したところ **SPC のポート(`$EA0001`〜`$EA001F`)を含む
`$EA0000`〜`$EA1FFF` は丸ごと `SCSI_Read` に落ち、ROM の内容を返すだけ**である。

- **SPC を直接叩くドライバは動かない。**動かすには SPC のエミュレーションが要り、
  いま進めている IOCS HLE より大幅に大きい仕事になる
- **SCSI IOCS 経由のドライバ**なら、HLE 実装後に動く可能性はある。ただし
  個々のドライバがどちらの方式かは**未確認**（中身は読まない方針なので、動かして測る）
- 権利上、第三者ドライバは同梱できない。利用者に別途入手させる設計は
  「開けば動く」という方針とも噛み合わない

`GOVERHD.x`（1GB超からの起動）は、**将来の容量上限の議論**で再び出てくる可能性がある。

## 外部スレッドの評価（2026-09-02）

利用者から nfggames のフォーラムスレッドの提示があった。
**方針上、第三者による解析記事は参照しない**と決めているため、
**中身を取り込まずに種類だけを判定する**形で確認した（要約器に分類のみを問い、
コード・番地・構造体定義を引用しないよう明示した）。

判定結果は「XM6 用の `scsiexrom.dat` / `scsiinrom.dat` を Human68k v3.02 の
フロッピーイメージから**ツールで生成する手順**の紹介」であり、
逆アセンブル結果やソースの掲載は無し、SCSI ROM の内部構造や登録の仕組みにも
触れていない、というものだった。**方針には抵触しない。**

### これは我々の近道にはならない

- XM6 は **SPC をエミュレートして本物の SCSI ROM を走らせる**方式である。
  px68k は逆に **SPC を実装せず IOCS を乗っ取る**方式であり、
  本物の ROM を持ってきても走らせる相手がいない
- 本物の ROM を使う route を採るなら **SPC のエミュレーションを書く**ことになり、
  HLE より大幅に大きい。加えて派生 ROM の配布可否という権利上の論点が増える

**得られた確証**は「実機では本当にドライバが ROM 側に居る」という、
資料の記述の裏付けである。近道ではなく、方向の確認として扱う。

### 同梱ディスクに SCSI ROM の実体は無い（実測）

`public/system/human302.xdf` を走査した。

- `SCSIEX` / `SCSIIN` の文字列は各4件見つかったが、**すべてプログラム内の文字列表**であった
  （`SWITCH.X` のパラメータ名 `BOOT`/`DEBUG`/`SCSI`/`SCSIIN`/`SCSIEX` の並び、
  および「SCSI ID が設定されていません」等のメッセージの近傍）。
  **ROM を探しに行く側のコードであって、ROM の実体ではない**
- ディレクトリエントリを全走査したが、名前に `SCSI` または `ROM` を含むファイルは無い

**したがって「SCSI ROM の実体が同梱ディスクにファイルとして入っている」ことは無い。**
それ以上（どこから生成されるのか）を追うには第三者バイナリの解析が必要になるため、
**方針により追わない。**

### 追記：ROM は `HUMAN302.XDF` から生成される（利用者提示の手順書、2026-09-02）

XM6 TypeG のクイックスタート（**エミュレータの利用手順書**であり解析記事ではない。
同じく分類のみを問う形で確認した）によれば、`SCSIEXROM.DAT` / `SCSIINROM.DAT` は
**XM6 のユーティリティで `HUMAN302.XDF` から生成する**ものであり、
XM6 TypeG では **SCSI を使うために必須**とされている。

つまり **SCSI ROM の中身は、我々が既に同梱している Human68k の配布物の中にある。**
ただしファイルとして独立してはおらず（前項の実測）、どこに埋まっているかを特定するには
第三者バイナリの解析が必要になる。**方針により行わない。**

**この違いは製品上の利点でもある。**XM6 系は SPC をエミュレートするため本物の
SCSI ROM を用意させる必要があるが、**px68k は IOCS を乗っ取る方式なので ROM の生成が要らない。**
我々が偽ROMにドライバを実装すれば、**利用者は何も生成せず、何も追加で入手しなくてよい。**

なお、仮に将来 route B（SPC エミュレーション＋本物のROM）を検討する場合は、
同梱の Human68k 配布物から派生した ROM を配布してよいかという権利上の判断が別途要る。
**現時点でその判断は不要であり、立ち入らない。**

## CONFIG.SYS の SCSIDEV を振って測った：既定は ON 相当だった（実測、2026-09-02）

「既定は `SCSIDEV=ON`/`OFF` のどちらか」を切り分けるため、実際に `CONFIG.SYS` を書き換えて測定した。

### 方法

同梱システムディスク `public/system/human302.xdf` の `CONFIG.SYS` に1行だけ足した変種イメージを作り、
`?fd1=` で FDD0 に挿して起動した。作成は新設の `scripts/make-config-variant.mjs`
（FAT12 を素の Node で解釈し、単一クラスタ前提で行を追加、書き出し後に読み戻して検証する）。
プローブ `scripts/probe-scsi-iocs.mjs` に `--fd1=` を足した。変種は追跡外の `public/test/` に置く。
`CONFIG.SYS` は 468 バイト・開始クラスタ60・1クラスタ=1024バイトで、行を足しても同じクラスタに収まる。

### 陽性対照（2段）

存在しないキーワード `ZZBOGUS = 1` を足した版は、起動もし、画面に何のエラーも出なかった。
**未知のキーワードは黙って無視される。よってこれは「CONFIG.SYS が読まれた」ことの証明にならない。**

そこで `DEVICE = \SYS\NOSUCH.X` を足した版で測り直したところ、画面に
「\SYS\NOSUCH.X が登録できませんでした」が出た。**追加した行が実際に読まれ処理されることを確認できた。**
スクリーンショットは追跡外の `_local/scsi/` に残した。

### 結果（IOCSベクタ $07d4 の値、SCSIボードROM($ea0020-$ea0053)の読み出し件数、SCSI IOCS の呼び出し件数）

| CONFIG.SYS | 起動 | $07d4 | ROM読み出し | SCSI IOCS |
|---|---|---|---|---|
| 既定（SCSIDEV 行なし） | 成功 | `$00ea004a` | 46件 | 0件 |
| `SCSIDEV = ON` | 成功 | `$00ea004a` | 46件 | 0件 |
| `SCSIDEV = OFF` | 成功 | `$f5ff1b1e`（既定値のまま） | **0件** | 0件 |

### 読み取れること

- `SCSIDEV = OFF` にすると Human68k は**ボードROMを1バイトも読まない**。
  つまり我々がこれまで観測してきた一連の動き（ID "SCSIEX" を探す → "Human68k" を検証する →
  ベクタ設定エントリを呼ぶ）は、**まるごと SCSIDEV=ON の処理である**と実測で確定した。
  資料の言う「SCSIROM のデバイスドライバを登録する」処理の入口はここである。
- 既定（行なし）は ON と完全に同じ挙動であり、**既定は ON 相当**と確定した。
  「既定が OFF だから登録されないのではないか」という仮説は**棄却**された。
- したがって欠けているのは設定ではなく、**ベクタ設定エントリが返すべき中身**である。
  ベクタ設定エントリは「IOCSベクタを設定する」だけの入口ではなく、
  **登録処理そのものの入口**だったことになる。

## d2 と a4 の両方を渡すと、Human68k はヘッダを読みに来た（実測、2026-09-02）

**やったこと**

- 偽SCSI-ROM の `$ea0100〜$ea04ff` を「書ける窓」にした（書き込みは実際に反映し、読み書きを全件ログ）。
  窓には 4バイト×64 要素の表を置き、要素 k は 10バイトのスタブ（それぞれ別の印を出して rts）を指す。
- 陽性対照：コア初期化時に `$ea0110` へ `$5a` を書いて読み返す検査を入れ、
  「窓への書き込みが効いている」ことを毎回確認している（実測で PASS）。

**観測系の欠陥を1つ踏んだ（先に書くこと）**

- プローブは `[SCSI-IOCS]` / `[SCSI]` / `[SCSI-ROM]` の3タグを列挙して照合していたため、
  新設した `[SCSI-WINREAD]` / `[SCSI-WRITE]` のログを**1行残らず捨てていた**。
  最初の実測は「窓は一度も読まれていない」という誤った結論になりかけた。
  タグの列挙をやめて `[SCSI` の前方一致に直した。**タグを増やす側と拾う側が別ファイルにあると無言で取りこぼす。**
...
|---|---|---|
| `d2=-1`（既定） | 成功 | 自己検査ぶんのみ |
| `d2=$00ea0100` のみ | 失敗 | **一度も読まれない** |
| `a4=$00ea0100` のみ | 成功 | 一度も読まれない |
| `d2=$00ea0100` かつ `a4=$00ea0100` | 失敗（画面は化ける） | **読まれた（下記）** |

両方渡したときの読み出し（自己検査ぶんを除く）:

| 番地 | PC | 幅 |
|---|---|---|
| `$ea0104` | `$00007152` | ワード（1件） |
| `$ea0106`〜`$ea0109` | `$0000df46` | ロング（4バイト連続） |

**読み取れること（実測）**

- **`d2` と `a4` の両方を渡してはじめて、Human68k は渡した構造体を読みに来る。** どちらか一方では読まない。
- 読まれたのは **+4 のワード**と **+6 のロング**である。
- `+6` を読んだ PC は `$0000df46` で、これは既に観測していた
  「`$00000000` へ飛んだときの戻り番地 `$0000df48` / `$0000df4e`」の**直前**である。
  つまり **+6 のロングを読み、それを呼んでいる**。今回落ちたのは、置いた表がでたらめで
  +6 に不正なアドレス（$020a00ea 相当）が並んでいたためと説明が付く。

**解釈（知識であって実測ではない）**

- Human68k のデバイスドライバヘッダは一般に
  「+0 次のヘッダ / +4 属性ワード / +6 ストラテジ / +10 インタラプト / +14 名前(8バイト)」の形をとる。
  今回実測した「+4 をワードで読み、+6 をロングで読んで呼ぶ」は**これと矛盾しない**。
  ただし **+10 と +14 が実際に使われるかは未実測**である。
- 次はこの形に沿ったヘッダを窓に置き、ストラテジ／インタラプトが呼ばれるかを実測する。

## ストラテジとインタラプトが呼ばれた：ヘッダ構造を実測で確定（2026-09-02）

**やったこと**：窓 `$ea0100` にデバイスドライバヘッダの形（+0 次ヘッダ=$ffffffff / +4 属性ワード（ホストから差し替え可、既定$0000） /
+6 ストラテジ / +10 インタラプト / +14 ユニット数$01+"SCSI   "）を置き、ストラテジとインタラプトにはそれぞれ別の印を出すだけのスタブを向けた。
`d2` と `a4` の両方に `$00ea0100` を渡して起動した。プローブに `--drv-attr=` を追加した。

**実測結果**
- **ストラテジが呼ばれた。**戻り番地 `$0000df48`、`a5=$000067de`、`a1=$00ea0100`（渡したヘッダ）、`a0=$00ea0020`（ボードROMヘッダ）
- **続いてインタラプトが呼ばれた。**戻り番地 `$0000df4e`。レジスタは `a4` 以外ストラテジ時と同じ
- 以前に観測していた「$00000000 へ2回飛ぶ、戻り番地 $df48/$df4e」の正体は、**この2つの呼び出しだった**
- `a5` が指す先（16バイト）: `16 00 00 00 6f fc 00 ea 00 20 00 ea 00 20 00 00`
  - `+0=$16`(=22、ヘッダ長) `+1=$00`(ユニット番号) `+2=$00`(**コマンド0＝初期化**) `+3=$00`
  - `+6..9` と `+10..13` に `$00ea0020`（ボードROMヘッダのアドレス）が入っている
- `a6=$00006734` は CONFIG.SYS の行のテキスト（`CONFIG  = \SYS...`）を指していた
- スタブは印を出して rts するだけなので、画面は「システムを起動できませんでした。リセットしてください」で止まる

**確定したこと**
- ヘッダの `+4` を**ワード**で、`+6` を**ロング**で読んで呼ぶ、`+10` のロングも読んで呼ぶ、を**実測で確認**した。
  「+0 次ヘッダ / +4 属性 / +6 ストラテジ / +10 インタラプト / +14 名前」という一般的な形は、
  少なくとも +4 / +6 / +10 については実測と一致する（+0 と +14 の意味は今回も未実測）
- **要求ヘッダはコマンド0（初期化）で、`a5` で渡される。**要求ヘッダ長は22バイトだが、
  今回のダンプは16バイトぶんしか取っていない。残り6バイト（`+16`〜`+21`）は未観測

**次**：ストラテジで `a5` を控え、インタラプトで要求ヘッダを読んで初期化の返答（エラーコード・ユニット数・
処理終了アドレス・BPBテーブルのポインタ）を書き戻す。返答の書き戻しは C 側（HLE）で行う。

## ヘッダを渡しているのは a4 だった。ただし返答の中身は結果を変えない（実測、2026-09-02）

### 実装したこと
- インタラプトで要求ヘッダを読み、初期化コマンド($00)への返答を C 側（HLE）で書き戻すようにした。
  返答の各欄（エラーコード `+3` / ユニット数 `+13` / 処理終了アドレス `+14` / BPB表ポインタ `+18` /
  ステータスワード `+4`）と、ヘッダの属性ワードは、**再ビルドせずホスト(JS)から差し替えられる**ようにした。
- 窓を `$ea0100`〜`$ea07ff` に広げ、`$ea0600` に BPB表ポインタ `$00ea0610` を置いた（BPB本体はゼロのまま。
  Human68k がどのバイトを読むかを実測するため）。読み出しログの上限は512件。

### 要求ヘッダの実測（初期化コマンド、26バイト）
処理前: `16 00 00 00 6f fc 00 ea 00 20 00 ea 00 20 00 00 7b 36 00 00 7f 76 02 00 67 34`
処理後（既定の返答を書いた場合）: `16 00 00 00 6f fc 00 ea 00 20 00 ea 00 01 00 ea 05 00 00 ea 06 00 02 00 67 34`
- `+14` と `+18` には元々 `$00007b36` と `$00007f76`（どちらもRAMを指す値）が入っていた。
  **これらは入力かもしれず、我々が上書きしてよい欄なのかは未確定。**

### 掃引しても結果が変わらなかった（重要）
次のどれを試しても **画面は「システムを起動できませんでした。リセットしてください」の1行だけで、
Human68k のバナー表示にすら達しない**。BPB（`$ea0610`）は**一度も読まれない**。

| 振った条件 | 値 | 結果 |
|---|---|---|
| ユニット数 `+13` | 1 / 0 | 変わらず |
| `+14`/`+18` | 我々の値 / 元の値のまま | 変わらず |
| 属性ワード `+4` | `$0000` / `$2000` / `$8000` | 変わらず |
| ステータスワード `+4`(要求ヘッダ側) | 未書き込み / `$0000` | 変わらず |
| `d2` | `$00ea0100` / `1` / `0` | 変わらず |

**返答の中身は結果を変えていない。**したがって、いま起動を止めているのは返答の欄ではない。

### d2 と a4 の役割を切り分けた（決定的）
`d2=$00ea0100`（ヘッダ）・`a4=$00ea0700`（窓の中の別番地、ゼロ）で走らせたところ、
読まれたのは **a4 の指す先**だった。

| 番地 | PC | 幅 |
|---|---|---|
| `$ea0704` | `$00007152` | ワード |
| `$ea0706`〜`$ea0709` | `$0000df46` | ロング |
| `$ea070a`〜`$ea070d` | `$0000df4c` | ロング |

- **`a4` がデバイスドライバヘッダのアドレスである。**`d2` はヘッダのポインタではない。
- `d2` は `-1` 以外なら値を問わない（`0` でも `1` でも `$00ea0100` でも同じ経路に入る）。
  **フラグ（デバイスの有無）に見えるが、意味は未確定。**
- これで以前の観測「`d2=0` にすると `$00000000` へ飛ぶ」の説明が付く。
  あのとき `a4` は `0` だったので、Human68k は**ヘッダを `$00000000` から読み**、
  `+6` のストラテジ（＝0）を呼んでいた。**過去の観測と辻褄が合う。**

### 分かっていないこと
- 返答の中身に関わらず起動が失敗する理由。画面が1行しか出ないことから、
  失敗は Human68k のごく初期（バナー表示より前）で起きている
- 要求ヘッダの `+13`/`+14`/`+18` が本当にユニット数・終了アドレス・BPB表ポインタなのか
  （**一般的な形からの当てはめであって、実測で確かめてはいない**）
- 次に振るのは「インタラプトから戻るときの `d0`」と「ヘッダ `+0`（次のヘッダ）の値」

### 追測：さらに4条件を振っても動かない。連鎖にも繋がれていない

- 追加で振った条件と結果（**すべて同じ失敗**。画面は「システムを起動できませんでした。リセットしてください」の1行のみ）:

| 振った条件 | 値 | 結果 |
|---|---|---|
| インタラプトからの戻り値 `d0` | 触らない / `0` | 変わらず |
| ヘッダ `+0`（次のヘッダ） | `$ffffffff` / `$00000000` | 変わらず |
| 要求ヘッダ `+4` のワード | 未書込 / `$0000` / `$0001` | 変わらず |

- **これで振った軸は9つになったが、結果は一度も動いていない。**返答の内容・戻り値・ヘッダの各欄のどれもが原因ではない。

- **停止後のゲストRAMを走査した（プローブに `--dump=<番地>:<バイト数>` を追加）。**
  `$10000`/`$11000`/`$6000` から各4KB を読み、我々のヘッダ（`$00ea0100`）を指す値を探した。

| 見つかった場所 | 値 |
|---|---|
| `$679c` | `$00ea0100` |
| `$67c8` | `$00ea020a`（インタラプトのアドレス） |
| `$67d8` | `$00ea0100` |

  いずれも要求ヘッダ（`$67de`）のすぐ手前の作業領域であり、**デバイスドライバの連鎖に繋がれた形跡は無い。**

- **副産物：Human68k 内蔵のディスクドライバのヘッダを実測できた。**`$00010da2` にあり、中身は
  `next=$ffffffff / 属性=$0000 / ストラテジ=$00010df0 / インタラプト=$00010df8 / $10 + "HARDDSK"`。
  **我々が組んだヘッダの形（+0 次 / +4 属性 / +6 ストラテジ / +10 インタラプト / +14 ユニット数+名前）が
  本物と同じ形であることの裏取りになる。**また失敗後もこのヘッダの `next` は `$ffffffff` のままで、
  **我々のドライバは連鎖の末尾にも繋がれていない。**

- **要求ヘッダの解釈がひとつ崩れた。**実測値では `+10` から `$00ea0020` のロングが入っており、
  「`+13` がユニット数」という当てはめは `+10` のロングと重なる。**MS-DOS 系の並びをそのまま当てるのは誤りである可能性が高い。**
  実測から言えるのは、22バイトが `長さ(1) / ユニット(1) / コマンド(1) / ?(1) / ワード(2) / ロング×4` に見える、というところまで。

- **分かっていないこと**：Human68k が何を根拠に登録を拒否しているのか。返答を読んでいる形跡すら無い。

## 本物のSCSI ROMをオラクルとして走らせた：SPCの叩き方を実測できた（2026-09-02）

### 方針（先に書く）

- 利用者の判断により、**本物の外部SCSIボードROM(8192バイト)を px68k に読み込ませて走らせ、
  「何番地を叩き、何を返すか」だけを実測する**ことにした。返答の仕様が手元の資料2冊からは決められず、
  9軸の掃引でも結果が1ミリも動かなかったためである。
- **逆アセンブルはしない。走らせて外から観測するだけ。**ROM は追跡外の `_local/scsi/` に置き、
  **リポジトリにも配布物にも入れない。**最終的に実装するのは自前のコードであり、
  本物ROMは仕様を決めるための基準器（オラクル）として使う。
- ROM は 2026-08-23 のXM6での移行作業時に生成されていたものが手元にあった（`Scsiexrom.dat` / `Scsiinrom.dat`）。

### 実装

- `SCSI_Init` で、ホスト(JS)から渡された 8192 バイトを自前スタブの代わりに使えるようにした
  （`--rom=<パス>`）。本物ROM使用時は自前スタブ向けの処理（d2/a4差し替え・観測用テーブル・窓selftest）を全て飛ばす。
  読めたことの確認として `$ea0044` が `"SCSIEX"` であることを毎回検査する（実測でPASS）。
- `$ea0000`〜`$ea1fff` の読み書きを `[SCSI-BUS]` として全件ログする。
  **最初の実装ではログ4000件が全て命令フェッチで埋まり、目的のSPCポートが1件も見えなかった。**
  px68k ではこの領域の命令フェッチも `SCSI_Read` を通るためである。
  「`adr` が PC の±8バイト以内なら命令フェッチとみなす」heuristic で除外し、
  **SPCのポート域($ea0000〜$ea001f)は除外判定に関わらず必ず出す**ようにして解決した。
  除外された命令フェッチは **9,031,710件**だった。

### 実測：本物ROMは何をするか

- ヘッダの読まれ方は自前スタブのときと同じ流れ（ID "SCSIEX" → `$ea0020` → "Human68k" の検証 → ベクタ設定エントリ）。
  本物ROMでは `$ea0020`〜`$ea003c` の8本すべてが `$00ea0092` を指し、
  `$ea0082`=`$00ea0154`（ベクタ設定エントリ）、`$ea0086`=`$00ea04ba`（IOCS入口）、`$ea008a` から `"Human68k"`、
  起動エントリは `$ea0092` にある。**IOCSベクタ `$07d4` は実際に `$00ea04ba` に設定された**（実測）。
- そのあと ROM は **SPC(MB89352)のレジスタを叩き始める**。最初の並び（抜粋、実測）:

| 番地 | 値 | 意味の候補 |
|---|---|---|
| `$ea0003` | `$90` | SCTL（リセット） |
| `$ea0001` | `$07` | BDID＝本体のSCSI ID 7（SRAMの既定値と一致） |
| `$ea0005` `$ea0011` `$ea0019` `$ea001b` `$ea001d` `$ea0017` | `$00` | 各レジスタの初期化 |
| `$ea0003` | `$10` | SCTL（動作開始） |
| `$ea0017` | `$01` | TEMP（選択する相手） |
| `$ea0019`/`$ea001b`/`$ea001d` | `$09`/`$c4`/`$03` | 転送カウンタ（タイムアウト値と思われる） |
| `$ea0005` | `$20` | SCMD（セレクト） |

  そのあと **`$ea0009`(INTS)を読み、`$ea000d` に `$05` を書く**ことを延々と繰り返す。
  px68k は SPC を実装しておらず読み出しが常に `$00` のため、**ROM はセレクト完了を待って永久に回り続ける**。
  画面は真っ暗のまま、Human68k のバナーも出ない。
- **番地の意味の対応付け（BDID/SCTL/SCMD/INTS など）は知識による当てはめであって、実測ではない。**
  実測として言えるのは「この番地にこの値を、この順で書き、この番地を読んで待っている」までである。

### これで分かったこと

- **本物ROMを動かすには最低限のSPCの応答が要る。**逆に言えば、SPCを「セレクトが成功し、
  コマンドとデータのやり取りができる」程度に作れば、**Human68k側の登録処理は本物ROMが全部やってくれる。**
  そこで d2/a4 に何が返り、要求ヘッダに何が書かれ、どのコマンドが来るのかを**実測で確定できる。**
- 得た規約で自前ROMスタブを実装すれば、**配布物に本物ROMは不要**という当初の利点は保てる。

### 次

最小限のSPC応答（セレクト成功 → コマンドフェーズ → INQUIRY / READ CAPACITY / READ(6) をホスト側イメージで応答）を
実装し、本物ROMを完走させて、Human68k とのやり取りを実測する。

## SPCの最小応答を実測で組み上げ、本物ROMがHuman68kへ戻るところまで来た（2026-09-02）

### 進め方

値を1つずつ変えて再実行すると256通りで時間が溶けるため、**「読み出しのたびに返す値を0→255と1ずつ変える掃引モード」**を
レジスタごとに用意した（`--spc-ssts=-2` `--spc-psns=-2`）。ROM は同じループを高速で回るので、**1回の実行で全値を試せる。**
またゲストの PC を60フレームごとにログへ出す診断（`[SCSI-PC]`）を足した。

### 観測系の誤りを1つ踏んだ（先に書く）

「SPCに応答したらバスアクセスがぱったり止まった」と一度判断したが、**誤りだった。**
実際は「同一PCからのアクセスが32件を超えたらログを止める」という圧縮規則が効いていただけで、
ROM はループを回り続けていた。**PCサンプラを入れて初めて `$ea1038` で回っていると分かった。**
ログの圧縮規則が「何も起きていない」ように見せることがある。

### 実測で確定した規則

| 事実（実測） | 内容 |
|---|---|
| セレクトの成立 | `$ea0005`(SCMD) に上位3ビット `001`（実測値 `$20`）が書かれたとき。`$ea0009` に `$08` を立てると ROM は先へ進む |
| `$ea000d` の bit7 | `pc=$ea1034` の読み出しは **bit7 が立っていると先へ進まない**。`pc=$ea109a` の読み出しは **bit7 が立つと抜ける**。つまり固定値では駄目で、**「セレクトで立ち、接続の終わりで下りる」状態**でなければならない |
| `$ea0009` への書き込み | 実測では `$00` と `$08` が書かれる。割り込み要因のクリアと解釈した（**未実測の解釈**） |

- レジスタ名（SCMD/INTS/SSTS/PSNS/PCTL/TEMP など）と番地の対応は**当てはめであって実測ではない**。
  実測として言えるのは「この番地にこの値を書き、この番地のこのビットを見て分岐している」までである。
- **実験的な措置**：再試行のたびに測定を1つ進めるため、`$ea0011`(PCTL) への書き込みでも bit7 を落とすようにした。
  **これは仕様として測ったものではない**（`--spc-clear-on-pctl=0` で外せる）。

### 到達点

上の規則を入れたところ、ROM は **PSNS を14回読んだ（＝セレクトを14回試した）あと ROM を抜け、
ゲストの PC が `$0000df48` に来た。**これは**Human68k がデバイスドライバのストラテジを呼ぶ、あの呼び出し口**である。
つまり **本物ROMは「デバイスは見つからなかった」なりの結果を持って Human68k へ戻り、ドライバとして呼ばれ始めた。**
起動そのものはまだ失敗（画面は真っ暗）で、SCSI IOCS の呼び出しは0件のまま。

### 次

**RAMへの書き込みを覗く仕掛けを足す。**本物ROMのドライバが要求ヘッダ（実測で `$67de` 付近）へ
何を書くのかを見れば、**我々が9軸の掃引でも決められなかった「初期化への返答の形」が実測で分かる。**

## 本物ROM＋最小SPCで Human68k が正常起動した（実測、2026-09-02）

### 道具として足したもの

- **ゲストRAMへの書き込み監視。**`x68k/mem_wrap.c` の `wm_cnt()`（RAM書き込みが集約される1箇所）にフックを入れ、
  番地範囲（`--ram-watch=<lo>:<hi>`）と**書いた側のPC範囲**（`--ram-watch-pc=<lo>:<hi>`）で絞ってログする。
  **陽性対照つき**（有効時に範囲内へ1バイト書いて、ログに出なければ ERROR）。実測で対照は通っている。
- PC範囲の絞りが要ったのは、最初の実測で**IPLのメモリクリア(pc=$ff9006/$ff900c/$ff9010)の書き込みだけでログ512件が埋まった**ため。

### 実測

- 本物ROM＋これまでに実測で組み上げた最小SPC応答（セレクト成立／SSTSのbit7状態機械／PSNS掃引）で、
  **Human68k が `A>` プロンプトまで正常起動した。**画面には通常どおり各ドライバのバナーが出る。
  IOCSベクタ `$07d4` は本物ROMの `$00ea04ba`。**SCSI IOCS の呼び出しは0件**（デバイスが見つかっていないため）。
- **判定を1つ訂正する。**その前の20秒実行で「画面が真っ暗のまま失敗」と書いたが、
  **単に起動の途中だった可能性が高い。**30〜40秒に延ばすと同じ条件で起動する。
  自前スタブでの失敗は画面に「システムを起動できませんでした」と明示されていたので、そちらは本物の失敗である。
- ROM のドライバが要求ヘッダ（`$67de` 付近）へ書き込む様子は**まだ観測できていない**。
  デバイスが1台も見つかっていないため、登録まで進んでいないと考えられる。
  なお `$67xx` への書き込みの大半は `pc=$ea0158`（ベクタ設定エントリ）の**レジスタ退避（スタック）**だった。

### 意味

- **「動く状態」が基準器として手に入った。**これまでは失敗しか観測できず、
  何を直せば良くなるのかを判断する足場が無かった。
- 次は SPC をもう一段進めて、**コマンドフェーズを成立させ、ROM にデバイスを1台見つけさせる。**
  そこまで行けば ROM がドライバとして登録され、**要求ヘッダに何を書くのかが実測できる**。
  それが、自前実装で9軸振っても決められなかった「初期化への返答の形」の答えになる。

## セレクト完了のビットと、転送待ちの正体（実測、2026-09-02）

### 観測系の穴をさらに2つ塞いだ

- **「同一PCから32件でログを止める」抑制をホストから外せるようにした**（`--bus-pc-limit=0`）。
  この抑制は既に一度、無限ループを「アクセスが止まった」と誤読させている。上限（`--bus-log-max=`）も可変にした。
- **ゲストのメモリ読み出し監視を足した**（`--mem-read-watch=<lo>:<hi>` と `--mem-read-watch-pc=<lo>:<hi>`、陽性対照つき）。
  書き込み監視と同じ作りで、`rm_main`（読み出しの共通経路）にフックする。
- **これで「待ちループは SCSI 領域を触っていない」という前の判断が誤りだと分かった。**
  実際は `pc=$ea144a` から `$ea000b`(PSNS)を**181回ポーリング**していた。
  `[SCSI-BUS]` に出ていなかったのは、**連続を圧縮した保留分が実行終了時に吐き出されない**ためだった。
  **同じ「絞り込みが沈黙を作る」誤りを、今日3回踏んでいる。**

### 実測で確定したこと

| 事実 | 内容 |
|---|---|
| セレクト完了のビット | `$ea0009`(INTS)に **`$10`** を立てると ROM は先へ進む。`$08`/`$01`/`$02` では駄目で、8個のIDを順に試して諦める。`$04` でも一応進むが `$10` の方が到達点が遠い |
| その後の並び（実測） | `$ea1138` で ROM 内変数を読む → `$ea0009` 読み → `$ea000b` 読み → `$ea0011`(PCTL)に `$02` を書く → `$ea000b` 読み → `$ea0017` に `$00` → **`$ea0005` に `$ec`（転送）** |
| 転送後の待ち | `pc=$ea144a` で `$ea000b`(PSNS)をポーリングし続ける |
| 待ちの正体 | **固定値では絶対に抜けない。0〜255の全値を返す掃引でも抜けない**（1回の実行で8周ぶん試した）。掃引は「読むたびに+1」なので連続する2読は必ず (v, v+1) にしかならない。**2値を交互に返すモード（`$8a` と `$0a` を交互）にしたところ、ROM はループを抜けた。** つまりこれは**ハンドシェイク待ち**（同じ値を返し続ける限り永久に待つ）である |
| 交互にしたときの結末 | ROM はループを抜け、Human68k は `A>` まで起動した。ただし `$ea0015`(DREG)へのアクセスは0件で、**コマンドの中身はまだ流れていない** |

- レジスタ名（INTS/PSNS/SSTS/PCTL/DREG）と番地の対応、`$8a` が「コマンドフェーズ」であること、
  `$ec` が「転送」であることは、**いずれも当てはめであって実測ではない**。
  実測として言えるのは「この番地にこの値を書き、この番地の値が変わるまで待つ」までである。

### 次

**転送の状態機械を作る。**ROM が DREG(`$ea0015`)へ書くコマンド記述子（CDB）を受け取り、
その中身をログに出すところまで行けば、**本物ROMが何を尋ねているのかが実測で分かる**。
そのうえで INQUIRY / READ CAPACITY / READ をホスト側イメージで応答すれば、
ROM はデバイスを1台見つけ、Human68k へドライバとして登録する。
**そこで初めて「初期化への返答の形」が実測できる。**

## SPCの転送を組み上げ、本物ROMがディスクを読み始めた（実測、2026-09-02）

### 実測で分かった転送の形

- **コマンド記述子(CDB)は DREG ではなく `$ea0017`(TEMP)経由で1バイトずつ送られる。**
  ROM は「TEMPに1バイト書く → `$ea0005`(SCMD)に上位3ビット `111`(`$ec`) → PSNSのREQが落ちるのを待つ」を繰り返す。
  **この形でCDBを6バイト受け取れた。最初のコマンドは `00 00 00 00 00 00`（TEST UNIT READY）だった。**
- **データ入力(DATAIN)は別の手順。**`$ea0011`(PCTL)に `$01`、
  転送数を **`$ea0019`(上位)/`$ea001b`/`$ea001d`(下位)** に書き（READ CAPACITY のとき **8** が入った＝応答長と一致。
  **並びはこれで確定**）、`$ea0005` に上位3ビット `100`(`$80`)。
  そのあと `$ea000d`(SSTS)をポーリングし、DREG(`$ea0015`)からバイトを読む。
- **STATUS/MSGIN も同じ枠組みで1バイトずつ受け渡す。**そのあと切断してバスが空く。

### 到達点：本物ROMがディスクを読んでいる

SSTS を「読み出しのたびに値を変える掃引」にした状態で走らせると、**一連のSCSIコマンドが流れた**（実測）:

| CDB | 意味 |
|---|---|
| `00 00 00 00 00 00` | TEST UNIT READY |
| `25 00 00 00 00 00` | READ CAPACITY |
| `28 ...` | READ(10) |
| `01 ...` | REZERO UNIT |
| `08 ...` | READ(6) |
| `1a 00 3f 00 04 00` | MODE SENSE |

READ CAPACITY の応答8バイト（`00 03 1f ff` / `00 00 02 00`＝最終LBA 204799・ブロック512）も渡っており、
**ホスト側イメージの実際の容量が ROM に届いている**。Human68k も起動する。

### まだ通っていないこと（正直に書く）

- **SSTS を固定値にすると2コマンド目で止まる。**掃引のときだけ流れる。
  掃引で抜けた瞬間の値は `$b0`（`$80`＋`$20`＋`$10`）だったが、
  **`$b0` を固定で返しても通らない。**PSNS のときと同じく「値が変わること」を見ている可能性が高いが、**未確定**。
- **観測系の同じ穴を今日4回踏んだ。**「連続する同じ値を圧縮する」仕組みが、
  **保留中の分を実行終了時に吐き出さない**ため、ポーリング中の値が1行も残らない。
  固定値のときに実際に何を返しているのかが見えず、切り分けが止まっている。**次はここを直す。**

### 道具として足したもの
`--spc-ints-sel=`（セレクト完了ビット、実測で `$10`）、`--spc-ssts-data-bit=`（掃引 `-2` 可）、
`--spc-ssts-tc0=`、`--spc-cdb-from-temp=`、`--bus-pc-limit=`、`--bus-log-max=`、
`--mem-read-watch=` / `--mem-read-watch-pc=`（読み出し監視、陽性対照つき）。

### 続き：沈黙の正体は自分たちの実験的規則だった

- **圧縮ログの保留分を定期的に吐き出すようにした。**60フレームに1回と実行終了時に、
  保留中のエントリを「(継続中)」付きで増分件数とともに出す。`[SCSI-BUS]` と `[SCSI-MEMR]` の両方に入れた
  （`[SCSI-RAM]` は元々1件ごとに出しているので対象外）。
- **直したその場で答えが出た。**DATAIN の待ちで実際に返していた値は `$b0` ではなく **`$30`** だった。
  つまり**接続中のビット `$80` が落ちていた**。
  犯人は**我々自身が入れた実験的規則「PCTL(`$ea0011`)書き込みで SSTS の bit7 を落とす」**である。
  ROM は DATAIN の開始時に PCTL を書くため、**転送のたびに接続が切れた扱いになっていた。**
  この規則は「再試行のたびに測定を1つ進める」ために入れた便宜的なもので、実機の仕様として測ったものではなかった。
- **`--spc-clear-on-pctl=0` にして固定値 `--spc-ssts-data-bit=0x30 --spc-ssts-tc0=0` にすると、
  掃引なしで READ(10) まで流れ、Human68k も起動する**（実測、CDB 5件:
  `00`/`25`/`28 ... 04`/`28 ... 00`/`28 ... 06`。LBA 4・0・6 を読んでいる）。
- **掃引(`-2`)は、この状態ではむしろ悪い**（接続中ビットまで壊すため2コマンド目で止まる）。
  **掃引は「値が分からないときに探す道具」であって、通す設定ではない。**
- 現時点の最良設定：`--spc-ints-sel=0x10 --spc-clear-on-pctl=0 --spc-ssts-data-bit=0x30 --spc-ssts-tc0=0`
- **まだ止まる。**上の設定でも `pc=$ea0ec0` の別のループで止まり、デバイスの登録までは進んでいない。
  SCSI IOCS の呼び出しは0件のまま。

**沈黙を作っていたのは対象ではなく、我々が観測と実験のために入れた仕掛けだった。今日これで4回目である。**

## 本物ROMは1台しか探していない（実測、2026-09-02）

- **セレクトに応答するIDを1つに絞れるようにした**（`--spc-target=`、既定1）。
  「どのIDでも成功する」ままだと ROM から同じディスクが8台に見えるのではないか、と疑ったため。
- **その疑いは外れた。**セレクトを1度も成功させない条件（`--spc-target=99`）で走らせ、
  16回のセレクト試行**すべての直前の書き込みを比べたところ、毎回まったく同じ**だった：

```
R $ea0001 = $07      (自分のID)
W $ea0003 = $10
W $ea0017 = $07      ← 毎回 $07 で変化しない
W $ea001b = $c4 / W $ea0019 = $09 / W $ea001d = $03
W $ea0005 = $20      (セレクト)
```

  **`$ea0017`(TEMP)は16回とも `$07` のまま。ROM は相手を変えて走査していない。**
  つまり **ROM が探しているのは1台だけ**であり、「8台に見えて混乱している」という仮説は棄却された。
  （`$07` が何を意味するのか——相手のIDなのか自分のIDなのか——は**未確定**。当てはめで断定しない）
- あわせて未対応だったコマンドに応答を足した：MODE SENSE(`$1a`)・REZERO(`$01`)・START-STOP(`$1b`)・
  REQUEST SENSE(`$03`)。INQUIRY は CDB の割り当て長で切るようにした。**いずれも当てはめ。**
- **登録はまだ起きていない。**内蔵HARDDSKヘッダ(`$10da2`)の next は `$ffffffff` のままで、
  ROM から RAM への書き込みもスタック操作だけだった（RAM書き込み監視、PC範囲=ROMで確認）。
- **SRAM を資料どおり(`$56`/`$07`/`$00`)にした条件でも変化は見られなかった**が、
  この実行は他の計測と時間が重なっており、**条件が揃っていない参考値**として扱う（再測が要る）。

## 計測系が遅くなった：原因はまだ特定できていない（2026-09-02）

**未解決の問題を正直に書く節。**以下はすべて親セッションの実測。

- **症状**：本物ROM＋SPC状態機械の実行が、**1回あたり6〜10分**かかるようになった。
  ゲストの進み方は **60フレームごとのPCサンプラで約540〜600フレーム／390秒＝毎秒1.4フレーム**
  （通常は毎秒55フレーム前後）。**約35倍遅い。**
- **基準は健全**：本物ROMを載せない既定経路は **38秒で起動する**（`elapsedMs` 26,675）。
  つまり環境や機械の問題ではなく、本物ROMを走らせる経路に固有である。
- **潰した仮説**（いずれも実測で否定）：
  1. **ホスト設定の取得がアクセスごとにJSへ往復している** → 24個の設定をフレーム単位のキャッシュに変更。**変わらず。**
     （陽性対照として、初期化時に読み込んだ設定値を1行出すようにした。値は正しく届いている）
  2. **ログ経路のコスト**（`$ea0000`〜`$ea1fff` の命令フェッチが毎回 `SCSI_Read` を通り、
     そのたびに `m68000_get_reg()` を呼ぶ。実測で除外件数 **900万件超**） →
     ログ上限到達後にログ経路ごと飛ばす早期脱出を実装し、**上限を50に下げて強制的に発火させても変わらず。**
  3. **セクタ読み出しの同期XHR** → 64KB単位のチャンクキャッシュ（保持16個=1MB上限）を実装。
     ただし**そもそもXHRは64回に達しておらず**（64回ごとのログが1行も出ない）、量が問題ではない。
- **分かっていること**：遅い最中のゲストのPCは **RAM上（`$1d33a`/`$1d354`/`$1cd6c`）**であり、
  ROM内で回っているわけではない。SCSIコマンドは3件しか観測できていないが、
  **これはログ上限で隠れている可能性がある**（上限を外して数え直す測定を実施中）。
- **影響**：1回の実験に6〜10分かかるため、掃引や条件振りの回転が落ちている。
  **原因の特定を先にやるべき状態である。**

**教訓**：基準を1本取ったから環境は健全だと分かった。基準が無ければ「機械が重い」で片付けていた。

## 本物のSCSIドライバのヘッダを実測できた（2026-09-02）

### 性能低下の正体（先に書く）
- 前の節で「原因不明」とした**6〜10分かかる問題は解決した。正体はセレクトの嵐だった。**
  ログ上限を外して数えたところ、**セレクト14,821回／INTSクリア14,893回に対し、通ったコマンドは36件**だけだった。
- 失敗している試行の発信元は **RAM上のコード(`pc=$0001cd8c`)** で、**TEMPに `$0f`** を書いていた。
  一方 **ROM内のルーチンは `$07`** を使う。こちらが `$07` にしか応答していなかったため、
  RAM側が延々と再試行していた。**どちらにも応答する（`--spc-target=-1`、既定）ようにしたところ、
  実行は61秒に戻り、コマンドは18件流れ、Human68k は正常起動した。**
- **TEMPの意味は依然として未確定**（ROMとRAMで違う値を使う理由は分かっていない）。

### 実測：本物のドライバはRAMに載って連鎖に繋がれる
起動後にゲストRAMを読んだところ、**内蔵HARDDSKヘッダ(`$10da2`)の次ポインタが
`$ffffffff` から `$0001a46c` に変わっていた**。つまり **SCSIドライバはRAMへ載せられ、
デバイスドライバ連鎖に繋がれている**。これまで「登録されていない」と判断していたのは、
**ROM内のPCで絞って監視していたためドライバがRAMにあることを見落としていた**からである。

`$0001a46c` の中身（実測）:

| オフセット | 値 | 意味 |
|---|---|---|
| `+0` | `$0001e7fc` | 次のヘッダ（さらに続く） |
| `+4` | **`$4000`** | 属性ワード |
| `+6` | `$0001a4da` | ストラテジ |
| `+10` | `$0001a4ec` | インタラプト |
| `+14` | `$01` | ユニット数 |
...
- **属性ワードは `$4000` だった。**自前スタブで振ったのは `$0000` / `$2000` / `$8000` の3つで、
  **`$4000` は一度も試していなかった。**
- ヘッダの形（+0 次 / +4 属性 / +6 ストラテジ / +10 インタラプト / +14 ユニット数＋名前）は、
  **我々が実測から組み立てた形と完全に一致している。**名前が7バイトである点も一致。

### これが意味すること
自前ROMスタブ（route A）で9軸を振っても起動が変わらなかった原因の**有力候補が1つに絞れた**：
**属性ワードが違っていた可能性**である。次は自前スタブで `--drv-attr=0x4000` を試す。

### まだ取れていないこと
- 初期化コマンドへの**返答の中身**（本物のドライバが要求ヘッダに何を書くか）は未捕捉。
  `$6734` を監視したが書き込みは0件で、そこは CONFIG.SYS のテキストを指すポインタだった。
  要求ヘッダの実体の位置を先に特定する必要がある。

## 属性ワード $4000 で自前スタブが受け入れられた（実測、2026-09-02）

### オラクルで得た値を自前スタブへ持ち帰った

本物ドライバのヘッダから得た **属性ワード `$4000`** を自前スタブに与えたところ、**画面の反応が変わった。**

| 条件 | 画面 |
|---|---|
| 属性 `$0000` / `$2000` / `$8000`（過去に9軸振った） | 「システムを起動できませんでした」のみ。**何を変えても不変だった** |
| 属性 **`$4000`**、BPB表ポインタ=`$00000000` | **「１セクタあたりのバイト数が大きすぎます」**（新しいメッセージ） |
| 属性 `$4000`、BPB表ポインタ=`$00ea0600`（中身はゼロ） | 上のメッセージが消え、「システムを起動できませんでした」に戻る |

- **Human68k は我々のドライバを受け入れ、BPB(ディスクの諸元)を読みに来ている。**
  ポインタを変えるとメッセージが変わることから、**渡したポインタを実際に辿っている**と分かる。
- **9軸を振っても結果が動かなかった原因は、属性ワードだった可能性が高い。**
  当時 `$4000` は候補に入れていなかった。**「変えても変わらない」ときに振っていない軸が残っていた。**

### 途中で見つけた欠陥（設定が無言で効かなくなっていた）

`--reply-units` などの返答系の設定が**まったく反映されていなかった**。
原因は、フレーム単位のキャッシュ関数 `SCSI_RefreshHostConfig()` の冒頭にあった
`if (!SCSIUsingRealRom) return;` で、**自前スタブ経路では返答系が一度も読み込まれず、C側のstatic既定値0のまま固定**されていた。
（属性ワードだけは別経路で効いていたため、「効く設定と効かない設定が混在」して見えた）

- 対策：早期returnを撤廃し、**起動時の陽性対照ログに全設定値を並べる**ようにした
  （`ints_sel / ssts_data_bit / target / clear_on_pctl / reply_err / reply_units / reply_end / reply_bpb /
  reply_status / reply_d0 / drv_attr / drv_next`）。**設定を足すときはキャッシュと陽性対照の両方に足す**とコメントにも明記した。

### 次

**正しい BPB を作って返す。**いまは `$ea0610` 以降がゼロのままなので、
ディスクの諸元（1セクタあたりのバイト数など）が0として読まれている。
ホスト側イメージのパーティションから作れるはずである。


## BPBを基準器イメージから写した／観測系の故障を見つけた（実測、2026-09-03）

### 1. BPBは知識で組み立てず、イメージから写した

自前スタブが初期化コマンド($00)で返す BPB($ea0610)を、**基準器イメージの
パーティション先頭ブートセクタから20バイトそのまま写す**ようにした（`x68k/scsi.c` の `SCSI_Init`）。

実測した構造（基準器 = 100MB の SCSI イメージ）:

- LBA0 先頭が `"X68SCSI1"`
- **LBA4(`$800`) にパーティション表**。`"X68K"` + 12バイトのヘッダのあと、16バイトのエントリ
  （名前8 + 開始4 + サイズ4、いずれもBE、**単位は1024バイトブロック**）。
  実測値は 名前=`"Human68k"` / 開始=`$20` / サイズ=`$18c00` で、`$18c00 * 1024` がイメージの実サイズと整合する
  （SASI の `.hdf` は256Bブロック単位だったので、**SCSI とは単位が違う**）
- パーティション先頭(`$20 * 1024 = $8000`)がブートセクタで、2バイト分岐 + 16バイトOEM(`"SHARP/KG    1.00"`)のあと **`+$12` から BPB**

`+$12` からの並びの解釈（16bit値はすべてBE）:

| 位置 | 値 | 意味 |
|---|---|---|
| +0 (word) | `$0400` | 1セクタのバイト数 = 1024 |
| +2 (byte) | 2 | 1クラスタのセクタ数 |
| +3 (byte) | 2 | FATの個数 |
| +4 (word) | 1 | 予約セクタ数 |
| +6 (word) | `$0200` | ルートディレクトリのエントリ数 = 512 |
| +8 (word) | 0 | 総セクタ数(16bit)。65535超なので下の32bit欄を使う |
| +10 (byte) | `$f7` | メディアバイト |
| +11 (byte) | 100 | FATのセクタ数 |
| +12 (long) | 101376 | 総セクタ数(32bit) |
| +16 (long) | 32 | パーティション開始 |

**この割り当ては独立計算で裏が取れている。** 総セクタ101376 / 2セクタ per クラスタ = 50688クラスタ、
FAT16なので `50688 * 2 = 101376` バイト = 1024バイトセクタで99セクタ。実際の値は100で整合する。
FAT数=2・予約=1 の側に読むと（DOS風に +3 を予約セクタ数のワードと読むと512予約/1FATになり）破綻するので、
**この読み方以外では辻褄が合わない。**

ただし「Human68k が実際にどの欄をどこまで読むか」は未実測なので、解釈はログに出すだけにして、
**写す範囲は `+$12` から20バイトそのまま**とした。

### 2. 観測系が壊れていた：PC絞り込みの既定値が全件を捨てていた

**ゲストRAM書き込み監視(`[SCSI-RAM]`)と読み出し監視(`[SCSI-MEMR]`)は、実CPUのアクセスを1件も観測できていなかった。**

- 「書いた側/読んだ側のPCで絞る」条件の判定が `if (pc_lo <= pc_hi)` だった。
  既定値は `pc_lo = pc_hi = -1` なので **`-1 <= -1` が真になり絞り込みが有効化され、
  実PC(必ず0以上)は必ず範囲外**になって全件捨てられていた
- **自己検査(陽性対照)だけが `selftest_active` でこの絞り込みを迂回する**作りだったため、
  「フックは繋がっている」という判定は通り続けた。**陽性対照が、自分だけが通れる抜け道を通っていた**
- 実測（故障の証明）: `--ram-watch=0x0:0xbfffff`（RAM全域）で起動しても、出力は自己検査の1行だけだった。
  RAM全域が丸ごと沈黙するのはありえないので、これは観測系の故障である

修正（`x68k/mem_wrap.c`）: 絞り込みの有効条件を **`pc_lo >= 0 && pc_lo <= pc_hi`** に変えた。
あわせて `src/core-shim.c` の refresh で、**監視範囲が変わったときに実効設定を1行出す**ようにした。

```
[SCSI-MEMR] 監視設定: adr=$ea0000..$ea0700 pc絞り込み=無効(pc_lo=-1 pc_hi=-1)
```

表示は本体と同じ判定式から作る（表示だけ別式にすると同じ事故が再発する）。
修正後は同じ条件で **226件** の読み出しが観測できるようになった（陽性対照として、
ROMヘッダ `$ea0020`〜、`"SCSIEX"` の照合、スタブ本体 `$ea0200` の読み出しがすべて見えている）。

**この故障の影響範囲は今日の測定だけではない。** 過去に `[SCSI-RAM]` / `[SCSI-MEMR]` で
「書き込みは0件だった」「一度も読まれていない」と判断した箇所は、**すべて偽の沈黙の可能性がある**。
該当するのは少なくとも次の2つで、いずれも結論を保留に戻す:

- 「`$6734` を監視したが書き込みは0件」（本物のドライバの要求ヘッダ探索）
- 「BPB(`$ea0610`)は一度も読まれない」（掃引しても結果が変わらなかった、の節）

### 3. 訂正：「属性 `$4000` で BPB を読みに来ている」は成立しない

前節「属性ワード $4000 で自前スタブが受け入れられた」の表は、**条件が揃っていない比較だった。**

当時「BPB表ポインタ=`$00000000`」として記録した試行(`own2.json` / `own4000.json`)のログを読み直すと、
実際の返答は **`unit数=0 終了addr=$00000000 BPB表ptr=$00000000 status=0`、つまり全欄ゼロ**だった。
これは返答系の設定が効いていなかった時期（`SCSI_RefreshHostConfig()` の早期return）のもので、
**「BPBポインタだけを0にした」試行ではない。**

今日、同じ全ゼロ条件を意図的に作って比較した:

| 条件（属性は `$4000`、a4=`$00ea0100`、d2=1 で固定） | 画面 |
|---|---|
| units=0 / end=0 / status=0 / **bpb=`$00000000`** | 「１セクタあたりのバイト数が大きすぎます」 |
| units=0 / end=0 / status=0 / **bpb=`$00ea0600`（中身は実物のBPB）** | **同じ**「１セクタあたりのバイト数が大きすぎます」 |
| units=1 / end=`$00ea0500` / status未指定 / bpb=`$00000000` | 「システムを起動できませんでした」 |
| units=1 / end=`$00ea0500` / status未指定 / bpb=`$00ea0600` | 「システムを起動できませんでした」 |

**メッセージを変えているのは BPB表ポインタではなく、units/end/status の側である。**
よって「渡したポインタを実際に辿っている」という当時の推論は**否定される**。

さらに、修正後の読み出し監視で `$ea0600`〜`$ea0640` を見張っても、
**自己検査以外の読み出しは0件**（監視設定の行で絞り込み無効を確認済み）。
`$ea0000`〜`$ea0700` に広げても、**インタラプトが `rts` した後に SCSI ROM の窓は一切読まれない**。
**Human68k は我々が返した BPB表ポインタを辿っていない。**

### 4. 返答の各欄を振っても結果は動かない（再確認、今度は観測系が生きた状態で）

属性 `$4000` / a4=`$00ea0100` / d2=1 を固定し、以下を1つずつ振ったが**画面は変わらない**
（いずれも「システムを起動できませんでした」、初期化コマンドは1回きり、以後の呼び出しは無い）:

| 振った欄 | 値 |
|---|---|
| `status`(+4) | 未指定 / `$0000` |
| `end`(+14) | `$00ea0500` / `$00007b36`（入力値のまま） |
| `units`(+13) | 1 / 0 |
| `d0`（戻り値） | 未指定 / 0 / `$00ea0500` |
| `bpb`(+18) | `$00ea0600` / `$00000000` / `$00007f76` / `$00007b36` |

また、インタラプトを呼んだ側（pc=`$0000df4e` 付近）は、**戻ってきた後に要求ヘッダを読み返していない**
（`--mem-read-watch=0x67de:0x67f8 --mem-read-watch-pc=0xd000:0xe000` で0件）。
返答を読む主体は別のPCにいる。

### 5. 本物ROM側の事実（再測定が必要）

本物ROMで起動したとき、内蔵HARDDSKヘッダ(`$10da2`)の次ポインタは
`$0002c12c`（RAM）になっており、**SCSIドライバはRAMに載って連鎖に繋がれている**（dump で確認）。
一方、本物ROM使用時に `$ea0000`〜`$ea0300` の**データ読み出しは0件**だった
——が、これは観測系が壊れていた時の測定なので、**修正後に取り直す必要がある**（宿題）。

### 次

1. **修正後の観測系で、本物ROMのときの ROM窓読み出し・`$10da2` 書き込みを取り直す。**
   誰が（どのPCが）連鎖を書き換えるのかが分かれば、自前スタブがどこで拒否されているかの切り分けになる。
2. 返答の欄ではなく、**「Human68k が我々のドライバをどう扱って捨てているか」**を追う。
   要求ヘッダの返答を読む主体のPCを、PC絞り込み無しの読み出し監視から特定する。
3. 上の2つが済むまで、BPBの中身は「正しく作れているが、まだ誰も読みに来ていない」状態として据え置く。


## 直した観測系で本物ROM側を取り直した（実測、2026-09-03）

宿題23。PC絞り込みの故障を直したうえで、本物ROMのときの ROM窓読み出しと
デバイスドライバ連鎖(`$10da2`)の書き込みを測り直した。

### 連鎖を書いているのは Human68k 自身だった

`--ram-watch=0x10da2:0x10da6` で、**書き込みの主体が pc=`$000070e0`** と分かった
（`$0002c12c` を4バイト書き込み。IPLの初期化 pc=`$00ff90xx` とは別）。
つまり **ROM が自分を繋いでいるのではなく、Human68k が繋いでいる。**

周辺のPCも取れた（`--mem-read-watch=0x0:0xffffff --mem-read-watch-pc=0x7000:0x7200`）:

| PC | 読んでいるもの |
|---|---|
| `$7152` | ドライバヘッダ +4（属性ワードの上位バイト） |
| `$70e8` | 同上（別経路） |
| `$7116` | `$0066b4` = `$00010da2`（内蔵HARDDSKヘッダへのポインタ） |
| `$7118` | そのヘッダの +0（次ポインタ）— 連鎖の末尾を探して歩いている |

**自前スタブでも pc=`$7152` から `$ea0104`(属性の上位)が読まれている**ので、
ここまでは同じ道を通っている。

### 自前スタブは連鎖に触れられてすらいない

同じ監視を自前スタブ条件（属性`$4000` / a4=`$00ea0100` / d2=1）で回すと、
`$10da2` への書き込みは **IPLの初期化ぶんだけ**で、pc=`$70e0` は一度も来ない。
**拒否は「初期化コマンドの返答」と「連鎖への追加」の間で起きている。**
（観測系を直した後の測定なので、今度は沈黙が本物であることに根拠がある）

### ボードROMヘッダの形（実ROMと自前スタブで同じ構造を読んでいる）

Human68k 側の同じコードが、`$ea0020` の指す先 P を基準に読む:

| 位置 | 実ROM | 自前スタブ | 読んでいるPC |
|---|---|---|---|
| `$ea0020` → P | `$00ea0092` | `$00ea0034` | `$7f16` |
| P-16 | `$00ea0154` | `$00ea0036` | `$7f32` |
| P-12 | `$00ea04ba` | `$00ea004a` | `$7f30` |
| P-8..P-1 | `"Human68k"` | `"Human68k"` | `$7f26` / `$7f1c` |

実ROMでは P-12 が指す `$ea04ba` が呼ばれ、`$ea0500` 台のコードが動き出す。

### ただし今日のオラクルは SCSIドライバを登録していない（重要）

連鎖を先頭から辿ったところ、今日の本物ROM起動では:

```
$010da2 HARDDSK → $02c12c PRN → $02c1bc LPT → $02c32e AUX → AUX0..AUX5
        → $02cfb6 FLOAT → $032290 CON → ...
```

で、**`SCHDISK` が居ない。**`$10000`〜`$54000` を丸ごと落として `"SCHDISK"` を検索しても0件。
2026-09-02 に「`$0001a46c` に SCHDISK が載って連鎖に繋がれた」と記録した状態が**再現していない。**

- 基準器イメージは同一（`_fixtures/scsi_bootable_20260823.HDS` と `~/Downloads/scsi_fixture_20260823_bootable.HDS`
  は SHA-256 一致 `ad7c8844…`）。ROM も設定も既定のまま
- SPC のやり取り自体は起きている（セレクト30件以上、フェーズ遷移あり）。
  ROMは TEMP=`$07`,`$17`… と**複数IDを順に叩いており**、こちらは `--spc-target=-1`（既定）で
  **どのIDにも成功を返している**
- 起動時間も 2026-09-02 の61秒に対し今日は29〜33秒と短い

**オラクルがドライバを登録しない状態のまま、そこから得た値を根拠に自前スタブを直すことはできない。**
次はまずオラクルの再現性を取り戻す。

### 副次的に分かったこと

- **命令フェッチも `rm_main` を通る**ため、読み出し監視には prefetch が混ざる
  （`adr` と `pc` が±10以内の行は命令フェッチとみなして落とすと実データだけになる）
- 自前スタブのログが `a0`〜`a6` の指す先を `cpu_readmem24` でダンプしているため、
  **その読み出しも監視に出る**（観測が観測に写り込む）。pc=`$00ea0208` の一群がそれ

### 次

1. **オラクルの再現性の回復。**`SCHDISK` が登録される条件（`--spc-target` を `$07` に絞る、
   ログ量、待ち時間など）を1つずつ振って、2026-09-02 の状態を再現する。
   再現できたら「登録された瞬間の pc=`$70e0` 前後で何を読んだか」を実ROM側で取り、
   自前スタブとの差分を取る。
2. それまで、自前スタブ側の返答の欄をこれ以上振らない（オラクルの裏付けが取れないため）。


## 拒否の正体：ドライバはRAMに無ければならない（実測、2026-09-03）

### 経緯：条件が結果ファイルに残っていなかった

宿題25（オラクルが `SCHDISK` を登録しない）の原因は**設定の食い違い**だった。
成功した測定 `_local/scsi/hdr2.json` は既定ではなく
`ints_sel=$10 / ssts_data_bit=48 / clear_on_pctl=0` で走っていた。
当時の陽性対照ログは設定の一部しか出しておらず、**どの `--spc-*` で走らせたのかが
結果ファイルから復元できなかった**。

対策として、プローブの出力JSONに `invocation`(= `process.argv.slice(2)`)を丸ごと残すようにした。
**代理(ログに出る一部の設定)ではなく、条件そのものを記録する。**

なお当時の `ssts_data_bit=48`($30)は、**`ssts_tc0_bit` がまだ存在しなかった時期**のもので
（`ssts_tc0_bit` は px68k `8b1ef81` で追加）、今のコードでは `data_bit=$20` と `tc0_bit=$10` に
分割されている。そのまま `$30` を与えると両者が競合し、ROMが `$ea13de` で
SSTS=`$a0` を延々ポーリングして進まなくなる（実測）。**古い結果ファイルの設定値は、
そのままでは今のコードに移せない。**

### 拒否している判定を、実行トレースから特定した

読み出し監視をPCで絞ると（`--mem-read-watch=0x0:0xffffff --mem-read-watch-pc=0x70c0:0x7160`）、
**命令フェッチも `rm_main` を通る**ため、その区間で実際に実行された命令列がそのまま取れる。
自前スタブ条件で取った並びは次のとおり:

```
$7140  move.b #$16,$0000(a5)     ; 要求ヘッダ +0 = 長さ22
$7146  move.b #$00,$0002(a5)     ; +2 = コマンド0(初期化)
$714c  btst   #5,$0004(a1)       ; ドライバヘッダ +4(属性の上位バイト)の bit5
$7152  beq    $715a              ; 属性$4000は bit5=0 なので分岐する
$715a  move.l d0,$0012(a5)       ; 要求ヘッダ +18 へ d0 を書く
        …(ここでストラテジ/インタラプトが呼ばれる)…
$70be  add.l  #$00010000,d0      ; d0 = 終了アドレス + $10000
$70c4  cmp.l  $1c00.w,d0         ; $1c00 の中身は $00200000 (実装RAMの上限)
$70c8  bcc    (後方へ)           ; 門1: 上限以上ならエラー経路へ
$70cc  sub.l  #$00010022,d0      ; d0 = 終了アドレス - $22
$70d2  cmp.l  a1,d0              ; a1 = ドライバヘッダの番地
$70d4  bcs    (後方へ)           ; 門2: ヘッダ番地より小さければエラー経路へ
```

（これは **Human68k 側のコード**を実行トレースとして観測したものであり、
第三者のSCSIボードROMは逆アセンブルしていない。ここで得たのは実装ではなく
「我々のドライバが満たすべき条件」＝インタフェースの取り決めである。）

### 実測：門1は通る、門2で落ちる

| 返した終了アドレス | 門1(`end+$10000 < $200000`) | 門2(`end-$22 >= a1`) | 結果 |
|---|---|---|---|
| `$00ea0500`（ROM窓・従来） | ✗（`$00eb0500` は上限超え） | — | `$70c8` で分岐、拒否 |
| `$00007b36`（RAM） | **○（`$70cc` へ進んだ）** | ✗（`$7b14 < $00ea0100`） | `$70d4` で分岐、拒否 |

**2つを同時に満たすには `a1 + $22 <= end < $001f0000` が必要**で、
`a1`（＝ドライバヘッダの番地）が `$00ea0100`（ROM窓）である限り**原理的に成立しない**。

### これが意味すること

**Human68k は、デバイスドライバのヘッダが RAM 上にあることを要求している。**
本物のSCSIボードROMが、自分のドライバを RAM（実測で `$0001a46c` / 実行のたびに変わる）へ
複写してから Human68k に渡していたのは、この判定を満たすためである。
「ROM上のドライバをそのまま登録させる」ことはできない。

これで、これまで「返答のどの欄を振っても結果が変わらない」と見えていた理由も説明が付く。
**振っていた欄はどれも門1・門2の手前では効かず、`a1` を変えない限り必ず同じ経路で落ちていた。**

### 次

1. **自前スタブのドライバヘッダとスタブ本体を、ゲストRAMへ置く。**
   ホスト側から書き込み、`a4` にその番地を返し、終了アドレスを `a4 + サイズ` にする。
   置き場所（Human68k が使ってよいと考えている番地）の決め方が次の課題。
   本物ROMが使った番地は実行のたびに変わるので、Human68k から渡された値
   （ベクタ設定エントリ呼び出し時のレジスタ、要求ヘッダの入力 +14 = `$00007b36` /
   +18 = `$00007f76`）のどれが「ここへ置いてよい」を意味するのかを実測で決める。
2. 属性ワードの bit5 が何を分けているのかは未確定（`$4000` では立たない側へ進む）。


## 自前ドライバがデバイスドライバ連鎖に登録され、BPBが読まれた（実測、2026-09-03）

前節の門1・門2を満たすため、**自前スタブのドライバヘッダとストラテジ/インタラプトの本体を
ゲストRAMへ置く経路**を実装した（`--drv-ram=<番地>` / `--drv-ram-from=<番地>`）。

- `SCSI_HostCommand()` のベクタ設定エントリ(`$03`)で、`cpu_writemem24` を使って
  ゲストRAMへヘッダ(+0 次 / +4 属性 / +6 ストラテジ / +10 インタラプト / +14 ユニット数 / +15 名前"SCSIHDD")と、
  `move.b #$40,$e9f802 / rts`・`move.b #$41,$e9f802 / rts` の10バイトずつを書く
- そのうえで ROMスタブが `a4` に積む即値(`$ea008a`)を、その番地へ差し替える
  （`SCSI_Init` のバイト入れ替え後なので `SCSIIPL[((addr) ^ 1) & 0x1fff]` で書く。
  差し替え前に `$28 $7c`(`movea.l #imm,a4`)であることを照合する）
- 初期化コマンドの返答の終了アドレスは `base + $34` にする（門2 の `+$22` 以上）
- 置き場所は `--drv-ram-from=0x66ca` で **Human68k の作業領域から取れる**。
  本物ROMの実行トレースで `$70dc` が `$0066ca` を読み、その値が次に登録されるヘッダの番地と
  一致していたことから、ここがドライバ領域の割り当てポインタだと当たりを付けた（今回は `$00190000` が入っていた）

### 実測：登録された

```
[SCSI-DRV] ドライバをRAMへ置いた base=$00190000 … 終了アドレス=$00190034
[SCSI-REQ] 初期化コマンド($00) 処理 #1: err=$00 unit数=1 終了addr=$00190034(RAM配置) BPB表ptr=$00ea0600
[SCSI-RAM] W adr=$010da2 data=$00 pc=$000070e0
[SCSI-RAM] W adr=$010da3 data=$19 pc=$000070e0
[SCSI-RAM] W adr=$010da4 data=$00 pc=$000070e0
[SCSI-RAM] W adr=$010da5 data=$00 pc=$000070e0
```

**内蔵HARDDSKヘッダの次ポインタに `$00190000`(我々のドライバ)が書かれた。**
書いたのは本物ROMのときと同じ pc=`$000070e0`。**自前スタブが Human68k の
デバイスドライバ連鎖に登録された。**

### 実測：BPBが読まれ、欄の割り当てが裏付けられた

`$ea0600`(BPB表ポインタ)→`$00ea0610`(BPB本体)が実際に辿られ、全欄が順に読まれた:

| 読まれた番地 | 値 | 読んだPC | 対応する欄 |
|---|---|---|---|
| `$ea0600..03` | `$00ea0610` | `$8328` | BPB表の要素0 |
| `$ea0610..11` | `$0400` | `$d32a` / `$d334` | 1セクタのバイト数 |
| `$ea0612` | `2` | `$d34a` | 1クラスタのセクタ数 |
| `$ea0613` | `2` | `$d35c` | FATの個数 |
| `$ea0614..15` | `1` | `$d372` | 予約セクタ数 |
| `$ea061b` | `100` | `$d37a` | FATのセクタ数 |
| `$ea0616..17` | `512` | `$d382` | ルートdirエントリ数 |
| `$ea0618..19` | `0` | `$d3b6` | 総セクタ数(16bit) |
| `$ea061c..1f` | `$00018c00` | `$d3bc` | 総セクタ数(32bit) |
| `$ea061a` | `$f7` | `$d3d8` | メディアバイト |

**ブートセクタから写した欄の割り当ては、Human68k 自身の読み方で裏付けられた。**
（`+16` のパーティション開始(`$ea0620..23`)は読まれない。BPBとして必要なのは `+0`〜`+15` の16バイト）

### まだ通っていないこと：アドレスエラーで止まる

BPBを読んだ直後、**「アドレスエラーが発生しました (SR=$2000 PC=$0000832x〜8380)」**で停止する。

- 初期化コマンドは**2回**呼ばれる。要求ヘッダの `+22` が1回目 `$02` → 2回目 `$03` と増える
  （ドライブ番号らしい）。2回目の入力では `+14` に我々が返した終了アドレスが、
  `+18` には Human68k 自身の値(`$00007f76`)が戻っている
- 置き場所を `$00190000`(作業領域から取得) と `$00180000`(直接指定)で振っても**同じ**なので、
  番地の選び方の問題ではない
- 初期化以外のコマンド(読み出し等)は**まだ1件も来ていない**

### 次

1. アドレスエラーの発生箇所を確定する。`--mem-read-watch` のPC絞り込みで
   `$8320`〜`$8390` の実行を追ったが、見えたのは `movem.l`＋`rts` までで、
   例外の報告するPCとずれている。**報告されたPCと、実際に落ちた命令の対応を先に確かめる**
   （例外フレームのPCが「落ちた命令」か「次の命令」かで読み方が変わる）
2. 初期化が2回呼ばれる理由（ユニット数の解釈、`+22` の意味）を切り分ける。
   `--reply-units` を 0/1/2 で振って呼び出し回数が変わるかを見る
3. `--drv-ram-from=0x66ca` の妥当性。今回は動いたが、`$66ca` がドライバ領域の
   割り当てポインタであるという当たりは**本物ROMの1回の観測から付けたもの**であり、
   Human68k がここを更新するかどうかまでは確かめていない


## 自前ドライバを登録したまま Human68k が正常起動した（実測、2026-09-03）

前節のアドレスエラーを追い、3つの規則が実測で確定した。

### 規則1：BPB表は「ユニットごとの4バイトポインタの配列」

`pc=$8328` が要素を4バイトずつ進みながら読む（`$ea0600` → `$ea0604` → `$ea0608` …）。
要素をひとつしか置いていないと、2つ目($ea0604=0)を辿って
「１セクタあたりのバイト数が大きすぎます」になる。要素は必要な数だけ置くこと。

### 規則2：ユニット数に 0 を返してはいけない（借り引きで暗転する）

登録ループの本体（Human68k、`$82d0`〜`$8356`）を実行トレースから読むと:

```
$82fe  move.b  $d(a0),d7        ; a0 = 要求ヘッダ。+13 = ユニット数
$8302  move.l  #$00010000,d5
$8308  swap    d7 / clr.w d7    ; d7 = ユニット数 << 16
$830c  movea.l $12(a0),a3       ; a3 = BPB表ポインタ(+18)
   …
$8326  movea.l (a3),a3          ; 表の要素を読む
   …
$8344  sub.l   d5,d7
$8346  cmp.l   d5,d7
$8348  bcs     抜ける            ; 符号なし比較
$8350  addq.l  #4,a3            ; 表の次の要素へ
$8356  bra     ループ先頭
```

**ユニット数0だと最初の減算で `$ffff0000` に借り引きし、符号なし比較では抜けられず
約65535回まわる。**その間ずっと BPB表の要素を消費するので、表の外（＝BPB本体）まで
読んで「１セクタあたりのバイト数が大きすぎます」に至る。
「もう無い」をユニット数0で伝えるのは**不可**。

### 規則3：ベクタ設定エントリは繰り返し呼ばれる。断り方は `d2`

ベクタ設定エントリ(`$03`)は**複数回**呼ばれる（Human68k が「次のドライバはあるか」を
聞き続けているとみられる）。毎回同じヘッダを返すと**同じドライバが2台ぶん登録され**、
DPBの並びが壊れて暴走ジャンプ（アドレスエラー）になる。

断り方は実測で1つに絞れた:

| 2回目の返し方 | 結果 |
|---|---|
| そのまま同じヘッダ | 二重登録 → アドレスエラー |
| 初期化コマンドで ユニット数0 | 規則2の暗転 → 「バイト数が大きすぎます」 |
| 初期化コマンドで ステータス +4 にエラービット`$8000` | 変わらず（無視されている） |
| **`a4` に 0** | Human68k が番地0をヘッダとして読み(`$831e btst #5,$4(a1)`)暴走 |
| **`d2` に `$ffffffff`** | **正常起動** |

（px68k の元のスタブが `d2` に `-1` を返していたのは、これが「無し」の意味だったからである）

### 到達点

```
booted=true (27.8秒、A>プロンプトまで)
[SCSI] ベクタ設定エントリ #2: 2回目以降のため d2 に $ffffffff を返す(もうドライバは無い)
[SCSI-REQ] 初期化コマンド($00) 処理 #1: err=$00 unit数=1 終了addr=$00190034(RAM配置) BPB表ptr=$00ea0600
$10da2 の内容: 00 19 00 00 …  (内蔵HARDDSK の次 = 我々のドライバ)
```

**Human68k は普段どおり `A>` まで起動し、そのうえで自前のSCSIドライバが
デバイスドライバ連鎖に残っている。**画面に異常は無い。

再現条件（このまま渡せば再現する）:

```
node scripts/probe-scsi-iocs.mjs --image=<基準器> \
  --drv-attr=0x4000 --init-a4=0x00ea0100 --init-d2=1 \
  --drv-ram-from=0x66ca --reply-init-once
```

### まだ通っていないこと

- **読み出しコマンドが1件も来ていない**（`未対応コマンド` のログが0件）。
  ドライブとして見えているか（ドライブレターが割り当たっているか）は未確認。
  ゲストに `DIR C:` を打って確かめる必要がある（プローブに打鍵経路が無いので、
  `?bridge=1` の Bridge 経由か、AUTOEXEC.BAT を仕込んだ変種イメージで測る）
- `--drv-ram-from=0x66ca` の妥当性は未確定のまま（宿題）
- ユニット数と BPB表の要素数の整合（いまは要素を4つ置いて多めに逃げている）


## SCSIディスクが読めた：`dir c:` が通った（実測、2026-09-03）

### ドライブは実在していた（DPBで確認）

起動後の Human68k の DPB 連鎖（先頭は `$1c3c.w`、各DPBは +0 ドライブ番号 /
+2 ドライバヘッダ / +6 次、$38バイト）を辿った:

```
DPB $014866: ドライブ番号=0 ユニット=0 ドライバ=$01097e 次=$01489e   (A:)
DPB $01489e: ドライブ番号=1 ユニット=1 ドライバ=$01097e 次=$190034   (B:)
DPB $190034: ドライブ番号=2 ユニット=0 ドライバ=$00190000 次=$ffffffff (C: ＝我々のドライバ)
```

DPBの中身にも我々のBPB由来の値が入っていた（`04 00`=1024バイト/セクタ、`02`=FAT2個、
`64`=FAT100セクタ、`02 00`=ルートdir512）。**Human68k は我々が返した終了アドレス
`$190034` を、そのままDPBの置き場所として使っている。**

### 打鍵経路を足した（プローブ `--type=`）

ドライブが見えているかは「誰かが触る」まで分からないので、起動後にゲストへ打鍵する
`--type=<文字列>` を足した。実測で2つ踏んだ:

- **JIS配列とのずれ**: `DIR C:` と打つと画面には `dir c;` と出た（US配列前提の
  Shift+Semicolon が JIS では `;` のまま）。`:` は JIS では独立キー（US配列の `Quote` の位置）
  なので、そこだけキー名で押す
- **押して離すのが速すぎると効かない**: puppeteer の `press` は down/up が同一フレームに
  収まり、ゲスト側のポーリングに拾われないことがある（Enter が効かずコマンドが実行
  されなかった）。**120ms 保持してから離す**ようにした
  （[[feedback_frame_gap_needs_two_polls]] と同じ性質）

### 読み出しコマンド `$04` の要求ヘッダ（実測）

`dir c:` を打つと、コマンド `$05` と `$04` が届いた。`$04` の中身:

```
1a 00 04 00 | 00 20 00 1e 55 00 00 00 00 | f7 | 00 01 55 16 | 00 00 00 01 | 00 00 00 c9
+0 長さ$1a  +1 ユニット  +2 コマンド  +3 エラーコード
+13 メディアバイト = $f7    ← 我々がBPBで返した値がそのまま来る
+14..17 転送先アドレス = $00015516
+18..21 セクタ数       = 1
+22..25 開始セクタ     = 201
```

**セクタ201は、我々のBPB(予約1 + FAT100セクタ×2個)から計算されるルートディレクトリの
先頭と一致する。**つまり開始セクタは**パーティション先頭を0とする論理セクタ番号**で、
1セクタ = BPBの「1セクタのバイト数」(=1024)。

### 実装と結果

`SCSI_HandleRequestHeader()` に `$04` を実装した。パーティション開始(1024ブロック単位)と
セクタサイズを `SCSI_Init` の BPB 組み立て時に控えておき、
論理セクタ `s` → ホストLBA `(開始*1024 + s*セクタサイズ) / 512` で 512バイトずつ読み、
`cpu_writemem24` で転送先へ書く。転送後の先頭16バイトを読み返してログに出す（陽性対照）。

実測ログ:

```
[SCSI-READ] 論理セクタ=201 個数=1 転送先=$00015516 (1セクタ=1024バイト パーティション開始=32ブロック 先頭LBA=466)
[SCSI-READ] 転送後の先頭16バイト: 48 55 4d 41 4e 20 20 20 53 59 53 24 …   ← "HUMAN   SYS$"
[SCSI-READ] 論理セクタ=1 個数=1 転送先=$00015106 (… 先頭LBA=66)
[SCSI-READ] 転送後の先頭16バイト: f7 ff ff ff 00 03 00 04 …               ← FATの先頭
```

画面:

```
CONFIG             SYS        468  93-03-20  12:00:00
Autoexec           bak        159  99-09-23   0:50:02
sys                    <dir>       26-08-23  17:14:44
bin                    <dir>       26-08-23  17:14:44
…
ETC                    <dir>       26-08-23  17:33:34
```

**SCSIディスクのディレクトリが Human68k から読めた。**ファイル名・サイズ・日付とも正しい。
...
node scripts/probe-scsi-iocs.mjs --image=<基準器> \
  --drv-attr=0x4000 --init-a4=0x00ea0100 --init-d2=1 \
  --drv-ram-from=0x66ca --reply-init-once --type="dir c:" --type-wait=10000
```

### 次

1. **コマンド `$05` の意味の特定。**9件届いており、いまは `err=$00` を書くだけ。
   要求ヘッダの中身から用途（メディアチェック等）を割り出す
2. **書き込みコマンドの実装。**まだ届いていないので、まず何番で来るかを実測する
3. **ファイルの読み出し（`type` や実行）まで通す。**いまはディレクトリまで
4. **恒久化。**いまは実験用スイッチ(`--drv-attr` / `--init-a4` / `--init-d2` /
   `--drv-ram-from` / `--reply-init-once`)を全部渡して初めて動く。既定値に落とし込み、
   `--drv-ram-from=0x66ca` の妥当性（`$66ca` がドライバ領域の割り当てポインタである
   という当たり）を確かめてからUIへ配線する


## ファイルの中身も読め、スイッチ無しの既定で動くようにした（実測、2026-09-03）

### ファイルの読み出し

`type c:\config.sys` を打つと、SCSIディスク上の CONFIG.SYS の中身が表示された。
...
読み出しは ルートディレクトリ(論理セクタ201) → FAT → データ(論理セクタ275) と辿られている。
**ディレクトリだけでなく、FATを辿ってファイル本体まで読めている。**

### 既定値を実働構成にした

ここまでは実験用スイッチを5つ渡して初めて動いていた。これを既定値にした
（JS側から明示指定すればそちらが勝つのは従来どおり）:
...
既定で名乗るようにすると、**SCSIイメージが無いときにゼロのBPBを掴ませてしまう**。
そこで `SCSIImageReady` を新設し、**基準器イメージからBPBを写せたときだけ 1** にする。
0 のときは:

- `SCSI_Init` 末尾で、JS側の設定に関わらず a4=`$00000000` / d2=`$ffffffff` を書く（名乗らない）
- ベクタ設定エントリ($03)でのRAMへのドライバ組み立ても丸ごと飛ばす

### 確認（親セッションが自分で走らせた）

| 条件 | 結果 |
|---|---|
| イメージあり・**スイッチ無し** `--type="dir c:"` | `booted=true`、ディレクトリが表示、読み出しコマンド101件（合格） |
| **イメージなし**・スイッチ無し | `booted=true`、`[SCSI] イメージが無いか読めないため、ドライバとして名乗らない`、読み出し0件（合格） |

（委譲先も同じ2件を確認しているが、[[feedback_agent_reports_done_without_committing]] の
とおり報告だけでは採らず、親セッションで独立に走らせて確かめた）

### 次

1. **書き込みコマンド。**まだ届いていない。何番で来るかを実測してから実装する
2. **コマンド `$05` の意味。**届いてはいるが `err=$00` を書くだけで通っている
3. **UI への配線。**いまはコアが `__webx68kScsiUrl` を見るだけで、
   ディスクライブラリやスロットUIとは繋がっていない
4. **`$66ca` の妥当性。**Human68k が我々の返した終了アドレス `$190034` を
   そのままDPBの置き場所に使っていることから、割り当てフロンティアとして
   整合はしている。ただし `$66ca` 自体がその変数だという確証はまだ無い


## セクタI/Oを差し替え可能にし、書き込み経路を端から端まで確かめた（実測、2026-09-03）

本命の書き戻し経路（決定2 の OPFS）は**コアをWorkerで回すのが前金**でまだ先である
（`?worker=1` は現時点でも裏フラグで、既定はメインスレッド。
`FileSystemSyncAccessHandle` はワーカー専用なので、既定経路では原理的に使えない）。
そこで先に**差し込み口だけ**用意し、テスト用のRAMオーバーレイで書き込み経路を検証した。

### 差し込み口

- `js_scsi_read_sector()` の冒頭で `globalThis.__webx68kScsiRead` を見る。
  **`-2` を返したときは従来のXHR経路へ落ちる**という約束にした（「自分では扱わない」の意味）
- `js_scsi_write_sector()` を新設。`globalThis.__webx68kScsiWrite` があればそれを呼び、
  無ければ `-1`（書き戻し経路が無い）
- どちらもフックが無ければ**従来と完全に同じ挙動**

コア側 `$08` は「書けるなら書く、書けないなら断る」にした。1セクタでも失敗したら
従来どおりステータスワードのエラーで断る。

### テスト用RAMオーバーレイ（`--scsi-ram-writes`）

プローブに `--scsi-ram-writes` を足した。`Map<lba, Uint8Array(512)>` に書き、
読み出しはオーバーレイにあればそれを返し、無ければ `-2` で従来経路へ委譲する。
**永続化しない。書き込み経路を確かめるためだけのもの**で、storage の設計とは別物である。

### 実測：書いて、読み返せた

```
node scripts/probe-scsi-iocs.mjs --image=<基準器> --scsi-ram-writes \
  --type="copy c:\config.sys c:\z.txt;;dir c:" --type-wait=12000
```

（プローブの `--type` は `;;` 区切りで複数コマンドを打てるようにした）

結果:

```
[SCSI-WRITE] 論理セクタ=201 個数=1 転送元=$000181c6 先頭LBA=466   ← ルートディレクトリ
[SCSI-WRITE] …                                                    ← FAT
…
z                  txt        468  93-03-20  12:00:00              ← dir c: に出た
```

**コピーが成功し、直後の `dir c:` に `z.txt`(468バイト、元のCONFIG.SYSと同じサイズ)が現れた。**
FATとディレクトリへの書き込みが受理され、読み返せている。断り分岐は0件。

これで**ドライバ側の読み書きは完成**した。残るのは「どこへ書くか」（決定2 の OPFS 経路）である。

### 途中で1回踏んだもの（記録のみ）

打鍵の途中で puppeteer が `TargetCloseError: Protocol error (Input.dispatchKeyEvent):
Session closed.` で落ちた（1回だけ。再実行で通った）。宿題20の
「Chrome起動待ちタイムアウト」とは別の現象。再発したら条件を足す。

### 次

1. **決定2 の OPFS 経路**。コアをWorkerで回すのを既定にするか、SCSI I/O だけ
   ワーカーへ出すかの設計判断が要る
2. UI（ディスクライブラリ・スロット）への配線。データ源が
   `__webx68kScsiUrl`(HTTP)のままでは配線できないので、1と一緒に決める
3. コマンド `$05` の意味。入力で安定しているのは `+13=$09` だけで、
   他の欄は未初期化バッファだった（9件を突き合わせて確認）。
   現状の返答(`err=$00`)で全て通っているため優先度は低い


## SASI と SCSI は同時に使える（実測、2026-09-03）

UIの形（HDDスロットの隣にSCSIを足すのか、置き換えるのか）を決めるため、
**SASIのHDDとSCSIを同時に挿して測った。**プローブに `--hdd=`(`?hdd=`)を足した。

```
node scripts/probe-scsi-iocs.mjs --image=<基準器> --hdd=/test/hd0.hdf \
  --type="dir c:;;dir d:" --type-wait=12000
```
...
| ドライブ | 中身 | 根拠 |
|---|---|---|
| **C:** | SASI の `hd0.hdf` | 日付が 99-09-22/23 系 |
| **D:** | SCSI の基準器 | 日付が 26-08-23 系、`82368K Byte 使用可能`(≒100MBのパーティション) |

**両方が同時にマウントされ、どちらも読める。**デバイスドライバ連鎖に
内蔵HARDDSK(SASI)と我々のSCSIドライバが並んでいるので、当然といえば当然だが、
推測ではなく実測で確かめた。

### UI設計への含意

- **SCSIはHDD(SASI)スロットの置き換えではなく、別スロットとして足す。**既存の
  40MBのSASI資産をそのまま使いながら、大容量はSCSIで、という並びになる
- **ドライブレターは固定できない。**SASIが入っていなければSCSIがC:、入っていればD:になる。
  UIに「C:」等と書いてはいけない
- SCSIスロットは**まず1本でよい**。1本のイメージに複数パーティションがあれば、
  本来はそれぞれが別のドライブレターになるはずで、「スロットを増やす」より
  「1本の中の複数パーティションを出す」ほうが先である
  （現状はパーティション0のみ・1ユニット固定）


## Worker 経路を既定にした（2026-09-03）

決定2（SCSIのI/OはOPFSの同期ハンドル経由）は `FileSystemSyncAccessHandle`＝**ワーカー専用**に
依存しており、メインスレッド経路では原理的に成立しない。よってコアをWorkerで回すのを既定にした。
`?worker=0` で従来の LocalCoreProxy へ戻せる。

### 先に踏んだ穴：SCSIの設定がWorkerから見えない

`?worker=1` で測ると **SCSIが丸ごと効かなかった**。原因は、SCSI関連の設定が
`globalThis.__webx68kScsiUrl` 等のグローバル経由で wasm から読まれているのに、
**それらはページ(main)側の window にしか無く、Worker の globalThis からは見えない**こと。

```
[SCSI-BPB] パーティション表(LBA4)の読み出しに失敗した
[SCSI] イメージが無いか読めないため、ドライバとして名乗らない
→ ゲストで dir c: すると「ドライブ名が無効です」
```

（**ガードは正しく効いた**。イメージが読めないときは名乗らないので、壊れずに「無い」状態になった）

対策として `InitPayload.hostGlobals` を新設し、**初期化時に1回だけ**
`__webx68k*` のうち値が number/string/boolean のものを Worker の globalThis へ写す。
Worker 側は写した件数と `__webx68kScsiUrl` の有無を1行で出す（陽性対照）。

- **関数は渡せない**（構造化クローン不可）。`__webx68kScsiRead`/`__webx68kScsiWrite` は関数なので
  渡らない。Worker 側で書き戻し経路を持つのは決定2のOPFS実装の仕事である
- **実行中に main 側で値を変えても Worker には反映されない**（1回だけの写し）。
  計測用の監視範囲を走行中に変える使い方をするなら、別途メッセージ経路が要る

### 検証（親セッションが自分で実行）

| 条件 | 結果 |
|---|---|
| 既定（`?worker` 指定なし） | `booted=true` / 読み出し101件 / `<dir>` 10行 / Worker橋のログあり — **2回連続で合格** |
| `?worker=0`（従来経路） | `booted=true` / 読み出し101件 / `<dir>` 10行 |
| `npx vitest run` | 42ファイル・655件すべて合格 |

### 副産物：検証道具が偽の失敗を作っていた

Worker既定にした直後の1回目が「読み出し0件」で落ちたが、**原因はSCSIではなく打鍵**だった。
画面には `A>dri c:` と出ており、`dir` の文字が入れ替わっていた。
`page.keyboard.type` の `delay` を 60ms → **120ms** に広げたところ再現しなくなった。

観測系の側が偽の失敗を作ると、直す必要のないものを直しにいくことになる。
**まず画面に何が出ているかを見る**のが先である（今回はSCSIのログが全て正常だったので気づけた）。


## SCSIのI/OをOPFSへ繋いだ：書き込みが永続化した（実測、2026-09-03）

決定2の本体。ワーカー内で `createSyncAccessHandle` を開き、既存の差し込み口
(`__webx68kScsiRead` / `__webx68kScsiWrite`)に繋いだ。

### 構成

- 新規 `src/scsi-opfs.ts` の `setupScsiOpfs()`。`src/core-worker.ts` が
  hostGlobals を写した直後・**コアを作る前に**呼ぶ
- OPFS の `scsi/scsi0.hds` を使う。**リモートのサイズと既存ファイルのサイズが違えば取り込む**
  （`fetch` のストリームを同期ハンドルへ流す）。同じなら取り込まない
- 読みは `handle.read(heap.subarray(ptr, ptr+512), {at: lba*512})`。
  **範囲外は例外でなく静かに0バイトが返る**ので、戻り値が512でなければ失敗として返す
- `flush()` は1セクタごとだと遅いので、dirty を立てて2秒ごとにまとめて行う。
  **取りこぼしの窓が最大2秒ある**（タブを閉じた瞬間の書き込みは失われうる）
- `!isSecureContext` / OPFS が無い環境では `mode:'none'` を返し、
  **従来のXHR経路（読み取り専用）へ落ちる**。理由を必ずログに出す
- コア側 `js_scsi_get_size` は `__webx68kScsiSize` があればそれを使う（XHRでサイズを取り直さない）

### 実測

プローブに `--scsi-opfs` と、**プロファイルを使い回す `--profile=<パス>`** を足した。
後者はOPFSがオリジンのストレージに紐づくため、「次の起動でも残っているか」を
2回の実行に分けて測るのに要る。

| 試行 | 条件 | 結果 |
|---|---|---|
| 1回目 | `--profile=P` で `copy c:\config.sys c:\z.txt` | `SCSI I/O: opfs (104857600 バイト, **取り込み=あり**)` / 書き込み2件 |
| 2回目 | **同じ** `--profile=P` で `dir c:` | `**取り込み=なし**` / **`z txt 468` が残っている** / 書き込み0件 |
| 陰性対照 | **新しい**プロファイルで `dir c:` | `取り込み=あり` / **`z.txt` は無い**（期待どおり） |

**ブラウザを起動し直しても書き込みが残り、かつ新しいプロファイルでは残らない。**
2回目は書き込みを1件も出していないので、画面に出た `z.txt` は
その実行で作ったものではなく、OPFSに残っていたものである。

退行確認: `--scsi-opfs` を付けない既定は `SCSI I/O: none (理由: __webx68kScsiOpfs が未設定)` で
従来のXHR経路のまま。`npx tsc --noEmit`・`npx vitest run`(655件)とも通過。

### まだやっていないこと

- `--scsi-opfs` はまだ**明示指定が要る**。既定にするのはUI配線(手順4)と一緒に決める
- **取り込みの進捗表示が無い。**100MBのコピーが起動前に入るので、UIでは待ち時間が見える形にする必要がある
- **flushの2秒窓**をどう詰めるか（`visibilitychange`/`pagehide` でのflushは未実装）
- OPFSの容量逼迫時の挙動（eviction）は未確認のまま（既存の宿題）


## 複数パーティションを別ドライブとして出せるようにした（実測、2026-09-03）

### 検証用の検体を作った

手元の基準器はパーティションが1本しかないので、2区画の検体を作った
（`scripts/make-scsi-multipart.mjs`、出力は追跡外の `_local/scsi/twopart.HDS`）。

- **区画の中身は Human68k の `FORMAT.X` が作った本物をそのまま複製する**
  （ブートセクタ・FAT・ディレクトリはすべて実物）
- **自前で書くのはパーティション表と先頭ヘッダの数値だけ**（書式は実測済み）

「自作の構造を自作の実装で読む」範囲をこれだけに絞った。
なお出来上がりは**同じ中身の区画が2つ並んだディスク**なので、目視では区別できない。
区画が別物であることは**片方へ書いてもう片方が変わらないこと**で確かめる。

（作る過程で先頭ヘッダの残りの欄も確定した:
`LBA0 +8..9` = 1セクタのバイト数、`+10..13` = 総512ブロック数-1。
`LBA4 +4` = 使用済み末尾、`+8`/`+12` = 総1KBブロック数-1。いずれも1024バイトブロック単位）

### 実装

- BPB表を `$ea0600` から4バイト×最大8、BPB本体を `$ea0620` から**32バイト刻み**で最大8に配置
  （`$ea0100`〜`$ea07ff` の窓に収まる。以前は本体が `$ea0610` にあり、表を8個に増やすと重なった）
- パーティション表を先頭から最大8個走査し、サイズ0で打ち切る。各区画のブートセクタから
  BPBを写し、開始ブロックとセクタサイズを配列に控える
- ユニット数（デバイスドライバヘッダ +$0e と初期化コマンドの返答 +13）を**区画数**にする
- **`$04`(読み)と`$08`(書き)は要求ヘッダ +1 のユニット番号で区画を選ぶ**。範囲外はエラー

### 実測

| 確認 | 結果 |
|---|---|
| 2区画が2ドライブになる | `[SCSI-BPB] 区画を 2 個みつけた` / `unit数=2(区画数を優先)` / `dir d:` が通る |
| **区画が別物である** | `copy c:\config.sys d:\mark.txt` → `dir d:\mark.txt` で**見つかり**、`dir c:\mark.txt` で**見つからない** |
| 書き込み先 | `[SCSI-WRITE] ユニット=1 論理セクタ=201 先頭LBA=203218` ＝ 区画1の領域（区画0なら466） |
| 1区画の基準器 | `区画を 1 個みつけた` / ディレクトリが出る（退行なし） |

### また検証道具が偽の失敗を作った

確認Bが1回目に落ちたが、原因は実装ではなく打鍵だった。画面には

```
A>coyp c:\coifng.sys d:\mark.txt
コマンドまたはファイル名が違います
```

と出ており、**文字が入れ替わっていた**（`copy`→`coyp`、`config`→`coifng`）。
`page.keyboard.type()` は `delay` を 120ms に広げても入れ替わる。
ゲストはフレーム単位でキーを拾うため、make/break が詰まると順序が壊れる。

対策: **`type()` を使わず、1文字ずつ「押す→保持→離す→間をあける」で送る**。
遅いが順序は壊れない。これで確認Bは通った。

**画面に何が出ているかを先に見ること。**SCSIのログが全て正常だったので、
実装ではなく道具を疑えた。

### 副産物：`--scsi-ram-writes` は Worker 既定では効かない

RAMオーバーレイはページ側の `globalThis` へ**関数**を差すが、
**関数は構造化クローンできない**ので Worker へ渡らない（`hostGlobals` は
number/string/boolean だけを写す）。プローブが警告を出すようにした。
使うなら `--worker=0` を併用する。書き込みの検証は `--scsi-opfs` のほうが本筋。


## SCSIスロットをUIに足した（2026-09-03）

手順4の1枚目。**ローカルのファイルを挿す／取り出す／ダウンロードする**まで。
ライブラリ一覧・ブランク作成・D&Dは次回。

- 新規 `src/scsi-store.ts`（ページ側）: OPFSの `scsi/` を扱う。同期ハンドルは使わず
  `createWritable()`/`getFile()`。取り込みは8MiBごとに進捗を返す
- `src/scsi-opfs.ts`（ワーカー側）に `__webx68kScsiOpfsPath` を追加。
  **既にOPFSにある物を開くだけ**の経路。URLからの取り込み経路(プローブ用)はそのまま残す
- `index.html` に `slot-scsi` を追加。**`slot-hdd` と同じ `.fd-slots` の中**に置くだけで、
  行の割り付けは既存の `flex-wrap` に任せる
- 起動中は差し替え不可、`isScsiStorageAvailable()` が false（secure context でない等）なら
  スロットごと無効にして理由を出す
- 取り出しは localStorage の紐付けを外すだけで、**OPFSのファイルは消さない**
  （取り出しただけで中身を捨てるのは危険。削除はライブラリUIの仕事）

### 並びを揃えた

スロットは `.fd-slots{display:flex;flex-wrap:wrap}` + `.fd-slot{flex:1 1 auto;min-width:220px}`
で、行を固定していない。ここにSCSIを足したところ、上下の行でボタンの位置がずれた。

| 指定 | 結果 |
|---|---|
| `flex: 1 1 auto`（元） | 中身の長さで幅が変わる。ボタン数の違う SCSI が入ると行ごとに分け方が変わりずれる |
| `flex: 1 1 0` | 幅が縮んで**1行に3つ**入ってしまい、名前が `h…` と潰れた |
| **`flex: 1 1 45%`** | **1行に2つを保ちつつ、どのスロットも同じ幅**。ボタン群が上下で揃う |

あわせて `.fd-label` の `min-width` を 20px → **32px**（`HDD` は3文字、`FDD0`/`SCSI` は4文字で
1文字ぶんずれるため）。

### 検証道具がまた偽の失敗を作った（2回目）

`dir c:` が `dirc :`、`copy` が `coyp` になる打鍵の壊れが続いた。調べると
**`booted=false` のまま打鍵していた**ことが分かった（プロンプトが出る前に押した
キーは落ちる。`A>ir a:` のように先頭が欠ける）。既定経路でも同じだったので、
**製品側の不具合ではなく道具側の問題**である。

プローブに次の2つを入れた:

1. **起動を検知できなかったら打鍵しない**（`typedMismatch` に理由を残す）
2. **打った文字列が画面にそのまま出ているかを照合する**（陽性対照）。
   一致しなければ `typedMismatch` に期待と実際を残す

これで「道具の失敗」と「製品の不具合」を取り違えなくなる。
**同じ罠に2回はまった**ので、照合を道具の側に固定した。
...
13. ~~Worker経路の打鍵落ちの原因調査。~~「起動・3ドライブ・音声の基準比較(実測、2026-09-01)」で、既定経路0件に対しWorker(修正前)9件・Worker(修正後)5件の打鍵落ちを確認した。修正後は5件がtrial1・2に集中しtrial3〜5は0件というウォームアップ性のある挙動に見えるが、原因は未特定。→ **2026-09-01、原因を特定し修正済み**（「make/breakの潰れの原因調査と修正(実測、2026-09-01)」参照）。**→ さらに2026-09-01夜、`a5aadbb`後の再計測(打鍵落ち修正・音声移行後)で1件・0件(2回の実行)に改善したことを確認し、解消と判定する**（「打鍵落ち修正・音声移行後の再計測(実測、2026-09-01 夜)」参照）。
...
15. **ドライブ応答時間の指標が観測側を測っている疑いの切り分け。**同上の節で、Worker(修正後)/既定の応答時間比がA:2.31倍・B:1.81倍と遅い一方、C:は既定/Worker=3.69倍で速く、D:はほぼ同値(0.95倍)と向きが割れた。同一経路のオーバーヘッドなら向きが揃うはずで、`readTextScreen`のスレッド境界越しのポーリング位相を測っている疑いが濃いが未確認。→ **2026-09-01夜、音声移行後の再計測でA:がWorker143.6〜145.8ms・既定137.8msと既定に近づいた**（「打鍵落ち修正・音声移行後の再計測(実測、2026-09-01 夜)」参照）。**主因は音声が未実装でWorker側の仕事量が少なかったことだった可能性が高いと判定するが、断定はしない。** ただしC:はWorkerの3回の実行で146.8/530.9/200.4msと大きくばらついており、**この指標(C:の応答時間)の実行間の安定性そのものが未解明のまま残る。**
16. **移行前基準がBattery Power下で取得されている件。**移行前基準(`newset2-20260825-audio.json`)は`powerSource: "Battery Power"`だが、2026-09-01の計測は全てAC Power。3指標とも基準をわずかに下回る方向へ系統的にずれており(起動 約-4%、beep/idle 各-0.3〜0.8ms程度)、電源条件の食い違いで説明が付くと判断した。電源条件を揃えて再計測するか、基準をAC電源側で取り直すかを決める。**2026-09-01夜の再計測(打鍵落ち修正・音声移行後)も全件AC Powerのままで、この宿題は未解消。**
17. ~~Worker側の音声移行(手順5・7の未完了部分)。~~ → **2026-09-01、実装・単体テスト・故障注入・実ブラウザでの機能確認(既定経路との対照・陰性対照込み)まで実施済み**（「ワーカー移行 手順5：音声出力の実装(2026-09-01)」参照)。音声サンプルはWorkerからmainへ運ばれ`queuedSec`が非ゼロで推移しアンダーフローが継続しないことを確認したが、**`queuedSec`中央値・p99の基準幅との正式な照合(時間の統計値)は今回のスコープ外で親セッションが別途行う。** → **2026-09-01夜、正式な計測ハーネスでidle/beepを再計測した**（「打鍵落ち修正・音声移行後の再計測(実測、2026-09-01 夜)」参照）。**ただしこの再計測は全件`contended`下で取得しており、静穏下での正式な`queuedSec`比較は依然として未実施のまま残る。**
18. **Worker経路の音声`queuedSec`が既定より深い件の原因調査と、静穏下での正式計測。**2026-09-01夜の再計測(`contended`下)では、idle区間の`queuedSec`中央値がWorker 143.991〜164.342msに対し既定86.179msで、約1.7〜1.9倍(beepまで含めると最大約2.3倍)深い。数字はいずれも`contended`下の参考値であり、原因調査と静穏下での取り直しの両方が必要。
19. **`measure-audio.mjs`のbeepシナリオがWorker経路で不安定な件の調査。**2026-09-01夜の再計測で、音声移行後のWorker経路は3回の実行(`wm-20260901-audio-worker-2/3/4.json`)中1回(`worker-3`)しか成功しておらず、失敗理由はいずれも「X-BASICの"Ok"プロンプトに到達できませんでした (lastLine=なし)」で画面テキストが読めていない。既定経路は同シナリオで毎回成功する。ワーカー移行 手順9で`screenText()`の未捕捉rejectionを`.catch(() => null)`で安全側へ倒した修正との関係が疑われるが未調査。
20. **puppeteerのChrome起動待ちタイムアウト(`Timed out after 30000 ms while waiting for the WS endpoint URL to appear in stdout!`)の再発。**8/25に一度発生し「再発したら記録する」としていたが、2026-09-01夜、load average 16以上の高負荷下で3本連続実行の1本目が失敗し再発した(該当する`wm-20260901-drives-worker-3.json`は作られていない)。高負荷が条件の候補として1つ絞れたが確定ではなく、再発時に条件を追加で記録する。 **→ 2026-09-02、load average 2.33 で再発した**（「SCSI HLE 着手」節参照）。同時刻に別プロファイルで Chrome を手動起動すると1秒で正常起動したため、**高負荷は必要条件ではなく、利用者の Chrome が起動していることも原因ではない**と分かった。原因は未特定のまま。
21. **SCSIデバイスの登録経路の特定。**「SCSI HLE 着手」節のとおり、起動しても SCSI IOCS は
    一度も呼ばれない。誰がSCSIドライブを見つけ、誰が Human68k へ登録するのかが未特定である。
    条件を変えた測定（SASIディスクを付けた状態、SRAM のブートデバイス設定を変えた状態、
    内蔵SCSI機種の IPL を使った状態）で IOCS が呼ばれるかを先に測り、
    ROMスタブの起動エントリに何を書けばよいかを決める。
22. **`localhost` と `127.0.0.1` の食い違いの棚卸し。**vite dev は既定で `[::1]` にしか
    bind しないため、接続先を `127.0.0.1` と書いたスクリプトは接続拒否になる。
    `.claude/launch.json` は `--host 127.0.0.1` を明示していて成立しているが、
    他のスクリプト・手順書に同じ書き方が残っていないか確認する。
23. **観測系の故障(PC絞り込みの既定値)で無効になった過去の測定の洗い直し。**
    `[SCSI-RAM]`/`[SCSI-MEMR]` で「0件だった」と判断した箇所は偽の沈黙の可能性がある
    （「BPBを基準器イメージから写した／観測系の故障を見つけた（実測、2026-09-03）」参照）。
    少なくとも `$6734` の書き込み監視と、本物ROM使用時の ROM窓読み出し・`$10da2` の書き込み監視は取り直す。
24. **陽性対照が「自分だけ通れる抜け道」を通っていないかの点検。**今回の自己検査は
    `selftest_active` でPC絞り込みを迂回する作りだったため、本番経路が全捨てされていても通り続けた。
    他の陽性対照（`$e9f800` の selftest、`$ea0110` の窓書き込み selftest 等）にも
    同じ形の抜け道が無いかを見る。
25. ~~**オラクル（本物ROM）が SCSIドライバを登録しない件。**~~ → **2026-09-03 原因特定**（設定の食い違い。
    成功例 `hdr2.json` は `ints_sel=$10 / ssts_data_bit=48 / clear_on_pctl=0` で走っており、
    しかも当時は `ssts_tc0_bit` が存在しなかったため、その値をそのまま今のコードへ移せない。
    「拒否の正体：ドライバはRAMに無ければならない（実測、2026-09-03）」参照）。以下は元の記述:
    2026-09-03 時点で、本物ROM起動時の
...
26. ~~SCSI UI 手順4の残り、ブランク作成。~~ → **2026-09-04 実装済み**（「SCSIのブランクディスクを作れるようにした（2026-09-04）」参照）。`dir c:` は実機（実ブラウザでのHuman68k起動）で確認できたが、
    **`copy` で新規ファイルを書き込む段（受け入れ条件2）が px68k-libretro 側の未解明の問題で通らない**ことが分かった。次のセッションは、まずこの節を読んでから
    px68k-libretro側の調査（別リポジトリ、後述のspawnタスク参照）を先に進めること。

## SCSIのブランクディスクを作れるようにした（2026-09-04）

### やったこと

- `src/api/fat.ts` に `createFormattedScsi(sizeMiB)` を追加した。`createFormattedHdd()`
  （SASI・ヘッダ無し・256Bブロック単位・固定40MB）とは完全に別の定数・別の関数にし、
  どちらかを直すときにもう片方を壊さないようにした。
- UIは他スロットの `btn-blank-*` に合わせて `#slot-scsi` へ `btn-blank-scsi` を追加。
  押すと `prompt()` でMB単位のサイズを聞く（既定値100、下記の実測どおり1〜2047の範囲）。
  1区画固定（複数区画は今回スコープ外）。
- 入力検証は `validateScsiBlankSizeMiB(input: string)` という純粋関数に切り出し
  (`test/fat-scsi.test.ts` で単体テスト可能)、非数値・小数・下限未満・上限超過を
  理由付きで弾く。理由ごとに文言(`scsiBlankSizeInvalidNotANumber`等)を`src/strings.ts`に
  日英そろえて追加した。
- 生成したイメージは `putScsiImage()` でOPFSへ書き込み、SCSIスロットへ割り当て、
  ライブラリ一覧にも出るようにした（`handleCreateBlankScsi()`。既存の
  `handleCreateBlankHdd()` と同じ導線）。起動中・OPFS利用不可のときは他のSCSIボタンと
  同様に無効化する（`updateScsiControls()`のlocked/scsiStorageAvailable分岐に追加）。

### 実測したBPB・ヘッダ（`_local/scsi/twopart.HDS`、FORMAT.Xが作った本物）

親セッションが実測した値に加え、今回の実装・検証で追加確認した値:
...
| +0 | 8 | `"X68SCSI1"` | シグネチャ |
| +8 | 2 (BE) | 512 | 物理セクタサイズ |
| +10 | 4 (BE) | 総512Bブロック数-1 |
| +14 | 2 | `01 00` | 意味不明の未知フィールド(仕様書に記載無し。今回は追わず0のままにした。動作に影響しないことを実測で確認済み) |
| +16 | 可変 | `"Human68K SCSI-DISK by Keisoku Giken"` | 機種名文字列(**写さず自前の`"WebX68k SCSI"`にした**) |

**パーティションテーブル(オフセット`$800`、1024バイト単位・BE)**

| オフセット | 実測値(twopart.HDS) | 意味 |
|---|---|---|
| +0 | `"X68K"` | シグネチャ |
| +4 | **203808**(=総1KBブロック数そのもの) | 使用済み末尾ブロック |
| +8 | **203807**(=総1KBブロック数-1) | 総1KBブロック数-1 |
| +12 | 203807 | +8と同じ |
| +16(entry0) | 名前`"Human68k"`+開始4B(BE)+サイズ4B(BE) | 区画0: 開始32・サイズ101376 |
| +32(entry1) | 同上 | 区画1: 開始101408・サイズ101376(区画0の直後に連続配置) |

**踏んだ落とし穴1: +4と+8は別の値。** 最初の実装は両方とも「総ブロック数-1」にしていた。
これだとHuman68kの整合性検査に引っかかり、**`copy`で「ディスクの管理領域が壊されています」
になることを実ブラウザで確認した**(下記「実ブラウザでの確認」参照)。+4は`total`、
+8/+12は`total-1`が正しい。

**パーティション先頭のブートセクタ(BPB、実測は前回セッションの表のとおり)。**
これに加えて今回、**FAT先頭の予約エントリの並びが `createFormattedHdd()`(SASI)の
想定と違う**ことを実測で見つけた:

- `createFormattedHdd()`のコメントは「Human68kはFAT16もBEで格納する」として
  `writeU16BE(base, 0xff00|media)`(バイト列 `[0xFF, media]`)を書く。
- 実測(twopart.HDS)のSCSI区画0のFAT先頭は `f7 ff ff ff`。クラスタ2以降の
  チェーン本体(`00 03 00 04 ...`、連番)は確かにBEだが、**予約エントリ0だけは
  `[media, 0xFF]`という逆順**になっている。
- これも直さないと「ディスクの管理領域が壊されています」を引き起こすことを実測で
  確認した(下記参照)。`createFormattedScsi()`は abstraction を介さず実測したバイト列を
  そのまま書くようにした。**`createFormattedHdd()`(SASI)側にも同じ誤りが潜んでいる
  可能性があるが、今回のスコープ外のため手を付けていない**(下記のspawnタスク参照)。

### サイズの上限・下限の実測

- **下限は1MiB**(`SCSI_BLANK_MIN_MIB`)。それ未満は`validateScsiBlankSizeMiB()`が
  `tooSmall`として拒否する。1MiBでも`computeScsiFatLayout()`は正常にspc/sectorsPerFatを
  導出できることを`test/fat-scsi.test.ts`で確認済み。
- **上限は2047MiB**(`SCSI_BLANK_MAX_MIB`)。根拠はFAT16のクラスタ数上限
  (65524、sectorsPerCluster側で回避できるので効かない)でもBPBのsectorsPerFat(1バイト、
  255以下)でもなく(2047MiBでもsectorsPerFat=128にしかならず255に遠い)、
  **SCSI HLE側の制約**である: `src/core-shim.c` の `js_scsi_get_size()` がイメージサイズを
  符号付き32bit整数(`int`)で返し、同関数内で `0x7fffffff`(2147483647バイト)を
  超える値をその値へクランプする。2048MiB(2147483648バイト)を作ると、コアが
  認識するサイズが実際のバイト数より1バイト小さい値に化けて静かに壊れる。
  MiB単位で切り下げた2047MiB(2146435072バイト、0x7fffffff未満)を上限にした。
  この境界は`test/fat-scsi.test.ts`の「上限MiB(2047)でも生成でき、開ける」テストで
  `image.length < 0x7fffffff`を確認している。
- `src/`・`px68k-libretro`側のLBA桁数を確認したところ、`lba`は`unsigned int`(32bit)で
  受け渡されており(`webx68k_scsi_read_sector`/`write_sector`)、512バイト単位で
  2^32セクタ(2TB超)まで表現できるため、この経路にサイズ上限は無い。
  したがって上記のint32クランプが実質的な唯一の上限である。

### 実ブラウザでの確認(実測、2026-09-04)

`npx tsc --noEmit`・`npx vitest run`(688件、667+21件)は通過。実ブラウザでの確認は
puppeteerではなくこのセッションのBrowserペイン(`http://127.0.0.1:5299/`、bundled
`human302.xdf`から起動)で、`window.__webx68kDebug.screenText()`で画面を読み、
`window.dispatchEvent(new KeyboardEvent(...))`でキー入力を合成する方式で行った
(自動化ツールの`type`アクションは`e.code`が空でゲストに届かないことを実測で確認したため、
`KeyboardEvent`を`code`付きで自前合成する方式に切り替えた。X68000のJIS配列では
`:`は`Quote`キー(`RETROK.QUOTE`)に割り当たっており、US配列の`Semicolon`キーでは
`;`になる。`src/keyboard.ts`の`charToKey()`のマップを参照して確定させた)。

1. **`dir c:`(受け入れ条件1): 合格。** 80MB/99MB/32MB(spc=1・spc=2の両方)いずれも
   `ボリュームがありません C:\` `0 ファイル 0K Byte 使用中 NNNNNK Byte 使用可能`
   `ファイルがありません`が出た。空き容量の値はHuman68k自身の計算(クラスタ数×クラスタ
   サイズ)と一致した。
2. **`copy a:\command.x c:\`(受け入れ条件2): 不合格のまま。** 未修正時は「ディスクの
   管理領域が壊されています。使用不可です 中止<A> 再試行<R> 無視<I>?」で拒否された。
   上記の2つの実バグ(パーティションテーブル+4、FAT予約エントリの並び)を修正した後も、
   **`copy`で新規ファイルを書き込もうとすると同じ拒否が起きる**ことを確認した
   (下記「切り分けた範囲」参照)。**この節の時点では受け入れ条件2は合格していない。**
3. **再読み込み後の永続化(受け入れ条件3)・故障注入(受け入れ条件4): 未実施。**
   受け入れ条件2が通らないため、その先に進めなかった。

### 切り分けた範囲(実測、2026-09-04)

上記の拒否がイメージのバイト列の問題か、別の要因かを切り分けるため、以下を実測した:

- **本物のFORMAT.X製イメージ(`_local/scsi/twopart.HDS`、既に`HUMAN.SYS`等が入っている)を
  そのままマウントすると、`copy a:\command.x c:\`は成功する**(`1 個のファイルを
  コピーしました`)。これを複数回、独立したOPFSコピーで再現した。
- **`createFormattedScsi()`が作ったブランク(sectorsPerCluster=1のケース・2のケースの
  両方、サイズ32MB/80MB/99MB全て)は、上記2つのバグを修正した後も一貫して`copy`で
  拒否される。** サイズやspcを変えても症状は変わらない。
- **`createFormattedHdd()`(SASI)が作ったブランクは`copy`が成功する**(`1 個のファイルを
  コピーしました`)。SASIはSCSIとは別の(px68k-libretro組み込みの、より枯れた)経路。
- px68k-libretro側にSCSIリクエストのログ(`[SCSI-REQ]`、実行中デバッグ用に既存)を
  仕込んで観測すると、**成功する場合も失敗する場合も、コマンド`$05`(この項の
  リクエストヘッダ構造は資料が無く未解読。両ケースで`err=$00`を返す無害なスタブが
  実装済み)が同じように何度も呼ばれており、これは差の説明にならない**ことを確認した。
- 失敗するケースでの詳細なトレース(1回のクリーンな`copy`試行)は次の順で止まる:
  1. ルートディレクトリ(セクタ163)を読む → 全ゼロ(空)
  2. FATセクタ1(クラスタ0/1の予約エントリ域)を読む → `f7 ff ff ff 00 00 ...`(空)
  3. コマンド`$05`(無害)
  4. **ルートディレクトリへ新規ディレクトリエントリを書く**(`COMMAND.X`、
     開始クラスタ=2、サイズ=0 という「仮」の状態で書き込み成功)
  5. コマンド`$05`(無害)
  6. FATセクタ1を**再度**読む → 内容は3と同じ`f7 ff ff ff 00 00 ...`(空のまま)
  7. ここで実行が止まり、利用者側では「ディスクの管理領域が壊されています」が出る。
     **FATチェーンの更新やファイル本体データの書き込みは一度も行われない。**
- 成功するケース(本物イメージ)では、同じ手順4相当の後、**複数回のコマンド`$08`
  (データ書き込み・FAT更新・ディレクトリエントリ確定と思われる)が続けて発行され、
  最終的に「1個のファイルをコピーしました」に到達する**。
- **「クラスタ2から確保するのが特別に悪いのか」を実測で否定した**: FAT[2]を
  あらかじめ`0xFFFF`(使用済み)に書き換えた「クラスタ3から確保させる」変種イメージでも、
  同じ「ディスクの管理領域が壊されています」を再現した。
- 以上から、**イメージのバイト列(パーティションテーブル・BPB・FAT)の形式そのものは
  もう本物と一致しており、`dir`は完全に正しく動く。残っている不具合は
  「一度も書き込まれたことが無い(=完全に空の)SCSI区画へHuman68kが初めて
  ファイルを書こうとする」という条件でだけ再現し、これは`px68k-libretro`
  リポジトリ(`x68k/scsi.c`、このリポジトリの外)側の挙動である可能性が高い。**
  SASI(px68k-libretro組み込みの別経路)は同じ「空の区画への初回書き込み」でも
  問題なく成功するため、Human68k自体の一般的な制約ではなく、SCSI側の
  ストラテジ/インタラプト処理(自前RAMドライバ)固有の何かだと考えられるが、
  `x68k/scsi.c`のコマンド`$05`(未解読)や`$08`の実装詳細まで踏み込む調査は
  今回のセッションのスコープ(SCSI UIのブランク作成)を大きく超えるため、
  ここで打ち切り、別セッションへ引き継ぐ。

## 空区画コピー失敗の追跡：書き込みは正しく落ちている／「CPU完全停止」は誤りだった(実測、2026-09-04)

前節を引き継ぎ、`px68k-libretro`側(`x68k/scsi.c`)を起点に調査した。**帰属の確定
(自作ブランク/本物の該当箇所を空にした版のいずれも拒否、非空の本物・SASIブランクは
成功)は親セッションが実測済みであり、ここではその結論を前提にする。**

**本節は一度「CPUが完全停止する」と誤って結論した記録を訂正したものである。**
訂正の経緯を含めてそのまま残す(誤った結論を消して正しい結論だけ書くと、
同じログ上限の罠に次の担当が再び落ちる)。

### やったこと(1周目)

1. `x68k/scsi.c`のコマンド`$08`(書き込み)ハンドラに、`webx68k_scsi_write_sector()`
   が成功を返した直後にその場で`webx68k_scsi_read_sector()`で同じLBAを読み返し、
   書いたバイト列と1バイト単位で照合する`[SCSI-WRITE-VERIFY]`ログを追加した
   (「書けたと返った」ことと「実際にそのLBAへ落ちた」ことは別、という調査方針に従う)。
2. `scripts/build-core.sh`でコアを再ビルドし、Puppeteerで実ブラウザを自動操作する
   使い捨てスクリプト(リポジトリには残していない)を書いて、`createFormattedScsi(80)`の
   ブランクへ`dir c:` → `copy a:\command.x c:\`を打鍵し、`[SCSI*]`ログ全件と
   画面テキストを回収した。
   - 打鍵は1文字ずつ押す→エコーを読み返す→ずれていたら全部消して打ち直す、を
     関数化して検証した(自動打鍵の脱落・入れ替わりを既知の落とし穴として警戒した)。
   - **陽性対照**として、同じ手順を本物の`_local/scsi/twopart.HDS`(非空)に対しても
     実行し、`1 個のファイルをコピーしました`で成功することを確認した
     (試験装置自体は正しく動作している)。

### 1周目の結果と、そこで犯した誤り

`[SCSI-WRITE-VERIFY]`で、ルートディレクトリへの`COMMAND X   `エントリ書き込み
(LBA390・391)が書いた16バイトと寸分違わず一致することを確認した。**「書けたと
言われたが実際は落ちていない」という仮説はここで否定できた(これは今も有効)。**

その後、FATセクタ1(LBA66)の再読み出しまでは前節の記録と一致したが、そこから先で
**「`[SCSI*]`ログが複数回の独立実行すべてで正確に2047件に達したところで完全に
途絶え、画面も一切変化しない」という現象を「ゲストCPUが完全停止した」と誤って
解釈した。** 観測(ポーリング)を止めても同じ2047件で止まったことを「観測が原因
ではない証拠」と扱ったが、これは的外れな反証だった。**2047(=2^11-1)という
出来過ぎた値そのものが、CPU側ではなく観測経路(Puppeteerのconsoleイベント収集、
または本テスト用ハーネス自作のログ配列)側に何らかの上限があることを疑うべき
シグナルだったのに、見落とした。**

### 訂正：ログに依存しない指標で再確認した(2周目)

以下はすべて`log_cb`(`[SCSI*]`コンソールログ)を一切経由しない指標で確認した。

1. **`window.__webx68kDebug.stat()`のfpsを、「停止」判定した地点から3秒空けて
   2回取得**したところ、`fps=61.46`で変化なし。コアのメインループ(rAF駆動)は
   正常に回り続けていた。
2. **`screenText()`の`diagnostics`(文字コード変換に頼らないセル単位の生の
   統計)を見ると、`nonEmptyPlaneCells=[865, 761, 0, 0]`だった。** 通常の
   `A>`プロンプト画面(1周目で「凍りついた」と誤認した直前の状態を含む)は
   2枚目のプレーン(反転・アトリビュート面に相当)がほぼ空(`[768, 768, 0, 0]`の
   ように両プレーンがほぼ同数)になるのに対し、この時点では2枚目プレーンだけ
   761セル埋まっており、`unknownCells`も201(通常時の約2倍)に増えていた。
   **これは画面上に、`screenText()`のフォント対応表では読めない何か
   (罫線・反転表示の窓)が実際に描画されていることを示す。**
3. **`A`(中止)キーを1回送ったところ、3秒後に画面が変化し、`A>`プロンプトへ
   戻った。** `diagnostics`も`unknownCells=98`・`nonEmptyPlaneCells=[802, 802,
   0, 0]`という「通常のプロンプト画面」の形に戻った。**キー入力に応答した
   ==CPUは止まっていなかった。**画面に描かれていたのは、前節で人間が実ブラウザで
   確認した「ディスクの管理領域が破壊されています。使用不能です
   中止<A> 再試行<R> 無視<I>」ダイアログそのものであり、`screenText()`の
   文字コード変換がその罫線・反転表示部分を読めなかっただけだった。
   (参考: `I`(無視)キーは1回試したが、この打鍵では画面に変化が見られなかった。
   打鍵方法自体を疑う余地が残るため、今回は「未確認」のまま次へ引き継ぐ。)

**結論: 「CPUが完全停止する」は誤りだった。実態は前節と同じ、既知のダイアログで
キー入力待ちになっているだけである。** ログが2047件で止まった理由(観測経路の
上限がどこにあるか)は未特定のまま(核心とは無関係と判断し、これ以上は追わなかった)。

### 分かったこと・分からなかったこと(訂正後)

- **分かったこと**: `$08`の書き込みは実装として正しい(バイト単位で照合済み)。
  失敗は「書き込みが落ちていない」ことでも「CPUが停止すること」でもなく、
  前節までに確定していたとおり、**FATセクタ1の2回目読み出し(空のまま)の後、
  Human68kが「ディスクの管理領域が破壊されています」ダイアログを出して
  キー入力待ちになる**ことである。`A`(中止)で`A>`プロンプトへ復帰することを
  自動化環境でも実測で確認した。
- **分からなかったこと**: 何がこのダイアログを引き起こしているか
  (コマンド`$05`の意味・Human68kがそこで何を根拠に「壊れている」と判断するか)
  は依然として未解明。`R`(再試行)・`I`(無視)を押した後の挙動も未確認
  (`I`は1回試して画面変化なしだったが、打鍵方法自体の再検証が必要)。
  本物のSCSI ROMは逆アセンブルしない方針のため、資料的に確定させる手段も無い。
- **直せなかった。** 受け入れ条件2(`copy`成功)は依然として不合格。
  条件3・4はその前提が崩れているため未実施。

### 次にやること

1. `R`(再試行)・`I`(無視)キーを、打鍵の陽性対照(押した後に画面か`stat()`が
   変化することの確認)込みで測り直す。`I`(無視)でコピーが強行された場合、
   その後のFAT状態が壊れるのか・使える状態になるのかは有力な手がかりになる。
2. ダイアログが出る条件そのもの(コマンド`$05`の意味・Human68kが「壊れている」と
   判断する根拠)を、`x68k/mem_wrap.c`の`webx68k_ram_watch_check`/PC範囲指定の
   仕組み(既存)でPCやメモリアクセスを絞り込みながら追う。
3. 上記が直ったら、受け入れ条件2〜4(コピー成功・再読み込み後の永続化・
   sectorsPerFatをずらす故障注入がその両方を実際に壊すこと)を実測し直す。
4. `createFormattedHdd()`(SASI)のFAT予約エントリが本項で見つけたのと同じ
   `[0xFF, media]`/`[media, 0xFF]`の並び違いを持っていないか、実機吸い出しイメージ
   (もしあれば)と突き合わせる。今回はSCSI側だけ実測して直し、SASI側は
   「動いている(実ブラウザでcopy成功済み)」という理由で触らなかったが、
   本当に正しいかどうかは未検証のまま。

## 本物ROMオラクルの復元を試みたが果たせず、自前ドライバのログ差分に切り替えた(実測、2026-09-04)

前節を引き継ぎ、「本物のSCSI ROMをオラクルとして走らせ、コマンド`$05`の処理前後で
要求ヘッダのどこがどう書き換わるかを観測する」方針で着手した。**結論から書く:
本物ROMのオラクルは今回のセッションでは復元できなかった。**その代わり、
自前ドライバ(`x68k/scsi.c`、既に安定して動く)のログから、`copy`が失敗する瞬間の
要求ヘッダとFATの中身を成功例・失敗例で直接突き合わせることはでき、
前節までの結論を裏付ける新しい実測が1つ増えた。順に記録する。

### 新しく見つけた穴: Worker既定経路では `--rom=` が無言で無効化されていた

`px68k_libretro.js`/`.wasm`を`scripts/build-core.sh`で再ビルドしたうえで、
`scripts/probe-scsi-iocs.mjs --rom=_local/scsi/Scsiexrom.dat --image=_local/scsi/twopart.HDS`
(worker指定なし=既定のWorker経路)を実行すると、**`booted=true`で`dir c:`も
正しく動くが、`[SCSI-BUS]`ログが1件も出ない**(`totalLogged: 0`)。
`rawUnparsed`を見ると、動いていたのは**自前スタブの初期化ログ
(`[SCSI] デバイスドライバヘッダ書き込み確認`)であり、本物ROMのログではなかった**。

原因は `src/main.ts` の `collectHostGlobals()`(3159行目)にあった:
...
**string/number/booleanしか転写しない。** プローブが `window.__webx68kScsiRomBytes` に
置く8192バイトの**配列**はここで無条件に落とされ、Workerの`globalThis`には一切届かない。
`x68k/scsi.c`側は`webx68k_scsi_rom_len()`が`$2000`のときだけ本物ROMとして扱うため、
**Worker経路では常に`rom_len=0`になり、`--rom=`を指定してもエラーも警告も出ないまま
自前スタブへ静かにフォールバックする。** これは関数を転写しない設計(コメントに明記済み)
と同じ理由だが、**配列も対象外になることはコメントに書かれておらず、今回まで
気づかれていなかった。** `--worker=0`(従来のメインスレッド経路)を使えば
`window.__webx68kScsiRomBytes`がそのまま`globalThis`なので問題なく届く。
**今後、本物ROMを使うプローブは必ず`--worker=0`を付けること。**

### `--worker=0` で本物ROMは読み込まれたが、SPCチューニングの再現に失敗した

`--worker=0 --rom=_local/scsi/Scsiexrom.dat --image=_local/scsi/twopart.HDS` で実行すると:

- **既定のSPC設定のまま**: 42秒でHuman68kが`A>`まで起動したが、`dir c:`は
  「ドライブ名が無効です」で、**SCSIデバイスは1台も認識されなかった**
  (2026-09-02時点の「本物ROM＋最小SPC」到達点と同じ状態)。
- **`--spc-ints-sel=0x10`を足す**(2026-09-02の記録が「セレクト完了のビット」として
  確定させた値): `--image`の有無に関わらず、**90秒〜6分半の待ちでも一度も`A>`へ
  到達しなかった**(画面は空白のまま)。`--image`無し・既定設定のみでの起動
  (`--rom`のみ)も同様に6分半到達せず、**2026-09-02に「30〜40秒で起動した」と
  記録された組み合わせが、今のコードでは再現しなかった。**
- 原因の候補として、doc自身が2026-09-03に書き残していた注意点が当てはまる:
  「`ssts_data_bit=48`は`ssts_tc0_bit`が存在しなかった時期の値で、今のコード
  (`spc-ssts-data-bit`/`spc-ssts-tc0`に分割済み)にそのまま与えると競合して
  `$ea13de`でSSTS=`$a0`を無限ポーリングする」という既知の罠と同じ症状に見える。
  つまり**再現には値の使い回しではなく、分割後のパラメータでの再チューニング
  (2026-09-02〜03相当の掃引をもう一度)が要る**。これは今回のセッション予算では
  完走できなかった。

**教訓**: 過去の「最良設定」は、コード側のパラメータ分割が進むと**無言で
別の状態に化ける**(今回は「無限ポーリング」)。invocationを丸ごと残す対策
(2026-09-03導入)はされているが、**「今のコードでも動くか」は別途確認しないと
過去のJSONを信じて時間を溶かす**、という点は次の担当への申し送りとして重要。

### オラクルを諦め、自前ドライバのログ差分で$05まわりを直接突き合わせた

本物ROMのSPC再チューニングは時間対効果が悪いと判断し、**方針を変えた**。
`x68k/scsi.c`の`SCSI_HandleRequestHeader()`は、コマンドの種類にかかわらず
**要求ヘッダ26バイト全体を処理前・処理後の両方でログに出す**実装が既に入っている
(`[SCSI-REQ] 処理前 addr=...` / `処理後 addr=...`)。これは自前ドライバ(ROM不要、
安定して速い)でも成功例・失敗例の両方から取れるので、**オラクルなしで
「`$05`の入出力が成功/失敗でどう違うか」を直接比較できる。**

`_local/scsi/twopart.HDS`(成功例)と`_local/scsi/real-emptied.HDS`(失敗例)に対して
`--scsi-opfs --type="dir c:;;copy a:\command.x c:\"`を実行し(陽性対照: 前者は
「1個のファイルをコピーしました」に到達することを確認済み)、両方の
`[SCSI-REQ]`/`[SCSI-READ]`/`[SCSI-WRITE]`ログを回収した。

**分かったこと:**

1. **`$05`の入出力パターンは成功/失敗で質的に同じ。**26バイトの長さ(`$1a`)、
   コマンド番号(`$05`)は共通で、うち`+3`(処理前は不定値、処理後は必ず`$00`)以外の
   全バイトは処理前後で一切変化しない(これは自前スタブの実装どおりで、
   ROMの有無に関係ない事実)。**呼ばれる回数もほぼ同じ**(この試行では失敗例19回・
   成功例22回)。前節までの結論(「`$05`単体は差の説明にならない」)を、
   今回改めて生ログで裏付けた。
2. **ディレクトリエントリの仮書き込みはどちらも成功する。**
   `COMMAND X`のエントリを書くコマンド`$08`(失敗例: LBA466、
   `43 4f 4d 4d 41 4e 44 20 58 20 20 20 00 00 00 00`)は成功例・失敗例とも
   同じように通る。
3. **本当の分岐点はその直後のFATセクタ1(ホストLBA=66)の再読み出しの中身。**

   - 失敗例(`real-emptied.HDS`): `f7 ff ff ff 00 00 00 00 00 00 00 00 00 00 00 00`
     (クラスタ2以降、見えている範囲は全て`$0000`=空き)。この読み出しの直後、
     `$05`が1回呼ばれた後で**それ以上コマンドが来ないまま止まる**
     (=ダイアログでキー入力待ちになる、前々節までの結論のとおり)。
   - 成功例(`twopart.HDS`): `f7 ff ff ff 00 03 00 04 00 05 00 06 00 07 00 08 …`
     (クラスタ2から順に3,4,5,6,7,8…と**既に使用中の連番チェーン**が並んでいる)。
     この読み出しの直後、Human68kは**そのまま複数回の`$08`
     (ディレクトリエントリ確定・27セクタ分のファイル本体書き込み)を発行して
     コピーを完了させる**。

4. **「クラスタ2から確保するのが特別に悪い」という前節の否定結果と矛盾しない。**
   前節は`FAT[2]`だけを`0xFFFF`(単体ファイルの終端)に書き換えて再現性を見たが、
   今回見えた成功例のFATは**そこから何十個も連続して「使用中」が続く長い塊**であり、
   1個だけ終端にしても同じ構造にはならない。「特定の1クラスタの値」ではなく、
   **もっと広い範囲の中身、あるいはFATそのものとは別の場所にHuman68kが持つ
   何らかの状態(空き容量キャッシュ・確保カーソル等)が分岐点である可能性が高い**、
   という前節の推測を追認する形になった。

5. **「本当に真っさらなFORMAT.X製の区画」のFATがどう見えるかは、今回も確認できなかった。**
   `twopart.HDS`の2区画は両方とも既にファイルが入っており(区画1のFATも同様に
   `f7 ff ff ff 00 03 00 04 … ff ff ff ff`という使用中チェーンだった)、
   「1個もファイルが無い、しかしFORMAT.X が作った直後の」区画のFATを直接見る
   手段がまだ無い。これは本物ROMオラクルが元々解決しようとしていた問いそのもの
   であり、今回はここへ戻ってきただけで終わった。

### 受け入れ条件

1〜4いずれも前節から進展なし(2は依然不合格、3・4は未実施)。**直せなかった。**

### 次にやること(更新、後述の実測により大幅に書き換え)

1. **本物ROMオラクルを再開するなら**、まず`--worker=0`を必須にすることを
   プローブのヘルプ/コメントに明記し、そのうえで`--spc-ssts-data-bit`/
   `--spc-ssts-tc0`を2026-09-02の値をそのまま使わず**ゼロから掃引し直す**。
   時間がかかることを見込んで別セッションに切り出すのが妥当。
2. **オラクルを使わない代替案**: 本物のFORMAT.Xが(もし起動ディスクに入っていれば)
   真っさらな未使用領域に対して実際にフォーマットを行い、そのときできる
   FATの中身を、自前ドライバ(安定して動く)経由で観測する。これができれば
   「本当に空のFATがどう見えるべきか」を、SPCチューニングというROM依存の
   難所を経由せずに確認できる。
3. `x68k/mem_wrap.c`の`webx68k_ram_watch_check`/読み出し監視の仕組み(既存)を使い、
   FATセクタ1の2回目読み出し(全て`$0000`のとき)の**直後**にHuman68kが読み書きする
   ゲストRAM番地を追い、「空き容量キャッシュ」や「確保カーソル」に相当する
   ワークエリアを特定する。今回、要求ヘッダとFATの中身までは一致させたので、
   次はHuman68k自身のワークエリア側を疑う番である。

**【重要】上記3項目は前提(「トリガーは空の区画」)がこの後の実測でほぼ否定された。
着手する前に次節を読むこと。**

## ゲスト自身に消させて帰属を確定：トリガーは「空の区画」ではなく「同名ファイルの削除直後の再作成」だった(実測、2026-09-04)

自作イメージのバイト列を一切介さず、**Human68k自身の`del`/`copy`だけ**で
トリガーを絞り込んだ。使ったのは本物のFORMAT.X製イメージ`_local/scsi/twopart.HDS`
(既に18エントリ・実ファイル1060個入りの実運用イメージで、コピーして使用)。

### 実験1: ゲストに消させる(素朴な「空にする」は規模的に無理だった)

`twopart.HDS`のC:区画をPythonでFAT16パーティションとして直接パースしたところ、
**ルートの18エントリの下に再帰的にファイル1060個・ディレクトリ82個・深さ5**が
実在した。1文字ずつキー入力するこのプローブの打鍵方式で全部を再帰削除するのは
現実的な時間で終わらないため、当初計画(区画を完全に空にする)は断念し、
**ルート直下だけを`del c:\*.*`で消す**方式に切り替えた(隠し属性の`HUMAN.SYS`は
既定で対象外のまま残る)。

再現条件: `--scsi-opfs --profile=<使い回すパス> --image=<twopart.HDSのコピー> --type="del c:\*.*;;y;;dir c:;;copy a:\command.x c:\;;dir c:"`

- **1回目**(`_local/scsi/exp/exp1.HDS`): `del c:\*.*`→`y`で8ファイル削除(残るのはディレクトリ10個のみ、`HUMAN.SYS`は非表示のまま健在)。直後の`copy a:\command.x c:\`で
  **`ディスクの管理領域が破壊されています。使用不可です 中止<A> 再試行<R> 無視<I>?`**
  が出た(画面をスクリーンショットで確認)。
- **2回目**(`_local/scsi/exp/exp1b.HDS`、別プロファイル・別コピーで独立に再実行): **同一の結果**。ダイアログの文言・レイアウトまで一致。

**ここで前節までの仮説が崩れた**: 両トライアルとも、直前の`[SCSI-READ]`が拾った
FATセクタ1(ホストLBA=66)の先頭16バイトは

```
f7 ff ff ff 00 03 00 04 00 05 00 06 00 07 00 08
```

で、これは**「成功例」として記録していたパターンそのもの**(クラスタ2から
`HUMAN.SYS`自身の使用中チェーンが続く形。`HUMAN.SYS`は`del *.*`の対象外なので
このチェーンは無傷)。**「FATセクタ1の先頭が全部`$0000`か」という前節までの
判定軸は、今回の失敗を説明できない。** ログはここで途切れ(2回とも同じ4278件で
打ち止め)、その後は新規SCSIコマンドが一切来ないまま停止した。

### 実験1': 単一ファイルの削除では起きない(陰性対照)

「ルートを丸ごと8ファイル削除」ではなく的を絞るため、`_local/scsi/exp/exp1c.HDS`で
**`command.x`とは無関係な`Autoexec.bak`だけ**を削除してから`copy a:\command.x c:\`を
実行した(`--type="del c:\autoexec.bak;;y;;dir c:;;copy a:\command.x c:\;;dir c:"`)。

**結果: 成功。**「`1 個のファイルをコピーしました`」が出て、`dir c:`にも
`COMMAND.X`が正しく現れた。**無関係なファイルの削除は無害。**

### 実験1'': 「消してすぐ同じ名前で作る」だけで再現した(核心)

実験1と1'の差は「削除したファイル数」だけではなく「削除した対象に、この後
コピーする`COMMAND.X`自身が含まれているか」でもある。これを直接切り分けるため、
**`command.x`だけを消してすぐ`copy a:\command.x c:\`を実行する**最小構成を試した。

再現条件: `--type="del c:\command.x;;dir c:;;copy a:\command.x c:\;;dir c:"`
(削除にワイルドカードを使わないため`Y/N`確認は出ない。他のファイルは一切触らない)

- **1回目**(`_local/scsi/exp/exp1d.HDS`): `del c:\command.x`で1ファイルだけ削除
  (`dir c:`で消えたことを確認)。直後の`copy a:\command.x c:\`で
  **同一の`ディスクの管理領域が破壊されています`ダイアログが出た**。
- **2回目**(`_local/scsi/exp/exp1e.HDS`、独立に再実行): **同一の結果**。

**これが実測できた最小の再現条件である**: 区画が空である必要は無く、
`bin`/`sys`等82ディレクトリ・1060ファイルが丸ごと無傷のままでも、
**「消したのと同じ名前のファイルを、消した直後に同じ場所へ書き込もうとする」
だけで壊れる。** 前節までの調査が追っていた「空の区画」「FATセクタ1が全部$0000」
という切り口は、この実験1''の状況(区画はほぼ満杯)には一切当てはまらない。
つまり**トリガーの取り違えだった**可能性が高い(過去の`real-emptied.HDS`実験は、
「空にする」過程で結果的に何らかの「直前に消して直後に書く」経路を踏んでいた
だけかもしれない、という疑いが残る)。

### 実験2: FORMAT.Xはそもそも通らなかった

システムFD(`public/system/human302.xdf`)に`FORMAT.X`(`X68k Format v2.20`)が
実在することを確認し、2区画目(`format d:`、ただし`dir d:`は`dir c:`と
全く同じ18エントリを返しており、C:とD:が同じ実体を指している疑いがある。
未検証のまま持ち越し)に対して`format d:`を実行した。

結果: バナー表示直後に**`初期化できませんでした`**→`他のディスクを初期化しますか？<Y/N>`
で終了。論理フォーマットの確認プロンプトにすら到達せず、**このSCSI HLEでは
FORMAT.Xの要求する手順(おそらく物理フォーマット系のSCSIコマンド)が
そもそも未実装で、ゲスト内フォーマットの経路は今のところ使えない。**
実験1''で核心が別に見つかったため、これ以上の深追いはしていない。

### 受け入れ条件との対応

「直せた場合の受け入れ条件」1〜4はいずれも未着手(今回はトリガーの特定が主眼)。
**直せていない。** ただし前節までの「空の区画が壊れる」という前提そのものが
実測で崩れたため、直すべき箇所の当たりが変わった。

### 次にやること(全面更新)

1. **最優先**: 実験1''の最小再現(`del c:\command.x`直後の`copy a:\command.x c:\`)を
   `x68k/scsi.c`の`[SCSI-REQ]`/`[SCSI-READ]`/`[SCSI-WRITE]`ログで再度追い、
   「削除の書き戻し($08)」から「コピーがそのディレクトリエントリ/FATを
   読み直す再読み出し」までの間で、**削除の結果が正しく反映されているか**
   (ホスト側が読んだ内容が、直前に書いた内容と矛盾していないか)を1バイト単位で
   突き合わせる。`src/scsi-opfs.ts`の`FLUSH_INTERVAL_MS=2000`(書き込みは2秒ごとに
   まとめてflush)は`flush()`が無くても同一ハンドルの`read()`は直前の`write()`を
   見えるはずだが、**未検証のまま**なので先に確認すること。
2. `createFormattedScsi()`(ブランク作成)の「潰してある説」の再検証は不要になった
   可能性が高い。**「空である」ことそのものは問題ではない**という実測が出たため、
   ブランクHDDでの`copy`失敗は今回見つかった「削除直後の再作成」経路とは
   別原因の可能性がある。ブランク作成直後(削除を経ない、最初から空)での
   単純な`copy`失敗を、今回と同じログ突き合わせ手法で再度確認すること。
3. **D:とC:が同じ内容を返す疑い**(実験2で気づいた)を検証する。もし実際に
   同一実体を指しているなら、2区画目のBPB/パーティション表の扱いに別のバグがある。
4. 本物ROMオラクル(SPCチューニング)の再開は、実験1''で代替の切り分け手段が
   得られたため優先度を下げてよい。

## 「削除直後の再作成」ではなく「新規に複数クラスタのチェーンを割り当てる書き込み」が真因だった(実測、2026-09-04)

前節の結論(「トリガーは削除直後の同名再作成」)は、条件表を追加で埋めた結果**説明できない
行が出たため撤回する**。正しい規則は実験で確定した:

> **新規に2クラスタ以上のチェーンを割り当てる書き込みだけが失敗する。
> 単一クラスタで足りる新規作成(`mkdir`、1クラスタに収まる新規ファイル)と、
> 既存のクラスタチェーンをそのまま再利用する上書きは、失敗しない。**

これは本物イメージ(`twopart.HDS`、実運用・1060ファイル入り)と自作ブランク
(`createFormattedScsi(100)`)の**両方**で成立することを実測した。

### 条件表(全行、2回以上実測)

| # | 操作 | 対象 | 新規クラスタ数 | 結果 |
|---|---|---|---|---|
| 1 | `copy a:\command.x c:\`(既存を上書き) | 本物 | 0(既存チェーン再利用) | **成功** |
| 2 | 無関係ファイル`Autoexec.bak`削除→`copy a:\command.x c:\` | 本物 | 0(既存チェーン再利用) | **成功** |
| 3 | `del c:\command.x`→同名で`copy` | 本物 | 28(新規チェーン) | **失敗** |
| 4 | `del c:\*.*`(ルート8ファイル削除)→`copy a:\command.x c:\` | 本物 | 28(新規チェーン) | **失敗** |
| 5 | 自作ブランクへ`copy a:\command.x c:\` | ブランク | 28(新規チェーン) | **失敗** |
| 6 (新規) | 一度も存在しない名前`zzz.x`へ`copy a:\command.x` | 本物 | 28(新規チェーン) | **失敗** |
| 7 (新規) | `del c:\command.x`後、**別の新規名**`zzz.x`へ`copy` | 本物 | 28(新規チェーン) | **失敗** |
| 8 (新規) | `mkdir c:\zzztest`(新規ディレクトリ、コピーではない) | 本物 | 1(新規1クラスタ) | **成功** |
| 9 (新規) | 一度も存在しない名前`zzz.bat`へ`copy a:\autoexec.bat`(179バイト=1クラスタ) | 本物 | 1(新規1クラスタ) | **成功** |
| 10 (新規) | 一度も存在しない名前`zzz.sys`へ`copy a:\uskcg.sys`(8028バイト=8クラスタ) | 本物 | 8(新規チェーン) | **失敗** |
| 11 (新規) | 自作ブランクへ、**一度も存在しない名前**`a.txt`へ`copy a:\config.sys`(468バイト=1クラスタ) | ブランク | 1(新規1クラスタ) | **成功** |
| 12 (新規) | 自作ブランクへ`copy a:\command.x c:\command.x`(28クラスタ) | ブランク | 28(新規チェーン) | **失敗** |

行6・7が前節の「削除直後の再作成」説を否定する: どちらも**削除を経ていない
(6)か、削除したのと別の名前(7)**にも関わらず失敗した。一方で行8・9・11は
**新規作成(削除もしていない、一度も存在しない名前、あるいは完全に空のディスク)
でも、必要なクラスタが1個だけなら成功する**ことを示す。行10・12は「大きい
チェーンだから」ではなく「複数クラスタのチェーンだから」であることを1〜8クラスタの
範囲でも確認する追加実測(正確な閾値(1クラスタと2クラスタの境界)は未検証)。

**すべての行(旧5行+新7行)が「新規複数クラスタチェーン割り当てだけが失敗する」
という1つの規則で説明できる。** 前節までの「空の区画」「削除→再作成」という
軸は、いずれもこの規則が生む見かけ上の相関に過ぎなかった(空の区画でのcopy failureは
たまたま毎回複数クラスタが要る大きいファイルで試していたため、削除→再作成failureは
たまたま毎回同じ大きいファイル(command.x)で試していたため、それぞれ「複数クラスタ
チェーン」という真の条件と一致して見えていた)。

### SCSI要求列の最初の分岐点

成功例(行9: `autoexec.bat`→`zzz.bat`、1クラスタ)と失敗例(行6: `command.x`→`zzz.x`、
28クラスタ)を、`--scsi-opfs`の`[SCSI-REQ]`/`[SCSI-READ]`/`[SCSI-WRITE]`ログで
先頭から突き合わせた。**両者は完全に同じ手順を、まったく同じFAT内容に対して
同じ順番で辿る**:

1. ディレクトリセクタ(論理セクタ201, ホストLBA466)を読む(新規作成先の
   ディレクトリを開く)。
2. FAT第1セクタから第99セクタまで(論理セクタ1〜99, ホストLBA66〜262)を
   **1セクタずつ、全件**読む(空き容量の把握、または空きクラスタの列挙)。
   この範囲に空き(`$0000`)は1個も無く、クラスタ2〜47はすべて使用中
   (`0003 0004 …`と続く鎖、またはチェーン終端`ffff`)であることを新設ログ
   (96バイト=クラスタ0〜47ぶん)で確認した。
3. ディレクトリセクタ(201/466)を数回読み、**新規エントリの仮書き込み
   (WRITE 201/466)を行う**(先頭16バイトはたまたまHUMAN.SYSのエントリで
   不変に見えるが、書き込み対象はセクタ内の別オフセットにある新規エントリ)。
4. FAT第1セクタ(1/66)を**もう一度**読む。

**ここまでは成功例・失敗例で1バイトも違わない。** 直後に分岐する:

- **成功例(1クラスタ)**: 手順4の直後に**データ領域のセクタ(論理セクタ505,
  ホストLBA1074)を読み**、続けてディレクトリセクタの読み書きを繰り返して
  コピーを完了させる。
- **失敗例(複数クラスタ)**: 手順4の後、**新しいSCSI要求が二度と来ないまま
  停止する**(=ダイアログでキー入力待ちになる)。データ領域へは一度も
  アクセスしない。

つまり最初の分岐点は「手順4の直後に何をするか」であり、**失敗例はFATセクタ1を
2回目に読んだ直後、続くFATセクタ2以降を読みに行かず(空きが無いと分かっている
はずのセクタ1だけを見て)、それ以上ディスクへ触れずに終わる**。この停止は
コア側(SCSI HLE)がエラーを返しているのではなく(そのようなログは一切無い)、
**Human68k側がCPU内の情報だけで「壊れている」と判断して以後I/Oを出さなくなった**
ことを意味する。これは前節までの結論(「ダイアログでのキー入力待ち」)と矛盾しない。

**分からなかったこと**: なぜ複数クラスタの割り当てだけがFATセクタ1の2回目読み出し
直後に諦めるのか、その中身(HUMAN68k側の判断ロジック)はここまでの実測では
分からない。読み出したFATセクタ1の中身は3回とも(仮書き込み前後を含め)完全に
同一で、コア側の書き込みが何かを壊した形跡は無い(前節のWRITE-VERIFYの結論と
整合)。空きクラスタが本当にセクタ2以降にあるはずなのに、なぜセクタ2を読みに
行かず諦めるのかは、Human68k自身(HUMAN.SYS、閉鎖ソース)の割り当てアルゴリズムの
内部状態を追わないと分からず、今回の実測の範囲を超える。

### 直せなかった

原因はHuman68k側(ゲストOS)の複数クラスタ新規割り当てロジックにあると強く
推測されるが、**ロジックの中身(なぜ1セクタだけ見て諦めるのか)を特定できて
いないため、`x68k/scsi.c`側の修正は行っていない。** 受け入れ条件(1〜4)は
未達成のまま。

代わりに、調査用として`x68k/scsi.c`のSCSI-READログを16バイト→96バイト
(クラスタ0〜47ぶん)に拡張した(挙動は変えていない、ログ出力のみの変更)。
これにより次回、FATセクタの生の中身をより広く見られる。

### 次にやること(更新)

1. **最優先**: HUMAN.SYS(ゲストOS本体)がFATをどう割り当てるかの実装を
   突き止める必要がある。手段の候補:
   - `x68k/mem_wrap.c`の`webx68k_ram_watch_check`/`webx68k_mem_read_watch_check`
     を使い、「FATセクタ1の2回目読み出し直後」から「次にCPUが何もSCSI要求を
     出さなくなるまで」の間に、HUMAN.SYSが読み書きするRAM番地(ワークエリア・
     DPB相当・エラーフラグ等)を追う。今回どこで止まるかは特定できたので、
     次はその直後のCPU側の動きを追う番。
   - 2クラスタちょうどのファイル(クラスタサイズ1024バイトなら1025〜2048
     バイト)を用意して、「1クラスタは成功・2クラスタ以上は失敗」という
     閾値が本当に「2」なのか、それとも別の条件(例: チェーンが1セクタの
     FATエントリ範囲をまたぐかどうか)なのかを確認する。今回は1クラスタ
     (成功)と8・28クラスタ(失敗)しか実測できていない。
2. **ブランク作成`createFormattedScsi()`の疑いは晴れた**: ブランクへの単一
   クラスタ新規作成(条件表11行目)は成功したため、ブランクのBPB/FAT自体は
   壊れていない。受け入れ条件3の「`sectorsPerFat`を1ずらすと壊れる」という
   想定の検証はまだ行っていないが、優先度は「HUMAN.SYS側のロジック特定」より
   低い(そもそもブランクの生成コードを疑う理由が今回の実測で失われたため)。
3. D:とC:が同じ内容を返す疑い(前節で気づいた)は今回未着手のまま。

## 「ログが途絶えた」の独立裏取りと、機構レベルの確定(実測、2026-09-04)

前節の結論(「新規複数クラスタ割り当てだけが失敗する」)には2つの弱点が指摘された:
(1) 「それ以降SCSI要求が来ない」がconsole.log/log_cb経由のログだけを根拠にしていた
(このプロジェクトで「ログが途絶えた=止まった」の誤読を複数回踏んでいる)、
(2) 「2クラスタ以上」が効いているのか「FATの複数セクタにまたがる」ことが
効いているのかを割れていなかった。両方を実測で裏取りした。

### 1. ログとは独立な経路での確認

`x68k/scsi.c`の`SCSI_HandleRequestHeader()`に、`log_cb`を一切経由しない生カウンタ
(`SCSIReqTotalCount`・`SCSIUnsupportedCmdCount`・`SCSIReadCount`/`SCSILastReadLogsec`・
`SCSIWriteCount`/`SCSILastWriteLogsec`)を追加した。これを`src/core-shim.c`の
getter経由でJSへ公開し(`_get_scsi_req_total`等)、`src/libretro-host.ts`の
`scsiDebugCounters()`、Worker経路では`FrameSnapshot.scsiDebugProbe`(keyBufProbeの
有効化フラグに相乗り)経由で`window.__webx68kDebug.scsiDebug()`から読めるようにした
(`scripts/probe-scsi-iocs.mjs`に`--poll-scsi-debug=<ms>`を追加)。

打鍵と並行して150ms間隔で連続ポーリングした結果(本物イメージ、
`copy a:\command.x c:\zzz.x`、Worker経路・既定設定):

| t(ms) | reqTotal | unsupported | readCount | writeCount |
|---|---|---|---|---|
| 307 | 1 | 0 | 0 | 0 |
| 1604 | 12 | 9 | 2 | 0 |
| 1914 | 55 | 9 | 45 | 0 |
...
| 9969 | 128 | 21 | 105 | 1 |
| 10120 | 130 | 22 | 106 | 1 |
| **10120〜27243(17秒間、34サンプル)** | **130で不変** | **22で不変** | **106で不変** | **1で不変** |

**カウンタは実際に上がる(=検出できることの陽性対照になっている)**。そのうえで
`reqTotal=130`に達した後、17秒間・34サンプルにわたって**1件も増えない**(未対応
コマンドの分岐にも1件も落ちていない)。これはconsole.logのキャプチャ上限や
Puppeteerのconsoleイベントの取りこぼしとは無関係な経路での確認であり、
前節の「それ以降SCSI要求が一切来ない」という結論を裏付ける。**本当に来ていない。**

### 2. 「2クラスタ以上」対「複数セクタにまたがる」の分離

`createFormattedScsi()`の`sectorsPerCluster`(spc)はサイズから自動計算されるため
(`computeScsiFatLayout()`、クラスタ数がFAT16上限65524に収まる最小の2の冪)、
**サイズを振ることでspcを操作できる**。これを使って2条件を測った:

- **spc=32のブランク(2047MiB、上限いっぱい)へ`command.x`(28382バイト)を新規コピー**:
  28382バイトはspc=32(1クラスタ=32768バイト)なら**1クラスタに収まる**。
  結果: **成功**(`1 個のファイルをコピーしました`、`dir c:`にも正しく出た)。
  同じバイト数・同じファイルが、spc=2(またはspc=1)のときは必ず失敗していたのに、
  spc=32では成功する。**「大きいファイルだから失敗する」のではなく
  「複数クラスタが要るから失敗する」ことが直接示された。**
- **spc=1の1MiBブランク(総クラスタ1024、FATは3セクタでその大半が第1セクタに
  収まる小ささ)へ、`copy a:\autoexec.bat+a:\config.sys+a:\key.sys c:\combo.txt`
  (179+468+712=1359バイト、spc=1なら2クラスタ)で新規作成**:
  結果: **失敗**(同一のダイアログ)。カウンタも同じパターンで停止を確認
  (`reqTotal`等が打鍵後は不変)。**FATがどれだけ小さくても(全域が実質1
  セクタに収まっていても)、2クラスタの新規チェーンというだけで失敗する。**
  「FATの複数セクタにまたがる」ことは条件ではない。

以上により規則は**「2クラスタ以上」が確定条件であり、「複数セクタにまたがるか」
は無関係**であることが機構レベルで裏付いた。

### 失敗直前に読んだFATの範囲

独立カウンタの`lastReadLogsec`は、**本物イメージ(28クラスタ必要)でも
1MiBブランク(2クラスタ必要)でも、失敗直前の最後の読み出しは必ず論理セクタ1
(パーティション先頭からのFAT第1セクタ)だった**。すなわち**2クラスタ目を
探しに行く前に諦めている**(セクタ1の1024バイトには少なくともクラスタ2〜
511ぶんのFATエントリが入っており、2クラスタ目の空き状況を知るための追加の
読み出しコマンドは不要のはずだが、それを発行する前にHuman68kは停止する)。
この「もう1回だけセクタ1を読んで、その中身をCPU内だけで判断してすぐ諦める」
という挙動そのものは、本物イメージ・自作ブランク(サイズ違い)の3条件すべてで
共通していた。

### 直せなかった(変わらず)

Human68k側の判断ロジックの中身(セクタ1を再読して何を見ているか)は今回も
特定できていない。ただし「2クラスタ以上の新規チェーン」という条件と、
「ログ非経由でも本当に停止する」という2点は機構レベルで確定した。

### 次にやること(更新)

1. **最有力**: `x68k/mem_wrap.c`の`webx68k_ram_watch_check`/
   `webx68k_mem_read_watch_check`で、「FATセクタ1の最後の読み出し」の直後に
   HUMAN.SYSが読み書きするワークエリア番地を追う。**使う前に必ず陽性対照
   (既知の番地への既知のタイミングでの書き込みが検出できること)を確認する**
   こと — PC絞り込みの既定値が全件を無言で捨てる不具合が過去にあった
   (本ドキュメントの別節参照)。
2. spc=32のような「大きいクラスタで1クラスタに収まる」回避策は、実用上の
   ワークアラウンドとして`createFormattedScsi()`側で使える可能性がある
   (ただし2クラスタ以上が必要になる十分大きいファイルでは依然失敗するため、
   根本的な解決ではない)。
3. `test/fat-scsi.test.ts`等に、今回のカウンタ(`scsiDebugCounters`)を使った
   回帰テストを足す余地がある(現状は手動のPuppeteer実測のみ)。

## SASI（成功する側）とSCSI（失敗する側）のDPB突き合わせ（実測、2026-09-04）

コーディネータの指摘: 同じHuman68k・同じFAT16で、SASIドライブへの「2クラスタ以上の
新規チェーン」は成功する。同じHuman68kが片方でできて片方でできないなら、
Human68kが持っている管理情報(DPB)のどこかに差があるはずだ、という指摘を受けて
実測した。

### 準備: SASIとSCSIを同時にマウント

`scripts/probe-scsi-iocs.mjs`の`--hdd=`にローカルパスを直接渡せるように拡張した
(従来はURLのみ受け付けていた。`--image`と同じ使い捨てHTTPサーバで配信する)。
`createFormattedHdd()`(SASI・40MB・spc=1)と`createFormattedScsi(40)`(SCSI・40MB・
spc=1、totalSectors=40960が65524以下に収まるためSASIとspcを揃えられる)を同時に
マウントして起動した。

**ドライブレターの確認**: `dir c:`と`dir d:`はどちらも0ファイル・約40MB使用可能
(想定どおり)。`copy a:\command.x c:\zzz.x`は**成功**、`copy a:\command.x d:\zzz.x`は
**失敗**(いつもの「管理領域が破壊されています」)。**C:=SASI、D:=SCSI(我々の
ドライバ)** と確定した。

### DPB連鎖の読み方(今回追加で判明)

前節「ドライブは実在していた」は「先頭は`$1c3c.w`」とだけ書いていたが、これは
ワード単体では24bitアドレスに足りない。実際には**`$1c3c`と`$1c3e`の2ワードを
連結したロングワード**がDPB連鎖の先頭ポインタだった(`peek(0x1c3c)=$0001`、
`peek(0x1c3e)`と連結して`$00014866`、既存記録の`DPB $014866`と一致)。
`__webx68kDebug.peek()`はWorker経路では`host`が常にnullを返すため
(`src/main.ts`のscsiDebugと同じ理由)、**この調査は`--worker=0`で行う必要がある**
(OPFS書き戻しフックはWorker専用なのでSCSI側の書き込みはこのモードでは
そもそも動かないが、DPBの中身を読むだけなら支障ない。SASI側の書き込みは
Worker非依存の別経路(`x68k/sasi.c`、レジスタレベルのエミュレーション)で動くため
`--worker=0`でも実際にコピーが成功する)。

今回の構成でのDPB連鎖(`+0`ドライブ番号、`+2..5`ドライバヘッダポインタ、
`+6..9`次DPBポインタ、`$38`=56バイト):

```
$014866 drive=0 (A:)
$01489e drive=1 (B:)
$0148d6 drive=2 (C: = SASI)
$190034 drive=3 (D: = SCSI、我々のドライバ)
```

### DPBの全バイト突き合わせ(コピー実行前)

C:(SASI)とD:(SCSI)のDPBを、`copy`を一度も実行する前に56バイト全部ダンプして
突き合わせた:

```
C:(SASI) 02 00 00 01 0d a2 00 19 00 34 04 00 00 00 00 01 02 51 02 00 00 b3 9f 50 00 a3 f8 0a 00 02 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00
D:(SCSI) 03 00 00 19 00 00 ff ff ff ff 04 00 00 00 00 01 02 51 02 00 00 b3 9f 50 00 a3 f7 0a 00 02 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00
```

差分は次の3箇所のみ、**すべて構造上当然の差**:

| オフセット | 意味(推定) | C:(SASI) | D:(SCSI) | 評価 |
|---|---|---|---|---|
| +0 | ドライブ番号 | 02 | 03 | 当然(別ドライブ) |
| +2..9 | ドライバヘッダポインタ・次DPBポインタ | (内部SASIドライバのアドレス・次=D:) | (我々のドライバ$190000・次=末尾) | 当然(別ドライバ・チェーン構造) |
| +26 | メディアバイト | f8 | f7 | `createFormattedHdd()`/`createFormattedScsi()`が別々に選んだ定数の差 |

**`+10`(1024バイト/セクタ)から`+25`(推定:予約セクタ・FAT数・ルートエントリ数・
総クラスタ数関連・セクタ/FAT)までのジオメトリ全体、および`+27`以降(FAT数の
バイト・空きクラスタ数/最終割当クラスタと思われる領域を含む)は、コピー実行前は
**1バイトも違わず完全一致していた。** つまり**「Human68kが持っている管理情報に
事前の差がある」という前提は、この2つのブランクでは成立しなかった。**

### 介入: メディアバイトを揃えても症状は変わらない

唯一の実データ差である`+26`のメディアバイトを、SCSI側だけ`createFormattedScsi()`の
定数`SCSI_MEDIA_BYTE`を`0xf7`→`0xf8`(SASIと同じ値)に一時変更してブランクを
作り直し、同じ2クラスタ新規コピーを試した。**結果は変わらず失敗した。** 変更は
確認後に`0xf7`へ戻した(差分無し)。メディアバイトは原因ではない。

### コピー実行後にDPBがどう動くか(成功/失敗の結果としての差)

同じ起動のまま、まずC:(SASI)へ`copy a:\command.x c:\zzz.x`(成功)、続けて
D:(SCSI)へ`copy a:\command.x d:\zzz.x`(失敗)を実行し、それぞれ直後にDPBを
再ダンプした:

- **C:(SASI)**: `+28..29`が`00 02`→`00 1d`に変化した(2→29。クラスタ2から
  29まで、ちょうど28クラスタ確保した計算と一致する)。**「最終割当クラスタ」
  相当の値がコピー成功後にきちんと進んでいる。**
- **D:(SCSI)**: `+28..29`は`00 02`のまま**変化しなかった**(コピーは失敗して
  いるので当然、クラスタは1つも確保できていない)。

**この差は「事前の管理情報の違い」ではなく「成功したか失敗したかの結果」に
過ぎない。** コピー実行前のDPBが完全一致していた以上、**DPBの中身そのものが
原因ではない**という結論になる(仮説は否定された。ただし否定できたこと自体が
前進であり、次の担当が同じ道を辿らずに済む)。

### 次の手がかり: SASIとSCSIはそもそも別のエミュレーション方式

`x68k/sasi.c`は`log_cb`によるログが一切無く、`SASI_Read`/`SASI_Write`という
**メモリマップドI/Oレジスタ読み書き**(実機のSASIコントローラのレジスタを
バイト単位でエミュレートする、セレクト・コマンド・データ・ステータスという
フェーズ遷移を伴う本物のプロトコル)として実装されている。

一方、我々の`x68k/scsi.c`(`SCSI_HandleRequestHeader`)は、SCSI IOCS呼び出し
($F5相当)を横取りして**RAM上の26バイト要求ヘッダを直接読み書きする自前の
ショートカット**であり、実機のSASI/SCSIバスプロトコル(フェーズ遷移・
ステータスバイト・センスデータ等)を一切経由しない。**Human68k自身のFAT
割り当てロジックは同じでも、SASIは実機同様の完全なプロトコルを通り、SCSIは
我々が作った簡略版プロトコル(`$00`初期化・`$04`読み出し・`$08`書き込み以外は
すべて`err=$00`を返すだけ)を通る**という、DPBより一段深いレベルでの違いが
残っている。**「新規複数クラスタ割り当ての直前・最中にHuman68kが追加で
求めている何か(コマンド`$05`の正しい応答、センスデータ、あるいは他の
未実装コマンド)を、我々のショートカットが無言で素通りさせている」可能性が
最も高い候補として残る。**

### 直せなかった

DPBの差分は「原因ではない」ことが分かったが、代わりの原因(プロトコルレベルの
未実装コマンド)はまだ特定できていない。

### 次にやること(更新)

1. **最有力**: `x68k/sasi.c`に`x68k/scsi.c`と同等の調査用ログ(`[SASI-CMD]`等、
   コマンドバイト列と応答)を一時的に追加し、**SASI側で2クラスタ以上の新規
   コピーを行ったときに実際にどんなコマンド列が流れるか**を実測する。
   SCSI側の`$05`(常に`err=$00`だけ返している未対応コマンド)に相当する
   処理をSASI側がどう扱っているかが分かれば、次の一手が見える。
2. `x68k/scsi.c`の`$05`コマンドの要求ヘッダの中身(まだ意味を特定していない、
   前々節参照)を、今回の知見(「SASIには対応物があるはず」)を前提に読み直す。
3. メモリ監視(`webx68k_ram_watch_check`)でHuman68k側のワークエリアを追う案は
   引き続き有効(前節参照、陽性対照を先に取ること)。

## 最小の対：同一ブランクへの「1クラスタ（成功）」対「2クラスタ（失敗）」、全バイト突き合わせ（実測、2026-09-04）

これまでの比較は削除の有無・イメージの違いを挟んでいた。今回は**唯一の変数を
コピー先ファイルのサイズだけ**にした最小の対を作った: `createFormattedScsi(1)`
(1MiB・spc=1)のブランクを2枚、**同一の初期状態(同一ファイルからのバイト単位
コピー)**から、片方には`a:\sys\sramdisk.sys`(924バイト、1クラスタ)を、
もう片方には`a:\sys\prndrv.sys`(1816バイト、2クラスタ)を単一ファイルとして
コピーした(前回の`a+b+c`のような結合コピーは複数ソースファイルのオープンという
余計な変数を持ち込むため今回は使わなかった)。

### ログ上限の確認(先に済ませた)

`x68k/scsi.c`のlog_cbを経由しない独立カウンタ(`reqTotal`)と、コンソールログの
`[SCSI-REQ] 処理前`行数を突き合わせ、**両方の実行で完全一致**することを確認した
(成功例: 50=50、失敗例: 33=33)。**途中で切れていない。**

### 要求ヘッダの全バイト突き合わせ

コマンド種別の並び(`$00`/`$04`/`$05`/`$08`)は、**先頭から33件目まで完全に同じ順序**
だった:

```
00 05 05 05 05 04 05 05 05 05 04 05 04 04 05 05 05 05 04 05 05 05 04 05 05 05 05 04 04 05 08 05 04
```

内容(処理前・処理後26バイトすべて)も、**#1〜#32が1バイトも違わず完全一致**した。
これには**`$08`(ディレクトリエントリの仮書き込み、#31)も含まれる**─both実行で
`1a 00 08 00 ec a0 00 00 cc 18 00 00 f0 f7 00 01 7d b6 00 00 00 01 00 00 00 07`と
同一。**`$05`は12回すべて(#2-5,7-10,12,15-18,20-22,24-27,30,32)成功例・失敗例で
完全一致した。** 条件の違うペア(前々節)での結論を、条件差を1つに絞った
今回のペアでも追認する。**`$05`は完全に白と言い切れる。**

### 最初に違ったところ

**#33(`$04`読み出し)で初めて分岐した。ここが失敗例の最後の要求でもある**
(前後の要求は一切来ない、独立カウンタでも確認済み)。

```
成功(#33) 1a 00 04 00 00 ba 00 00 b6 92 00 00 eb f7 00 01 79 a6 00 00 00 01 00 00 00 18
失敗(#33) 1a 00 04 66 00 00 03 18 00 00 00 9d 00 f7 00 01 79 a6 00 00 00 01 00 00 00 01
```

`+14..17`(転送先アドレス)は両方とも`$0179a6`で同じ、`+18..21`(個数)も両方`1`で
同じ。**唯一違うのは`+22..25`(開始論理セクタ): 成功例は`$18`(24)、失敗例は
`$01`(1)。** この構成(reserved=1・numFats=2・sectorsPerFat=3)では論理セクタ1〜3が
FAT第1面、ルートディレクトリが7から始まり、データ領域(クラスタ2)は23から
始まる計算になる。**成功例はクラスタ3(論理セクタ24)を読みに行き、失敗例は
FAT第1セクタ(論理セクタ1)を読みに行って、そのまま二度と要求を出さずに終わる。**
前々節・前節までに得た「最後に読むのはFATセクタ1」という観測を、
条件差を1つに絞った最小の対で再確認できた。

### 解釈(まだ確定していない)

924バイト(1クラスタ)のコピーで「次のクラスタ(クラスタ3)」を読みに行く理由は、
1クラスタしか要らないはずなのに一見不要に見える。可能性として、Human68kの
割り当てアルゴリズムが「今使うクラスタの次のクラスタ」を先読みして次回の
割り当て開始位置(空きクラスタの探索カーソル)を更新している、というのが
最有力の推測だが、これはCPU側のロジックを直接見ないと確証できない
(前々節までの「見るべき場所」がそのまま該当する)。

1816バイト(2クラスタ)の場合は、クラスタ2への書き込み後、**クラスタ3を
確保する段になって**FATセクタ1を(既にFAT全域を走査済みのはずなのに)もう一度
読み、その中身だけで「壊れている」と判断して止まる。**1クラスタで済む場合は
この「もう一度FATセクタ1を読む」分岐そのものに入らない**(成功例の#33はFATでは
なくデータ領域を読んでいる)ことが、今回はっきり確認できた。

### 直せなかった

原因(なぜ2クラスタ目の確保時だけFATセクタ1の再読み出しで止まるか)はHuman68k
自身のロジックの中にあり、今回もその中身までは追えていない。受け入れ条件は
未達成のまま。

### 次にやること(最終更新)

1. **`x68k/mem_wrap.c`のワークエリア監視で、#32(`$08`処理後)から#33の分岐までの
   間にHuman68kが読み書きするRAM番地を追う。** 今回、分岐点そのもの(#33の
   開始セクタが`$18`か`$01`かを決める直前の瞬間)まで正確に絞り込めたので、
   監視すべき「窓」はごく狭い(この2つの要求の間のCPU実行区間だけ)。
   **使う前に陽性対照(既知のタイミングでの既知アドレスへの書き込みが検出
   できること)を必ず確認すること**(PC絞り込みの既定値が全件を無言で
   捨てる不具合が過去にあった)。
2. `x68k/sasi.c`への調査用ログ追加(前節)と組み合わせ、SASI側が同じ場面
   (2クラスタ目の確保)で何を読みに行くかを実測できれば、Human68kの
   ロジックそのものを推測ではなく実測で埋められる。

## 分岐点の2要求間を追った：Human68kは「更新せず諦めている」（実測、2026-09-04、本セッション区切り）

前節で確定した最小の対（同一ブランクへ1クラスタ成功／2クラスタ失敗）の、
**要求#32→#33の間で、Human68kがFATバッファ(要求#33の転送先=`$0179a6`)を
CPU側から更新しているか**を実測した。

### 陽性対照(本番と同じ経路)

`webx68k_ram_watch_selftest()`(自己検査専用関数、PC絞り込みを迂回する)は使わず、
**要求#33自体がこのバッファへ行う書き込み**(我々のドライバが`cpu_writemem24()`
経由でFAT/データの内容を配達する、`wm_cnt`を通る本番と全く同じ経路)を
陽性対照として使った。成功例・失敗例とも、`$0179a6`〜`$0179b5`(16バイト、
`--ram-watch=0x179a6:0x179b5`、PC絞り込みは使わず無効のまま)で**期待どおり
16件ずつ検出できた**(成功例: `pc=$00190032`＝我々のドライバの配達コード、
中身はクラスタ3の内容=全ゼロ。失敗例: 同じPCで中身はFATセクタ1の先頭16バイト
`f7 ff ff ff 00 00 00 00 …`)。**フックは本番と同じ設定で確実に検出できている。**
(独立カウンタ`reqTotal`と`[SCSI-REQ]処理前`の件数も両方一致(50=50、33=33)を
確認済みで、ログも切れていない。)

### 結果: 更新していない

- **成功例**: 要求#33(クラスタ3を読む、内容は全ゼロ=空き)の直後、**要求#34
  ($05)の処理中に、我々のドライバとは無関係なPC(`$0000c7e0`、Human68k自身の
  コード)から同じバッファへの書き込みが実際に観測できた**(`48 55 00 00 … 03 1a`
  へ変化)。Human68kはこのバッファを次の作業に使い回している。
- **失敗例**: 要求#33(FATセクタ1を読む、内容は`f7 ff ff ff 00 00 00 00 …`
  ＝クラスタ2・3ともに空き)の処理が終わった**直後、ログが完全に停止する。
  このバッファへの追加の書き込みは1件も無く、新しいSCSI要求も1件も来ない**
  (独立カウンタでも確認済み、前節参照)。

**Human68kは、FATセクタ1を読んで(クラスタ2・3が空いていることを確認できる
はずの内容を受け取って)から、何も書き込まずに、その場で処理を止めている。**
コーディネータの立てた二択のうち、**「確保する前に諦めている」の方だった。**
書き戻し要求が消えているのではなく、**そもそも書き戻そうとした形跡が無い。**

### 測れなかったこと(正直に)

- 監視した範囲はこの16バイト(`$0179a6`〜`$0179b5`)だけである。Human68kが
  **この範囲外の別のワークエリア**(例えばDPBの空きクラスタ欄や、エラー
  フラグ、ダイアログ表示のトリガとなるフラグ等)に何か書き込んでから
  止まっている可能性は、今回の監視範囲では捉えられない。「何も書いていない」
  と言えるのはこの16バイトについてだけである
- 「何を見て諦めたか」(読んだ16バイトの、具体的にどの値・どの判定が
  諦めの引き金になったか)はまだ特定できていない。クラスタ2・3ともに
  `$0000`(空き)であり、素朴には「空きが無いから諦める」という説明とは
  逆の内容なので、**単純な「空き不足」ではない何か**を見ている可能性が高い

## これまでに潰した説（一覧、次の担当への申し送り）

このセッション（および前セッション群）で、以下はすべて**実測で否定済み**。
同じ道を再度歩かないための一覧:

| 説 | 結果 | 根拠 |
|---|---|---|
| イメージのバイト列が壊れている | 否定 | WRITE-VERIFY(書いた直後の読み返し)で常に一致 |
| ブートセクタの`55 AA`シグネチャ | 否定 | 本節の対象操作と無関係(区画自体は正常に見えている) |
| `$08`(書き込みコマンド)の実装 | 否定 | 成功例・失敗例で1バイトも違わず完全一致(本セッション) |
| クラスタ2固有の問題 | 否定 | 空のブランクでもクラスタ2は空きのまま読めており、単体のクラスタ値操作では再現しない |
| `$05`(未対応コマンド)の内容 | 否定 | 条件を1つに絞った最小の対で12回すべて完全一致(本セッション) |
| CPUが完全停止(ハング)している | 否定 | 実際はダイアログのキー入力待ち。要求ログはlog_cb非依存カウンタでも0で確定 |
| 「空の区画」であること自体 | 否定 | 空のブランクでも1クラスタなら成功する |
| FATが全ゼロであること | 否定 | 全ゼロでも1クラスタなら成功する。今回、FATセクタ1が空き(≒ゼロ多め)でも中身自体は問題にならない例を確認 |
| ファイルサイズ(バイト数)そのもの | 否定 | 同じ28KBファイルでもクラスタが1個で足りる大きいクラスタでは成功する |
| FATが複数セクタにまたがること | 否定 | 全域が1セクタに収まる極小ブランクでも2クラスタなら失敗する |
| DPB(ドライブ管理情報)の差 | 否定 | 操作前のDPBはSASI/SCSIで(メディアバイト以外)完全一致。メディアバイトを揃える介入も無効 |
| 削除直後の同名再作成がトリガー | 否定 | 削除を経ない新規名でも失敗し、削除後でも1クラスタなら成功する |
| SCSIバッファへの書き戻し要求が消えている | 否定(本セッション) | 書き戻そうとした形跡自体が無い(要求が来る前に諦めている) |

**確定している規則**: 新規に2クラスタ以上のFATチェーンを割り当てる書き込みだけが
失敗し、単一クラスタの新規作成・既存チェーンの上書きは失敗しない。SASI
(実機同様のレジスタ/フェーズ遷移プロトコル)では同じ操作が成功するが、
操作前のDPBはSCSIと同一であるため、DPBの中身ではなくプロトコルレベルの
何か(あるいはHuman68k内部の一時的な計算)が原因と考えられる。

## 次にやること(このセッションの最終申し送り)

1. **最優先**: 要求#33の処理中(FATセクタ1の内容がCPUへ渡った直後)から、
   Human68kが完全に沈黙するまでの間に、**$0179a6-b5以外のどこかへ**
   書き込み・読み出しが無いかを、監視範囲を広げて(例えば`$000000:$00ffff`
   の低位ワークエリア全域、または的を絞るなら`m68000_get_reg(M68K_A5)`/
   `A6`等のレジスタが指す構造体まわり)確認する。今回は「このバッファは
   更新されない」というピンポイントの答えは出せたが、**Human68kがどこかで
   諦めの判断をしている痕跡そのもの**はまだ見えていない
2. `x68k/sasi.c`への調査用ログ追加(前節)を実行し、SASIが同じ場面
   (2クラスタ目の確保)で実際に何を読み書きするかを実測する。今回SCSI側で
   「クラスタ2・3は空きだが何も書かずに諦める」ことが分かったので、
   SASI側で同じ内容を読んだときに何が違う判断をしているか(あるいは
   同じ内容でも先に進む何らかの追加ステップがあるか)を比べれば、
   Human68kの分岐条件そのものに迫れる
3. 監視対象を広げる際は、**今回のように必ず生きた書き込み(自己検査の
   バイパスではなく、本番経路で実際に起きるとわかっている書き込み)を
   陽性対照に使うこと**(本節で有効だった手法)。

## 本日の計測を2回まるごと無効にした条件の欠落（2026-09-04、要注意）

**先に結論**: このセッションの前半に行った2回の計測（SASIとの要求列突き合わせ／その後の対照確認）は、
**実験条件の欠落によりどちらも成立していなかった**。結論として書きかけた
「分岐点はディレクトリ書き込みへの応答の直後」は**取り下げる**。

### 何が欠けていたか

両方の実行が `--worker=0 --scsi-opfs` で行われていた。しかし:

- `--scsi-opfs`（`src/scsi-opfs.ts`）は **Worker専用**である。`createSyncAccessHandle()` が
  Worker専用APIで、ファイル冒頭のコメントにも「このモジュールは Worker 内からだけ呼ばれる想定」と
  明記されている
- `--scsi-ram-writes` は逆に **メインスレッド専用**である（ページの `globalThis` へ関数を差すので
  Worker へは構造化クローンで渡らない）
- よって **`--worker=0 --scsi-opfs` は、SCSIの書き込み経路がどちらも生きていない唯一の組み合わせ**

実際、`_local/scsi/probes/run3-scsi-fail.json` と `baseline1-1cluster.json` のログには
`[SCSI-REQ] 書き込みコマンド($08) を断った` が1件あり、`[SCSI-WRITE]`（成功）行と
WRITE-VERIFY行は**0件**だった。**Human68kは「書き込みを断られた」から止まっていた**のであって、
調査対象の欠陥ではない。

Worker経路がその日ハングしていたため `--worker=0` へ退避したのは妥当だったが、
**退避先では書き込み能力そのものが落ちる**ことに気づけなかった。

### なぜ気づけなかったか（同じ失敗を繰り返さないために）

**「書き込みが断られたときの症状」と「調査対象の欠陥の症状」が見分けられなかった。**
どちらも「ダイアログが出て要求が尽きる」という同じ顔をする。陽性対照として置いた
「SCSI側で失敗が再現すること」は、**失敗している理由を問わないので通ってしまった**。

さらに、比較相手のSASIは書き込みが生きている側だったため、実際には
**「書けるドライブ」対「書けないドライブ」を比べていた**。

### 入れた防御（`scripts/probe-scsi-iocs.mjs`、コミット `ff27bf4`）

- `--scsi-opfs` と `--worker=0` の同時指定、および `--worker=0` 無しの `--scsi-ram-writes` を
  **起動時にエラーで拒否**する。エラーメッセージには**代替経路**（`--worker=0` のときは
  `--scsi-ram-writes` を使う）を書いてある
- 実行結果に `書き込みコマンド($08) を断った` が1件でも含まれていたら、出力JSONに
  `writeRefused: true` を立て、標準エラーへ枠付きで「この実行は書き込み経路が死んでいるため無効」と出す
- `[SCSI-WRITE]`（成功）の件数を `writeSucceededCount` としてJSONに必ず載せる

### 記録の穴（次の担当への申し送り）

最小の対（1クラスタ成功／2クラスタ失敗）を最初に測ったときの**起動コマンドラインが、
このドキュメントにも `_local` のJSONにも残っていなかった**ため、条件を復元できず遠回りした。
**以後、実測を記録する節には必ず invocation を丸ごと貼ること。**

## 対照を再建して分かったこと：「コピーしました」は書き込みの証明ではない（実測、2026-09-04）

`--worker=0 --scsi-ram-writes` で最小の対を測り直した（ブランクは実行ごとに新規作成）。

```
node scripts/probe-scsi-iocs.mjs --worker=0 --scsi-ram-writes \
  --image=_local/scsi/probes/ctrl-a-blank.hds \
  --type='dir c:;;copy a:\sys\sramdisk.sys c:\sramdisk.sys;;dir c:' \
  --type-wait=8000 --poll-scsi-debug=8000 --timeout=90000
```

`createFormattedScsi(1)`（1MiB・spc=1）へ `a:\sys\sramdisk.sys`（924バイト＝1クラスタ）:

- 画面には **「1 個のファイルをコピーしました」** が出る（`writeRefused=false`、書き込み経路は生きている）
- **しかしディスクへ落ちるのはルートディレクトリ（論理セクタ7）への書き込み2回だけ**。
  1回目がサイズ0の仮書き込み、2回目でサイズ924に確定。**ファイル本体のクラスタもFATも一度も書かれない**
- 要求総数50は、以前「成功例」として記録した回と一致する。**以前の「成功」も中身は同じだった**

**受け入れ条件として使っていた「画面に『コピーしました』が出ること」は、書き込みが
ディスクへ落ちたことの証明になっていなかった。**以後、判定は `[SCSI-WRITE]` のログ
（データ領域のセクタとFATセクタへの書き戻しが現れること）で行う。

### SASI（成功する側）との突き合わせ

同じ操作をSASIブランク（`createFormattedHdd()`、40MB）で行うと:

| 場面 | SASI | SCSI |
|---|---|---|
| ディレクトリエントリ仮書き込み | 書く | 書く |
| **ファイル本体のクラスタ** | **書く** | **書かない** |
| ディレクトリエントリ確定 | 書く | 書く |
| **FATチェーン** | **書く** | **書かない** |

SASI側のLBAは区画構造と完全に整合する（区画開始LBA8、FAT論理セクタ1=LBA12、
ルート先頭163=LBA660、データ先頭179=LBA724）。2クラスタの場合は
dir(660-663) → 本体(724-727) → dir(660-663) → 本体(728-731) → FAT(12-15) の順に書かれ、
**SASIはクラスタ2から素直に確保している**。

### ドライバ呼び出しは途中で落ちてはいない

`[SCSI] ストラテジ: 要求ヘッダのアドレスを控えた` の件数と、実際に処理した要求の件数は
**完全に一致する**（50=50 / 78=78 / 47=47）。**Human68kは本体書き込みの要求を用意すらしていない。**
ドライバ呼び出しの途中で落ちているのではなく、Human68k側が発行しないことを選んでいる。

### 既存チェーンの上書きなら本体は書かれる

過去の実測（`twopart.HDS` の `COMMAND.X` 上書き）では `[SCSI-WRITE] 論理セクタ=291 個数=27` と
**本体27セクタぶんが書かれている**。書き込み経路そのものは機能する。
**新規にクラスタを確保する場合だけ、本体とFATが書かれない。**

## 欠陥1：小さいSCSIブランクをFAT16で作っていた（実測・修正済み、2026-09-04）

### 見つけ方

1MiBブランクへの新規コピーで、Human68kが書いたディレクトリエントリの**先頭クラスタが3**だった
（空のFATなのにクラスタ2を飛ばしている）。我々のFAT先頭 `f7 ff ff ff 00 00 …` を
**FAT12として読む**と、これがそのまま説明できる:

| エントリ | FAT12での読み方 | 値 |
|---|---|---|
| entry0 | バイト0,1 | `$FF7`（メディアIDとして正当） |
| entry1 | バイト1,2 | `$FFF`（EOCとして正当） |
| **entry2** | **バイト3,4** | **`$0FF`（空きではない → 飛ばす）** |
| entry3 | バイト4,5 | `$000`（空き → ここを確保） |

### ホスト側だけで取れた裏

`src/api/fat.ts` の読み取り側 `openFat()` は**総クラスタ数4085未満をFAT12と判定**する
（`readFatEntry()` も12bit対応済み）。一方 `createFormattedScsi()` は**サイズによらず常にFAT16**で
作っていた。つまり**自作のフォーマッタと自作のリーダが食い違うイメージ**を出していた
（修正前の実測値）:

| サイズ | `openFat()`の判定 | 総クラスタ | FAT12として読んだentry2 |
|---|---|---|---|
| 1MiB | FAT12 | 1001 | **`$0FF`** |
| 2MiB | FAT12 | 2021 | `$0FF` |
| 4MiB | FAT12 | 4061 | `$0FF` |
| 5MiB | FAT16 | 5081 | （FAT16では `$0000`＝空き） |

### 実機での確認

区画サイズだけを振って8MiB（総クラスタ8192＝FAT16域）で同じコピーを行うと、
**先頭クラスタが2になった**（1MiBでは3）。FAT12誤読の説明と一致する。
SASIブランクが無事だったのは `createFormattedHdd()` が**40MB固定**（約4万クラスタ＝常にFAT16）
だったからで、偶然ではない。

### 直した（コミット `26567a6`）

- FAT12/FAT16の判定を `pickFatType(totalClusters)` に**1箇所へ集約**し、`openFat()` と
  `computeScsiFatLayout()` が同じ規則を共有するようにした
- 型の判定には `openFat()` と**同じ厳密なクラスタ数**（reserved/FAT/root分を差し引いた数）を使う。
  従来の `floor(総セクタ数 / spc)` という概算のままだと境界付近で「作った型」と「読まれる型」が
  再びずれる
- 型 → `sectorsPerFat` → データ開始セクタ → クラスタ数 → 型、という循環は不動点反復で収束させた
- FAT12のときの予約エントリは `f7 ff ff`、`sectorsPerFat` は1.5バイト/エントリで算出
- **FAT16側の `sectorsPerFat` は修正前と同じ値のまま**（5/6/8/64MiB で 11/13/17/65）。
  実機イメージ `twopart.HDS` と突き合わせた既存の実測を壊していない
- 回帰テストは**修正前に4サイズ（1/2/3/4MiB）で落ちる**ことを確認してから入れた。全725テスト通過

修正後の検算:

```
   1MiB  FAT12  1003クラスタ  spf=2   f7 ff ff …      entry2=$0 entry3=$0
   4MiB  FAT12  4067クラスタ  spf=6        〃              〃
   5MiB  FAT16  5081クラスタ  spf=11  f7 ff ff ff…        〃
2047MiB  FAT16 65495クラスタ  spf=128      〃              〃
```

### ただしこれは主症状ではない

8MiB（FAT16域）でも、**本体とFATが書かれない主症状は不変**だった。
FAT12/16の取り違えは「どのクラスタを確保するか」だけを説明する別の欠陥である。

## 欠陥2：ファイルの中身はバッファに載っているのに書き戻されない（実測、2026-09-04）

`x68k/mem_wrap.c` のゲストRAM監視で、データクラスタ用バッファ（`$0179a6`、
Human68kがデータ領域のセクタを読み込んだ先）を監視した。

- **陽性対照**: 我々のドライバ自身がそのバッファへ配達する書き込み（`pc=$00190032`）を
  161件検出。フックは本番と同じ経路で確実に働いている
- **結果**: **Human68k自身のコード（`pc=$0000c7e0`）が、そのバッファへ
  `a:\sys\sramdisk.sys` の先頭32バイトを1バイト差なく書き込んでいる。**
  にもかかわらず、同じ実行の `[SCSI-WRITE]` にはこのデータクラスタへの書き込みが1件も現れない
- **SASIオラクル**: 同じ手順で `lba=724`（データクラスタ）が同じ中身で実際に書かれ、
  続いて `lba=12`（FAT）も書かれる

**よって原因は「データを作る側」ではなく「書き戻す側」（フラッシュ／ダーティ管理）にある。**

### 次の手がかり：バッファは16バイトのヘッダ＋1024バイトの本体

観測されたバッファ番地 `$17596 / $179a6 / $17db6 / $181c6 / $185d6` はすべて
**間隔 `$410`（=1040=16+1024）**で並ぶ。番地 X のバッファのヘッダは `X-16` にあるとみられる。

そして**同じ1回の実行の中に対照が同居している**:

- **書き戻される**バッファ: ルートディレクトリ用
- **書き戻されない**バッファ: データクラスタ用

同一実行・同一ドライブ・同一ドライバなので、**この2つのヘッダの差が、書き戻される／されないを
分けている条件**である可能性が高い。ここが次に見る場所である。

## これまでに潰した説（追記）

| 説 | 結果 | 根拠 |
|---|---|---|
| `$05` はディスク交換チェックで、返答欄を埋めれば直る | 否定 | `+13`/`+14` へ「交換されていない」「交換された」のどちらを書いても書き込みは1件も増えない。介入が届いていることは `[SCSI-CMD-ANSWER]` ログで確認済み（2026-09-04） |
| 分岐点はディレクトリ書き込みへの応答の直後 | 取り下げ | その実行は書き込み経路が死んでおり比較として成立していなかった（本ドキュメント冒頭の節） |
| 書き込み経路そのものが壊れている | 否定 | 既存チェーンの上書きでは本体27セクタが書かれる。WRITE-VERIFYもバイト単位で一致 |
| Human68kがデータを作れていない | 否定 | バッファにファイルの中身が1バイト差なく載っていることを実測（欠陥2の節） |
| ドライバ呼び出しの途中で落ちている | 否定 | ストラテジ呼び出し数と要求処理数が完全一致（50=50 / 78=78 / 47=47） |
| 区画サイズ・クラスタ数が主因 | 否定 | 8MiB（FAT16域）でも主症状は不変。FAT12誤読は確保クラスタ番号だけを説明する別の欠陥 |

### バッファヘッダの対照比較は「差が無い」ではなく「区別できない」だった（実測、2026-09-04）

同一実行内の「書き戻されるバッファ（ルートディレクトリ用 `$17db6`）」と
「書き戻されないバッファ（データクラスタ用 `$179a6`）」について、それぞれのヘッダ
（バッファ番地-16 から16バイト）への書き込みを監視して突き合わせた。

- 陽性対照は通っている（A=157件・B=153件、オフセット0〜15すべてに書き込みを検出）
- 「ヘッダは番地-16、16バイト」という前提は構造的に成立した（書き込みが正確にその16バイトへ収まる）
- **書き込むPCの集合は17個中16個まで一致し、「書き戻される側にだけある書き込み」は見つからなかった**

ただし**この否定は弱い**。両ヘッダとも同じ汎用のバッファ記述子ステートマシン
（割当 → `eb2c`/`eb30`/`eb40`/`eb46`/`eb3e` の周回 → `$ff` で解放）を通っており、
**実行中に何度も別セクタ（FAT／ルート／データを問わず）へ使い回されている**。
つまり固定番地を見張っても「その瞬間そこに載っている種別」を追えていない。
**「差が無い」ではなく「この見方では区別できない」**と読むべきである。

次に見るべきは、**使い回されない状態**、すなわちDPB（ドライブごとに固定）である。

### DPBを突き合わせた：確保そのものはSASIと同じに行われている（実測、2026-09-04）

バッファと違いDPBは**ドライブごとに固定で使い回されない**ので、固定番地の監視が有効に働く。

- SCSI側のDPB番地は `$190034`（`[SCSI-DRV] … 終了アドレス=$00190034` と一致。
  さらに `+10..11=$0400`(1024B/セクタ)・`+16=$02`(FAT数)・`+17=$02`(sectorsPerFat) が
  `createFormattedScsi(1)` の実測ジオメトリと一致）
- SASI側のDPB番地は `$0148d6`（DPB連鎖 `$1c3c/$1c3e→$014866→$01489e→$0148d6` を辿って特定。
  `+10..11=$0400`・`+14..15=$0001`・`+16=$02`・`+17=$51`(=81) が `createFormattedHdd()` の
  ジオメトリと**複数フィールドで一致**）
- 陽性対照: SCSI実行で351件・SASI実行で278件の書き込みを検出（上限2000件には未到達）

**結果: クラスタの確保そのものは両者で同じように行われている。** どちらも
DPB `+$1c` へ `$0002`（クラスタ2を確保）を書く（SCSI `pc=$eed0` / SASI `pc=$ef5a`、値は同一）。
差が出るのはその後で、SASIは実ディスクへの書き込み（dir→本体→FAT）へ進むが、SCSIは進まない。

#### 読み違いの訂正: `pc=$b786/$b78a` は巻き戻しではない

当初、確保の直後に現れる `pc=$b786/$b78a` によるDPB近傍（`+$1e..21` と `+$2a..2d`）の
ゼロ書きを「確保の巻き戻し＝中止経路」と読んだが、**誤りだった**。生ログを時系列で読み直すと、
この書き込みは **`$05` 要求の直後に毎回起きる定型処理**であり、正常に動作する `dir` の最中にも
数十回現れる。中止の印ではない。

（教訓: 狭い窓だけを見ると、定型処理が「その場面に固有の異常」に見える。
**同じ書き込みが正常時にも起きていないかを、必ず実行全体で数えること。**）

### まだ一度も試していない軸が残っていた

`SCSI_HandleRequestHeader()` の**成功経路は要求ヘッダ `+3` に `$00` を1バイト書くだけ**で、
`+4` 以降は前の値（不定値）のまま、`d0` は `SCSIReqReplyD0 = -1` のまま＝呼び出し時の値を放置している。
MS-DOS系の要求ヘッダでは `+3..+4` がステータスワード（bit15=エラー / bit8=完了 / 下位8bit=エラーコード）
とされ、**「完了」ビットを立てていない**可能性がある。

この検証は2026-09-04の途中で計画されたが、**当時ベースラインが壊れていて実施されないまま流れていた**。
陽性対照には `twopart.HDS` への**既存チェーンの上書きコピー**が使える
（SCSI経路で本体27セクタが実際に書かれる、数少ない本物の成功例）。

### 応答の中身は原因ではなかった：`+3` は読まれ、`+4` と `d0` は読まれていない（実測、2026-09-04）

陽性対照に **`twopart.HDS` への既存チェーン上書きコピー**（`copy a:\command.x c:\`）を使った。
これはSCSI経路で**本体27セクタが実際に書かれる、数少ない本物の成功例**である
（`論理セクタ=291 個数=27`、`shasum` は実行前後で不変＝元イメージは汚していない）。

これを土台に1軸ずつ故障注入した:

| 注入 | 結果 | 読み取れること |
|---|---|---|
| `+3` に `$800a` | **壊れた**（「ドライブ名が無効です」、書き込み0件） | `+3` は読まれている |
| `+4` に `$800a` | 壊れなかった（24件注入しても正常完了） | `+4` は読まれていない |
| `d0` に `$800a` | 壊れなかった | `d0` は読まれていない |

さらに `+3` へ `$0100`（MS-DOS流の「完了ビット」のつもり）を入れるとマウントごと壊れた。
**`+3` はワードの上位バイトではなく1バイトの結果コードで、非ゼロなら即エラー**である。
すなわち**現状の「`+3` に `$00` を書く」は正しく、応答の中身は本件の原因ではない。**

（この軸は2026-09-04の途中で一度計画されながら、ベースラインが壊れていて実施されないまま
流れていたもの。**「計画したが実施していない」ものが宙に浮くと、後から
「もう試した」と誤って扱われかねない。実施の有無は必ず記録に残すこと。**）

### 次に割るべき二択

成功例（`twopart.HDS` の上書き）と失敗例（自作ブランクへの新規確保）は、**2点が同時に違う**:

1. **既存チェーンの上書き** か **新規確保** か
2. **実機フォーマット済みイメージ** か **`createFormattedScsi()` の自作イメージ** か

**自作イメージにホスト側であらかじめファイルを書いておき、それを上書きコピーする**という
1変数だけの対を作れば割れる。書ければ「新規確保」が効いており、書けなければ
「イメージの作り」が効いている。

### 二択が割れた：効いているのは「新規確保」で、イメージの作りではない（実測、2026-09-04）

`createFormattedScsi(8)` の8MiBブランクに、**ホスト側で** `fatWriteFile()` を使って
`USKCG.SYS`(8028バイト＝8クラスタ)を先に書き込んだイメージを用意した。
ゲスト起動後の `dir c:` にそのファイルが**正しい名前とサイズで見える**ことを先に確認している
（ホスト側の書き込みがHuman68kから読めていることの前提確認）。

同一イメージ種別・同一打鍵・同一待ち時間で、**変えたのは「上書きか新規確保か」の1点だけ**:

| 条件 | `[SCSI-WRITE]` のデータ領域書き込み |
|---|---|
| 既存チェーンの**上書き**（同名・同サイズを`copy`） | **あり**（論理セクタ51・7セクタ） |
| 同じブランクへの**新規確保** | **なし**（ルートのセクタ1回のみ） |

**自作イメージでも上書きなら書ける。** よって「実機フォーマット済みか自作か」は原因ではなく、
**効いているのは「新規にクラスタを確保すること」のほう**だと確定した。

（この対は、以前の `twopart.HDS`（実機品・上書き）対 自作ブランク（新規確保）という
**2点が同時に違う比較**を、1点だけに絞り直したもの。成功例と失敗例が複数の点で違うときは、
どちらが効いているか分からない。）

## 現在地と、次にやること（2026-09-04 セッション末）

### 確定していること

- **書き込み経路そのものは機能する**（上書きなら本体もWRITE-VERIFYも通る）
- **データはバッファに正しく載っている**（Human68k自身のコードが1バイト差なく書き込む）
- **クラスタ確保そのものは正常**（DPB `+$1c` へ確保クラスタを書く。SASIと同一）
- **ドライバ呼び出しの取りこぼしは無い**（ストラテジ数＝処理数）
- **我々の応答は正しい**（`+3` は読まれており `$00` でよい。`+4`・`d0` は読まれていない）
- **失敗の条件は「新規確保」のみ**（イメージの作り・区画サイズ・クラスタ数は無関係）

つまり **Human68kは、新規に確保したクラスタのバッファだけを、書き戻さずに捨てている。**

### 次にやること（優先順）

1. **最優先**: バッファ記述子を、**同一イメージ・同一ドライブでの1変数の対**で比べる。
   本節で「上書きならデータ領域が書かれる」対が手に入ったので、
   **「書き戻されるデータバッファ（上書き時）」対「書き戻されないデータバッファ（新規確保時）」**
   という、どちらも同じ役割のバッファ同士の比較ができるようになった。
   （以前のルートディレクトリ用との比較は役割が違ううえ、バッファの使い回しで種別を追えなかった。
   今回の対なら「同じ役割・同じ場面・確保の有無だけが違う」）
2. 書き込み要求が**別のドライブへ逸れていないか**の裏取り（コピー元のフロッピーA:へ
   書かれていないか）。`_get_fdd_dirty_mask` はコアにあるが `--worker=0` の
   `window.__webx68kDebug` に露出していないため、小さな計装が要る。
   「要求が来ない」を「どこにも行っていない」と言い切るには、この裏取りが要る。
3. 上の2つで決まらなければ、確保直後にHuman68kが読む**ワークエリアの読み出し**を追う
   （書き込みではなく読み出しの監視。`--mem-read-watch` が既にある）。

### 訂正の材料：「成功した上書き」も端数セクタを書いていなかった（実測、2026-09-04）

前節で「効いているのは新規確保のほう」と結論したが、**その対は実は1変数になっていなかった。**
書き込みログを数え直すと、**成功したはずの上書き2件は、どちらも端数セクタを書いていない**:

| 例 | ファイルサイズ | 書かれたデータセクタ | 端数 |
|---|---|---|---|
| 自作8MiBへ `USKCG.SYS` 上書き | 8028B（7セクタ＋860B） | 論理51から**7セクタ** | **書かれない** |
| `twopart.HDS` へ `COMMAND.X` 上書き | 28382B（27セクタ＋734B） | 論理291から**27セクタ** | **書かれない** |

つまり通っているのは**全セクタの直接転送だけ**で、**バッファ経由で遅延される書き込み
（端数セクタ・FAT）は、上書きの場合ですら一度も書き戻されていない**。
ディレクトリのセクタだけは別経路（即時書き）で書かれている。

さらに、対にした「新規確保・8028B」の実行は**途中で停止しており完了メッセージも出ていない**。
**データ転送に到達する前に終わっている**ので、その実行から「新規確保だとデータが書けない」とは言えない。

よって二択はまだ割れていない:

- **説A**: 新規にクラスタを確保すること自体が書き込みを止めている
- **説B**: **遅延される書き戻し（バッファのフラッシュ）が一度も起きない**のが本体で、
  新規確保はFAT更新（＝遅延書き戻し）を必要とするから詰まるだけ

**ファイルサイズをセクタ(1024B)の整数倍にすれば割れる**（端数が無ければバッファ経由の書き戻しが
要らないので、新規確保でもデータが書けるかどうかを単独で見られる）。

（教訓: **「成功した」と記録した例も、何が書かれ何が書かれなかったかまで数えること。**
完了メッセージと「全部書けた」は別。今回は成功例のほうに欠落が隠れていた。）

## 説Aと説Bは二択ではなかった：遅延書き戻しが届かない、という1つの根（実測、2026-09-04）

コピー元もSCSIドライブ側に置き（フロッピーを経路から外し）、`createFormattedScsi(8)` の
8MiBイメージにホスト側で中身を用意して、サイズだけを設計した3つの対を測った。
各実行とも先に `dir c:` で置いたファイルが正しい名前とサイズで見えることを確認している。

| 実行 | 条件 | `[SCSI-WRITE]` |
|---|---|---|
| 1 | **新規確保**・2048B（セクタ整数倍＝端数なし） | ルート`35`のみ。**要求が完全に停止**（30秒ポーリングで `reqTotal`/`writeCount` 不変）。完了メッセージも出ない |
| 2 | **上書き**・2048B（端数なし） | ルート`35`×2、データ`53`×2セクタ（中身も一致）。**FAT(`1`)は0回** |
| 3 | **上書き**・1500B（端数あり） | ルート`35`×2、データ`53`×**1セクタのみ**。**端数476Bの2セクタ目は書かれない**。FATも0回 |

```
node scripts/probe-scsi-iocs.mjs --worker=0 --scsi-ram-writes \
  --image=_local/scsi/probes/split-run1-new.hds \
  --type="dir c:;;copy c:\src.dat c:\dst.dat;;dir c:" \
  --type-wait=3000 --poll-scsi-debug=30000 --timeout=140000
```
（実行2は `split-run2-overwrite.hds`、実行3は `split-run3-fraction.hds` と `src2/dst2` に差し替え）

### 読み方

説A（新規確保が止めている）と説B（遅延書き戻しが起きない）は**二択ではなく、同じ根の2つの顔**だった:

> **バッファ経由の遅延書き戻しが、我々のドライブに対して一度も届かない。**
> - 遅延書き戻しが**要らない**操作（セクタに整列した全量の直接転送）は通る（実行2）
> - 遅延書き戻しが**要る**操作（端数セクタ）は**黙って落ちる**（実行3）
> - 遅延書き戻しが**必須**な操作（新規確保＝FAT更新）は、**そこで止まる**（実行1）

### 実用上の重さ（見落とせない）

**実行3は「1 個のファイルをコピーしました」と表示されたうえで、末尾476バイトがディスクに落ちていない。**
つまり現状のSCSI書き込みは、**成功表示のまま静かにデータを壊す**。
新規作成が失敗する件だけの問題ではないので、UIへ配線する前に必ず解くこと。

### 次に割ること

「遅延しているだけ（契機が来ていない）」のか「永久に来ない（そもそも汚れたと見なされていない）」のか。
バッファを大量に使い回させて（churn）、端数セクタの書き戻しが後からでも現れるかを見る。
SASI側で同じ端数ありの上書きを行い、**いつ端数が書かれるか**を見本として取る。

### 「遅延している」ではなく「永久に来ない」（実測、2026-09-04）

端数ありの上書き（`SRC2.DAT` 1500B → `DST2.DAT` 1500B）を行ったあと、
**ディスクバッファを大量に使い回させて（churn）**、端数セクタの書き戻しが後から現れるかを見た。

- churn が実在したことの根拠: `[SCSI-READ]` の論理セクタが34種（1〜16の起動系、35のディレクトリ53回、
  51〜68のデータ全域）に広がり計243件。置いた14ファイルすべてに `type` が届き内容が読まれている
- **SCSI: 端数セクタ（論理54）への書き込みは、churn を通しても一度も現れなかった。**
  論理54は書き込み前に1回読まれたきり、以後は読みも書きも無い
- **SASI（オラクル）: 同じ操作で、`copy` 直後・churn より前に端数が書かれた**（`lba=736..739`）

**よって「契機が来ていないだけ」ではなく「永久に来ない」。** SASIでは即時に書かれるので、
Human68kが遅延させているのではなく、我々のドライブに対してだけ書き戻しが発行されない。

### 属性ワードは原因ではない（実測、2026-09-04）

比較相手であるHuman68k内蔵のHARDDSKドライバのヘッダ（`$00010da2`）を実測した:

```
次ヘッダ=$0002c164  属性ワード=$0000  ストラテジ=$00010df0
インタラプト=$00010df8  +14=$10  名前="HARDDSK"
```

（名前が "HARDDSK" と読める文字列で出ていることが、番地が正しいことの根拠。）

我々のドライバの属性 `$4000` との差は bit14 の1ビットだけだったので、`$4000`（対照）と
`$0000`（内蔵ドライバの実測値）の2条件で足りた。**どちらでもマウントは成功し、
どちらでも端数セクタ（論理54）への書き込みは現れなかった。属性は原因ではない。**

### 絞られた規則

> **ディレクトリのセクタ書き込みと、セクタに整列した全量の直接転送だけが届く。
> データバッファとFATバッファの書き戻しは一度も届かない。**

ディレクトリも「セクタの一部を書き換えて書き戻す」操作なので、
**「端数だから」ではなく「どのバッファか」で分かれている。**
次はバッファ記述子（バッファ番地-16 の16バイト）の**値**を、
書き戻される2つ（論理35・53）と書き戻されない1つ（論理54）で突き合わせる。
（前回の記述子比較は「どのPCが書いたか」しか見ておらず、値を比べていなかった。）

### コマンド `$05` の結果は見られていた（実測、2026-09-04。過去の「白」を訂正）

`$05` は引数を持たず、1回のコピーで数十回届く。我々は中身を見ずに要求ヘッダ `+3` へ
`$00`（成功）だけを返している。ここに**エラー（`$0a`）を返したところ**:

- `$08`（書き込み）の発行が **3件 → 0件**（`[SCSI-WRITE]` が1行も出ない）
- 画面は `copy` の途中で停止し、正常時のように完了しない
- `$04`（読み出し）は37回発行されており、コマンド発行そのものは続いている

**つまり `$05` の結果はHuman68kに見られており、書き込み経路の関門になっている。**

これは本ドキュメントの前の節にある「`$05` は完全に白と言い切れる」という記述の**訂正**である。
当時測ったのは「**返答欄として `+13` と `+14` の2箇所**に値を書いても書き込みが増えない」という
限定的な事実であって、`$05` の結果そのものが見られているかは測っていなかった。

（教訓: **「Xは無関係」と書くときは、Xの何を測ったのかまで書くこと。**
「`$05` は白」とだけ残っていたため、以後の検討から `$05` が丸ごと外れてしまっていた。）

### バッファ記述子の値に差があった（実測、2026-09-04）

要求のたびに転送先アドレスの直前16バイト（＝そのバッファの記述子）を、処理の前後で
ログに出すようにして突き合わせた（`px68k-libretro` `d181f1a`）。

| 種別 | 論理セクタ | byte0-3 |
|---|---|---|
| 書き戻される（ルートディレクトリ） | 35 | `00 01 79 96`（番地に見える） |
| 書き戻される（データ先頭・直接転送） | 53 | `00 1f 06 30`（番地に見える） |
| **書き戻されない（端数）** | 54 | **`ff ff ff ff`** |

処理の前後で記述子自体は変化していない（Human68kはこの16バイトを書き換えていない）。
収集した全記録を通して、**byte0-3 が「書き込まれるバッファ＝番地らしき値／読み出しだけの
バッファ＝`ff ff ff ff`」に例外なく分かれていた。** 同じバッファについて、読み出し時の
byte8-11 の値が、後の書き込み時には byte0-3 に移っている例も観測された。
意味は断定しないが、**書き戻し待ち行列のリンクである可能性**があり、次の手がかりとして残す。

### `$05` の返答欄は要求ヘッダの中には無い（実測、2026-09-04）

`--cmd-fill=05:4:25:<値>`（コマンド`$05`の要求ヘッダ `+4`〜`+25` を指定バイトで埋める）を足して掃引した。
`+3`（結果コード）は非ゼロにするとマウントごと壊れるため埋めていない。

| 埋めた値 | マウント | `[SCSI-WRITE]` の論理セクタ | 端数(54) |
|---|---|---|---|
| `$01`（「交換されていない」相当） | OK | 35, 53, 35 | **出ない** |
| `$ff`（「交換された」相当＝故障注入） | OK | 35, 53, 35 | **出ない** |
| `$00`（不定値を消すだけ） | OK | 35, 53, 35 | **出ない** |

埋めた結果が `[SCSI-REQ] 処理後` に実際に現れることは条件ごとに確認済み。
**故障注入の `$ff` でも悪化しなかった**ので、この範囲は読まれていない。
すなわち **`$05` は「結果コード(`+3`)だけが見られる、引数も返答欄も持たない関門」**である。

### 外から値を振る手は出尽くした

実測で否定した軸（すべて本ドキュメントに記録あり）:
FATの中身 / `$05` の返答欄 / 要求ヘッダの応答（`+3` は読まれ `$00` でよい・`+4`・`d0` は読まれない）/
DPBのドライバ参照・連鎖・ジオメトリ / メディアバイト / 属性ワード（`$4000` も内蔵実測値 `$0000` も同じ）/
区画サイズ・クラスタ数 / イメージの作り / 「遅延しているだけ」説。

**次は外から振るのをやめ、内側の実行を見る。** ゲストRAM監視のフックは
**アクセスごとにPCを返す**ので、窓を絞れば実質的な実行トレースになる。
そして比較相手のSASIは**同じHuman68kの同じコード**を通って成功するので、
**2つのトレースが分岐した場所が答え**になる。

トリガは「端数セクタの読み出しを処理し終えた瞬間」（SCSI側は論理セクタ54の `$04`、
SASI側は対応するLBAの読み出し）。それ以降の一定件数だけを記録し、
**打ち切ったかどうかを必ずログに出す**（途中で切れたことを「起きなかった」と読み違えないため）。

## 分岐点を実行トレースの差分で特定した（実測、2026-09-04）

端数セクタの読み出しを処理し終えた瞬間をトリガにして、それ以降のPCを記録するゲートを
コアに足し（`px68k-libretro` `0511b0b`）、**SASI（成功する側）とSCSI（失敗する側）で
同じ操作のトレースを取って突き合わせた。**

- 両実行ともトリガは1回発火。各10000件で打ち切り（打ち切りログあり）
- 記録したPCの範囲は `$00008000`〜`$00020000`（Human68k本体の想定域）
- トリガ直後は呼び出しの深さが違うので一致しないが、**両者は `pc=$0000df44`〜`$0000df48` の
  共通ループへ合流し、そこから4732件連続で完全一致**した

### 最初に食い違ったところ

```
SCSI: ... df02, df44×2, df46×6, df48×6, df4a×2, df4c×6, df4e×6, df50×6, d51a, ...
SASI: ... df02, df44×2, df46×6, df48×6, 10df2×2, 10df6×7, 10df8×6, df4a×2, df4c×6, df4e×6, 10dfa×2, 10dfc×8, ...
```

**`$df48` の直後で分岐する。**

- **SASI**: `$00010df2 → $00010df6 → $00010df8` へ入る。これは実測済みの内蔵HARDDSKドライバの
  ストラテジ（`$00010df0`）／インタラプト（`$00010df8`）である。以後もループを回るたびに
  `$10dfx` を踏み続ける
- **SCSI**: 同じ地点でそこへ行かず `$df4a` へ進む

**すなわち `$df48` の条件分岐で、Human68kが「このドライブのドライバは呼ばない」と判断している。**

### この観測の限界（前担当が自分で明記した）

記録範囲を `$8000`〜`$20000` に絞ったため、**我々のドライバ（`$190000` 付近）は範囲外**である。
よって正しくは「**この地点でSASI側は可視域内のドライバへ分岐するが、SCSI側は可視域内には
分岐しない**」までしか言えない。SCSI側が `$190000` 側へ分岐している可能性は未排除。
次の計測で `$180000`〜`$1a0000` も同時に記録して潰す。

（なお、SASI側のトリガLBAは事前の見当（736-739）ではなく、実測で **737**（ファイル末尾を含む
端数セクタ＝実際に書き戻される側）と確定させている。738・739はファイル実データの範囲外で
読むだけのセクタであり、SCSI側の「端数」に対応しない。**見当で決めず実測で選び直したのは正しい。**）

### 次: 分岐条件の入力を、逆アセンブルせずに実測する

`$df48` の直前にHuman68kが**どの番地から何を読んだか**を、読み出し監視のPC絞り込み
（`$df30`〜`$df50`）で記録し、SASIとSCSIで突き合わせる。値が違えば、その番地が判断材料である。
既に実測済みの番地（SCSIのDPB `$190034` / SASIのDPB `$0148d6` / ドライバヘッダ `$190000`・`$00010da2` /
バッファ記述子）と突き合わせれば、逆アセンブルせずに構造を言い当てられる。

### 訂正: `$df48` の「分岐」は記録範囲が作った見せかけだった（実測、2026-09-04）

前節で「`$df48` の条件分岐でHuman68kが我々のドライバを呼ばないと判断している」と書いたが、**誤りだった。**
記録するPC範囲に `$00180000`〜`$001a0000` を追加して取り直すと、**SCSI側も同じループで
我々のドライバを呼んでいた**:

- SCSI: `pc=$df46` → ストラテジ入口 `$190020`（`$190022→24→28→2a` を実行）、
  `pc=$df4c` → インタラプト入口 `$19002a`（`$19002c→2e→32→34`）
- SASI: `pc=$df46` → `$00010df0`、`pc=$df4c` → `$00010df8`

**完全に対称であり、分岐していない。** 前回 `$8000`〜`$20000` に絞っていたため、
我々のドライバ（`$190000` 付近）への分岐が**見えていなかっただけ**である。

分岐直前にHuman68kが読んでいた番地も、両者ともデバイスドライバヘッダの
`+6`（ストラテジ）と `+10`（インタラプト）で、**構造としては同一**だった
（SASI=`$010da8`→`$00010df0` / SCSI=`$190006`→`$00190020`）。値が違うのは別ドライバなので当然で、
分岐条件の差ではない。

（教訓: **絞り込みで見えなくなったものを「無い」と読まない。**
このドキュメントには同じ形の事故が複数記録されている——PC絞り込みの既定値が全件を捨てていた件、
ログ上限を「CPU停止」と誤読した件。**範囲を絞ったら、絞った外を必ず一度は見ること。**）

### そのかわりに出た観測: SCSIだけ待ちループから抜けていない

トレース末尾（10000件目の手前）を見ると:

- **SASI**: 既にコピー完了処理らしき別のコード（`$85f8`〜`$8760` 付近）へ進んでいた
- **SCSI**: まだ `$df44` のループでドライバ呼び出しを繰り返していた

**「呼ばれない」のではなく「待ちループから抜けられていない」。**
このループが毎回発行しているものの正体は、1回のコピーで数十回届く `$05` であるとみられる
（`$05` は引数も返答欄も持たず、結果コード `+3` だけが見られる＝**状態を問い合わせるポーリング**の形）。

次は、**ドライバから戻った直後にHuman68kが要求ヘッダのどの欄を読んで「もう一度回るか」を
決めているか**を、読み出し監視のPC絞り込みで実測する。

### SCSI側だけが回っているループは、我々のドライバ自身の呼び出しだった（実測、2026-09-04）

トリガ以降のPCトレースを数え直すと、`pc=$df44` は SCSI で6回・SASI で8回しか踏まれておらず、
**「`$df44` のループを回り続けている」という前節の読みも誤りだった。**

実際にSCSI側だけが多数回踏んでいるのは:

| PC | 回数 | 正体 |
|---|---|---|
| `$00190028` | 383 | 我々のドライバの**ストラテジの `rts`**（`$190020`+8） |
| `$00190032` | 638 | 我々のドライバの**インタラプトの `rts`**（`$19002a`+8） |

**この2つはSASI側のトレースには1回も現れない。**
つまり **Human68kは我々のドライバを数百回呼び続けている。**

一方 `[SCSI-REQ]` のコンソールログはこの窓で56件しか出ていない。**この食い違いは、
ログ上限で切れている可能性が高い**（本ドキュメントに同種の事故が複数記録されている）。
**ログ非依存の独立カウンタで数え直すこと。**

もし本当に数百回なら、**Human68kは `$05` を延々とポーリングして待っている**ことになり、
「書き戻しが発行されない」の正体は「**待っている条件が永久に満たされない**」になる。

### まだ潰していない置き場所: 戻り値 `d0`

`$05` の返答欄は要求ヘッダの中には無いと確定している（`+4`〜`+25` をどう埋めても不変）。
残る置き場所は**ストラテジ／インタラプトから戻る `d0`** である。

**注意**: 本ドキュメントには「`d0` は読まれていない」という実測が既にあるが、
**あれは書き込みが成功する上書き経路（`twopart.HDS` の `COMMAND.X` 上書き）で測ったもので、
今回の待ちループを通っていない。あの否定はこの場面には効かない。**
（[[同じ形の誤り]]: 「Xは無関係」と書くときは、**どの経路で測ったか**まで書くこと。）

### `d0` も原因ではない／ポーリング嵐でもなかった（実測、2026-09-04）

**ログ非依存カウンタでの裏取り**: 試験片（1500Bの上書き）で打鍵前後の増分は
`reqTotal` が 82、うち未対応コマンド（`$05`）が 44。**数百件ではなく数十件**だった。
コンソールの `[SCSI-REQ]` 行から独立に数えた件数ともカウンタは完全に一致し、
**ログは切れていない**（前節で疑った「ログ上限で切れている」は否定）。

**`d0` の掃引**: `--reply-d0` を `0` / `1` / `$0200` / 対照（付けない）で振った。
`d0 を … に設定した` のログで**220回実際に上書きされたことを確認**したうえで、
4条件とも `[SCSI-WRITE]` は `35, 53, 35` のみで**論理セクタ54は現れず**、
`reqTotal`・`unsupported`・`writeCount` も4条件で完全に同一だった。**`d0` は原因ではない。**

### SCSI IOCS 経由でもない（実測、2026-09-04）

「Human68kのバッファ書き戻しが、デバイスドライバではなく SCSI IOCS（`$F5`）経由で
行われているのではないか」という筋を確かめた。`x68k/scsi.c` の IOCS フックは
「動作を変えずログだけ取る」実装のままなので、もしそこを通っていれば**何も起きずに消える**——
観測と一致するはずだった。

結果: **フックの自己検査（`[SCSI-IOCS] selftest ok`）は通っているのに、
コピー中の実際のIOCS呼び出しは0件。** 陽性対照つきで否定できた。**この経路は通っていない。**

### 残った未解明の数字

トリガ以降のPCトレースで我々のドライバの `rts`（`$190028`=383回 / `$190032`=638回）が
踏まれている件数が、要求総数（コピー全体で83件）と**逆転**している（狭い窓のほうが多い）。
説明は2つある:

- **説P（本物の取りこぼし）**: スタブが叩く `move.b #$41,$e9f802` がフックへ届かない回がある。
  **本当ならこれ自体が原因になりうる**（このプロジェクトには `$e9f800` が書き込み表で
  握りつぶされていた前例がある）
- **説Q（数え方の膨らみ）**: トレースはメモリアクセスごとに記録するので件数が膨らむだけ

`SCSITableCallCount[]`（ストラテジ／インタラプトの呼び出し回数、既存）をJSへ公開して
要求総数と突き合わせれば割れる。**この数字を放置しないこと。**

### 未解明だった数字は「数え方の膨らみ」だった（実測、2026-09-04）

`SCSITableCallCount[]` 相当をlog_cb非依存でJSへ公開し（`px68k-libretro` `43dab90`）、
打鍵前後の増分を突き合わせた:
...
**3系列が厳密に一致。取りこぼし（説P）は無い。**
トレース上の `rts` 件数（`$190028`=439 / `$190032`=694）が多く見えたのは、
**トレースがメモリアクセス単位で記録されること**と、**上限10000件で打ち切られていたこと**による。
`[SCSI-TRACE] 上限10000に達したため打ち切った` のログも出ていた。
**トレースの件数は「呼び出し回数」の指標として使えない。**

## 2026-09-04 セッション末：現在地と申し送り

### 直したもの

- **小さいSCSIブランクをFAT16で作っていた欠陥**（`26567a6`）。判定を `pickFatType()` に集約し、
  `openFat()` と同じ厳密なクラスタ数で型を決めるようにした。修正前に落ちる回帰テスト付き
- **プローブの防御**（`ff27bf4`）。成立しない実行条件（`--worker=0` × `--scsi-opfs`）を起動時に拒否し、
  書き込みが断られた実行は `writeRefused` で自動失効させる

### 残っている本命

**Human68kが、バッファ経由の書き戻しを我々のドライブに対して一度も発行しない。**

- 届くもの: ディレクトリのセクタ書き込み、セクタに整列した全量の直接転送
- 届かないもの: **端数セクタ**（上書きでも。churnしても永久に来ない）、**FAT**
- そのため、新規確保（FAT更新が必須）はそこで止まり、
  **上書きは「コピーしました」と表示したまま末尾を落とす**（＝静かにデータが壊れる）

### 実測で否定した軸（同じ道を再度歩かないこと）

FATの中身 / `$05` の返答欄（`+4`〜`+25` をどう埋めても不変）/ 要求ヘッダの応答
（`+3` は読まれ `$00` でよい・`+4` は読まれない）/ **`d0`（この待ち場面でも220回上書きして不変）** /
DPBのドライバ参照・連鎖・ジオメトリ / メディアバイト / 属性ワード（`$4000` も内蔵実測値 `$0000` も同じ）/
区画サイズ・クラスタ数 / イメージの作り / 「遅延しているだけ」説 / **SCSI IOCS 経由**
（自己検査は通るのに呼び出し0件）/ ドライバ呼び出しの取りこぼし（3系列が厳密一致）。

### 次にやること

1. **`$05` の正体を、コマンド番号の意味から詰める。** `$05` は「引数も返答欄も持たず、
   結果コード `+3` だけが見られる」関門で、エラーを返すと `$08` が一切発行されなくなる。
   ここが書き込み経路の門であることは確実なので、**成功以外の返し方**（例: 何も書かない、
   `+3` 以外の欄を特定の値にする、呼び出しごとに変える）をまだ試していない
2. **ディレクトリ書き込みと端数書き込みの、Human68k側の発行元PCを比べる。**
   ディレクトリは届くので、**届く経路の発行元**を特定できれば、届かない経路と対比できる
   （今回はトレースを「トリガ以降」でしか取っていない。ディレクトリ書き込みの瞬間を
   トリガにする窓をまだ取っていない）
3. **調査用ログの片付け。** 現在コア側に `[SCSI-REQ]`/`[SCSI-DESC]`/`[SCSI-TRACE]` 等の
   詳細ログが常時入っている。**master へ入れる前に必ず既定オフにするか削ること**
   （このままだと配布物で毎フレーム大量のログが出る）

### 計測の作法（このセッションで確立した／踏み直した）

- **実行は `--worker=0 --scsi-ram-writes` の組でしか成立しない**（`--scsi-opfs` はWorker専用）
- **判定は `[SCSI-WRITE]` のログで行う。**「コピーしました」の表示は書き込みの証明にならない
- **試験片**: 8MiBブランク＋`SRC2.DAT`(1500B)/`DST2.DAT`(1500B) の上書きコピー。
  **合否が「論理セクタ54への書き込みが出るか」の1行で読める**
- **「Xは無関係」と書くときは、Xの何を・どの経路で測ったかまで書く。**
  このセッションだけで、この不足による誤った打ち切りを3回踏んだ
  （`$05`／`$df48` の分岐／`d0`）

## 成功する側の要求ヘッダを直接覗いた：`$05` の `+13` と、来ていない `$01`（実測、2026-09-04）

これまでSASI（成功する側）とSCSI（失敗する側）の比較は、**別プロセスの2回の実行**で行っていた。
本ドキュメントには、条件が揃っていなかったせいで計測を2回まるごと無効にした記録がある。
今回は `--hdd=`（C: = SASI）と `--image=`（D: = SCSI）を**同時にマウントした1回の起動**の中で、
C: と D: へ同じコピーを行い、両方を同じ瞬間に観測した。

### 足した道具: デバイスドライバ入口フック

`px68k-libretro` `x68k/mem_wrap.c` に `webx68k_drv_hook_*` を新設した（既定オフ）。
ストラテジ／インタラプトの入口PCをホストから指定すると、その時点の `a5`（要求ヘッダ番地）を控え、
26バイトを `[DRV-HOOK] S-before / I-before / I-after` としてダンプする。
**逆アセンブルはしていない。本物を走らせて外から値を読んだだけ。**

```
node scripts/probe-scsi-iocs.mjs --worker=0 --scsi-ram-writes \
  --image=_local/scsi/probes/pair1-scsi.hds \
  --hdd=_local/scsi/probes/pair1-sasi.hds \
  --drv-hook-strategy=0x00010df0 --drv-hook-interrupt=0x00010df8 \
  --type='copy c:\src2.dat c:\dst2.dat;;copy d:\src2.dat d:\dst2.dat' \
  --type-wait=3000 --timeout=200000
```

試験片は `scripts/_gen-pair-fixtures.mts` で作る（SASI/SCSIに同一内容の `SRC2.DAT`/`DST2.DAT` を1500Bずつ）。

### 同じ起動の中での対比

| | 受け取ったコマンド | 書き込み |
|---|---|---|
| C:（SASI・内蔵ドライバ） | `$00`×1 / `$05`×37 / **`$01`×31** / `$04`×5 / `$08`×5 | 20セクタ（データ・ディレクトリ・**FAT**） |
| D:（SCSI・我々のドライバ） | `$00`×1 / `$05`×35 / **`$01`×0** / `$04`×17 / `$08`×3 | 3セクタ（ディレクトリ35×2・データ53のみ） |

**`$01`（ディスク交換チェック）が、我々のドライバには1回も来ていない。**
内蔵ドライバ側では `$05` の直後に必ず `$01` が同じ要求ヘッダ番地で続く。

内蔵ドライバの `$08` には `cnt=1 sec=1`（＝FATセクタ）が含まれる。
**探していた「バッファ経由の書き戻し」は、成功する側では確かにドライバ経由で発行されている。**

### 内蔵ドライバが書き戻していた値（処理前→処理後の差分、実測）

| コマンド | 書き戻していた欄 |
|---|---|
| `$05` | `+3` = `$00`、`+4` = `$00`、**`+13` = `$42`**（毎回必ず `$42`） |
| `$01` | **`+14` = `$01`**（`+3`・`+4` は直前の `$05` の応答で既に `$00`） |

我々は `$05` も `$01` も「未対応コマンド」分岐で `+3` に `$00` を書くだけで、
**`+13` も `+14` も書いていない。**

`$42` と `$01` の**意味は分かっていない**（資料で確かめていない）。実測した値をそのまま写す。

### なぜ過去の掃引で見つからなかったか

`--cmd-fill=05:4:25:VAL` は「`+4`〜`+25` を**一様に同じ値で**埋める」掃引だった。
`+13` だけを `$42` にする形は試していない（一様に `$42` で埋めると他の欄が壊れる）。
**「どう埋めても不変」は「どの欄も効かない」ではなかった。**

### 次

`$05` に `+13=$42`、`$01` に `+14=$01` を返す応答を入れ、
**`[SCSI-WRITE]` に論理セクタ54とFATが現れるか**を1変数で見る。

### 今回踏んだ事故（同じ道を歩かないこと）

- **プローブのコンソール収集がタグの列挙だった。** `[SCSI` / `[SASI` / `[WebX68k-worker]` を含む行しか
  拾っておらず、新設した `[DRV-HOOK]` は1行も残らなかった。コアには確かに入っているのにログが0件で、
  「フックが呼ばれていない」と誤って追いかけた。**新しいタグを足したら収集側にも足すこと。**
- **入口PCの完全一致では当たらない。** メモリアクセス時に読めるPCはオペコード取得後の値で、
  入口番地そのものは現れない（我々のドライバでも `$190020` は現れず `$190022` から出る）。窓で受ける。
- **「処理後」の採取点を雑に決めると、返答を1つも観測できない。** 当初は「PCが境界より下になったら」で
  採っており、`$04`（読み出し）まで含めて全件「変化なし」になった。
  インタラプト入口でスタックから戻り先を読み、**そこへ戻った瞬間**に採ることで初めて差分が出た。
- **ビルドが落ちたのに気づかず、古いwasmを測った。** `build-core.sh` の終了コードを見ずに
  `grep error` の結果だけ見ていた。必ず rc を確認すること。

## 解決：`$05` の `+13=$42` と `$01` の `+14=$01`（実測、2026-09-04）

前節で実測した内蔵ドライバの応答をそのまま写した。**意味は分かっていない。実測値を返しているだけ。**

- `$05` → `+3`=`$00`、`+4`=`$00`、**`+13`=`$42`**
- `$01`（ディスク交換チェック）→ `+3`=`$00`、**`+14`=`$01`**

`$01` は「`$05` にきちんと答えて初めて来る」ので、2つは別々の変数ではなく1本の連鎖である。

### 合否（試験片は従来どおり、`--scsi-oracle-reply` の有無だけが変数）

| 実行 | 受け取ったコマンド | `[SCSI-WRITE]` の論理セクタ |
|---|---|---|
| 応答なし（従来） | `$00`×1 / `$05`×35 / **`$01`×0** / `$04`×17 / `$08`×3 | `35, 53, 35` |
| 応答あり | `$00`×1 / `$05`×37 / **`$01`×31** / `$04`×5 / `$08`×5 | `35, 53, 35, **54**, **1**` |

**受け取るコマンドの内訳が、同じ起動の内蔵ドライバ（`$00`×1 / `$05`×37 / `$01`×31 / `$04`×5 / `$08`×5）と
完全に一致した。** 端数セクタ（論理54）とFAT（論理1）の書き戻しが出ている。

### 新規確保も通った

これまで「要求が完全に停止して完了メッセージも出ない」状態だった**新規ファイル作成**を確かめた。

```
node scripts/probe-scsi-iocs.mjs --worker=0 --scsi-ram-writes --scsi-oracle-reply \
  --image=_local/scsi/probes/pair1-scsi.hds \
  --type='copy c:\src2.dat c:\new1.dat;;dir c:' --type-wait=4000 --timeout=200000
```

- 書き込み論理セクタ: `35`(ディレクトリ) / `55` / `56` / `1`(FAT)
- `dir c:` に `new1.dat 1500` が現れ、使用量が 4K → 6K に増えた
- `[SCSI-WRITE-VERIFY]` の不一致は0件

**「成功表示のまま静かにデータを壊す」状態は解消した。**

### まだ済んでいないこと

- **OPFS（本命の書き戻し経路、Worker）での確認。** 現状 `--scsi-opfs` の実行は
  **起動が200秒たっても終わらない**。調査用ログが Worker 境界を毎フレーム大量に越えているためとみられる
  （本ドキュメントに同型の記録がある）。ログを既定オフにしてから測り直すこと
- **リロードをまたぐ永続化の検証**（`scripts/verify-disk-persistence.mjs` に相当するSCSI版が無い）
- ~~`$42` の意味。資料で裏を取れていない~~ → **2026-09-05 裏取り済み**（末尾「`$42` と `$01` の意味の裏を取った」参照）
...
## リロードをまたぐ永続化を「中身まで」検証した（実測、2026-09-04）

2026-09-03 の永続化実測は**判定がディレクトリ表示だけ**だった。直前まで踏んでいた不具合は
「ディレクトリには正しい名前とサイズが出るのに末尾の端数バイトが落ちる」というものなので、
その判定では見逃す。中身を読み返すハーネスを作った。

- `scripts/_gen-scsi-marker.mts`: 8MiBブランクに `SRC.TXT` を1つ置く。
  **1行目が `HEAD-<id>`、最終行が `TAIL-<id>`**、間は64文字の埋め行。
  **サイズはセクタ境界に一致させない**（954バイト＝末尾セクタ端数442バイト。
  端数が出ない検体では、直した不具合を踏まない）
- `scripts/verify-scsi-persistence.mjs`: `probe-scsi-iocs.mjs` を子プロセスとして呼ぶ
  オーケストレータ（**打鍵処理は自作しない**。何度も事故が起きて固められているため）。
  終了コードは 合格=0 / 不合格=1 / **ハーネスエラー=2**
...
| 段 | 条件 | 結果 |
|---|---|---|
| 主判定 | プロファイル `P` で `copy` → **同じ `P`** で `type` | **合格**。`HEAD`/`TAIL` とも読み返せた。`取り込み=なし`・`writeCount=0` の裏取りつき（＝この実行で作ったものではない） |
| 陰性対照 | **新しい**プロファイル `Q` で `type` | `HEAD`/`TAIL` とも出ない（＝この検査は落ちうる） |
| 陽性対照（故障注入） | `--scsi-oracle-reply=0` で `copy` → 同じプロファイルで `type` | **画面上は「コピーしました」が出たのに、リロード後は `HEAD` も `TAIL` も無い** |

**陽性対照が、直した不具合そのもの（成功表示のままデータが残らない）を再現している。**
この検査は「修正が入っているかどうか」を見分けられる。

### 判定の作りで踏んだこと

最初の実行は**実質合格なのにハーネスエラーになった**。`type` の出力がテキスト画面
（96列×32行）を溢れさせ、**コマンドのエコー行と `HEAD` が上へ流れて消えた**ためで、
データは残っていた。2つ直した:

- 検体の埋め行を16文字から64文字にして、`HEAD`〜`TAIL` が1画面に収まるようにした
- **打鍵エコーの照合は「目印が1つも見つからなかったとき」だけ効かせる。**
  目印が読めているのに手続きの都合でエラーにするのは、証拠を捨てているだけ。
  逆に目印が無いときは「残っていない」と「打鍵が化けた」を区別できないので、
  不合格ではなくハーネスエラーにする
- **陰性対照と故障注入では緩めない**（どちらも「目印が出ないこと」が期待なので、
  打鍵が化けると中身に関わらず期待どおりに見えてしまう）
...
### この検査で言えること・言えないこと（flushについて）

`--flush-grace=0`（書き込み直後の固定待ちを置かない）で**主判定は合格**した
（`HEAD`/`TAIL` とも読み返し、`取り込み=なし`・`writeCount=0` の裏取りつき）。
固定の待ちを置かなくても内容が読み返せる、というところまでは言える。

**ただし、この検査は「`flush()` が呼ばれたこと」を示していない。**
リロードは同じブラウザセッション内で起きるので、`flush()` していなくても
直前の `write()` の内容がそのまま読めてしまう可能性がある。
この検査が区別できるのは「書けたか」であって「ディスクへ確定したか」ではない。

`flush()` が実際に呼ばれること・呼ばれなくなること（故障注入）は
**単体テスト `test/scsi-opfs.test.ts` の側で確かめている**
（`__webx68kScsiFlushDebounceMs=0` で自動flushが止まることを含む）。
...
## 起動しなくなったのはマージのせいではなかった：固定タイムアウトが初期化中のコアを殺していた（実測、2026-09-04）

master を取り込んだ直後、プローブでの起動確認が通らなくなった。直前に大きなマージをしていたため
「マージの退行」と考えたが、**それは誤りだった。**

### 切り分け

| 確認したこと | 結果 |
|---|---|
| 実ブラウザ（温まったプロファイル）でマージ後のコードを開く | **正常に `A>` まで起動** |
| 同じブラウザで、プローブと同じ間隔・同じ呼び方で `screenText()` を叩く | 17回すべて正常 |
| 可視性（`visibilityState: hidden`） | **無関係**（Worker は動き続け frameNo は 33228 まで進む） |
| マージ**前**のツリーを別 worktree に出して同じコマンド | **起動する（46秒）** |
| マージ**後** | 起動しない |

「マージ前後で違う」ところまでは出たが、**同じコードが自分のブラウザでは動く**ため、
コードの差だけでは説明が付かなかった。

### 決め手は「ログに依存しない数値」

`workerStats()` の `frameNo` を起動待ちと並行して2秒おきに記録したところ、**+28s まで 0 のまま**だった。
これで症状を **「Workerが応答しない」ではなく「そもそもフレームが1枚も回っていない」** と
言い直せた。応答タイムアウトはその**結果**であって原因ではなかった。

### 機構

`WorkerCoreProxy` は全コマンドに固定 10 秒の応答タイムアウトを張り、
超えると `WORKER_FAILURE` として**コアを恒久的に異常終了扱い**にする。

**まっさらなプロファイル（＝冷えた初回アクセス相当）では、コアが最初のフレームを出すまで約22秒かかる。**
その最中にアプリが `readTextScreen` を投げると 10 秒で見切られ、コアが死ぬ。
画面には「Workerコアが異常終了しました: 応答timeout(10000ms): readTextScreen」とだけ出て、**復帰できない**。

タイムアウトを 60 秒へ伸ばすと同条件で起動した（56.5秒）ことで機構を確定させた。

**マージ前後の差は、初期化時間がこの固定しきい値をまたいだかどうかだけだった。**
つまりこれは**マージの不具合ではなく製品の不具合**で、遅い端末や初回アクセスで同じことが起きる。

### 直した内容（`43da624`、その前段が `f0642f1`）

死活監視の目的は残したまま、**初期化中のWorkerを「死んだ」と誤判定しない**ようにした。

- 起動完了の境界は **最初の `frame` イベントを受け取った時点**。コアが実際に動き始めたと
  言えるのはそこ
- 起動中は無タイマではなく `STARTUP_RESPONSE_TIMEOUT_MS`（既定120秒）を張る。
  **無タイマにすると、本当に起動に失敗して固まったWorkerを永久に検出できなくなる**
- 最初のフレーム以降は従来どおり 10 秒。起動後の挙動は変えていない

**1回目の修正（`f0642f1`、境界を `initialize` の応答に置いた）は効かなかった。**
`initialize` の応答は先に返り、重い処理はその後にあったため。
単体テストは4本とも通っていたが、テストは「想定した境界」を確かめていただけで、
**その境界が実際の起動シーケンス上どこにあるか**は確かめていなかった。

### 実測（修正後）

`booted True` / 61.5秒 / `frameNo` は +28s で 4 まで進行 / 「異常終了」のログ無し。

### まだ測っていないこと

マージ前 46 秒に対しマージ後 61.5 秒で、**起動が約15秒遅い**。コアを別の emsdk
（`$HOME/emsdk` 6.0.7）で作り直したことが効いている可能性があるが、**未測定**。
実行ごとのばらつきも取っていないので、差として確定していない。

## Worker経路の速度無制限モード（実装・実測、2026-09-04）

master を取り込んだ時点では、**無制限モードは Worker 経路（＝既定）で何も起きなかった**。
`applySpeedChangeToWorker()` が `speedMultiplier` しか送らず、無制限のときその値は 1 だったため。
UIのボタンは光るのに等倍のまま、という無言の失敗だった。

master の実装（`UNLIMITED_TICK_BUDGET_MS = 12` 等）は**そのまま移植できない**。
あの 12ms は「コア実行と画面提示とUIが同じスレッドに同居する」前提から逆算された値
（実測: コア12.37ms + 提示6.56ms で占有率95.9%、ページが固まった）。
Worker 経路ではコアは Worker、提示はメイン、と別スレッドに分かれる。

### 実測（同一実行内で比較。`workerStats().frameNo` の増分）

| 段階 | 等倍 | 無制限 | 4倍(この環境の上限) |
|---|---|---|---|
| 1. 予算を「tick間隔の割合」で決めた | 55.6 | **62.5** | 122.7 |
| 2. 予算を実時間ベースへ（占有率0.9） | 55.6 | 173 | 192 |
| 3. 無制限中の音停止・提示間引きを追加 | 55.4 | 183.5 | 213.5 |
| 4. 占有率の上限を撤廃（1.0） | — | **136** | 121.6 |

（絶対値は実行ごとの機械の負荷で動くので、**同一実行内の比較だけが意味を持つ**。
段階4だけ全体に低いのはそのため。比率で読むこと。）

### 段階ごとに分かったこと

- **段階1の 62.5fps は設計ミス。** この環境の1フレームは約8.3ms、予算は `16ms × 0.7 = 11.2ms`、
  そこから「入らないフレームは始めない」ぶん8.3msを引くので、**2フレーム目が原理的に入らない**。
  `1フレーム/tick × 62.5tick/秒 = 62.5fps` と観測値がぴったり一致した。
  **無制限モードは刻みをまたいで回れないと意味がない**、という点が抜けていた
- **段階2で 173fps。** ただし `192 × 0.9 = 173` で占有率の上限に張り付いており、
  実装は意図どおりだが**固定倍率(4倍)より遅い**という妙な関係のままだった
- **段階3の音停止・提示間引きは、Worker では効かなかった**（183.5/213.5 = 0.86 で比率が変わらない）。
  **Worker では音も提示も律速ではない**。提示コストはメイン側の別スレッドにあり、
  Worker の予算を食っていなかった。既定経路と同じ対称性は保つ意味があるので実装は残す
- **段階4で 4倍を追い越した**（無制限136 > 4倍121.6）。Worker はUIを持たないので、
  メインスレッド用の「UIを守るための取り分」を確保する理由が無い。
  イベントループへは `setInterval` の刻みの間に自然に戻る

### 入力が生きていることの確認

無制限中に打鍵し、画面に `A>dir` が出ることを実測で確認した（占有率1.0でも入力は届く）。
frame event は `WORKER_UNLIMITED_PRESENT_INTERVAL_MS = 33`（約30fps）までしか間引かない。
**main 側は frame event を契機に InputUpdate を送るため、これより粗くすると入力が遅れる。**

### 途中で踏んだ不具合

**frame event が完全に止まり 0fps になった**（`frameNo` が 2723 で凍結）。
原因は、送信の判定が `ranFrames > 0 && shouldSendFrame` の両方なのに、
**「提示した時刻」を `shouldSendFrame` だけで更新していた**こと。
占有率の上限で `ranFrames = 0` になった tick でも時計が進み、次は間引かれる——
これが噛み合うと永久に送られない。更新を実送信と同じ条件へ揃えて解決した。

**2つのゲートを独立に計算し、片方だけで状態を進めてはいけない。**

## CPUクロックの走行中変更と∞MHzを Worker 経路へ配線した（実測、2026-09-05）

master から取り込んだ「CPUクロックの走行中変更」「∞MHz(ホスト次第)」は、
**Worker 経路（＝既定）に1つも配線されていなかった。** 見つかった穴は4つ:

| 穴 | 症状 |
|---|---|
| 設定変更が `host?.setCoreOption(...)` だけ | `host` は既定経路でしか作られない。**`?.` が機能の欠落を無言で飲み込む** |
| 占有率ゲートが `runTick()` に無い | 追いつけない設定でキー・ボタンが効かなくなる（master が既定経路で直した症状） |
| 1フレームの実測コストがメインに無い | ∞MHz は「実際に実時間を維持できたか」で判断するので、Worker ではクロックが決まらない |
| `updateSpeedActualDisplay()` が `loop()` からしか呼ばれない | **∞MHz の測定クロックが画面に一切出ない。** 実装が動いていても何も起きていないように見える |

`bootWorkerCore()` が生の `cpuSpeed` を渡していた箇所も見つかった。
ブランチ側が「両経路が同じ `buildCoreOptions()` から値を取る」構造にしていたのは、
以前これで `px68k_cpuspeed` が食い違い**コア既定の10MHzのまま Worker が起動していた**ため。

### 実測（Worker 経路、実ブラウザ）

`16Mhz` 固定 → `auto`(∞MHz) へ走行中に切り替えたときのクロックの推移:

```
16Mhz固定 →  実測 99%
auto  +3s →  実測 99% / CPU  31MHz
auto  +6s →  実測 98% / CPU  76MHz
auto  +9s →  実測 99% / CPU 119MHz
auto +12s →  実測 99% / CPU 138MHz
auto +15s →  実測 99% / CPU 151MHz
```

**実時間を99%維持したままクロックが登っていく。** これで次の3つが同時に実証された:

1. 走行中のコアオプション更新が Worker へ届いている（届かなければクロックは変わらない）
2. Worker が測った1フレームのコストがメインへ戻っている（戻らなければ判断できない）
3. 表示の配線（この節で直した4つ目の穴）

∞MHz(143MHz)のまま打鍵し、画面に `A>dir` が出ることも確認した（占有率ゲートが効いている）。

### 観測できないものは直っていないのと同じ

4つ目の穴（表示が更新されない）は一見「見た目だけ」の問題だが、
**これを直すまで、他の3つが動いているかを確かめる手段が無かった。**
`fps` は追いつけている間クロックに依らず 55.5 のままで、判別に使えない。
実装の配線を直したら、**その結果を外から読める窓も同時に用意すること。**

## Web Serial を Worker 経路へ配線した（実装、2026-09-05）。**通信そのものは未検証**

master から取り込んだ Web Serial（SCCチャネルA / RS-232C）は、Worker 経路（＝既定）に
配線されていなかった（`src/libretro-host.ts` に55箇所あるのに `src/core-worker.ts` には2箇所）。
配線したが、**実際に通信できるかは確かめていない。**

### 確かめたこと / 確かめていないこと

| | 状態 |
|---|---|
| 既定経路（メインスレッド）の無退行 | 確認済み（既存テスト無変更で通過） |
| 配線の存在・純粋ロジックの単体テスト | 確認済み（9件） |
| Worker 経路での起動・描画・入力の無退行 | 実測で確認済み |
| **実際にシリアル機器と通信できるか** | **未確認。物理的な機器が必要** |

**この状態は「動く」ことを意味しない。** 本ドキュメントには、単体テストが通っているのに
実機で効いていなかった例が同じ日に3件ある（応答タイムアウトの境界、無制限モードの予算モデル、
frame event の間引き）。いずれも実測で見つけた。**Web Serial だけはその手段が無い。**

### 方針: 報告ベースで対応する

機器が用意できないため、**動かないという報告があった時点で調べる**。

**注意: 失敗は無言になる。** 例外もエラー表示も出ないので、利用者は「シリアルが使えない」と
思うだけで報告に至らない可能性がある。報告が来たら幸運、くらいに考えること。

### 報告が来たときの切り分け手順

1. **まず `?worker=0` で試してもらう。** 既定経路（メインスレッド）は master 由来の実装が
   そのまま動いているので、**そちらで通信できて Worker 経路でできなければ、原因はこの配線**。
   これが一番速い切り分けで、最初に聞くべきこと
2. Worker 経路で、frame event のスナップショット `snapshot.serial` が届いているかを見る
   （送信バイト・ゲスト側ボーレートが載る）。届いていなければ Worker → main が切れている
3. 受信側は**メイン側のミラー（キュー残量）で背圧をかける**作りなので、
   ミラーが下がらないまま張り付いていないかを見る。張り付くと受信が止まる
4. 切断・再接続で直るなら、`SERIAL_RESET_KIND` によるキューとミラーのクリアを疑う

### 設計上の注意（次に触る人へ）

- `navigator.serial` は**メインスレッド専用**。搬送層（`src/serial.ts`）は Worker へ移せない
- 受信の背圧は**同期の往復ができない**。既定経路は `onData` の戻り値で搬送層を待たせるが、
  Worker へは同期で問い合わせられないため、キュー残量のミラーで代替している。
  **ここを「とりあえず全部受理」に変えると、静かにデータを落とす**
- 送信の失敗時は**再送しない**（既定経路と同じ。順序の重複を招くため）

## `$42` と `$01` の意味の裏を取った（資料調査、2026-09-05）

「実測した値をそのまま返しているが意味は分かっていない」として残していた2つの値の裏を取った。
**実装は変えていない。返していた値は意味の上でも正しかった。**

### 結論

| 値 | 意味 |
|---|---|
| `$05` の `+13` = `$42` | **ドライブ状態バイト**。`bit6`＝イジェクト禁止、`bit1`＝メディア挿入済み。固定ディスクとして正しい |
| `$01` の `+14` = `$01` | **メディアは交換されていない**（media not changed） |

参照した資料:
- Human68k のドライバ仕様をまとめた DDK 系の記述（NFG/GameSX wiki `x68000:writing_drivers`。Lydux 氏が自作 DDK の副産物として整理したもの）
- Data Crystal `X68k/DOSCALL`（`_DRVCTRL`）および `X68k/FS`（ドライバのコマンド一覧）

### `$05` はブロックデバイスでは「ドライブ制御／状態検査」

キャラクタデバイスの `$05` は「無停止読み込み」だが、**ブロックデバイスの `$05` はまったく別のコマンド**で、
ドライブの動作指定と状態の返却を行う。要求ヘッダの `+13` は入力としては内部コマンド
（DOS コール `_DRVCTRL` の `MD` に相当）、出力としてはドライブ状態が返る欄である。
**同じ番号で中身が違うので、キャラクタデバイス側の説明を読むと外す。**

ドライブ状態バイトの並びは `_DRVCTRL`（コール番号 `$ff0f`、`MD=0` で状態検査）の戻り値と同じ:
...
| 4 | 利用者によるイジェクト禁止 |
| 3 | 書き込み禁止（1＝禁止） |
| 2 | 準備できていない（1＝not ready） |
| 1 | メディア挿入済み |
| 0 | 挿入エラー |

`$42` = `0100_0010b` = **bit6（イジェクト禁止）＋ bit1（メディア挿入済み）**。
**「毎回必ず `$42`」だったのは、固定ディスクの状態が変わらないからだった。**

### `$01` は資料が食い違い、実測が裁定した

メディアチェックの戻り値について、参照した2つの資料は**逆のことを書いている**。

| 資料 | `+14` の `$01` の意味 |
|---|---|
| DDK 系（MS-DOS の慣習と同じ） | 交換されていない |
| Data Crystal | **交換された**（交換されていない側が負数） |

**どちらが正しいかは、既に手元にある実測が決めていた。** 同じ起動で内蔵ドライバが受け取った
コマンドの内訳は `$00`×1 / `$05`×37 / **`$01`×31** / `$04`×5 / `$08`×5 で、
**`$02`（BPB取得）が1回も無い。** `$02` は「交換された」と報告された後にだけ発行されるコマンドなので、
31回の `$01` はすべて「交換されていない」と答えていたことになる。
よって `+14`=`$01` は **交換されていない**で確定。Data Crystal の記述のほうが誤り。

**資料が2つあっても票にはならなかった。決めたのは、意味を調べる前に取ってあった実測のほうだった。**

### 出自について

**我々の実装はこれらの資料から作ったものではない。** 値は本物のドライバを走らせて外から観測して
決めたもので、資料は後から意味を当てただけである。順序が逆だったら、
第三者の解析に依存した実装になっていた。この順序は今後も守ること。

参照した2件はどちらも第三者による解析・整理であって、シャープの一次資料ではない。
手元の `Inside X68000`（1992）と `Human68k 3.0 ユーザーズマニュアル` には
デバイスドライバの要求ヘッダの仕様は**入っていない**（前者は SCSI バスと SPC の層まで、
後者は利用者向け）。両方とも全文検索して確認済みなので、次の担当は探し直さなくてよい。

### この裏取りで見えた、実装の粗

`$42` は定数ではなく**状態バイト**だった。いま我々は固定値を返しているので、次が置き去りになっている。

- **bit6（イジェクト禁止）は本来デバイス依存。** SCSI はハードディスクだけでなく MO も繋がり、
  イジェクト可否はディスク先頭のデバイスパラメータのフラグに書かれている
  （`Inside X68000` 2-5「SCSI デバイスパラメータ」）。イジェクト可能なデバイスを繋いだとき、
  我々は「イジェクト禁止」と嘘をつくことになる
- **bit3（書き込み禁止）も固定で 0。** 書き込み禁止のイメージを扱えるようにしたら、ここを立てる必要がある

いまは固定ディスクしか作れないので実害は無い。**種類を増やすときに一緒に直すこと。**

### 実測で確かめるなら（未実施）

資料で意味が付いただけで、**この意味づけ自体はまだ実測していない**。確かめるなら:

1. ゲストから `_DRVCTRL`（`MD=0`）を C: に対して呼び、`$42` が返ることを見る
2. 我々の `+13` の応答を別の値に変え、1 の結果が追随することを見る（1変数の介入）
3. FD ドライバに同じ `webx68k_drv_hook_*` を当て、**違う値**が返ることを見る

3 が効く。**「固定ディスクだから `$42`」が正しいなら、固定でないドライブは違う値でなければならない。**
固定ディスクと同じ値が返ってきたら、この意味づけを疑うこと。

## バーチャルトラックパッドが Worker 経路（＝既定）で丸ごと死んでいた（実測・修正、2026-09-05）

`urlWorkerMode` の既定は `true`（`src/main.ts`）で、Worker 経路では `host` が常に null。
タッチ操作のマウス入力だけが `applyMouseDelta()`/`applyMouseButton()` の窓口を通さず
`host` を直接触っていたため、**スマホ・タブレットのマウス操作が既定で1つも効いていなかった。**

### 見つけ方（機械的に洗った）

「本流の新機能が片側だけ実装される」は既に記録がある失敗なので、
`host?.` と `if (!host)` を全部拾い、**それぞれの関数に Worker 分岐があるか**を機械的に判定した。
4件が「分岐なし」で残り、うち3件が本物だった（`pollDiskAccess()` は frame event 経由の
別配線がコメント付きで存在する誤検出）。

### 実測（陽性対照つき。変数は経路だけ）

同じ座標のドラッグを、同じ手順で両経路に当てた。

| 経路 | パネルが受けたイベント | ゲストのカーソル |
|---|---|---|
| `?worker=0`（既定経路） | down1 / move2 / up1 | (2,2) → **(252,142)** |
| Worker（＝既定、修正前） | **同一** | (764,99) → **不変** |
| Worker（修正後） | 同一 | (2,2) → **(252,142)**（既定経路と一致） |

**イベント数が両経路で同一**なのが効いた。これが無いと「効かない」と「操作が届いていない」を
分けられず、測定失敗を不具合と読み違える。

### 必要条件は2つあった

デルタの配線を直しても**タップだけは出なかった**。原因は独立した2つ目の欠落で、
`stepTouchTrackpad()` が `loop()`（既定経路のフレームループ）からしか呼ばれておらず、
Worker 経路では長押し判定もクリックパルスも一度も回っていなかった。
Worker 経路の毎フレームの唯一の呼び出し口は frame event ハンドラである。

**片方だけ直しても症状が変わらないので、直した側まで疑いたくなる。**
`loop()` が呼ぶ関数と frame event ハンドラが呼ぶ関数の差を機械的に取り、
13件のうち別配線が確認できないのが `stepTouchTrackpad()` だけだと確かめてから直した
（`pollAutoSave`→`pollWorkerAutoSave`、`pollDiskAccess`→`snapshot.disk.access`、
シリアルは `snapshot.serial`、`stepMouseTracking` は Worker 内で完結、が確認できた分）。

### 観測できないときは、まず観測系を疑った

タップが出なかったとき、`__webx68kDebug.mouseButton()`（確実に届く経路）で同じ指標が
0→1 になることを先に確かめた。**指標が動くことを見てから「タップが出ていない」と言った。**
これをやらずに「ボタンは直っていない」と書いていたら、次の担当が指標の側を疑って時間を溶かす。

### protocol を1つ増やした理由

Worker 側の閉ループは main から送られた目標比率へカーソルを引き続ける。
既定経路の `trackpadMoveBy()` は相対移動の前に `mouseTracker.clearDesiredRatio()` で
目標を捨てて綱引きを防いでいるが、**Worker へ「目標を捨てた」を伝える手段が protocol に無く**、
`applyMouseTrackUpdate()` は常に `setDesiredRatio()` を呼んでいた。
mouse track 更新メッセージに `hasRatio` を足し、false のとき Worker 側で
`clearDesiredRatio()` を呼ぶようにした。

### なぜテストが966件通ったまま素通りしたか

`applyMouseButton()` 自体は正しく分岐するので、**ラッパを単体テストすると通る**。
見ていなかったのは「呼び出し側がラッパを使っているか」だった。
`test/mouse-track.test.ts` は閉ループ追従を見ており、トラックパッドには1件も触れていない。

### まだやっていないこと

- **入力の窓口を静的に検査する回帰テスト。** 「入力系は必ず `apply*()` を通る」
  「`loop()` だけが呼ぶ毎フレーム処理を増やさない」を実ファイル読み取りで検査する
  （`test/core-worker-build-format.test.ts` と同じ流儀。限界も同じで、形しか見ない）
- 実機（iOS/Android）での確認。今回の実測はデスクトップのブラウザでポインタを合成している

## ポーズが Worker 経路（＝既定）で表示だけだった（実測・修正、2026-09-05）

利用者からの指摘（「ポーズ中と出ているのにカーソルが点滅している」）が発端。
**指摘のとおりで、コアは全速で走っていた。**

`setPaused()` は `pausedByUser` と UI を変えるだけで、実際に止めていたのは
`loop()` 冒頭の早期 return と `scheduleNext()`。どちらも既定経路専用で、
**protocol にポーズは1つも存在しなかった**（`grep pause src/core-protocol.ts src/core-worker.ts` が0件）。
トラックパッドと同じ「片側だけ配線」で、ポーズは master から取り込んだ機能である。

### 実測（`workerStats().frameNo` の増分）
...
Worker の `tick()` は経過時間から走らせるフレーム数を決めるので、**ポーズ中も時計が進むと
解除の瞬間に止まっていた分をまとめて走らせる**。ポーズのゲートで `lastTickAtMs` を更新し
`accumulator` を 0 に落としてからreturnすることで防いだ。

**「解除直後300ms＝16フレーム」がその証拠**である。2秒ぶんを取り戻していたら
ここが120前後になる。ポーズが効いていることだけを見て合格にすると、この穴は見えない。

### 利用者の観測のほうが正しかった

「ポーズ中」の表示は出ていたので、**UI だけを見ると正常に見える**。
気づけたのは画面のカーソルが点滅し続けていたからで、
これは我々の合格条件（オーバーレイが出るか）が**失敗状態のほうを強く満たしていた**例である。
合格条件は必ず**末端の量**（ここでは frameNo）で置くこと。

## 起動可能なSCSIを挿していると、FDからも起動できない（実測、2026-09-05）

利用者からの報告（URL で FD を指定して開くと「システムを起動できませんでした」）を追った結果、
**原因は SCSI だった。** 指摘してくれたのは利用者のほうで、こちらは別の筋を2つ潰してから辿り着いた。

### A/B（変数は SCSI の有無だけ。同じ FD・同じ操作）

| SCSI | 結果 |
|---|---|
| 起動可能な `.HDS` を挿す | **「システムを起動できませんでした。リセットしてください」** |
| 挿さない | FD から正常起動 |

決定6は「FD から起動して SCSI をデータドライブにする」を前提にしていたが、
**起動可能なイメージではその前提が崩れている。** SCSI から起動できないだけでなく、
**FD 起動を巻き添えにする。**

機構の見込み（**未検証**）: IPL が SCSI 側の起動可能な区画（`X68SCSI1` 署名＋Human68k 区画）を
見つけてそちらから起動しようとし、我々の HLE に起動エントリが無いので失敗し、
FDD0 へフォールバックせずに諦めている。傍証として、過去のプローブは
`createFormattedScsi()` 製の**空（起動不可能）**な SCSI を挿した状態で FD から起動できていた。
ただしこれは記録からの引用で、「起動可能かどうか」を変数にした実測ではない。

### 潰した説（先に間違えた2つ。同じ道を歩かないこと）

1. **URLパラメータの切り詰め。** `fd1=` の値を URL エンコードしていないため
   `&st=`/`&dl=0` が WebX68k 側のパラメータとして食われるのは事実だが、
   **切り詰めた URL でも Dropbox は 206 + `application/octet-stream` を返す**ので原因ではない。
   CORS も全 Origin で `*`
2. **`run=1` の起動レース。** `resolveUrlSlotContent()` に30秒の遅延を注入し、
   IndexedDB を全消去して初回相当にしたところ、**FDD0 未挿入・コア停止のまま32秒待ち、
   両方が同時に切り替わった。** 取得を待たずに起動する経路は無い。この説は否定された

**利用者の観測（SCSIを挿したままだった）のほうが、こちらの2つの仮説より正しかった。**

### 対処1: URL がディスク構成を指定したら、指定外のスロットを解除する

共有リンクは「誰が開いても同じ状態で起動する」のが本来で、前のセッションの残りが混ざるのは
不自然である（この不具合そのものがその実例）。`fd1`/`fd2`/`hdd`/`system` のいずれかがあるとき、
**URL が指定していないスロットを解除する**ようにした。`run=1` だけの URL は
「いま挿さっているもので起動する」意味なので何も触らない。

- **SCSI は URL から指定する手段が無い**ので、この規則が働くときは常に解除される
- 解除は**スロットから外すだけ**で、OPFS 上の実体もライブラリのレコードも消さない
- 判定は純粋関数 `slotsToUnmountForUrl()`（`src/url-params.ts`）に切り出して単体テスト対象にした

### 告知が消えていたのを直した（実測）

最初の実装では告知トーストが**実ブラウザで見えなかった**。`showToast()` はトースト要素が
1つしかなく後勝ちで、直後の「FDD0: 前回保存した◯◯を復元しました」に上書きされていた。
**一番必要な場面で無言になる。**読み込みが全部終わったあと（各 `return` の直前と起動の直前）に
出すよう移した。実測で t=6.5s に表示されることを確認済み。

### まだやっていないこと

- **本体の修正。** UI から起動可能な SCSI を挿した人は、依然として FD から起動できない。
  機構を1変数（起動可能／不可能なイメージ）で確定させてから、IPL が SCSI を
  起動先に選ばないようにする必要がある
- **起動前へ戻る手段（電源OFF相当）が無い。** いま詰まると URL を編集してリロードするしかない。
  リセットは再起動なので、HDD/SCSI のロックは外れない

## SCSIを挿すと起動しないディスクがある（実測、2026-09-05。**未解決**）

前節の「起動可能なSCSIを挿すとFDからも起動できない」を、FD側も振って測り直した結果、
**結論が2回変わった。**最終的な実測は次のとおり。

### 実測（全部 `?worker=0`。SCSIは `__webx68kScsiUrl` で挿した）

| FDD0 | 併設 | 結果 |
|---|---|---|
| CHACHA.XDF | なし | 起動する |
| CHACHA.XDF | **SASI (HDD 40MB, 空)** | **起動する** |
| CHACHA.XDF | SCSI 空1MiB | **失敗**（IPLの「システムを起動できませんでした」） |
| CHACHA.XDF | SCSI 空100MB | **失敗** |
| CHACHA.XDF | SCSI 起動可能100MB | **失敗** |
| TDQ1.2HD | SCSI 空1MiB | **起動する** |
| 同梱システムディスク | SCSI 空1MiB | **起動する** |

9/4 のプローブ（同梱システムディスク＋SCSI）を今日そのまま走らせても起動し、
SCSIドライバの `$05`/`$01` が処理されるログまで出た。**SCSI機能そのものは壊れていない。**

### 分かっていること

- **SCSIのときだけ落ちる。** SASI(HDD)を繋いでも CHACHA は起動する。
  「ハードディスクが繋がっていること」が原因なら SASI でも落ちるはずで、落ちない。
  したがって**我々のSCSI実装が原因**である線が濃い
- **影響を受けるディスクは限られる。** 3枚中1枚だけが落ちた
- 症状の文言は IPL がゲスト側で出すもので、アプリ側は異常を検知していない

### 2回の誤りとその原因（同じ道を歩かないこと）

1. **「起動可能なSCSIだけが悪い」**と書いた。空の(起動不可能な)イメージでも落ちたので誤り
2. **「どんなSCSIイメージでもFD起動が止まる」**と書いた。
   これは**FD側をCHACHAに固定したままSCSIだけを振っていた**ためで、
   FD側を振ると TDQ1・システムディスクは通る。**1変数にしたつもりで、
   もう一方の変数を固定していたことに気づいていなかった。**

**「Aを振ったら症状が変わった」は「Aが原因」までしか言えない。**
「Aがあると常に壊れる」と書くには、**固定していた側も振る**必要がある。

### 次にやること（次の担当はここから）

黒箱の切り分けはここで尽きた。コア側のトレースに移る。

1. **SASI と SCSI で IPL の実行トレースを差分する。** 同じ CHACHA.XDF で、
   SASI併設(通る)と SCSI併設(落ちる)を比べ、**どこで分岐して起動を諦めるか**を出す。
   過去に分岐点 `$df48` を同じ手法で特定した実績がある（本ドキュメント該当節参照）
2. SCSIマウント時の**SRAM書き込み**が起動デバイス設定に触れていないかを見る
   （利用者のログに `sram write enable = 1` が出ていた）
3. 再現手順は上の表のとおり。`scripts/_gen-scsi-blank.mts` で空イメージを作り、
   `public/` へ置いて `window.__webx68kScsiUrl` に URL を入れてから起動ボタンを押せばよい
   （`?worker=0` でも既定でも同じ結果）

### 本日の状態

- **master 取り込みの阻害要因とまでは言えない**（SCSI機能全体は動く）が、
  「SCSIを挿すと起動しなくなるディスクがある」という既知の欠陥は残る
- 前節の「URL指定時に指定外スロットを解除する」により、**URL経路で報告された症状は止まる**
- 残っている宿題: この件 / 起動前へ戻る手段(電源OFF相当)が無い件 /
  使い方・READMEのSCSIの節 / `ipe-profile-select` の重複id

## 「SCSIを挿すと起動しないディスクがある」の原因を特定し、安全側へ倒した（実測・修正、2026-09-05）

### 決め手

前の担当の申し送りは「SASI併設とSCSI併設のIPL実行トレース差分から入る」だったが、**トレースを取る前に、同じプローブでFDだけを振ったログ差分で決着した**。同じ `scripts/probe-scsi-iocs.mjs` を、変数をFDだけにして2回走らせて突き合わせた（＝両側で物差しが同じ）。

- 失敗する側: `node scripts/probe-scsi-iocs.mjs --worker=0 --no-system --fd1=/test/CHACHA.XDF --image=_local/scsi/probes/blank1m.hds`
- 成功する側: `node scripts/probe-scsi-iocs.mjs --worker=0 --image=_local/scsi/probes/blank1m.hds`（同梱システムディスク）

### 原因（実測）

- `SCSI_HostCommand()` の cmd==`$03`（ベクタ設定エントリ）は、ゲスト番地 `$66ca` の中身を「ドライバヘッダを置くRAM番地(base)」として読む（`webx68k_scsi_drv_ram_from()` の既定値 `0x66ca`）
- 同梱システムディスクでは `$66ca` に `$00190000` が入っており、`[SCSI-DRV] ドライバをRAMへ置いた base=$00190000` が出て正常に登録される
- **CHACHA.XDF の Human68k では `$66ca` が `$00000000` のまま**。`--mem-read-watch=0x66ca:0x66cd` で、我々のベクタ設定エントリ（pc=`$00ea0088`）が `$00` を読んでいることを直接観測した。`--ram-watch` でも、この起動中にゲストが `$66ca` へ書いた形跡は IPL の `$ff` 埋め（pc=`$00ff0010`）以外に無い
- base==0 のとき、旧コードは **ログも出さずに** RAM配置を飛ばしていた。その結果ヘッダは ROM窓 `$ea0100` のままで、初期化コマンドの返答が `終了addr=$00ea0500` になり、Human68k の門（終了アドレス+`$10000` < RAM上限 / 終了アドレス-`$22` >= ヘッダ番地）を満たせず、**起動そのものが止まっていた**
- 両ディスクとも Human68k は **version 3.02** で同じ。要求ヘッダの番地（`$67de`）も内蔵HARDDSKヘッダ（`$00010da2`）も一致しており、版ずれや配置ずれではない。成功側の要求ヘッダ +$0e は `$00007b36`、失敗側は `$00010da2` と異なる

### 陽性対照

`--drv-ram-from=0 --drv-ram=0x190000` で RAM配置を強制すると、**CHACHA.XDF + SCSI でも起動した**（booted=true）。1変数の介入で症状が消えることを確認済み。

### 直したこと(安全側へ倒しただけ。$66ca に代わる正しい番地はまだ分かっていない)

px68k-libretro `1e5ffd5`。base が 0・奇数番地・RAM上限（`Config.ram_size`）を超えて門を満たせない、のいずれかのときは、理由を1行ログに出したうえで **ドライバとして名乗らない**（d2 に `$ffffffff` を返す。a4 は触らない＝0を返すと番地0をヘッダとして読んで暴走するため）。base が使えるときの動作は無変更。
実際に出るログ:
`[SCSI-DRV] ドライバの置き場が取れない (from=$0066ca の中身=$00000000, RAM=$00200000)。ドライバとして名乗らない(このまま登録すると Human68k が起動を諦めるため)`

### 修正後の実測（3本、いずれも `--worker=0`）

| FDD0 | SCSI | booted |
|---|---|---|
| CHACHA.XDF | 空1MiB | **true**（修正前 false） |
| 同梱システムディスク | 空1MiB | true（`base=$00190000` のログも従来どおり。退行なし） |
| CHACHA.XDF | なし | true |

### 前の節の見込みは誤りだった

9146行の節に書いた「IPL が SCSI 側の起動可能な区画を見つけてそちらから起動しようとして諦めている」という機構の見込み（未検証と明記されていた）は**誤り**。ログの順序から、失敗する起動でも **Human68k は動いており**、我々のドライバのストラテジ／インタラプトが呼ばれて初期化コマンド `$00` まで処理されている。止まるのはその**あと**である。IPLの文言が出るからといって IPL の段階で止まっているとは限らない。

### 残っていること

- **`$66ca` に代わる、版・構成に依らないドライバ置き場の求め方**。今回の修正は「取れないときは名乗らない」だけで、CHACHA.XDF のようなディスクでは SCSI が使えないままである。`$66ca` が0のままだったのは、CONFIG.SYS でデバイスドライバを読み込まない構成だと Human68k がこの変数を設定しないためではないか、という**未検証の見込み**がある（成功側の要求ヘッダ近傍 a6 には `CONFIG = \SYS...` の文字列が見えたが、失敗側には無い）
- 起動前へ戻る手段（電源OFF相当）が無い件 / 使い方・README の SCSI の節 / `ipe-profile-select` の重複id

## 置き場は「借りた番地」でしか取れない：a1 は空きメモリの先頭だった（実測、2026-09-05）

### `$66ca` の中身はディスクごとに違う

ベクタ設定エントリの時点で観測した `$66ca` の中身:

| 起動ディスク | `$66ca` |
|---|---|
| 同梱システムディスク | `$00190000` |
| CONFIG.SYS から DEVICE 行だけ消した変種 | `$00190000`（同じ） |
| CONFIG.SYS ごと消した変種 | `$00000000` |
| CHACHA.XDF | `$00000000` |

CHACHA.XDF には **CONFIG.SYS が無い**（収録は HUMAN.SYS / CHA2BEN.MSL / MASL.X / AUTOEXEC.BAT / COMMAND.x / FLOAT2.X / HIOCS.X）。**DEVICE 行の有無ではなく、CONFIG.SYS の有無で分かれる**ことが分かった。

### 検体を1本作った

検体として `public/test/SYS_NOCONFIG.XDF`（同梱システムディスクから CONFIG.SYS を消したもの）を作成。CHACHA と同じ `$66ca=$00000000` を再現しつつ **A> まで来るので `dir c:` で機能を試せる**（ゲームディスクの CHACHA では `dir c:` を打てる場面が無い）。

### a1 が「次に置ける番地」に見えた

ベクタ設定エントリに入った時点のレジスタを観測した（コア `07da27e`）。a1=a4 が「次に置ける番地」に見えた。

| 起動 | a1 |
|---|---|
| 同梱システムディスク（1回目） | `$0001a46c` |
| CONFIG.SYS無し | `$00019eae` |

同梱システムディスクの**2回目**の呼び出しでは a1=`$0019006c` で、これは我々が返した終了アドレス `$00190034` の直後だった。ここまでは a1 が確保領域の直後を指しているように見え、a1 をフォールバックに使えるのではという見込みが立った。

### a1 フォールバックは「1回通った」だけだった

a1 をフォールバックに使うと、`SYS_NOCONFIG.XDF` でも `dir c:` が通り `MARKER.TXT` が見えた。**しかしこれは安全の証明ではなかった。**

`--ram-watch` でドライバヘッダの置き場を監視した結果:

| 起動 | base | 我々以外の書き込み |
|---|---|---|
| 同梱システムディスク（`$66ca` 由来） | `$00190000` | `pc=$000070e0` の4バイトだけ＝Human68k のドライバ連鎖リンク（正当） |
| CHACHA.XDF（a1 フォールバック） | `$00019eae` | `pc=$00040778` が `$00039fd0` を書き込み＝**メモリ管理ブロックの体裁。プログラムへ払い出されている** |
| CHACHA.XDF（自前で `$190000` を強制） | `$00190000` | `pc=$001b6778` が `$001affd0` を書き込み＝同上 |

したがって **a1 は「空きメモリの先頭」であって、我々が使ってよい番地ではない**。`$66ca` 由来の番地だけが Human68k に予約されている。返した終了アドレスによる予約は、CONFIG.SYS の無い起動では効いていない（a1 は進まず、そのまま払い出された）。ドライバヘッダも、ストラテジ／インタラプトの本体も、後から Human68k に潰されうる＝**黙って壊れる**。「1回 `dir c:` が通った」は安全の証明にならない。

### 撤回した

a1 フォールバックは撤回した（px68k-libretro `9d77400`、追加コミット `6d61366` の revert）。現状は前節どおり「置き場が取れなければ名乗らない」＝**CONFIG.SYS の無いディスクでは SCSI が使えないが、起動は止まらない**。

### 残っていること

CONFIG.SYS の無い起動で「予約された番地」を得る方法。次に試すなら、ベクタ設定エントリの時点でゲストRAM低位を走査して a1 と同じ値を持つ番地を探し（`$66ca` に相当する変数がどこにあるか）、そこを我々が進めて予約できるかを実測する筋がある（未着手）。

## 「予約された置き場」を探して全滅、ただし資料が方向を1つ示した（実測、2026-09-05）

### 試したことと結果

1. **走査で `$001c24` を特定した。** ベクタ設定エントリ入口で a1 と同じ値を持つ番地をゲストRAM `$000000-$01ffff` から探すと、**両方の起動でちょうど1件** `$001c24` だけ一致した（CONFIG.SYS無し a1=`$00019eae` / CONFIG.SYSあり a1=`$0001a46c`）。番地は焼き込まず毎回走査して特定する実装にした（`$66ca` を焼き込んだのが今回の不具合の原因なので同じ轍を踏まないため）
2. **`$001c24` を進めて「自分で予約する」を試した**（コア `69061d5`、のち撤回 `ed7fc42`）。結果:

| 検体 | 結果 |
|---|---|
| `SYS_NOCONFIG.XDF` + `dir c:` | 通る。MARKER.TXT が見え、ヘッダ範囲への他者の書き込みは0件 |
| CHACHA.XDF | **不合格**。修正前と同じ `pc=$00040778` が `$00039fd0` をヘッダ先頭へ書き、潰れる |

   `SYS_NOCONFIG.XDF` 側の「0件」は**短い操作の間に確保が起きなかっただけの可能性があり、安全の証明にならない**。よって撤回した
3. **`$001c24` の一生を観測した**（CHACHA.XDF、ドライバを置かない状態）。この番地は起動中に何度も書き換わる:
   `$0001407a` → `$00014866` → `$000148d6` → `$00019eae`(pc=`$8246`) → 〔ここで我々のベクタ設定エントリ〕 → `$0002b76e`(pc=`$771c`)。
   **a1 は「次に読み込むものの読み込み先」**であり、我々の書き込みは後から Human68k 自身に上書きされる（`$0002b76e` は `$00019eae` + COMMAND.X のサイズに見える）
4. **ROM窓のヘッダのまま名乗れないかを掃引した**（実験用スイッチ `--rom-header-end=`、コア `d7c4b13`／`src/core-shim.c`）。ヘッダを `$00ea0100` に置いたまま、初期化コマンドの返答の終了アドレスを6通り振った:
   `$00010da2` / `$0001a000` / `$00019ee2` / `$001f0000` / `$00ea0122` / `$00100000` → **6通りとも `booted=false`（起動が止まる）。全滅。**
   陽性対照として「ROM窓のヘッダのまま名乗る」ログが出ていることを毎回確認した（前回、分岐に一度も入らないまま6件を測った空振りがあったため）
5. **本物のSCSIボードROMは基準器にならなかった。** `--rom=_local/scsi/Scsiexrom.dat` で起動すると、CONFIG.SYS の有無に関わらず `dir c:` が「ドライブ名が無効です」になる。我々の SPC が部分実装なので、本物ROMもドライブを登録できていない

### 資料に1件だけ記述があった

『Human68k version 3.0』1.3.2「SCSIデバイスドライバの標準装備」に、**FDから起動した場合などでも `SCSIDRV.SYS` の組み込み無しに SCSI ドライブが使える、SCSIドライバをROM内に持っているため**、という記述がある。すなわち**実機では CONFIG.SYS の無いディスクから起動しても ROM 常駐のドライバが登録される**。
（『Inside X68000』側は SCSI-ROM の置き場所と識別ラベル、SRAM の追加3番地までで、登録手順・要求ヘッダ・メモリ確保の記述は無い。両書とも「ストラテジ」の語が出ない）

### いまの読みと、次にやること

- 本ドキュメント前出の門1・門2（`$70be`〜`$70d4` の実行トレース）は「ヘッダがRAM上にあること」を要求しており、今回の掃引6件全滅はそれと整合する。**ヘッダをRAMへ置くこと自体は避けられない**
- 同じ節に「本物ROMは自分のドライバを RAM(`$0001a46c`)へ複写してから渡していた」と記録がある。**その番地は a1 と同じ値**であり、本物も a1 を置き場にしていた可能性が高い（未確定）
- 食い違っているのは**予約**のほう。CONFIG.SYS のある起動ではベクタ設定エントリが**2回**呼ばれるが、無い起動では**1回**しか呼ばれていない（観測用ログ `07da27e` の文脈ダンプで確認）。次はここから入る:
  1. 「2回目が来ない」ことと「読み込み先が進まない」ことが同じ根かを確かめる
  2. 2回目の呼び出しで `d2=$ffffffff`（もう無い）を返している既存の処理が、進める処理を止めていないかを振ってみる（`--reply-init-once` を切る）
  3. CHACHA.XDF で潰した `pc=$00040778` の書き込みが、置き場を進めたときにどこへ移るのかを測る（移らないなら、進める先が間違っている）

## 解決: 置き場は a1（実測、2026-09-05）

### 前節の結論は誤りだった

前節で「a1 に置いた領域はプログラムに払い出され、ヘッダごと潰れる」と書いて経路を撤回したが、**誤りだった**。根拠にしたのは `--ram-watch` で見えた `pc=$00040778` の書き込みだが、これは**4バイトしか書いていない**。Human68k のメモリ管理ブロックなら16バイト書く。**書き込みの大きさを見ていなかった。**

### 実際に何が書かれていたか(peek で中身を読んだ)

プローブに `--peek=<番地>:<長さ>` を足して、実行の最後にゲストメモリを読んだ(`ae3fbd3`)。

- 我々のヘッダ `$019eae` から48バイト:
  `00 03 9f d0 | 40 00 | 00 01 9e ce | 00 01 9e d8 | 01 | "SCSIHDD" | … | 13 fc 00 40 00 e9 f8 02 4e 75 …`
  → **変わったのは +0〜+3 の「次のドライバ」だけ**。属性 `$4000`・ストラテジ `$00019ece`・インタラプト `$00019ed8`・ユニット数・名前 `SCSIHDD`・スタブ本体(`move.b #$40,$e9f802 / rts`)はすべて無傷
- その `+0` が指す先 `$039fd0` から48バイト:
  `ff ff ff ff | c0 20 | 00 04 01 5e | 00 04 01 6a | 40 | "IOCS   " | 48 e7 …`
  → **これはデバイスドライバのヘッダそのもの**(次=`$ffffffff`=連鎖の末尾、属性 `$c020`、ストラテジ/インタラプトが自分の中を指す、名前 `IOCS   `)

つまり `pc=$00040778` の正体は、**あとから常駐した IOCS ドライバが自分を連鎖に繋ぐ書き込み**だった(CHACHA.XDF には HIOCS.X が入っている)。破壊ではなく正当な動作である。

### 予約も効いていた

置き場を指定して登録を進ませると、CHACHA.XDF でも**ベクタ設定エントリは2回呼ばれ、a1 が `$00019eae` → `$00019f1a` へ進む**(我々が返した終了アドレス `$00019ee2` の先)。同梱システムディスクで `$00190034` → `$0019006c` と進むのと同じ形。**Human68k は我々の領域を飛ばしており、`$001c24` を自分で書き換える必要は無い**(その案は撤回済み `ed7fc42`)。

### 端から端までの実測

`SYS_NOCONFIG.XDF`(CONFIG.SYS を消した同梱システムディスク)で、**ドライバが後から常駐しても SCSI ドライブが生き残る**ことを確認した:

```
A>dir c:      → MARKER TXT 1500 が見える
A>\sys\iocs.x → Console/Graphic IOCS Version 1.50 …(＝ここで連鎖に繋がる)
A>dir c:      → MARKER TXT 1500 が見える(生きている)
```

### 現在の実装と実測(コア `69383a2`)

`$66ca` の中身が使えないときだけ、ベクタ設定エントリ入口の a1 を置き場に採る(既存の検査＝0でない・偶数・`base+0x34`と`+0x10000`がRAM上限未満、を通ったときのみ)。通らなければ従来どおり「名乗らない」。

| 起動 | 結果 |
|---|---|
| CHACHA.XDF + SCSI | `booted=true`、base=`$00019eae`、ヘッダ無傷 |
| `SYS_NOCONFIG.XDF` + SCSI + `dir c:` | `booted=true`、MARKER.TXT が見える |
| 同梱システムディスク + SCSI + `dir c:` | `booted=true`、base=`$00190000`(従来経路・無変更)、MARKER.TXT が見える |

### 資料との整合

『Human68k version 3.0』1.3.2 の「FDから起動した場合などでも SCSIDRV.SYS 無しに SCSI ドライブが使える(ドライバを ROM 内に持っているため)」という記述と、これで整合が取れた。本ドキュメント前出の「本物ROMは自分のドライバを RAM(`$0001a46c`)へ複写してから渡していた」という観測とも合う(`$0001a46c` は、その起動での a1 の値と同じ)。

### 付随して分かったこと

`reply_init_once` を 0 にして「2回目以降も断らない」ようにすると、**同梱システムディスク・CHACHA.XDF のどちらも起動しなくなる**(初期化コマンドが2回処理され、ドライバが二重に登録されるため)。既定の 1 が正しい。
