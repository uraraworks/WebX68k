...
## 空区画コピー失敗の追跡：書き込みは正しく落ちている／CPUが完全停止する(実測、2026-09-04)
...
2. `scripts/build-core.sh`でコアを再ビルドし、Puppeteerで実ブラウザを自動操作する
   使い捨てスクリプト(`_debug-copy-tmp.mjs`、リポジトリには残していない)を書いて、
   `createFormattedScsi(80)`のブランクへ`dir c:` → `copy a:\command.x c:\`を打鍵し、
   `[SCSI*]`ログ全件と画面テキストを回収した。
...
**ブランク(`createFormattedScsi(80)`)への`copy`は、ダイアログではなく完全なハング
で終わる。** 再現手順は毎回同一で、決定的だった(複数回実行して同じ地点で停止):

1. ルートディレクトリ(セクタ163、ホストLBA390)を読む → 全ゼロ
2. FATセクタ1(ホストLBA66)を読む → `f7 ff ff ff 00 00 ...`(空)
3. コマンド`$05`
4. ルートディレクトリへ`COMMAND X   `のエントリを書く(`$08`)。
   **`[SCSI-WRITE-VERIFY]`で同じLBA(390・391)を即座に読み返し、書いた16バイト
   `43 4f 4d 4d 41 4e 44 20 58 20 20 20 00 00 00 00`と一致することを実測で確認した。
   「書けたと言われたが実際は落ちていない」という仮説は否定できた。**
5. コマンド`$05`
6. FATセクタ1を再度読む → 3と同じ(空のまま。ここまでは前節の記録と完全に一致する)
7. コマンド`$05`が数回続く
8. **ここでゲスト側のあらゆる出力(`[SCSI*]`ログはもちろん、無関係な`sram write
   enable`のようなログも含めて)が完全に途絶える。** ポーリングを一切行わず
   Enter後にまとまった時間(15秒・30秒・5分)放置してから1回だけ状態を見ても、
   ログ件数(2047件、複数回の独立した実行すべてで再現)・画面とも一切変化しない。
   `A>copy a:\command.x c:\` / `a:\COMMAND           .X` の次の行(コピー結果や
   エラーダイアログが出るはずの行)は空白のまま凍りつく。

**これは「ディスクの管理領域が壊されています」ダイアログとは別の症状である。**
前節の実ブラウザ確認(手動)ではダイアログが出たと記録されているが、今回の
自動化環境では同じ操作から**ダイアログではなく無期限のハング**に到達した。
どちらも「空のSCSI区画への初回書き込み」でだけ起きる点は一致しており、
ダイアログ表示とハングは同じ根本原因の異なる現れ方(タイミング依存)である
可能性が高いが、未確認。

**「観測(ポーリング)がWorkerを止めているのでは」という仮説は否定した。**
Enter後に一切`screenText()`を呼ばずに放置しても同じ地点で同じ件数(2047件)で
止まるため、自動化ツール側の観測が原因ではない。

### 分かったこと・分からなかったこと

- **分かったこと**: `$08`の書き込みは実装として正しい(バイト単位で照合済み)。
  失敗は「書き込みが落ちていない」ことではなく、**2回目のFATセクタ1読み出し
  (すべて空)の後、複数回のコマンド`$05`を経てゲストCPUが進行を完全に止める**
  ことである。停止は決定的で、複数回の独立実行すべてで同じログ件数
  (2047件)に達して止まった。
- **分からなかったこと**: 何がこの停止を引き起こしているか(コマンド`$05`の
  意味・Human68kがそこで何を待っているか)は未解明のまま。`x68k/scsi.c`の
  ログはこの時点で完全に途絶えるため、ゲストCPUがこちらの処理系(SCSIハンドラ)
  を呼ばない場所で止まっている可能性が高く、`[SCSI*]`ログだけでは追えない。
  本物のSCSI ROMは逆アセンブルしない方針のため、`$05`の意味を資料的に
  確定させる手段も無い。
...
1. **ゲストCPUが実際にどこで止まっているかを、`[SCSI*]`ログに頼らずに特定する。**
   `x68k/mem_wrap.c`の`webx68k_ram_watch_check`/PC範囲指定の仕組み(既存)を使い、
   停止直前のPCの範囲を broadに監視するか、JS側からタイマーで`m68000_get_reg(M68K_PC)`
   を(log_cbを経由せず直接)一定間隔でサンプリングして、停止後もPCが変化して
   いる(細かいループを回っている)のか、本当に停止している(STOP命令や
   割り込み待ちで一切実行していない)のかを切り分ける。
2. コマンド`$05`の意味を、呼ばれる文脈(今回の対比: 成功する非空イメージでも
   同じ`$05`は呼ばれているので、$05そのものより「$05が呼ばれた後、何を
   読みに行くか／行かないか」に着目する)から絞り込む。
