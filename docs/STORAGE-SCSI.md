...
- **マウスキャプチャモード(`requestPointerLock`による相対移動)のWorker経路での動作は未確認。** `requestPointerLock`はユーザー操作から同期的に呼ぶ必要があり自動化できなかった。**人手での確認が必要な宿題**として残す。
