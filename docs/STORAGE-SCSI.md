...
20. **puppeteerのChrome起動待ちタイムアウト(`Timed out after 30000 ms while waiting for the WS endpoint URL to appear in stdout!`)の再発。**8/25に一度発生し「再発したら記録する」としていたが、2026-09-01夜、load average 16以上の高負荷下で3本連続実行の1本目が失敗し再発した(該当する`wm-20260901-drives-worker-3.json`は作られていない)。高負荷が条件の候補として1つ絞れたが確定ではなく、再発時に条件を追加で記録する。
