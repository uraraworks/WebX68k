...
1. **px68k-libretro側(別リポジトリ、`/Users/haruurara/MyProject/_emulator/X68K/px68k-libretro`)
   での`copy`失敗の原因調査。** `x68k/scsi.c`のストラテジ/インタラプト処理
   (特にコマンド`$05`の意味の特定と、コマンド`$08`のログをもっと細かく取ること)を
   起点に、「空のSCSI区画への初回書き込み」だけで再現する条件を絞り込む。
   再現手順はこの節の「切り分けた範囲」を参照(`createFormattedScsi(80)`で作った
   ブランクへ`copy a:\command.x c:\`するだけで再現する)。
2. 上記が直ったら、受け入れ条件2〜4(コピー成功・再読み込み後の永続化・
   sectorsPerFatをずらす故障注入がその両方を実際に壊すこと)を実測し直す。
3. `createFormattedHdd()`(SASI)のFAT予約エントリが本項で見つけたのと同じ
