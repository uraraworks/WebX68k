...
- KeyBufプローブのWorker経路対応(`scripts/measure-key.mjs`が同期API前提のため、正式なキー入力計測はまだ回せない。今回はTVRAM経由の簡易確認にとどまる)
- 正式な計測ハーネスによる移行前基準との比較(起動・3ドライブ・キー入力・音声)は未実施
