...
1. **2組目を別日に取る。**条件は (a) 同梱アセットのSHA-256が今日と一致、(b) `src/` が未変更、(c) 計測前後で `verdict: quiet`。`contended` の組は採らない。
2. 音声の `outputLatency`（0.168秒／0.016秒）の切り分け。今日は0.016秒だった。**2組続けて0.016秒なら、0.168秒だった旧set1のほうが例外だった可能性が上がる**ので、2組目でこの値を必ず確認する。
3. キー入力1回目の KeyBuf が1回目だけ遅い件は、2組目の同じ値と突き合わせる。再現すれば「1回目は暖機」として基準の読み方に反映し、再現しなければ今日の1点として扱う。
