...
- **実ブラウザでの追従動作の確認は最小限(起動と手動操作の生存確認程度)にとどめた。** 継続的な追従の収束性・違和感の有無は未確認。親セッションが`?worker=1`でコアを起動し、追従モード(非キャプチャ)でマウスを動かしてカーソルが追いかけてくること、「マウス再同期」ボタンが機能すること、キャプチャモード(右ダブルクリック)との切り替えが機能することを確認する必要がある。
- **マウス入力のレイテンシは計測していない**(「まだ確かめていないこと」節にあった宿題のうち、今回はマウス移動のレイテンシ計測自体は行っていない。閉ループのWorker移行のみを実施した)。
- **stall(90フレーム)まで実際に追い込んで諦める挙動が実ブラウザで発生するか**は未確認(単体テストでは`reflectImmediately=false`のfakeで再現しているが、実コア・実IOCSワークでの再現は見ていない)。
