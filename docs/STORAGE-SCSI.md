...
8. **長時間バックグラウンド化の確認。**今回は14.9秒しか試していない。数分〜数十分の凍結やメモリ逼迫下でのハンドル生存・ワーカー生存を確認する。
9. **2タブでの排他の実地確認。**単一タブ内の多重取得では排他が効くことを確認済みだが、実際に2枚のタブを開いての相互確認（`manualCrossTab`）は未実施のまま。案内UIの出し方も含めて確認する。
