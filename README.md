...
- `src/api/fat.ts` / `sjis.ts` / `archive.ts` / `archive-util.ts` / `lzh.ts` / `zip.ts` … WebNP2の
  `src/api/` から移植(移植元は無変更)。`fat.ts` のみジオメトリ定数をX68000の2HD(1232KB)向けに変更。
- `src/filemanager.ts` … WebNP2の `src/ui/filemanager.ts` を移植。FmTargetの表示ラベルを
  呼び出し側(main.ts)で組み立て済み文字列として受け取る形に簡略化(WebX68kはFDD1/FDD2/HDDの
  3スロット構成で、HDD非対応の理由をラベルに含めたいため)。
...
- **HDD(.hdf)は非対応**: Human68kのHDDイメージは先頭にHuman68k独自のパーティションテーブル
  (シグネチャ`"X68K"`、パーティションエントリはオフセット0x410から16バイト単位、
  値は256バイトセクタ単位)を持つ。実データ(`hd0.hdf`)で解析したところ、パーティション自体の
  位置は正しく特定できた(`33 * 256 = 0x2100`バイト目、"Hudson soft 2.00"というOEM名が見つかる)
  ものの、そのブートセクタは標準のMS-DOS BPBレイアウトと異なる(ジャンプ命令が2バイトのみ、
  OEM名が16バイト等、フィールドの並びが標準からシフトしている)。時間の制約上この独自レイアウトを
  確実に解析しきれなかったため、本実装ではHDDのFAT読み書きは非対応とした。ファイルマネージャの
  ターゲット一覧にはHDDスロット/ライブラリ内HDDイメージも表示されるが「(HDD非対応)」と表示され
  選択できないようになっている(誤って壊す心配はない)。
...
- `hd0.hdf`(Human68k入りHDD)は上記の理由によりFAT非対応。ターゲット一覧には表示されるが
  編集不可として扱われる。
